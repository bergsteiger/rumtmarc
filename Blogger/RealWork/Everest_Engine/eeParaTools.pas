...
{ $Id: eeParaTools.pas,v 1.1 2010/04/21 16:52:40 lulin Exp $ }
...
begin
 ...
 begin
  if (aPara as Ik2Tag).QT(InevPara, l_Para, aEditor.Processor) then
  begin
   l_ParaLeaf := evFindNumberedPara(l_Para, aSearchDown);
   if not Assigned(l_ParaLeaf) OR
      not Supports(TeePara.Make(l_ParaLeaf, aEditor), IeeLeafPara, Result) then
    Result := nil;
  end
  else
  begin
