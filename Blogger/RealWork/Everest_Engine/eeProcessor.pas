...
{ $Id: eeProcessor.pas,v 1.3 2013/07/31 12:24:48 morozov Exp $ }
...
procedure TeeProcessor.DoCheckParaOp(const aParent  : Ik2Tag;
                                     var aChild     : Ik2Tag;
                                     anOp           : Tl3Operation;
                                     anIndex        : TnevParaIndex);
  //override;
...
var
 ...
 procedure CorrectParaStyle(const aPara: Ik2Tag);

 var
  l_Op : InevOp;

  procedure CorrectParaStylePrim(const aPara: Ik2Tag);
  var
   l_Index : Integer;
  begin
 ...
 procedure lp_NotifyOtherTexts(const anObj: InevObject);
  ...
  begin
   ...
   if not l3IEQ(l_AnotherContainer, l_Container) AND
    l_AnotherContainer.IsSameText(l_Container) then
   begin
    ...
    begin
     ...
     begin
      ...
      if (l_ListItemObj.InheritsFrom(k2_idCommentPara)) then
      begin
       if l_ListItemObj.QT(InevObject, l_ParaObject) then
       begin
        ...
        Assert(false);
      end;//(l_ListItemObj.InheritsFrom(k2_idCommentPara))
      l_ListItemObj := nil;
  ...
  try
   Case anOp of
    ...
    begin
     ...
     if (aChild.InheritsFrom(k2_idCommentPara)) then
     begin
     ...
     end;// aChild.InheritsFrom(k2_idCommentPara)
     if not InUndo then
      if not IsCheckOff then
       if not evInPara(aChild, k2_idCommentPara) then
       // - проверяем, что вставляем внутрь комментария
        if evInPara(aChild, k2_idDocument) then
    ...
    begin
     ...
     begin
      ...
      if evInPara(aParent, k2_idCommentPara) then
      ...
      try
       ...
       if evInPara(aParent, k2_idSBS) then
        // - не даем вставлять комментарии внутри подписи.
        raise EeeCannotInsertPara.Create(cCannotInsertPara);
       if not aChild.InheritsFrom(k2_idCommentPara) then
       begin
        if (anIndex > 0) AND ((anIndex < aParent.ChildrenCount) AND
           aParent.Child[Pred(anIndex)].InheritsFrom(k2_idCommentPara)) then
         raise EeeCannotInsertPara.Create(cCannotInsertPara);
        l_Child := k2_typCommentPara.MakeTag;
        try
         l_Child.AddChild(Ik2Tag(aChild));
         aChild := l_Child;
        finally
         l_Child := nil;
        end;//try..finally
       end;//not aChild.InheritsFrom(ee_idCommentPara)
      finally
       l_Para := nil;
      end;//try..finally
     end;//not InUndo
