...
{ $Id: eePara.pas,v 1.4 2012/04/26 11:38:26 lulin Exp $ }
...
class function TeePara.Make(const aTagWrap   : Ik2Tag;
                            const aDocument  : IeeDocumentEx;
                            const aProcessor : InevProcessor = nil): IeePara;
  //reintroduce;
  {-}
...
begin
 ...
 begin
  if aTagWrap.InheritsFrom(k2_idSBS) then
   l_Para := TeeSBS.Create(aTagWrap, aDocument, aProcessor)
  else 
  if aTagWrap.InheritsFrom(k2_idParaList) then
...
function TeePara.InheritsFrom(aType: TeeTypeID): Windows.Bool;
  {-}
begin
 Result := TagInst.InheritsFrom(aType);
end;
...
begin
 ...
 if not Result then
 begin
  if Supports(aPara, Ik2Tag, l_TagWrap) then
   try
    Result := TagInst.IsSame(l_TagWrap);
   finally
    l_TagWrap := nil;
   end;//try..finally
 end;//not Result
...
function TeePara.Get_Parent: IeePara;
  {-}
begin
 if (Document = nil) OR InheritsFrom(k2_idDocument) OR (TagInst = nil) OR
...
begin
 ...
 end;//not InheritsFrom(k2_idDocument)
end;

function TeePara.Get_Subs: IeeSubList;
  {-}
begin
 Result := TeeSubList.Make(TagInst, Document, ev_sbtSub);
end;

function TeePara.Get_Markers: IeeSubList;
  {-}
begin
 Result := TeeSubList.Make(TagInst, Document, ev_sbtMarker);
end;

function TeePara.Get_Bookmarks: IeeSubList;
  {-}
begin
 Result := TeeSubList.Make(TagInst, Document, ev_sbtBookmark);
end;

function TeePara.Get_Marks: IeeSubList;
  {-}
begin
 Result := TeeSubList.Make(TagInst, Document, ev_sbtMark);
end;
...
function TeePara.TagOwner: Ik2Tag;
  {-}
begin
 if (TagInst = nil) then
  Result := k2NullTag
...
begin
 ...
 begin
  if evInPara(TagInst, k2_idCommentPara) then
  ...
  begin
   ...
   if not Result then
    with rAtomEx([k2_tiSegments,
                k2_tiChildren, k2_tiHandle, ev_slView]) do
...
class function TeeStyledLeafPara.Make(const aTagWrap   : Ik2Tag;
                                      const aDocument  : IeeDocumentEx;
                                      const aProcessor : InevProcessor = nil): IeeStyledLeafPara;
  //reintroduce;
  {-}
