...
{ $Id: CsListener.pas,v 1.10 2013/05/31 07:20:42 lulin Exp $ }
...
// - нова€ процедура KeepAlive
// - cleanup
//
// Revision 1.6  2006/09/01 13:55:00  narry
// - при подключении клиента к серверу провер€ютс€ версии клиента и сервера
//
// Revision 1.5  2006/08/02 12:18:45  narry
// - ловушка дл€ сообщений с сервера
//
...
type
 ...
 TcsUDPListenerThread = {$IfDef XE} TidUDPListenerThread {$Else} TObject {$EndIf};
 ...
 private
  ...
  procedure OnUdpServerRead(Sender: TcsUDPListenerThread;
                            {$IfDef VER250}const {$EndIf}aData: {$IfDef VER240} array of Byte {$Else} TIdBytes {$EndIf};
 ...
 begin
  if Write2Log then
   with l_Notification do
    l3System.Msg2Log('”ведомление: %s (Number: %d, Text: "%s")'#13#10, [GetEnumName(TypeInfo(TCsNotificationType), ord(NotificationType)), Number, Text]);
  l_List := f_Notifications.LockList;
  try
