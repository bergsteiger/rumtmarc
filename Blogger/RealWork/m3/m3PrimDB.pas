...
// Generated from UML model, root element: <<SimpleClass::Class>> Shared Delphi::m3::m3DB::Tm3PrimDB
//
...
type
 Tm3PrimDB = class(Tl3CacheableBase)
  {* Заготовка для абстрактной базы }
 protected
 // protected methods
   function MainStorage(aMode: Tm3StoreAccess = m3_saRead): Im3IndexedStorage; virtual; abstract;
     {* Возвращает корень постоянной части }
   function GetVersionsStorage(aMode: Tm3StoreAccess = m3_saRead): Im3IndexedStorage; virtual; abstract;
     {* Возвращает корень переменной части }
   function GetBackupStorage(aMode: Tm3StoreAccess = m3_saRead): Im3IndexedStorage; virtual; abstract;
...
var
  ...
  begin
   ...
   begin
    ...
    begin
     ...
     l_ObjStream := IStream(l_ObjectStorage.OpenStore(aStoreInfo.rPosition, m3_saRead, anIndex, m3_stStream));
 ...
 begin
  ...
  try
   l_ObjectStorage.IterateF(m3L2SA(@DoObject));
  finally
   l_ObjectStorage := nil;
  end;//try..finally
 end;
