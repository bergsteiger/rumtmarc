...
{ $Id: k2TagTool.pas,v 1.93 2012/10/26 19:12:34 lulin Exp $ }
 ...
 Tk2TagPointer = class(_k2TagBox_, Ik2TagListner, Ik2TagTool)
    ...
    // interfaces methods
      ...
      procedure pm_SetParentTool(const aValue: Ik2TagTool);
        {-}
      function  IsSame(const aTool: Ik2Tag): Boolean;
        override;
        {-}
    protected
    // property methods
    ...
    // internal methods
      ...
      procedure cvReset(out theValue: Tk2Integer);
        overload;
        {-}
      function cvCheck(const aValue : Tk2Str;
                       out theValue : Tl3PCharLen): Boolean;
        overload;
        {-}
      function cvSet(const aValue : Tl3PCharLen;
                     out theValue : Tk2Str): Tl3PCharLen;
        overload;
        {-}
      procedure TagChanged(const anOld, aNew: Ik2Tag);
        virtual;
        {-}
    ...
    // public methods
      constructor Create(const aTag: Ik2Tag);
        ...
        virtual;
        {-}
      class function Make(const aTag: Ik2Tag): Ik2TagTool;
        reintroduce;
        virtual;
        {-}
  ...
  Tk2ProcTagTool = class(Tk2TagTool)
...
class function Tk2TagPointer.Make(const aTag: Ik2Tag): Ik2TagTool;
  //reintroduce;
  //virtual;
  {-}
var
...
procedure Tk2TagPointer.Assign(aSource: TPersistent);
  //override;
  {-}
begin
...
procedure Tk2TagPointer.pm_SetParentTool(const aValue: Ik2TagTool);
  {-}
begin
 if (f_ParentTool = nil) OR not f_ParentTool.IsSame(aValue) then
  f_ParentTool := aValue;
end;

function Tk2TagPointer.IsSame(const aTool: Ik2Tag): Boolean;
  {-}
begin
 Result := (Ik2TagTool(Self) = aTool);
 if not Result then
 begin
  if (aTool = nil) then
  begin
  ...
  begin
   if (Redirect = nil) then
    ...
    Result := Redirect.IsSame(aTool);
  end;//aTool = nil
 end;//not Result
end;

procedure Tk2TagPointer.pm_SetTarget(const aValue: Ik2Tag);
  {-}
begin
 SetTagQT(aValue);
end;
...
function Tk2TagPointer.cvCheck(const aValue : Tk2Str; out theValue: Tl3PCharLen): Boolean;
  //overload;
  {-}
begin
 if not aValue.rIsSet then
...
function Tk2TagPointer.cvSet(const aValue : Tl3PCharLen;
                             out theValue : Tk2Str): Tl3PCharLen;
  //overload;
  {-}
begin
...
procedure Tk2TagPointer.DoSetTag(const aTag: Ik2Tag);
  //override;
  {-}
begin
 if (aTag = nil) then
...
begin
 ...
 begin
  ...
  begin
   ...
   Assert(not l3IEQ(Self, l_Tag));
   TagChanged(Redirect, l_Tag);
   Redirect := l_Tag;
   if (Redirect <> nil) then
   begin
...
procedure Tk2TagPointer.TagChanged(const anOld, aNew: Ik2Tag);
  //virtual;
  {-}
begin
 ClearTagCache;
...
// start class Tk2ProcTagTool

constructor Tk2ProcTagTool.Create(const aTag: Ik2Tag; const aProcessor: Ik2Processor);
  //reintroduce;
  //virtual;
  {-}
begin
...
class function Tk2ProcTagTool.Make(const aTag: Ik2Tag; const aProcessor: Ik2Processor): Ik2TagTool;
  //reintroduce;
  //virtual;
  {-}
var
 l_Tool : Tk2ProcTagTool;
begin
 l_Tool := Create(aTag, aProcessor);
 try
  Result := l_Tool;
 finally
  l3Free(l_Tool);
...
procedure Tk2ProcTagTool.Cleanup;
  //override;
  {-}
begin
 inherited;
...
procedure Tk2ProcTagTool.DoFire(const anEvent : Tk2Event;
                              const anOp    : Ik2Op);
  //override;
  {* - сообщение о том, что с тегом что-то произошло. }
begin
