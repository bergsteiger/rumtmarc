...
// - new behavior: в Tk2TagGenerator не поднимаем Exception 'Вызовов Finish больше чем вызовов Start' при прерывании загрузки документа.
//
...
// - undo behavior: опять поднимается Exception при попытке вызвать Finish больше, чем Start*.
//
   ...
   procedure Set_CurrentVersion(aValue: Integer);
    ...
    // public methods
      ...
      procedure AddStringAtomClone(TagID: Long; Value: Tl3CustomString);
        {* - добавить копию строкового атом. }
