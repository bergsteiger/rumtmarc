...
{ $Id: k2StackGenerator.pas,v 1.66 2013/04/04 11:21:38 lulin Exp $ }
...
function Tk2BaseStackGenerator.GetObjectID(const aName: AnsiString; aChild: Bool): Long;
 ...
 begin//Recover
  ...
  l_CT := CurrentType;
  Result := l_CT.DefaultChildTypeID;
  if (Result <> k2_idEmpty) then
...
begin
 ...
 begin
  ...
  begin
   ...
   Result := GetEnumValue(TypeInfo(Tk2TypeID), '_k2_id' + aName)
 end//aChild
 else
  Result := GetEnumValue(TypeInfo(Tk2TagID), '_k2_ti' + aName);
 if (Result < 0) then
  RaiseNotDefined;
...
procedure Tk2CustomStackGenerator.StartChild(TypeID: Long);
  {override;}
  {-}
begin
 inherited;
 Assert(f_Types <> nil);
 f_Types.Add(TypeTable[TypeID]);
end;
...
procedure Tk2CustomStackGeneratorEx.AddAtom(AtomIndex: Long; TK: Tk2TypeKind; const Value);
  {override;}
  {-}
begin
 inherited;
 ...
 if (AtomIndex >= 0) then
  TranslateAddAtom(AtomIndex, TK, Value);
end;
