...
{ $Id: k2DocumentGenerator.pas,v 1.52 2013/04/04 11:21:37 lulin Exp $ }
...
// - new method: TevDocumentBuffer.RootChanged.
//
// Revision 1.56  2004/01/12 16:20:03  nikitin75
// + default value: TevDocumentBuffer.Create(anOwner: Tk2TagGeneratorOwner = nil);
//
...
// - добавлена функция VariantAsInteger.
//
   ...
   {* Генератор образа документа в оперативной памяти. }
    ...
    // internal methods
      ...
      function  NeedPreAddChild(const aChild: Tk2StackAtom): Bool;
        virtual;
        {-}
      procedure ChildWasPreAdded(const aChild: Ik2Tag);
        virtual;
        {-}
      procedure Cleanup;
        override;
    ...
    // public methods
      ...
      procedure StartTag(TagID: Long);
        override;
        {-}
      procedure AddAtomToDocument(AtomIndex: Long; TK: Tk2TypeKind; const Source);
        {-}
      procedure AddAtom(AtomIndex: Long; TK: Tk2TypeKind; const Source);
        override;
        {-}
    protected
...
begin
 inherited;
 f_Tags := TevAtomStack.Create(Self);
end;
   ...
   begin
    ...
    begin
     ...
     if (l_Child.Parent <> nil) then
      l_Child.Parent.Box.Attr[l_Child.TagID] := nil;
    end;//DoBeforeFinishAtom(l_Child)
...
procedure Tk2DocumentGenerator.StartChild(TypeID: Long);
  {override;}
  {-}
...
(*  with A.TagType do
  ...
  l_T := TypeTable[TypeID];
  l_Top := f_Tags.Push(l_T, l_T.MakeTag, true, -1);
  DoStartAtom(l_Top^);
...
begin
 ...
 Assert(l_Prop <> nil,
        ...
        [GetEnumName(TypeInfo(Tk2TagID), TagID), TagID,
...
procedure Tk2DocumentGenerator.DoAddAtom(const Atom : Tk2StackAtom;
  ...
  {-}
begin
 if Assigned(f_OnAddAtom) then f_OnAddAtom(Self, Atom, Prop, TK, Source);
end;

procedure Tk2DocumentGenerator.AddAtomToDocument(AtomIndex: Long; TK: Tk2TypeKind; const Source);
  {-}
var
 l_Atom : PevStackAtom;
 l_Prop : Tk2Prop;
begin
 inherited AddAtom(AtomIndex, TK, Source);
 ...
 begin
  ...
  begin
   l_Atom.Box.cAtom(k2_tiChildren, Context).ChildrenCapacity := Long(Source);
  end//AtomIndex = k2_tiChildren
  else
   try
    if (AtomIndex = k2_tiFrame) AND
       (Integer(Source) = 0) AND
    ...
    begin
     try
      Case TK of
       k2_tkInteger:
        f_Tags.Top.Box.IntW[AtomIndex, Context] := Integer(Source);
       k2_tkBool:
        f_Tags.Top.Box.BoolW[AtomIndex, Context] := Boolean(Source);
       k2_tkObject:
        {$IfDef Nemesis}
        ...
        {$IfEnd}
        {$EndIf Nemesis}
         f_Tags.Top.Box.ObjW[AtomIndex, Context] := TObject(Source);
       k2_tkTransparent:
       begin
        l_Prop := Tk2Prop(f_Tags.Top.TagType.Prop[AtomIndex]);
        if (l_Prop <> nil) then
         f_Tags.Top.Box.AttrW[AtomIndex, Context] := Tk2Type(l_Prop.AtomType).TransparentTag;
       end;//k2_tkTransparent
       k2_tkStream:
       begin
        l_Prop := Tk2Prop(f_Tags.Top.TagType.Prop[AtomIndex]);
        if (l_Prop <> nil) then
         f_Tags.Top.Box.AttrW[AtomIndex, Context] := Tk2Type(l_Prop.AtomType).ObjToTag(TObject(Source));
       end;//k2_tkTransparent
//        f_Tags.Top.Box.ObjW[AtomIndex, Context] := TObject(Source);
       else
        Assert(false);
     ...
     DoAddAtom(l_Atom^, AtomIndex, TK, Source);
    end;//RewriteAtoms..
   except
...
procedure Tk2DocumentGenerator.AddAtom(AtomIndex: Long; TK: Tk2TypeKind; const Source);
  {override;}
  {-}
begin
 AddAtomToDocument(AtomIndex, TK, Source);
end;
...
function Tk2StackAtom.InheritsFrom(ID: Integer): Boolean;
  //overload;
  {* - проверяет наследование данного типа от типа с идентификатором ID. }
begin
 Result := f_Inst.InheritsFrom(ID);
end;

function Tk2StackAtom.InheritsFrom(const anIDs : array of Long): Bool;
  //overload;
  {* - проверить наследование. }
begin
 Result := f_Inst.InheritsFrom(anIDs);
end;

function Tk2StackAtom.InheritsFrom(anAtomTypeID  : Long;
                                   const Exclude : array of Long): Bool;
  //overload;
  {* - проверить наследование. }
begin
 Result := f_Inst.InheritsFrom(anAtomTypeID, Exclude);
end;
