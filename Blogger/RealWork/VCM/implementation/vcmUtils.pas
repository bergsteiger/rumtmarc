...
{ $Id: vcmUtils.pas,v 1.44 2012/07/11 18:25:09 lulin Exp $ }
...
// new function: vcmGetHelpKeyword - перетащил из vcmEntityForm
//
 ...
 begin
  ...
  begin
   ...
   if (TvcmEntityForm(l_Form).Entities <> nil) AND
    ...
    l_Entity := TvcmEntitiesCollectionItem(TvcmEntityForm(l_Form).Entities.
     Entities.FindItemByName(aEntityName));
   if Assigned(l_Entity) then
   begin
    // Операция
    l_Operation := TvcmOperationsCollectionItem(l_Entity.Operations.
     FindItemByName(aOperationName));
    // Action
