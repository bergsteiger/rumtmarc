...
{ $Id: vcmRepOperationsCollectionItem.pas,v 1.48 2013/08/28 16:38:01 lulin Exp $ }
...
// change: _TvcmFormActivate наследник от TvcmBase;
...
type
  ...
  TvcmFormActivate = class(Tl3CBase)
 ...
 if (f_Holders <> nil) then
  with f_Holders do
   for l_Index := 0 to Pred(Count) do
    TvcmOperationsCollectionItem(Items[l_Index]).SetRep(nil);
 vcmFree(f_Holders);
 inherited;
end;
...
procedure TvcmRepOperationsCollectionItem.AddHolder(aHolder: TObject);
  {-}
begin
 ...
 begin
  ...
  if {$IfDef vcmNeedL3}
     (f_Holders.IndexOf(Integer(aHolder)) < 0)
     {$Else vcmNeedL3}
     (f_Holders.IndexOf(Pointer(aHolder)) < 0)
     {$EndIf vcmNeedL3} then
  begin
   f_Holders.Add(Integer(aHolder));
   TvcmOperationsCollectionItem(aHolder).SetRep(Self);
  end;//f_Holders.IndexOf(Pointer(aHolder) < 0
 end;//aHolder Is TvcmOperationsCollectionItem
end;

procedure TvcmRepOperationsCollectionItem.RemoveHolder(aHolder: TObject);
  {-}
begin
 if (aHolder Is TvcmOperationsCollectionItem) then
 begin
  ...
  TvcmOperationsCollectionItem(aHolder).SetRep(nil);
 end;//aHolder Is TvcmOperationsCollectionItem
end;
