...
{ $Id: vcmOperationsCollectionItem.pas,v 1.64 2013/07/11 15:25:19 morozov Exp $ }
...
var
 ...
 begin
  {$IfNDef DesignTimeLibrary}
  if (f_OperationType in vcmToolbarOpTypes) and
...
function TvcmOperationsCollectionItem.CaptionStored: Boolean;
  //override;
  {-}
begin
 if not Linked then
  Result := inherited CaptionStored
 else
  Result := false;
end;
...
function TvcmOperationsCollectionItem.GroupIDStored: Boolean;
  //override;
  {-}
begin
 if not Linked then
  Result := inherited GroupIDStored
 else
  Result := false;
end;
...
function TvcmOperationsCollectionItem.OperationTypeStored: Boolean;
  //override;
  {-}
begin
 if not Linked then
  Result := inherited OperationTypeStored
 else
  Result := false;
end;
...
function TvcmOperationsCollectionItem.IsDefaultStored: Boolean;
begin
 if not Linked then
  Result := inherited IsDefaultStored
 else
  Result := false;
end;
...
procedure TvcmOperationsCollectionItem.pm_SetIsDefault(
