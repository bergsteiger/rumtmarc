...
{ Библиотека "vcm"        }
{ Автор: Люлин А.В. ©     }
{ Модуль: vcmBaseOperationState - }
{ Начат: 21.02.2003 19:09 }
{ $Id: vcmBaseOperationState.pas,v 1.10 2013/04/25 14:22:38 lulin Exp $ }
...
// - bug fix: для состояния операции был пустой Hint (CQ OIT5-6070).
//
...
function TvcmBaseOperationState.CaptionStored: Boolean;
  //override;
  {-}
begin
...
begin
 ...
 begin
  Result := Caption;
  Hint := Result;
  // - для того, чтобы запустить цепочку присвоений Hint -> LongHint -> Description  
 end;//Result = ''
end;
