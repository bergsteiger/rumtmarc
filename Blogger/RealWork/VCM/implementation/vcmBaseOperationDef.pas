...
{ $Id: vcmBaseOperationDef.pas,v 1.43 2013/07/05 09:10:33 lulin Exp $ }
...
type
  ...
  TvcmBaseOperationDef = class(TvcmIdentifiedUserFriendlyWithCategory, IvcmOperationDef, IvcmOpHolder)
   {* Базовый класс описания операции. }
    ...
    public
    // public methods
      constructor Create(anOp: TvcmBaseOperationsCollectionItem);
        reintroduce;
        {-}
      class function Make(anOp: TvcmBaseOperationsCollectionItem): IvcmOperationDef;
...
constructor TvcmBaseOperationDef.Create(anOp: TvcmBaseOperationsCollectionItem);
  //reintroduce;
  {-}
begin
...
class function TvcmBaseOperationDef.Make(anOp: TvcmBaseOperationsCollectionItem): IvcmOperationDef;
  {* - возвращает базовый описатель операции. }
var
...
begin
 if Assigned(f_Op) and Assigned(f_Op.States) then
  ...
  if Assigned(f_States) then
   Result := f_States.Items[anIndex] as IvcmUserFriendlyControl
  else
   Result := nil;
end;
...
begin
 ...
 begin
  ...
  begin
   f_States := TvcmInterfaceList.Make;
