...
{ $Id: vcmEntityForm.pas,v 1.558 2013/08/07 10:24:59 morozov Exp $ }
...
// new behaviour: инициализируем держателей интерфейса IvcmSettingsSource после конструирования формы и установки UserType;
//
...
// bugfix: не проверяли на существование _Entities в GetInner
//
...
// change: перетащил vcmGetHelpKeyword в vcmUtils
//
...
// change: работа контекстных операций из выпадающего окна тулбара (vcmEntityForm.getInner)
//
...
// - new method: _TvcmEntityForm.GetEntityID.
//
  ...
  TvcmEntityForm = class(_afwApplicationDataUpdate_,
   ...
   function DoGetTabInfo(out theCaption: Il3CString;
    ...
    // public methods
      ...
      class procedure ClassGetEntitiesDef(aList      : TvcmInterfaceList;
                                          aUserTypes : TvcmInterfaceList);
        {*- возвращает список описаний сущностей. }
      procedure GetUserTypes(aList : TvcmInterfaceList);
        {-}
...
// Toolbars := [vcm_tbpTop];
 ...
 if (UserType < UserTypes.Count) then
  ...
  HelpKeyword := Name;
 HelpType := htKeyword;
 f_DockList := TvcmLongintList.Make;
...
class procedure TvcmEntityForm.ClassGetEntitiesDef(aList      : TvcmInterfaceList;
                                                   aUserTypes : TvcmInterfaceList);
  {-}
begin
 with Create(nil) do
...
procedure TvcmEntityForm.GetUserTypes(aList : TvcmInterfaceList);
  {-}
var
 l_Index   : Integer;
...
procedure TvcmEntityForm.GetEntitiesDef(aList : TvcmInterfaceList);
  //virtual;
  {-}
begin
...
begin
 ...
 begin
  f_EntitiesList := TvcmInterfaceList.Make;
  try
...
type
  IvcmStateItem = interface(IUnknown)
    ['{91EEAA76-7E66-4B63-8EED-0442E9F8BACE}']
    // public methods
...
type
  IvcmFormState = interface(IvcmBase)
    ['{5933D76E-3BF2-434B-A684-369134CEFB23}']
    // public methods
      procedure LoadState(aForm      : TvcmEntityForm;
                          aStateType : TvcmStateType);
        {-}
  ...
  TvcmFormState = class(TvcmInterfaceList,
                        IvcmBase,
                        IvcmFormState)
    private
    // interface methods
    ...
    // public methods
      constructor Create;
        reintroduce;
        {-}
      procedure AddState(const aName: String; const aState: IUnknown);
        {-}
  end;//TvcmFormState

constructor TvcmFormState.Create;
  //reintroduce;
  {-}
begin
 Make;
...
procedure TvcmFormState.AddState(const aName: String; const aState: IUnknown);
  {-}
begin
 if (aState <> nil) then
...
begin
 for l_Index := Lo to Hi do
  IvcmStateItem(Items[l_Index]).LoadState(aForm, aStateType);
end;
 ...
 begin//SaveControls
  with aControl do
   ...
   begin
    ...
    l_Name := l_Child.Name;
    if (l_Name <> '') AND
       Supports(l_Child, IvcmState, l_State) then
     try
      if l_State.SaveState(l_St, aStateType) then
      begin
       if (l_FormState = nil) then
        l_FormState := TvcmFormState.Create;
       l_FormState.AddState(l_Name, l_St);
      end;//l_State.SaveState(l_St)
     finally
      l_State := nil;
     end;//try..finally
//    if (l_Child Is TWinControl) then
//     SaveControls(TWinControl(l_Child));
   end;//for l_Index
 end;//SaveControls
...
begin
 l_FormState := nil;
 try
  Result := SaveOwnFormState(theState, aStateType);
  SaveControls(Self);
  if (l_FormState <> nil) then
  begin
  ...
  if (g_AllowModal <= 0) then
   if (Application.MainForm <> nil) then
    if not seHasModalWorker then
     raise EvcmTryEnterModalState.Create('Здесь надо обрабатывать модальность формы: ' + ClassName);
 {$EndIf InsiderTest}
 try
  if (g_MenuManager <> nil) then
...
begin
 ...
 for l_Index := 0 to Pred(f_DockList.Count) do
  if Supports(TObject(f_DockList.Items[l_Index]), IvcmDock, l_Dock) then
  try
   // Вычисляем по высоте
   if aHeight then
    case l_Dock.Pos of
...
procedure TvcmEntityForm.UserTypeWeightChanged(aSender: TObject);
  {-}
var
 l_Tab: IafwWeightListner;
begin
 if not (csLoading in ComponentState) and
  (CurUserType = TvcmBaseUserTypesCollectionItem(aSender)) and
  Supports(Parent, IafwWeightListner, l_Tab) then
 try
...
begin
 Result := false;
 if not (ZoneType in [vcm_ztModal, vcm_ztManualModal, vcm_ztSimpleFloat]) then
  if Supports(NativeMainForm, IvcmMainForm, l_Main) then
   try
    if Assigned(l_Main.History) then
     Result := l_Main.History.InBF;
   finally
    l_Main := nil;
   end;//try..finally
end;
...
begin
 ...
 begin
  try
   if (g_AllowModal <= 0) AND seExecuteCurrentModalWorker then
   begin
...
begin
 ...
 begin
  ...
  begin
   ...
   if ((l_Main = Self) OR (Self Is TvcmMainForm)) AND
      (Dispatcher.FormDispatcher.MainFormsCount = 1) then
