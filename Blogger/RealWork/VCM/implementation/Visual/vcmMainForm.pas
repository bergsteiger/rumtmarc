...
{ $Id: vcmMainForm.pas,v 1.466 2013/08/26 14:46:23 morozov Exp $ }
...
// добавляем в shell.mo.en объекты vg_scene
//
...
// change: при освобождении объектов из ObjectsForFree пользуем vcmFree вместо FreeAndNil
//
...
// - new prop: IafwMainForm.StatusBar.
//
...
type
   ...
   {* Базовый класс для главной формы приложения построенного на библиотеке vcm. }
    ...
    // messages
      ...
      procedure vcmMsgCloseForm(var Message: TMessage);
        ...
        {-}
      procedure vcmMsgCloseChildForm(var Message: TMessage);
        message vcm_msgCloseChildForm;
        {-}
      procedure vcmmsgFreeComponent(var Message: TMessage);
        message vcm_msgFreeComponent;
        {-}
      procedure vcmMsgClearPopup(var Message: TMessage);
        message vcm_msgClearPopup;
        {-}
    ...
    // public methods
      ...
      function MakeClone: TvcmMainForm;
        { Description
  ...
  TvcmDispatcher = class(TvcmCacheableBase,
    ...
    // internal fields
      ...
      f_ObjectsForFree        : TvcmLongintList;
      f_InvalidateForms       : TvcmIEntityFormPtrList;
      f_UpdateForms           : TvcmIEntityFormPtrList;
      f_InvalidateCtrls       : TvcmLongintList;
      f_DockManagers          : TvcmLongintList;
      f_LockListeners         : TvcmLongintList;
      f_FDListeners           : TvcmLongintList;
      f_Aggregates            : TvcmLongintList;
      f_Lock                  : Integer;
...
begin
 ...
 f_ObjectsForFree := TvcmLongintList.Make;
 ...
 f_DockManagers := TvcmLongintList.Make;
 f_LockListeners := TvcmLongintList.Make;
...
begin
 if f_FDListeners <> nil then
  for l_Index := 0 to Pred(f_FDListeners.Count) do
   IvcmFormDispatcherListener(f_FDListeners.Items[l_Index]).StartNewMainForm;
...
begin
 if f_FDListeners = nil then
  f_FDListeners := TvcmLongintList.Make;
 l_Index := f_FDListeners.IndexOf(Integer(aListener));
 if l_Index = -1 then
  f_FDListeners.Add(Integer(aListener));
end;

procedure TvcmDispatcher.RemoveFDListener(const aListener: IvcmFormDispatcherListener);
begin
 if f_FDListeners <> nil then
  f_FDListeners.Remove(Integer(aListener));
end;
 ...
 if (f_LockListeners <> nil) then
  ...
  begin
   ...
   begin
    ...
    try
    {$EndIf}
     with IvcmLockListener(Items[l_Index]) do
      case aLock of
       vcm_lnLock:
        Lock;
       vcm_lnBeforeUnlock:
  ...
  begin
   ...
   if (f_DockManagers <> nil) then
    ...
    begin
     ...
     while (l_Index <= Hi) do
     begin
      l_DM := IDockManager(Items[l_Index]);
      try
       l_DM.EndUpdate;
      finally
       l_DM := nil;
...
var
 l_Index : Integer;
begin
 l_Index := f_LockListeners.IndexOf(Integer(aListener));
 if (l_Index < 0) then
  f_LockListeners.Add(Integer(aListener));
end;//AddLockListener
...
begin
 if (f_LockListeners <> nil) then
  f_LockListeners.Remove(Integer(aListener));
...
begin
 ...
 begin
  l_Index := f_DockManagers.IndexOf(Integer(aDockManager));
  if (l_Index < 0) and not f_UnlockProtection then
  begin
   f_DockManagers.Add(Integer(aDockManager));
...
begin
 ...
 if (l_CMF <> nil) and (l_CMF.VCLWinControl Is TvcmMainForm) then
 begin
  if Locked then
   // - не обновляем заголовок окна при залочке
   f_NeedUpdateMainCaption := true
  else
   (l_CMF.VCLWinControl As TvcmMainForm).UpdateMainCaption;
 end;//l_CMF <> nil..
end;
...
procedure TvcmDispatcher.AddObjectForFree(O: TObject);
  {-}
begin
 if (f_ObjectsForFree.IndexOf(Integer(O)) < 0) then
  f_ObjectsForFree.Add(Integer(O));
end;
...
begin
 if (f_Aggregates = nil) then
  f_Aggregates := TvcmLongintList.MakeSorted;
 f_Aggregates.Add(Integer(anAggregate)); 
end;

procedure TvcmDispatcher.RemoveAggregate(const anAggregate: IvcmAggregate);
  {-}
begin
 f_Aggregates.Remove(Integer(anAggregate));
end;
...
begin
 ...
 if (f_Aggregates <> nil) then
  with f_Aggregates do
   for l_Index := Lo to Hi do
   begin
    theAggregate := IvcmAggregate(Items[l_Index]);
    ...
    try
     ...
     try
      ...
      OpenLevel(c_vcmModule,
                Format(cTitID, [Trim(ev_psPrefix(Self.Caption, '-')),
                                Self.FormID.rName]));
      try
       OpenLevel('forms');
       try
...
function TvcmMainForm.MakeClone: TvcmMainForm;
  {-}
begin
      ...
      with aForm.UserTypes do
       ...
       begin
        ...
        begin
         ...
         l3CatSuff(l_Suffix, Format('%s="%s"', [c_vcmLongCaption, l_UT.SettingsCaption]));
...
procedure TvcmMainForm.vcmMsgCloseChildForm(var Message: TMessage);
  //message vcm_msgCloseChildForm;
  {-}
begin
 if (Message.LParam <> 0) then
  // http://mdp.garant.ru/pages/viewpage.action?pageId=425036429
  if TvcmSlayedEntityFormsList.Instance.IndexOf(TvcmEntityForm(Message.LParam)) >= 0 then
   TvcmInterfacedForm(Message.LParam).KillMe;
end;

procedure TvcmMainForm.vcmmsgFreeComponent(var Message: TMessage);
  //message vcm_msgFreeComponent;
  {-}
begin
 if (Message.LParam <> 0) then
 begin
  if (Dispatcher = nil) OR not Dispatcher.InOp then
   FreeAndNil(TObject(Message.LParam))
  else
   Dispatcher.AddObjectForFree(TObject(Message.LParam));
 end;//Message.LParam <> 0
end;

procedure TvcmMainForm.vcmMsgClearPopup(var Message: TMessage);
  //message vcm_msgClearPopup;
  {-}
begin
 if (Message.LParam <> 0) then
 begin
  TPopupMenu(Message.LParam).Items.Clear;
  {$IfDef vcmExportHelp}
  if (TPopupMenu(Message.LParam) Is TvcmPopupMenu) then
   TvcmPopupMenu(Message.LParam).ClearInPopup;
  {$EndIf}
  if (TPopupMenu(Message.LParam).Owner = nil) then
  begin
   //Assert(false, 'А такое бывает, что контекстное меню ни к кому не привязано?');
   // - бывает для TvcmButtonPopupMenu - тесты не прошли
   TPopupMenu(Message.LParam).Free;
  end;//TPopupMenu(Message.LParam).Owner = nil
 end;//Message.LParam <> 0
end;
...
procedure TvcmDispatcher.CascadeWindows;
var
 lIndex        : Integer;
...
begin
 ...
 for lIndex := 0 to Pred(f_MainForms.Count) do
 begin
  SetWindowPos(Get_MainForm(lIndex).VCLWinControl.Handle, HWND_TOP,
...
procedure TvcmDispatcher.ArrangeAll;
const
 ROWS = 3;
var
 lAddHeight    : Integer;           
 ...
 begin
  ...
  begin
   ...
   if lIndex = Pred(lCount) then
    lAddHeight := lRest;
   // позиционируем
   SetWindowPosition(Get_MainForm(lCursor).VCLWinControl.Handle,
...
begin
 ...
 // остаток окон
 lWindowsRest := f_MainForms.Count mod ROWS;
 // остаток высоты
 lHeightRest := lScreenHeight mod ROWS;
 // количество форм в строке
 lWindowsInRow := f_MainForms.Count div ROWS;
 // строим
...
procedure TvcmDispatcher.PrepareWindows;
var
 l_Index : Integer;
begin
 for l_Index := 0 to Pred(f_MainForms.Count) do
  vcmDropConstraints(f_MainForms[l_Index].VCLWinControl As TCustomForm);
end;
...
begin
 PrepareWindows;
 if (f_MainForms.Count > TILE_COUNT) then
  ArrangeAll
 else
 begin
  ...
  begin
   l_RestDiv := l_Height mod f_MainForms.Count;
   l_Step    := l_Height div f_MainForms.Count;
   l_Cursor  := l_Monitor.Top;
  end//aType = vcm_twHorizontal
  else
  begin
   l_RestDiv := l_Width mod f_MainForms.Count;
   l_Step    := l_Width div f_MainForms.Count;
  ...
  begin
   ...
   if l_Index = Pred(f_MainForms.Count) then
    l_Step := l_Step + l_RestDiv;
   // установим позицию формы
   case aType of
    vcm_twHorizontal:
     SetWindowPosition(Get_MainForm(l_Index).VCLWinControl.Handle, l_Monitor.Left, l_Cursor, l_Width, l_Step);
    vcm_twVertical:
     SetWindowPosition(Get_MainForm(l_Index).VCLWinControl.Handle, l_Cursor, l_Monitor.Top, l_Step, l_Height);
   end;//case aType
...
procedure TvcmDispatcher.TileWindowsHorizontal;
begin
 TileWindows(vcm_twHorizontal);
end;

procedure TvcmDispatcher.TileWindowsVertical;
begin
 TileWindows(vcm_twVertical);
end;
...
procedure TvcmDispatcher.CloseAllWindows(const aCurrent : IvcmEntityForm = nil);
var
 l_Index: Integer;
begin
 ...
 for l_Index := Pred(f_MainForms.Count) downto 0 do
  if f_ClosingAllWindows or
    (f_MainForms[l_Index] <> aCurrent) then
   f_MainForms[l_Index].SafeClose;
end;
