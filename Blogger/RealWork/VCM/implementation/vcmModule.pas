...
{ $Id: vcmModule.pas,v 1.81 2012/11/01 09:42:19 lulin Exp $ }
...
// - TvcmModule реализует IUnknown через шаблон.
//
...
// - change: из TvcmModule убран _ModuleDef, так как было сложно редактировать его в наследниках от TvcmModule.
//
...
// - change: TvcmModule теперь наследуется от _TDataModule.
//
...
// - rename method: TvcmModule._GetOperations -> TvcmModule.GetOperationsDef.
//
  ...
  TvcmModule = class(_l3Unknown_)
   {* Базовый класс, представляющий модуль приложения построенного на основе библиотеки vcm. }
    ...
    // public methods
      ...
      class procedure GetEntitiesDef(aList      : TvcmInterfaceList;
                                     aUserTypes : TvcmInterfaceList);
        virtual;
...
class procedure TvcmModule.GetEntitiesDef(aList      : TvcmInterfaceList;
                                          aUserTypes : TvcmInterfaceList);
  //virtual;
  {*- возвращает список описаний сущностей. Для перекрытия в потомках. }
var
