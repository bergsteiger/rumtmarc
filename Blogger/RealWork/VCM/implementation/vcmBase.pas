...
{ $Id: vcmBase.pas,v 1.258 2013/07/01 08:26:33 lulin Exp $ }
...
// - new methods: Tafw.BeginOp/_EndOp.
//
...
// - TvcmBase переведен на "шаблон" l3Unknown.
//
...
// - bug fix: Stack Overflow без директивы vcmNeedL3.
//
...
// - new method: TvcmModule._MakeAggregate.
...
type
  ...
  TvcmInterfaceList = class(Tl3InterfacedInterfaceList)
   {* Список. }
    public
    // internal methods
      class function IsCacheable: Boolean;
  ...
  TvcmBase = class(_l3UnknownPrim_)
  end;{ Базовый класс для создания иерархии классов библиотеки VCM. }

  TvcmCacheableBase = class(TvcmBase)
  end;//TvcmCacheableBase

  TvcmWString = class(TvcmCacheableBase, Il3CString)
    private
    // interfnal fields
      f_St       : PAnsiChar;
      f_Len      : Integer;
    ...
    // property methods
      ...
      procedure pm_SetAsWStr(const aStr: Tl3WString);
        {-}
      function  pm_GetAsString: AnsiString;
      procedure pm_SetAsString(const aStr: AnsiString);
        {-}
    protected
    // internal methods
      procedure Cleanup;
    ...
    // public methods
      constructor Make(const aStr: AnsiString);
        overload;
        {-}
      constructor Make(const aStr: Tl3WString);
        overload;
        {-}
      class function MakeI(const aStr: AnsiString): Il3CString;
        overload;
  ...
  TvcmCWideString = class(TvcmCacheableBase, Il3CString)
    private
    // internal fields
      f_S : Tl3DString;
    protected
    ...
    public
    // public methods
      class function MakeI(const aStr: Tl3DString): Il3CString;
        {-}
  end;//TvcmCWideString

  TvcmList = class(TvcmBase)
    private
    // internal fields
      f_List : TObject;
    protected
    // property methods
      ...
      procedure pm_SetCount(aValue: Integer);
        {-}
      function  pm_GetItem(anIndex: Integer): Pointer;
      procedure pm_SetItem(anIndex: Integer; aValue: Pointer);
        {-}
    protected
    // internal methods
      procedure Cleanup;
    ...
    public
    // public methods
      constructor MakeIUnknown;
        {-}
      constructor MakeLongint;
        {-}
      ...
      function  Add(const anItem: IUnknown): Integer;
        overload;
        {-}
      procedure Insert(anIndex: Integer; const anItem: IUnknown);
        overload;
        {-}
      function  Add(anItem: TClass): Integer;
        overload;
        {-}
      function  Add(anItem: Integer): Integer;
        overload;
        {-}
  ...
  TvcmLongintList = class(TvcmList)
    protected
    // property methods
      function  pm_GetItem(Index: Integer): Integer;
        {-}
      procedure pm_SetItem(Index: Integer; Item: Integer);
        {-}
    public
    // public method
      constructor Make;
        {-}
      constructor MakeSorted;
        {-}
  ...
  TvcmObjectList = class(TvcmList)
    protected
    // property methods
      function  pm_GetItem(Index: Integer): TObject;
        {-}
      procedure pm_SetItem(Index: Integer; Item: TObject);
        {-}
    public
    // public method
      constructor Make;
  ...
  TvcmInterfacePtrList = class(TvcmList)
    protected
    // property methods
      function  pm_GetItem(Index: Integer): IUnknown;
        {-}
      procedure pm_SetItem(Index: Integer; const Item: IUnknown);
        {-}
    public
    // public method
      constructor Make;
        {-}
      procedure Remove(anItem: IUnknown);
        {-}
    public
    // public properties
      property Items[Index: Integer]: IUnknown
        ...
        default;
        {* - элементы списка. }
  end;//TvcmInterfacePtrList

  TvcmClassList = class(TvcmList)
    protected
    // property methods
      function  pm_GetItem(Index: Integer): TClass;
        {-}
      procedure pm_SetItem(Index: Integer; Item: TClass);
        {-}
    public
    // public method
      constructor Make;
  ...
  TvcmInterfaceList = class(TvcmList)
    protected
    // property methods
   function pm_GetFirst: IUnknown;
   function pm_GetLast: IUnknown;
   procedure AssignList(const aList: Il3InterfaceList);
      function  pm_GetItems(Index: Integer): IUnknown;
        {-}
      procedure pm_SetItems(Index: Integer; const Item: IUnknown);
        {-}
    public
    // public method
      constructor Make;
        {-}
      procedure Remove(const Item: IUnknown);
        reintroduce;
        {-}
      function  IndexOf(anItem: IUnknown): Integer;
        reintroduce;
        {-}
    public
    // public properties
...
{$IfNDef vcmNeedL3}
function vcmUse(O: TObject): Pointer;
  {-}
var
 Obj : Pointer;
begin
 if (O <> nil) then O.GetInterface(IUnknown, Obj);
 Result := O;
end;
{$EndIf  vcmNeedL3}
...
begin
 ...
 {$Else  vcmNeedL3}
 Result := (I1 = I2);
 if not Result AND (I1 <> nil) AND (I2 <> nil) then begin
  I1.QueryInterface(IUnknown, iu1);
  try
   I2.QueryInterface(IUnknown, iu2);
   try
    Result := (iu1 = iu2);
   finally
    iu2 := nil;
...
// start class TvcmList

constructor TvcmList.MakeIUnknown;
  {-}
begin
 inherited Create;
...
constructor TvcmList.MakeClass;
  {-}
begin
 inherited Create;
 f_List := TList.Create;
...
constructor TvcmList.MakeLongint;
  {-}
begin
 inherited Create;
 f_List := TList.Create;
...
function TvcmList.Add(const anItem: IUnknown): Integer;
  //overload;
  {-}
begin
 Result := (f_List As TInterfaceList).Add(anItem);
end;

procedure TvcmList.Insert(anIndex: Integer; const anItem: IUnknown);
  //overload;
  {-}
begin
 (f_List As TInterfaceList).Insert(anIndex, anItem);
end;

function TvcmList.Add(anItem: TClass): Integer;
  //overload;
  {-}
begin
 Result := (f_List As TList).Add(anItem);
end;

function TvcmList.Add(anItem: Integer): Integer;
  //overload;
  {-}
begin
 Result := (f_List As TList).Add(Pointer(anItem));
end;

procedure TvcmList.Delete(anIndex: Integer);
  {-}
begin
 if (f_List Is TList) then
  TList(f_List).Delete(anIndex)
 else if (f_List Is TInterfaceList) then
  TInterfaceList(f_List).Delete(anIndex);
end;

function TvcmList.IndexOf(anItem: Pointer): Integer;
  {-}
begin
 if (f_List Is TList) then
  Result := TList(f_List).IndexOf(anItem)
 else if (f_List Is TInterfaceList) then
  Result := TInterfaceList(f_List).IndexOf(IUnknown(anItem))
 else
  Result := -1;
end;

procedure TvcmList.Remove(anItem: Pointer);
  {-}
var
 l_Index : Integer;  
begin
...
function TvcmList.Empty: Boolean;
  {-}
begin
 Result := (Self = nil) OR (Count <= 0);
end;

function TvcmList.Lo: Integer;
  {-}
begin
 Result := 0;
end;

function TvcmList.Hi: Integer;
  {-}
begin
 Result := Pred(Count);
end;

procedure TvcmList.Clear;
  {-}
begin
 if (f_List Is TList) then
  TList(f_List).Clear
 else if (f_List Is TInterfaceList) then
  TInterfaceList(f_List).Clear;
end;
  
function TvcmList.pm_GetCount: Integer;
  {-}
begin
 if (f_List Is TList) then
  Result := TList(f_List).Count
 else if (f_List Is TInterfaceList) then
  Result := TInterfaceList(f_List).Count
 else
  Result := 0;
end;

procedure TvcmList.pm_SetCount(aValue: Integer);
begin
 if (f_List Is TList) then
  TList(f_List).Count := aValue
 else if (f_List Is TInterfaceList) then
  TInterfaceList(f_List).Count := aValue;
end;

function TvcmList.pm_GetItem(anIndex: Integer): Pointer;
  {-}
begin
 if (f_List Is TList) then
  Result := TList(f_List).Items[anIndex]
 else if (f_List Is TInterfaceList) then
  Result := Pointer(TInterfaceList(f_List).Items[anIndex])
 else
  Result := nil;
end;

procedure TvcmList.pm_SetItem(anIndex: Integer; aValue: Pointer);
begin
 if (f_List Is TList) then
  Items[anIndex] := aValue
 else if (f_List Is TInterfaceList) then
  TInterfaceList(f_List).Items[anIndex] := IUnknown(aValue);
end;
{$EndIf vcmNeedL3}
...
constructor TvcmWString.Make(const aStr: Tl3WString);
  //overload;
  {-}
begin
 Create;
...
function TvcmWString.pm_GetAsWStr: Tl3WString;
  {-}
begin
 if (Self = nil) then
 ...
 begin
  Result.S := f_St;
  Result.SLen := f_Len;
  Result.SCodePage := f_CodePage;
 end;//Self = nil
end;

procedure TvcmWString.pm_SetAsWStr(const aStr: Tl3WString);
  {-}
var
 l_P  : PAnsiChar;
 l_Sz : Integer;
begin
 f_CodePage := aStr.SCodePage;
 if (aStr.S <> nil) AND (aStr.SLen > 0) then
 begin
  f_Len := aStr.SLen;
  l_P := f_St;
  l_Sz := f_Len + 1;
  ...
  if (f_CodePage = CP_Unicode) then
   l_Sz := l_Sz * SizeOf(WideChar);
  Move(aStr.S^, f_St^, l_Sz);
  vcmFreeMem(l_P);
 end//aStr.S <> nil
 else
 begin
...
function TvcmWString.pm_GetAsString: AnsiString;
  {-}
begin
 if (Self = nil) then
  Result := ''
 else
  Result := vcmStr(AsWStr);
end;

procedure TvcmWString.pm_SetAsString(const aStr: AnsiString);
  {-}
begin
 AsWStr := vcmWStr(aStr);
end;
...
constructor TvcmLongintList.Make;
  {-}
begin
 MakeLongint;
end;

constructor TvcmLongintList.MakeSorted;
  {-}
begin
 Make;
 // !STUB!
...
procedure TvcmLongintList.Remove(Item: Integer);
begin
 inherited Remove(Pointer(Item));
end;

function TvcmLongintList.pm_GetItem(Index: Integer): Integer;
  {-}
begin
 Result := Integer(inherited Items[Index]);
end;

procedure TvcmLongintList.pm_SetItem(Index: Integer; Item: Integer);
  {-}
begin
 inherited Items[Index] := Pointer(Item);
end;

// start class TvcmObjectList

constructor TvcmObjectList.Make;
  {-}
begin
 MakeLongint;
end;

function TvcmObjectList.pm_GetItem(Index: Integer): TObject;
  {-}
begin
 Result := TObject(inherited Items[Index]);
end;

procedure TvcmObjectList.pm_SetItem(Index: Integer; Item: TObject);
  {-}
begin
 inherited Items[Index] := Pointer(Item);
end;

// start class TvcmInterfacePtrList

constructor TvcmInterfacePtrList.Make;
  {-}
begin
 MakeLongint;
end;

function TvcmInterfacePtrList.pm_GetItem(Index: Integer): IUnknown;
  {-}
begin
 Result := IUnknown(inherited Items[Index]);
end;

procedure TvcmInterfacePtrList.pm_SetItem(Index: Integer; const Item: IUnknown);
  {-}
begin
 inherited Items[Index] := Pointer(Item);
end;

procedure TvcmInterfacePtrList.Remove(anItem: IUnknown);
  {-}
begin
 inherited Remove(Pointer(anItem));
end;

// start class TvcmClassList

constructor TvcmClassList.Make;
  {-}
begin
 MakeClass;
end;

function TvcmClassList.pm_GetItem(Index: Integer): TClass;
  {-}
begin
 Result := TClass(inherited Items[Index]);
end;

procedure TvcmClassList.pm_SetItem(Index: Integer; Item: TClass);
  {-}
begin
 inherited Items[Index] := Pointer(Item);
end;
...
// start class TvcmInterfaceList

{$IfDef vcmNeedL3}

class function TvcmInterfaceList.IsCacheable: Boolean;
  //override;
  {-}
begin
 Result := true;
...
constructor TvcmInterfaceList.Make;
  {-}
begin
 MakeIUnknown;
end;

procedure TvcmInterfaceList.Remove(const Item: IUnknown);
begin
 inherited Remove(Pointer(Item));
end;

function TvcmInterfaceList.IndexOf(anItem: IUnknown): Integer;
  //reintroduce;
  {-}
begin
 Result := inherited IndexOf(Pointer(anItem));
end;

function TvcmInterfaceList.pm_GetFirst: IUnknown;
  {-}
begin
 Result := pm_GetItems(0);
end;

function TvcmInterfaceList.pm_GetLast: IUnknown;
begin
 Result := pm_GetItems(Count - 1);
end;

procedure TvcmInterfaceList.AssignList(const aList: Il3InterfaceList);
var
 i   : Integer;
 l_C : Integer;
begin
...
function TvcmInterfaceList.pm_GetItems(Index: Integer): IUnknown;
  {-}
begin
 Result := IUnknown(inherited Items[Index]);
end;

procedure TvcmInterfaceList.pm_SetItems(Index: Integer; const Item: IUnknown);
  {-}
begin
 inherited Items[Index] := Pointer(Item);
end;
...
begin
 ...
 if (aLen <> vcmLen(aStr)) then
 begin
  l_S := vcmStr(aStr);
  SetLength(l_S, aLen);
  aStr := vcmCStr(l_S);
 end;//aLen <> vcmLen(aStr)
 {$EndIf vcmNeedL3}
end;
...
begin
 ...
 Result := l3CStr(anID);
 {$Else  vcmNeedL3}
 Result := TvcmCWideString.MakeI(anID);
 {$EndIf vcmNeedL3}
end;
...
begin
 ...
 {$Else  vcmNeedL3}
 Result.S := PAnsiChar(aString);
 Result.SLen := Length(aString);
...
begin
 ...
 Result := l3PCharLen(aString);
 {$Else  vcmNeedL3}
 Result.S := PAnsiChar(PWideChar(aString));
 Result.SLen := Length(aString);
 Result.SCodePage := CP_Unicode;
 {$EndIf vcmNeedL3}
end;
...
begin
 ...
 Result := l3Str(aString);
 {$Else  vcmNeedL3}
 if (aString.SCodePage = CP_Unicode) then
 begin
  Result := PWideChar(aString.S);
  SetString(Result, PAnsiChar(Result), aString.SLen);
 end//aString.SCodePage = CP_Unicode
 else
  SetString(Result, aString.S, aString.SLen);
 {$EndIf vcmNeedL3}
end;
...
begin
 ...
 Result := (aString = nil);
 if not Result then
  with aString.AsWStr do
   Result := (S = nil) OR (SLen = 0);
 {$EndIf vcmNeedL3}
end;
...
begin
 ...
 Result := l3Cat([aS1, aS2]);
 {$Else  vcmNeedL3}
 Result := vcmCStr(vcmStr(aS1) + vcmStr(aS2));
 {$EndIf vcmNeedL3}
end;
...
begin
 ...
 Result := l3Fmt(anID, aParams);
 {$Else  vcmNeedL3}
 Result := vcmCStr(Format(vcmStr(anID), aParams));
 {$EndIf vcmNeedL3}
end;
...
begin
 ...
 {$Else  vcmNeedL3}
 if aIgnoreCase then
  Result := (AnsiCompareText(S1, vcmStr(S2)) = 0)
 else
  Result := (S1 = vcmStr(S2));
 {$EndIf vcmNeedL3}
end;
...
begin
 ...
 {$Else  vcmNeedL3}
 if aIgnoreCase then
  Result := (AnsiCompareText(vcmStr(S1), S2) = 0)
 else
  Result := (vcmStr(S1) = S2);
 {$EndIf vcmNeedL3}
end;
...
begin
 ...
 {$Else  vcmNeedL3}
 if aIgnoreCase then
  Result := (WideCompareText(vcmStr(S1), S2) = 0)
 else
  Result := (vcmDStr(S1) = S2);
 {$EndIf vcmNeedL3}
end;
...
begin
 ...
 {$Else  vcmNeedL3}
 if aIgnoreCase then
  Result := (AnsiCompareText(vcmStr(S1), vcmStr(S2)) = 0)
 else
  Result := (vcmStr(S1) = vcmStr(S2));
 {$EndIf vcmNeedL3}
end;
...
begin
 ...
 {$Else  vcmNeedL3}
 Result := vcmCStr(System.Copy(vcmStr(aStr), anIndex + 1, aCount));
 {$EndIf vcmNeedL3}
end;
...
function vcmReplaceEnters(const aString   : IvcmCString): IvcmCString;
 ...
 function lp_Replace(const aStr: AnsiString): AnsiString;
 var
  l_IDX: Integer;
  l_Result: AnsiString;
 begin
  Result := aStr;
  for l_IDX := 1 to Length(Result) do
   if Result[l_IDX] in [#13, #10] then
    Result[l_IDX] := ' ';
  repeat
   l_Result := Result;
   Result := StringReplace(Result, '  ', ' ',[rfReplaceAll]);
  until l_Result = Result;
 end;
...
begin
 ...
 {$Else  vcmNeedL3}
 Result := vcmCStr(lp_Replace(vcmStr(aString)));
 {$EndIf vcmNeedL3}
end;
...
// start class TvcmCWideString

class function TvcmCWideString.MakeI(const aStr: Tl3DString): Il3CString;
  {-}
var
 l_S : TvcmCWideString;
begin
...
procedure TvcmCWideString.Cleanup;
  //override;
  {-}
begin
 f_S := '';
...
function TvcmCWideString.pm_GetAsWStr: Tl3WString;
  {-}
begin
 Result := vcmWStr(f_S);  
end;//TvcmCWideString
