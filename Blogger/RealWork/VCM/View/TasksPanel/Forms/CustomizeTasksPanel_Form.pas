...
{ Начат      : 26.02.2007               }
{ $Id: CustomizeTasksPanel_Form.pas,v 1.9 2013/08/05 12:21:56 morozov Exp $ }
...
type
  TvcmCustomizeTasksPanelForm = class(TvcmEntityFormRef)
  ...
  private
  // fields
    f_DeleteGroups     : TvcmInterfaceList;
    f_DeleteOperations : TvcmInterfaceList;
    f_TasksPanel       : IvcmCustOps;
    f_IsChanged        : Boolean;
    f_UserTypeDescrList: TvcmUserTypeDescrList;
  private
  // property methods
    function pm_GetDeleteGroups: TvcmInterfaceList;
      {-}
    function pm_GetDeleteOperations: TvcmInterfaceList;
      {-}
  ...
  private
  // properties
    property DeleteGroups: TvcmInterfaceList
      read pm_GetDeleteGroups;
      {-}
    property DeleteOperations: TvcmInterfaceList
  ...
  // methods
    procedure Cleanup;
      override;
      {-}
  end;//TvcmCustomizeTasksPanelForm

  TCustomizeTasksPanelForm = TvcmCustomizeTasksPanelForm;
...
function TvcmCustomizeTasksPanelForm.tvRepOpsGetItemImage(Sender: TObject;
...
procedure TvcmCustomizeTasksPanelForm.cbGroupNameChange(Sender: TObject);
begin
 UpdateState;
end;//cbGroupNameChange

procedure TvcmCustomizeTasksPanelForm.btnAddGroupClick(Sender: TObject);
var
 l_Root : Il3Node;
 l_Node : Il3Node;
begin
 if (cbGroupName.Text <> '') and (tvRepGroups.TreeStruct <> nil) and
...
procedure TvcmCustomizeTasksPanelForm.tvRepGroupsMakeTreeSource(
...
procedure TvcmCustomizeTasksPanelForm.btnChangeGroupClick(Sender: TObject);
begin
 with tvRepGroups do
 begin
...
procedure TvcmCustomizeTasksPanelForm.UpdateState;
begin
 with cbGroupName do
 begin
  btnAddGroup.Enabled := Text <> '';
  btnChangeGroup.Enabled := (tvRepGroups.CurrentCNode <> nil) and (Text <> '');
 end;
...
procedure TvcmCustomizeTasksPanelForm.tvRepGroupsCurrentChanged(
...
procedure TvcmCustomizeTasksPanelForm.btnDeleteGroupClick(Sender: TObject);
var
...
function TvcmCustomizeTasksPanelForm.pm_GetDeleteOperations: TvcmInterfaceList;
  {-}
begin
 if f_DeleteOperations = nil then
  f_DeleteOperations := TvcmInterfaceList.Make;
 Result := f_DeleteOperations;
end;//pm_GetDeleteOperations

function TvcmCustomizeTasksPanelForm.pm_GetDeleteGroups: TvcmInterfaceList;
begin
 if f_DeleteGroups = nil then
  f_DeleteGroups := TvcmInterfaceList.Make;
 Result := f_DeleteGroups;
end;//pm_GetDeleteGroups

procedure TvcmCustomizeTasksPanelForm.Cleanup;
begin
 g_TasksPanel := nil;
...
procedure TvcmCustomizeTasksPanelForm.btOkClick(Sender: TObject);
begin
 Save;
end;//btOkClick

procedure TvcmCustomizeTasksPanelForm.btApplyClick(Sender: TObject);
begin
 Save;
end;//btApplyClick

function TvcmCustomizeTasksPanelForm.FindRepGroupOps(const aGroup: IvcmCustOpsRepGroupOperations): Il3Node;
  {-}
...
procedure TvcmCustomizeTasksPanelForm.Save;
 ...
 begin
  for l_Index := 0 to Pred(DeleteGroups.Count) do
   if Supports(DeleteGroups.Items[l_Index], IvcmCustOpsRepGroup, l_Group) then
   try
 ...
 begin
  ...
  for l_Index := 0 to Pred(DeleteOperations.Count) do
   if Supports(DeleteOperations.Items[l_Index], IvcmCustOpsGroupOperation, l_Op) then
   try
    if l_Op <> nil then
     l_Op.Delete;
   finally
...
procedure TvcmCustomizeTasksPanelForm.btnMoveGroupToPanelClick(Sender: TObject);
var
 l_Dest     : Il3Node;
 l_RepGroup : ItpRepGroupNode;
 l_Node     : Il3Node;
...
procedure TvcmCustomizeTasksPanelForm.tvRepOpsCurrentChanged(
  Sender: TObject; aNewCurrent, aOldCurrent: Integer);
begin
 UpdateState;
end;//tvRepOpsCurrentChanged

procedure TvcmCustomizeTasksPanelForm.UpdateControls;
var
...
procedure TvcmCustomizeTasksPanelForm.tvTasksPanelCurrentChanged(
  Sender: TObject; aNewCurrent, aOldCurrent: Integer);
begin
 UpdateControls;
 UpdateState;
...
procedure TvcmCustomizeTasksPanelForm.btnMoveOpToPanelClick(
  Sender: TObject);
var
 l_Current   : Il3Node;
 l_DestGroup : Il3Node;
...
function TvcmCustomizeTasksPanelForm.tvTasksPanelGetItemImage(
...
procedure TvcmCustomizeTasksPanelForm.btnChangeItemClick(Sender: TObject);
begin
 with tvTasksPanel do
 begin
  if CurrentCNode <> nil then
...
procedure TvcmCustomizeTasksPanelForm.btnMoveGroupToRepClick(
  Sender: TObject);
begin
...
procedure TvcmCustomizeTasksPanelForm.RemoveNode(const aNode: Il3Node);
var
 l_Parent: Il3Node;
begin
...
procedure TvcmCustomizeTasksPanelForm.ReturnRepOp(const aRepOp: IvcmCustOpsRepOperation);
var
 l_Group: Il3Node;
begin
...
procedure TvcmCustomizeTasksPanelForm.DeleteOp(const aNode: Il3Node);
var
 l_Op: ItpGroupOperation;
begin
...
procedure TvcmCustomizeTasksPanelForm.RemoveGroup(const aGroup: Il3Node);
...
function TvcmCustomizeTasksPanelForm.GetSelectedUserType: IvcmUserTypeDef;
...
procedure TvcmCustomizeTasksPanelForm.btnMoveOpToRepClick(Sender: TObject);
begin
 with tvTasksPanel do
 begin
  // Удалить группу:
...
procedure TvcmCustomizeTasksPanelForm.cbForAllUseCasesClick(
  Sender: TObject);
var
 l_Op: ItpGroupOperation;
begin
...
procedure TvcmCustomizeTasksPanelForm.UpdatePositions(const aParent: Il3Node);
...
procedure TvcmCustomizeTasksPanelForm.MoveTasksPanelItem(const aUp        : Boolean;
                                                         const aNewParent : Il3Node);
const
...
procedure TvcmCustomizeTasksPanelForm.UpBitBtnClick(Sender: TObject);
begin
 MoveTasksPanelItem(True, tvTasksPanel.CurrentCNode.ParentNode.PrevNode);
 IsChanged := True;
end;//UpBitBtnClick

procedure TvcmCustomizeTasksPanelForm.DownBitBtnClick(Sender: TObject);
begin
...
function TvcmCustomizeTasksPanelForm.pm_GetTasksPanel: IvcmCustOps;
begin
 Result := f_TasksPanel;
end;

procedure TvcmCustomizeTasksPanelForm.pm_SetTasksPanel(
...
procedure TvcmCustomizeTasksPanelForm.cbFormsChange(Sender: TObject);
var
 l_NeedFree : Boolean;
...
begin
 ...
 begin
  if (vcmShowMessageDlg(Tl3Message_C(l3CStr(lblConfirmSave.Caption),
                                     'TvcmCustomizeTasksPanelForm.cbFormsChange',
                                     mtInformation,
...
procedure TvcmCustomizeTasksPanelForm.vcmEntityFormInit;
begin
 IsChanged := False;
end;

procedure TvcmCustomizeTasksPanelForm.pm_SetIsChanged(
  const Value: Boolean);
begin
 f_IsChanged := Value;
end;
