...
{ $Id: CustomizeTools_Form.pas,v 1.22 2013/08/08 14:37:12 morozov Exp $ }
...
// new: protected property TvcmCustomizeToolsForm.RestoreBtnVisible;
...
// new: method TvcmCustomizeToolsForm.LoadGlyphSize;
...
// bugfix: уничтожаем форму vcmFree, а не Free(наследие того, что форма была не vcm'ная)
...
// - bug fix: TvcmCustomizeToolsForm сделан наследником TvcmEntityForm (CQ OIT5-6447).
//
...
uses
  ...
  ctButtonEdit, ctAbstractEdit, absdrop, treedrop, EditableBox, FakeBox,
  OvcBase,
  vcmToolbarMenuRes,
  vcmMenuManager,
  vcmUserTypeDescr
...
type
  ...
  TvcmCustomizeToolsForm = class(TvcmEntityFormRef)
  ...
  public
    ...
    class function Execute(aMenuManager : TvcmBaseMenuManager;
      ...
      {-}
  end;//TvcmCustomizeToolsForm

  TCustomizeToolsForm = TvcmCustomizeToolsForm;
...
// start class TvcmCustomizeToolsForm

procedure TvcmCustomizeToolsForm.AddChangedShortCut(aAction : TvcmAction);
begin
...
procedure TvcmCustomizeToolsForm.MoveSelected(aTo, aFrom : TvtCustomOutliner);
var
...
procedure TvcmCustomizeToolsForm.MoveSelectedA(aList : TvtCustomOutliner; aMoveUp : Boolean);
var
...
function TvcmCustomizeToolsForm.AddShortCut(const aAction : IActionNode;
                                            aShortCut     : TShortCut;
...
procedure TvcmCustomizeToolsForm.UpdateShortCut(const aActionNode : IActionNode);
var
...
constructor TvcmCustomizeToolsForm.Create(anOwner: TComponent);
  //override;
  {* - создает форму. }
begin
...
class function TvcmCustomizeToolsForm.Execute(aMenuManager : TvcmBaseMenuManager;
                                               aDock        : TComponent): Boolean;
  {-}
var
 l_Form: TvcmCustomizeToolsForm;
begin
...
procedure TvcmCustomizeToolsForm.TuneBoxes;
  {-}
begin
...
procedure TvcmCustomizeToolsForm.FillObjects(aMenuManager    : TvcmBaseMenuManager;
...
procedure TvcmCustomizeToolsForm.SetForm(aForm         : TvcmEntityForm;
                                         aToolbar      : TComponent;
                                         aNeedFreeForm : Boolean);
  {-}
var
...
procedure TvcmCustomizeToolsForm.FillSelected;
...
procedure TvcmCustomizeToolsForm.FillAvalailable;
  {-}
begin
 FillOperations(f_AvailableOperationsList, true, false, false);
end;

procedure TvcmCustomizeToolsForm.LoadGlyphSize;
var
...
procedure TvcmCustomizeToolsForm.FillAll;
  {-}
begin
 f_IgnoreModifications := True;
 try
...
procedure TvcmCustomizeToolsForm.FillOperations(aTarget : TvtCustomOutliner;
                                                aAvalailableOnly: Boolean;
                                                aResetShortcuts: boolean;
                                                aFillShortcuts: boolean);
...
procedure TvcmCustomizeToolsForm.SaveToolbars;
var
 l_DockName: string;
  {-}
...
begin//SaveToolbars
 ...
 begin
  ...
  try
   ...
   (* Размер иконок *)
   (g_MenuManager as TvcmCustomMenuManager).GlyphSize := GetCheckedGlyphSize;
   (g_MenuManager as TvcmCustomMenuManager).StoreGlyphSize;
   (* Глубина цвета иконок *)
   (g_MenuManager as TvcmCustomMenuManager).GlyphColordepth := GetCheckedColorDepth;
...
function TvcmCustomizeToolsForm.ExecutePrim(aMenuManager : TvcmBaseMenuManager;
                                             aToolbar     : TComponent): Boolean;
  {-}
var
...
procedure TvcmCustomizeToolsForm.AddBitBtnClick(Sender: TObject);
begin
 MoveSelected(f_SelectedOperationsList, f_AvailableOperationsList);
end;

procedure TvcmCustomizeToolsForm.RemoveBitBtnClick(Sender: TObject);
begin
 MoveSelected(f_AvailableOperationsList, f_SelectedOperationsList);
end;

procedure TvcmCustomizeToolsForm.UpBitBtnClick(Sender: TObject);
begin
 MoveSelectedA(f_SelectedOperationsList, true);
 UpdateEnableFlags;
end;

procedure TvcmCustomizeToolsForm.DownBitBtnClick(Sender: TObject);
begin
 MoveSelectedA(f_SelectedOperationsList, false);
 UpdateEnableFlags;
end;

function TvcmCustomizeToolsForm.pm_GetModified: Boolean;
  {-}
begin
 Result := f_Modified;
end;

function TvcmCustomizeToolsForm.pm_GetRestoreBtnVisible : Boolean;
{-}
begin
 Result := ResetToDefaultButton.Visible;
end;

procedure TvcmCustomizeToolsForm.pm_SetRestoreBtnVsible(const aValue : Boolean);
{-}
begin
...
procedure TvcmCustomizeToolsForm.pm_SetModified(aValue: Boolean);
  {-}
begin
 f_Modified := aValue;
...
procedure TvcmCustomizeToolsForm.UpdateEnableFlags;
var
 l_Node   : IActionNode;
...
procedure TvcmCustomizeToolsForm.f_AvailableOperationsDblClick(Sender: TObject; Index : Longint);
begin
 MoveSelected(f_SelectedOperationsList, f_AvailableOperationsList);
end;

procedure TvcmCustomizeToolsForm.f_SelectedOperationsDblClick(Sender: TObject; Index : Longint);
begin
 MoveSelected(f_AvailableOperationsList, f_SelectedOperationsList);
end;

procedure TvcmCustomizeToolsForm.cbObjectChange(Sender: TObject);
begin
 ...
 begin
  if vcmShowMessageDlg(Tl3Message_C(l3CStr(ConfirmSavaLabel.Caption),
                                    'TvcmCustomizeToolsForm.cbObjectChange',
                                    mtConfirmation,
...
procedure TvcmCustomizeToolsForm.ReloadData;
var
...
procedure TvcmCustomizeToolsForm.ResetToDefaultButtonClick(
  Sender: TObject);
begin
 ReloadData;
end;

procedure TvcmCustomizeToolsForm.btApplyClick(Sender: TObject);
begin
 SaveToolbars;
end;

procedure TvcmCustomizeToolsForm.cbToolBarChange(Sender: TObject);
var
 l_Pos : TvcmEffectiveToolBarPos;
begin
...
procedure TvcmCustomizeToolsForm.ShortCutEditChange(Sender: TObject);
var
 l_Node   : IActionNode;
 l_Result : Boolean;
begin
...
procedure TvcmCustomizeToolsForm.AllOperationsChange(Sender : TObject; aNewCurrent : Longint;  aOldCurrent : Longint);
begin
 UpdateShortCutItemState;
end;

procedure TvcmCustomizeToolsForm.AddButtonClick(Sender: TObject);
var
 l_Node : IActionNode;
begin
 l_Node := f_AllOperationsList.CurrentCNode as IActionNode;
...
procedure TvcmCustomizeToolsForm.UpdateShortCutItemState;
var
 i        : Integer;
 l_Node   : IActionNode;
 l_Action : TvcmAction;
...
procedure TvcmCustomizeToolsForm.lwShortCutsChange(Sender: TObject;
...
procedure TvcmCustomizeToolsForm.RemoveButtonClick(Sender: TObject);
var
 i         : Integer;
 l_Node    : IActionNode;
 l_Action  : TvcmAction;
...
procedure TvcmCustomizeToolsForm.WhotShowRadioGroupClick(Sender: TObject);
Var
 l_Node : IActionNode;
begin
 l_Node := f_SelectedOperationsList.CurrentCNode as IActionNode;
...
procedure TvcmCustomizeToolsForm.Cleanup;
begin
 inherited;
...
procedure TvcmCustomizeToolsForm.ResetAllToDefaultButtonClick(Sender: TObject);
Var
 l_OldCursor    : TCursor;
 l_Index        : integer;
 ...
 try
  ...
  with (f_MenuManager As TvcmCustomMenuManager).UserTypes do
   for l_Index := 0 to Hi do
    with IvcmUserTypeDef(Items[l_Index]) do
    begin
     l_Form := vcmFormCustomize.MakeVirtualForm(IvcmUserTypeDef(Items[l_Index]), l_NeedFreeForm);
     for l_Pos := Low(TvcmEffectiveToolBarPos) to High(TvcmEffectiveToolBarPos) do
     begin
     ...
     if l_NeedFreeForm then
      l_Form.SafeClose;
    end;//with IvcmUserTypeDef(Items[l_Index])
...
procedure TvcmCustomizeToolsForm.ShowUserType(const aUserType     : IvcmUserTypeDef;
...
procedure TvcmCustomizeToolsForm.ClearShortcutButtonClick(Sender: TObject);
begin
 ShortCutEdit.Text := '';
 ShortCutEdit.Brush.Color := clWindow; 
end;

function TvcmCustomizeToolsForm.GetListImage(Sender : TObject; Index : Longint; var aImages : TCustomImageList):
    Integer;
var
 l_Node: IActionNode;
begin
...
procedure TvcmCustomizeToolsForm.OperationsBeginDrag(const aNode: Il3SimpleNode; var aOkEffect: Longint);
begin
 //
end;

function TvcmCustomizeToolsForm.SelectedDragAcceptData(const aData: IDataObject): Boolean;
begin
 Result := True;
end;

procedure TvcmCustomizeToolsForm.OpListChange(Sender : TObject; aNewCurrent : Longint;  aOldCurrent : Longint);
begin
 UpdateEnableFlags;
end;

function TvcmCustomizeToolsForm.AvailableDragAcceptData(const aData: IDataObject): Boolean;
var
 lNode: Il3Node;
begin
 Result := False;
...
function TvcmCustomizeToolsForm.Drop2AvailOperations(const aData  : Il3SimpleNode;
                                                     aWantMenu    : Boolean;
                                                     var dwEffect : Longint): Boolean;
var
 lNode: Il3Node;
...
function TvcmCustomizeToolsForm.Drop2SelectedOperations(const aData  : Il3SimpleNode;
                                                        aWantMenu    : Boolean;
                                                        var dwEffect : Longint): Boolean;
var
...
procedure TvcmCustomizeToolsForm.vcmEntityFormResize(Sender: TObject);
var
 l_GroupWidth,
 l_GroupOffset,
...
procedure TvcmCustomizeToolsForm.rgGlyphSizeClick(Sender: TObject);
begin
 if not f_IgnoreModifications then
  Modified := True;
end;

procedure TvcmCustomizeToolsForm.rgGlyphColorClick(Sender: TObject);
begin
 if not f_IgnoreModifications then
  Modified := True;
end;

procedure TvcmCustomizeToolsForm.LoadGlyphColordepth;
var
 l_GlyphColorDepth : TvcmGlyphColorDepth;
begin
 l_GlyphColorDepth := (f_MenuManager as TvcmCustomMenuManager).GlyphColordepth;
...
procedure TvcmCustomizeToolsForm.vcmEntityFormRefCreate(Sender: TObject);
begin
 IconSizeComboBox.Visible := False;
 IconSizeComboBox.Text := l3CStr('крупный');
end;

function TvcmCustomizeToolsForm.GetCheckedColorDepth: TvcmGlyphColorDepth;
begin
...
function TvcmCustomizeToolsForm.GetSelectedUserType: IvcmUserTypeDef;
// http://mdp.garant.ru/pages/viewpage.action?pageId=460277543
const
 c_DividerDash = ' - ';
var
...
function TvcmCustomizeToolsForm.GetCheckedGlyphSize: TvcmGlyphSize;
begin
