...
//#UC END# *4FD0A98C01BDci*
 ...
 // overridden public methods
   ...
   function ImageExtent(anIndex: Integer): TPoint; override;
    {$IfEnd} //not DesignTimeLibrary AND not XE
    {$If not defined(DesignTimeLibrary) AND not defined(XE)}
...
//#UC START# *4FD095010268_4ACB4A5201FC_impl*
 ...
 begin
  if Enabled then
   ...
   l_CacheTop := Height;
  g_BlendOp.SourceConstantAlpha := CalcTransparenctColor(cFullAlpha, HalfTransparent);
  l_Painted := AlphaBlend(Canvas.Handle,X,Y,Width,Height,f_AlphaDrawCache.Canvas.Handle,Index*Width,l_CacheTop,Width,Height,g_BlendOp);
  g_BlendOp.SourceConstantAlpha := cFullAlpha;
 end;
 if not l_Painted then
 begin
