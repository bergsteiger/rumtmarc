...
// Версия     : $Id: vtShockwaveEx.pas,v 1.44 2012/11/01 09:42:28 lulin Exp $
////////////////////////////////////////////////////////////////////////////////
...
// ПРИ СМЕНЕ ВЕРСИИ ФЛЕША В ОБОЛОЧКЕ ПРОВЕРИТЬ СРАВНЕНИЕ FlashVersion
  if Supports(Obj, IShockwaveFlash, l_Obj) and (l_Obj.FlashVersion <> $90000) then
  begin
...
(*   Width := Max(1024, Trunc(TGetPropertyAsNumber('/', 8)));
  ...
  // http://mdp.garant.ru/pages/viewpage.action?pageId=335151860 
  f_Height := Max(f_Height, Min(5000, LoadedHeight));
  {$IfEnd}
  if (AlignMode = 5) then
   AlignMode := 15;
