...
// $Id: vtForm.pas,v 1.25 2013/04/05 12:02:45 lulin Exp $
...
begin
 ...
 if g_BatchMode then
 begin
  if seExecuteCurrentModalWorker then
  begin
 ...
 if g_BatchMode then
  if (g_AllowModal <= 0) then
   if (Application.MainForm <> nil) then
    if not seHasModalWorker then
     raise EvcmTryEnterModalState.Create('Здесь надо обрабатывать модальность формы');
 {$EndIf InsiderTest}
