...
type
 ...
 ESkipOperation = class(Exception)
 end;//ESkipOperation
...
//#UC START# *508F825303E4_4CFFBEEA0109_impl*
 ...
 inherited DoOnGetItemFont(aItemIndex, aFont);
...
//#UC START# *51629F26036A_4CFFBEEA0109_impl*
 ...
 if (ClientWidth = 0) then
  Exit;
 CR := Rect(0, 0, ClientWidth, MaxInt);
 l_Canvas := l3CrtIC;
 ...
 begin
  ...
  l_Canvas.Font.AssignFont(Font);
  DoOnGetItemFont(aItemIndex, l_Canvas.Font);
...
var
 ...
 begin
  ...
  if (N = Current) and (voShowSelectAsArrow in f_ViewOptions) then
  begin
   lRegion := Tl3Region.Create;
   try
    with CRFull do
