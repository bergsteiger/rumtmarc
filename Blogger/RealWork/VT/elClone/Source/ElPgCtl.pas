...
// $Id: ElPgCtl.pas,v 1.79 2013/04/05 12:02:49 lulin Exp $
...
type
  ...
  TElTabSheet = class(TafwCustomCommonTextControl, IafwWeightListner)
  private
  // messages
  ...
  begin
   if (ControlCount = 1) then
    if (Controls[0] Is TWinControl) then
     ...
     begin
      if (Controls[0].Width <> ClientWidth) then
       SetWindowPos(TWinControl(Controls[0]).Handle, 0,
       ...
       SetWindowPos(TWinControl(Controls[0]).Handle, 0,
                    ...
                    SWP_FRAMECHANGED + SWP_NOZORDER + SWP_NOACTIVATE);
      TWinControl(Controls[0]).Realign;
