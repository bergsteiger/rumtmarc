...
{ $Id: m4DB.pas,v 1.25 2013/04/12 16:25:08 lulin Exp $ }
...
// - теперь метод Im3DB.Start имеет два ОБЯЗАТЕЛЬНЫХ параметра - режимы открытия частей.
//
...
class function Tm4DB.Make(const aBaseName   : AnsiString;
  ...
  {-}
begin
 Result := inherited Make(aBaseName, aOnYield,
                          aFileMeter, aFilesMeter,
                          aFilesProcessed, aFilesProcessedEx) As Im4DB;
end;
...
begin
 ...
 //  l_IndexExt := '_back' + l_IndexExt;
  ...
  if not f_FakeIndex AND (f_TextIndex = nil) then
   f_TextIndex := Tm3FullModeStorageManager.
...
var
 ...
 var
  ...
  function DoWord(const aStoreInfo : Tm3StoreInfo; const aName : Tl3PCharLen): Bool;
  ...
  begin//DoWord
   if Stopped then
    ...
    m2COMWriteBuffer(l_HashInfo, PWideChar(aName.S)^, aName.SLen * SizeOf(WideChar));
    ...
    if Assigned(f_FilesMeter) then
     f_FilesMeter(piCurrent,
                  anIndex,
                  IntToStr(anIndex) + ' ' + l3PCharLen2String(aName, CP_ANSI, @l_ConvertBuff));
    Yield;
    with aStoreInfo do
     l_InWordStream := IStream(
   ...
   if (l_HashWords <> nil) then
    with l_HashWords do
     if BinSearchByKey(PWideChar(aName.S), aName.SLen, l_FoundIndex) then
      Delete(l_FoundIndex);
  end;//DoWord
  ...
  begin//DoNewWord
   if Stopped then
    ...
    l_InWordStream := m3COMOpenStream(l_DeltaHashElement,
                                      g_WideStringManager.StringAsPCharLen(
 ...
 begin//DoHash
  if Stopped then
   ...
   l_HashWords := Tm3HashHandle(l_DeltaWords.Items[anIndex]).Data.BranchHandleList;
   if (anIndex mod 50 = 0) AND Assigned(f_FilesProcessed) then
    f_FilesProcessed(anIndex);
   if (anIndex mod 100 = 0) AND Assigned(f_FilesProcessedEx) then
    f_FilesProcessedEx(anIndex, Cm2HASDefCount, Now - l_Total);
   if Assigned(f_FilesMeter) then
    f_FilesMeter(piCurrent, anIndex, IntToStr(anIndex));
   Yield;
   with aStoreInfo do
    l_InHashElement := Im3IndexedStorage(
  ...
  begin//_NewWord
   ...
   if (aWord^ <> nil) then begin
    with Tm3BranchHandle(aWord^).Data do
     m2COMWriteBuffer(l_HashInfo, Value^, g_WideStringManager.ItemLength(Value) * SizeOf(WideChar));
    m2COMWriteBuffer(l_HashInfo, cc_NullVar, SizeOf(WideChar));
   end;//aWord^ <> nil
  end;//_NewWord
 ...
 begin//_CheckNewHash
  if Stopped then
   ...
   if (aHashWords^ <> nil) then begin
    if (anIndex mod 100 = 0) AND Assigned(f_FilesProcessedEx) then
     f_FilesProcessedEx(anIndex, Cm2HASDefCount, Now - l_NewHashTotal);
    if (Tm3HashHandle(aHashWords^).Data.BranchHandleList.Count > 0) then begin
     {$IfDef m4OutNewHash}
     Gm0EXCLibDefSrv.SaveMessage(Cm0EXCLibINF, 'Появился новый хеш ' + IntToStr(anIndex));
     {$EndIf m4OutNewHash}
     Tm3HashHandle(aHashWords^).Data.BranchHandleList.IterateAllF(l3L2IA(@_NewWord));
     l_DeltaHashElement := m3COMOpenStorage(l_DeltaHash,
                                             anIndex,
                                             m3_saRead,
                                             false);
    ...
    end;//Tm3HashHandle(aHashWords^).Data.BranchHandleList.Count > 0
   end;//aHashWords^ <> nil
  end;//Stopped
 end;//_CheckNewHash
  ...
  begin
   ...
   begin
    ...
    try
     l_ConvertBuff := nil;
     try
      l_InIndex := Tm3ReadModeStorageManager.MakeInterface(l_InIndexName);
      try
       ...
       l_OutIndex := Tm3FullModeStorageManager.MakeInterface(l_UpdateName);
       try
        CreateSummary;
          ...
          try
           ...
           try
            ...
            try
             ...
             {$IfNDef l3ConsoleApp}
             if Assigned(f_FilesMeter) then
              f_FilesMeter(piStart, Cm2HASDefCount, 'Обновление индекса');
             try
             {$EndIf l3ConsoleApp}
             ...
             {$IfNDef l3ConsoleApp}
             if Assigned(f_FilesMeter) then
              f_FilesMeter(piStart, l_DeltaWords.Count, 'Проверка новых хешей');
             try
             {$EndIf l3ConsoleApp}
              l_NewHashTotal := Now;
