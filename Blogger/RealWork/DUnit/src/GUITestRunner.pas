{ $Id: GUITestRunner.pas,v 1.96 2013/06/07 14:03:44 dinishev Exp $ }
{: DUnit: An XTreme testing framework for Delphi programs.
   @author  The DUnit Group.
   @version $Revision: 1.96 $ 2001/03/08 uberto
}
(*
...
uses
  ...
  elCustomButtonEdit, ctButtonEdit, ctAbstractEdit, absdrop, treedrop,
...
type
  ...
  protected
    ...
    function DictionariesPath: string;
    ...
    function SetNodeEnabled(aNode: TTreeNode; aValue: Boolean): Boolean;
...
function TGUITestRunner.SetNodeEnabled(aNode: TTreeNode; aValue: Boolean): Boolean;
var
 l_Test: ITest;
begin
 l_Test := NodeToTest(aNode);
   ...
   begin
    ...
    begin
     ...
     if l_Test.HasScriptChildren then
      ...
      begin
       l_Test.ClearEtalons;
       DeleteEtalons(l_Test.Name, l_Test.GetSubFolder, l_ParentTest <> nil);
...
begin
 with (Sender as TFileOpen) do
 begin
  LoadTestNamesFromFile(Dialog.FileName);
...
function TGUITestRunner.DictionariesPath: string;
begin
 Result := NormalDir(FileFromCurrent(csDictionarySubDir, True, False));
end;
...
begin
 ...
 l_FileName := DictionariesPath + (aSender as TMenuItem).Caption;
 if FileExists(l_FileName)
  then //ShellExecute(0, nil, 'notepad', PChar(l_FileName), PChar(ExtractFilePath(l_FileName)), SW_SHOWNORMAL)
       OpenFile(l_FileName)
  else ShowMessageFmt('Словарь %s не найден. Возможно, он был удалён.'#13#10'Попробуйте перечитать тесты.', [l_FileName]);
end;

procedure TGUITestRunner.LoadDictionariesMenu;
begin
 Dictionaries1.Clear;
 ReadSubDirs(Dictionaries1, csDictionaryExt, csDictionarySubDir, DictionaryMenuItemClick);
end;
...
begin
 ...
 LoadTestNamesFromFile(l_PresetName);
 ChangeState4Test(True, False);
end;
...
begin
 ...
 try
  ProcessDirectory(l_Dir, TFileProcessingFunc(l_FPStub));
 finally
  l3FreeLocalStub(l_FPStub);
 end;
