...
{ $Id: Dt_Equal.pas,v 1.31 2010/09/24 12:11:14 voba Exp $ }
...
Type
 ...
 TEqualItemsTbl = Class(TPrometTbl)
  ...
  public
   Constructor Create(aFamily : TFamilyID; aDLType : TDLType); Reintroduce;
...
constructor TEqualItemsTbl.Create(aFamily : TFamilyID; aDLType : TDLType);
begin
...
begin
 ...
 htSearch(@fSrchList,TmpSab, Handle, aField, EQUAL, @aID, Nil);
 Try
  If aField = eqFID_Fld
   then
...
Begin
 RefreshSrchList;
 l3FillChar(TmpSab,SizeOf(SAB));
 htSearch(@fSrchList,TmpSab,Handle,eqFID_Fld,EQUAL,@aID,Nil);
 Try
  If (TmpSab.gFoundCnt<>0) then
   if StartTA then
    try
 ...
 l3FillChar(TmpSab,SizeOf(SAB));
 htSearch(@fSrchList,TmpSab,Handle,eqSID_Fld,EQUAL,@aID,Nil);
 Try
  If (TmpSab.gFoundCnt<>0) then
   if StartTA then
    try
...
begin
 ...
 try
  RefreshSrchList;
  HtCreateEmptySAB(@fSrchList,lLongSab,Handle,eqFID_Fld,Res_Value);
  try
...
begin
 ...
 try
  RefreshSrchList;
  l_Arr.Capacity := fSrchList.gFoundCnt;
  l_Iter := TValuesIterator.Create(fSrchList, eqOpenFldArr, Self);
  try
   l_ArrIndex := 0;
