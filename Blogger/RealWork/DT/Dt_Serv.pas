...
// - add verload function StartTransaction список таблиц подается как aTables : Il3CBaseList
//
...
// исправлена THTServer.CheckLocalTime
//
...
// в CreateHtEx добавлен вывод исключения в лог
//
...
// изменены методы THTServer.CheckExportResults и THTServer.CheckLocalTime под новый Dt_NetComm
//
...
{+------------------------------------------------------------------------+
 ...
 THTServer = Class(Tl3ChangeNotifier)
  ...
  public
   ...
   (* Работа со списком открытых результатов *)
   ...
   class procedure   SabToList(var aSrcSab: Sab; aDstList: Tl3LongintList);
    // Перенос целочисленных значений из Sab в список
   ...
   property  OnIdle: TOnHtIdleProc read f_OnIdle write f_OnIdle;
                // Часто вызывается HyTech'ом при выполнении операций.
                // Лучше всего использовать для перерисовки окна приложения.
                // При уничтожении объекта (владельца метода) - это свойство надо обнулить!!!
...
begin
 if (GlobalHtServer <> nil) and Assigned(GlobalHtServer.OnIdle) then
  ...
  except // все исключения гасим, чтобы не повредить работу HyTech'а
   on E: Exception do
    l3System.Exception2Log(E);
  end;
end;
...
{$IFNDEF InsiderTest}
 If Not StationConfig.ReadParamStr('StationName', theStationName) and aStandAlone then
  l3System.Msg2Log('Не задно имя станции! Задайте параметр StationName в секции [%s] файла %s.', [BaseSection, StationConfig.GetCfgFile]);
{$ENDIF InsiderTest}

 If Not ServerConfig.ReadParamStr('TablePath',theFolders.TblPath) and aStandAlone  then
  l3System.Msg2Log('Не задан путь к базам! Задайте параметр TablePath в секции [%s] файла %s.', [BaseSection, StationConfig.GetCfgFile])
 else
 begin
  ...
  begin
   l3System.Msg2Log('Неправильно указан путь TblPath : %s', [theFolders.TblPath]);
   exit;
  end;
 end;
 ...
 If Not ServerConfig.ReadParamStr('LockPath', theFolders.LockPath) and aStandAlone  then
  l3System.Msg2Log('Не задан путь к разделяемым ресурсам! Задайте параметр LockPath в секции [%s] файла %s.', [BaseSection, ServerConfig.GetCfgFile])
 else
 begin
...
Begin
 ...
 Begin
  ...
  fpfYield := @fn_OnHtIdle;     (* Функция активации других процессов *)
 ...
 Try
  ...
  htShut;
  raise EHtErrors.CreateFmt('Не удалось открыть таблицу %s', [AnsiString(ControlTblFName)]);
 end;
...
begin
 ...
 if (Abs(DeltaTime)*24*3600) > 10 then
  begin
   ...
   if DeltaTime < 0 then
    DeltaStr:='-'  + DeltaStr;
   l3System.Msg2Log('Время на сервере: ' + DateTimeToStr(PDateTime(aMessage)^) +
                    '; Delta = ' + DeltaStr);
  end;
end;
...
Begin
 ...
 {
 // сначала ищем аналогично GetTblObject
 for I := 0 to fOpenTbls.Count - 1 do
...
begin
 ...
 {$IFDEF TestDb}
  ...
  if not FileExists(l_PathToFlagFile) then
   raise Exception.Create(SysUtils.Format('Работа на БД "%s" запрещена.',
                                          [l_DbPath]));
 {$ENDIF}
  ...
  l3System.Msg2Log('Не найдена папка %s', [IfThen(DirectoryExists(DocBaseFolder), AdminBaseFolder, DocBaseFolder)])
end;
...
begin
 ...
 end (* depricated по самое не балуйся
 else
 if ServerConfig <> nil then
 begin
  SaveSection:=ServerConfig.Section;
