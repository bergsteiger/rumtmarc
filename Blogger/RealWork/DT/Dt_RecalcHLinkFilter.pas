...
{ $Id: Dt_RecalcHLinkFilter.pas,v 1.42 2013/05/13 17:23:52 lulin Exp $ }
...
begin
 ...
 CT := CurrentType;
 if f_InAddress and CT.InheritsFrom(k2_idAddress) and f_InHyperlink then
 begin
  // - закрываем именно адрес ссылки
  l_WasInAddress := true;
  f_InAddress := False;
 end//f_InAddress..
 else if f_InHyperlink and CT.InheritsFrom(k2_idHyperlink) then
 begin
  // - закрываем именно ссылку
  BeforeCloseHyperLink;
  f_InHyperlink := False;
 end//f_InHyperlink
 else if f_InPara and CT.InheritsFrom(k2_idTextPara) then
 begin
...
procedure TRecalcHLinksFilter.StartChild(TypeID: Long);
var
 CT : Tk2Type;
begin
 CT := TypeTable[TypeID];
 if CT.InheritsFrom(k2_idAddress) and f_InHyperlink then
  BeforeOpenAddress;
 ...
 if not f_InPara then
  f_InPara := CT.InheritsFrom(k2_idTextPara)
 else if not f_InHyperlink then
  f_InHyperlink := CT.InheritsFrom(k2_idHyperlink)
 else if not f_InAddress then
  f_InAddress := CT.InheritsFrom(k2_idAddress);
