...
{ $Id: Dt_EGen.pas,v 1.108 2013/07/18 07:29:09 voba Exp $ }
...
Type
 TDocExportGenerator = Class(Tk2CustomReader)
  ...
  protected
   ...
   Procedure   SetBaseOrTopic(aVal : TDiapasonRec);
  ...
  public
   ...
   Procedure   ExportDictionary(aDictType : TDLType; aFile : TFileName;
   ...
   {Array of documents for export = Record : Type + Data;
          Type : None (on initialise), Single (one document),
                 Group (Doc Group), All (Full base), Search (search results)
          Data : ID (of Doc or of Group) or SrchRes (Search)
   }
   Property    Diapason   : TDiapasonRec write SetBaseOrTopic;

   Property    ExportType : TDiapType read fDiapasonType;
...
Uses
     ...
     m2xltlib,
     dt_ImpExpTypes,
     Dt_Doc, Dt_Link, Dt_LinkServ, Dt_Err, Dt_Serv, dt_Log, Dt_BBLog, Dt_Lock,
     l3String, l3Date,
     Dt_Prior, Dt_User,
     dt_DictExport,
     l3FieldSortRecList
...
Begin
 ...
 fDiapasonType:=tdNone;
 fBreakFlag:=False;
 With fCurOpenMode do
  Begin
   openMode:=ROPEN_READ;
   FldArr:=@ExportFldArr;
   Count:=ExportFldCount;
  end;
 fRelated:=False;
...
Procedure TDocExportGenerator.SetBaseOrTopic(aVal : TDiapasonRec);
Begin
 ClearExportData;
 fDiapasonType:=aVal.Typ;
 Case aVal.Typ of
  tdGroup  :
   begin
    fCurDocSAB := MakeSab(LinkServer(fFamily).Links[dlBases]);
    fCurDocSAB.Select(lnkDictIDFld, aVal.ID);
    fCurDocSAB.ValuesOfKey(lnkDocIDFld);
    fCurDocSAB.TransferToPhoto(docIdFld, DocumentServer(fFamily).FileTbl);
    fCurDocSAB.RecordsByKey;
   end;

  tdAll :
   begin
    fCurDocSAB := MakeSab(DocumentServer(fFamily).FileTbl);
    fCurDocSAB.SelectAll;
   end;

  tdNumList :
   begin
    if aVal.ExternalFormat then
    begin
     fCurDocSAB := MakeSab(LinkServer(fFamily).Renum, aVal.List);
     fCurDocSAB.RecordsByKey(rnImportID_fld);
     fCurDocSAB.ValuesOfkey(rnRealID_fld);
     fCurDocSAB.TransferToPhoto(docIdFld, DocumentServer(fFamily).FileTbl);
     fCurDocSAB.RecordsByKey;
    end
    else
    begin
     fCurDocSAB := MakeSab(DocumentServer(fFamily).FileTbl, aVal.List);
 ...
 fCurValDocSAB.ValuesOfkey(rnRealID_fld);

 GetDoubleRelatedList;
 GlobalHtServer.BigBrother.LogExport(fFamily, fCurDocSAB.Count);
end;
...
var
 ...
 begin
  ...
  begin
   ...
   if lStr.SLen > 0 then
    Generator.AddPCharLenAtom(k2_tiShortName, lStr);
  end;

  lStr := l3ArrayToPCharLen(lDocRec.FullName, SizeOf(lDocRec.FullName));
  if lStr.SLen > 0 then
   Generator.AddPCharLenAtom(k2_tiName, lStr);
  ...
  if lStr.SLen > 0 then
   Generator.AddPCharLenAtom(k2_tiNameComment, lStr);
 end;
 ...
 begin
  ...
  begin
   Generator.StartChild(k2_idAnnoTopic);
   try
    WriteAnnoHeader;
   finally
    Generator.Finish;
...
Begin
 ...
 lOutStream := Tl3FileStream.Create(aFile, l3_fmCreateReadWrite);
 Try
  aDictTbl.RefreshSrchList;
  If aDictTbl.fSrchList.gFoundCnt=0 then
    exit;

  If Assigned(aProgress) then
   aProgress(piStart,aDictTbl.fSrchList.gFoundCnt,aTitle);

  Ht(htOpenResults(aDictTbl.fSrchList,ROPEN_BODY,Nil,0));
  Try
   While htReadResults(aDictTbl.fSrchList,
                       aDictTbl.FullRecord,
    ...
    Begin
     ...
     If (aDictTbl.DictType = dlBases) or
        (aDictTbl.DictType = dlAccGroups) then
      Begin
   ...
   htCloseResults(aDictTbl.fSrchList);
  end;
 finally
  If Assigned(aProgress) then
   aProgress(piEnd, 0, '');
...
Procedure TDocExportGenerator.ExportDictionary(aDictType : TDLType;
                                               aFile : TFileName;
                                               aProgress : Tl3ProgressProc);
begin
 Case aDictType of
