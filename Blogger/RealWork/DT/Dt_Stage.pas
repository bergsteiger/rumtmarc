...
{ $Id: Dt_Stage.pas,v 1.78 2013/04/19 15:27:38 lulin Exp $ }
// $Log: Dt_Stage.pas,v $
...
begin
 ...
 if (aUserID <= 0) then
  aUserID := GlobalHtServer.CurUserID;
  ...
  try
   //GetFromFullRec(stAuthor_Key, TmpUser);
     { запрет закрытия этапа открытого другим пользователем (не админом)
      If Not GlobalHtServer.AdminRights
      then
       If (TmpUser<>GlobalHtServer.CurUserID)
        then
...
begin
 ...
 begin
  RefreshSrchList;
  htSearch(@fSrchList,DocStages,Handle,stDocID_Key,Equal,@aDocID,nil);
 end;
 try
...
Begin
 ...
 htSearch(@fTable.fSrchList,Result,fTable.Handle,stDocID_Key,Equal,@aDocID,Nil);
end;
...
Begin
 ...
 begin
  ...
  Case StageFlag of
   stfNone :
     Begin
      htSearch(@fSrchList,TmpSab1,Handle,stBDate_Key,Less_Equal,@EndDate,Nil); // начавшиеся до конца периода
      Try
       htSearch(@fSrchList,TmpSab2,Handle,stEDate_Key,Great_Equal,@BeginDate,Nil); // закончившиеся после начала периода
       Try
        htSearch(@fSrchList,TmpSab3,Handle,stEDate_Key,EQUAL,@BlankDate,Nil);  // незаконченные
        Try
 ...
 //     htSearch(@fSrchList,Result,Handle,stBDate_Key,Less_Equal,@EndDate,Nil);
      htSearch(@fSrchList,Result,Handle,stBDate_Key,IN_RANGE,@BeginDate,@EndDate);
     end;
   stfFinished :
     Begin
 //     htSearch(@fSrchList,Result,Handle,stEDate_Key,Less_Equal,@EndDate,Nil);
      htSearch(@fSrchList,Result,Handle,stEDate_Key,IN_RANGE,@BeginDate,@EndDate);
     end;
   stfUnfinished :
     Begin
      htSearch(@fSrchList,TmpSab1,Handle,stEDate_Key,Great,@EndDate,Nil);
      Try
       htSearch(@fSrchList,TmpSab2,Handle,stEDate_Key,Equal,@BlankDate,Nil);
       Try
    ...
    For I:=stInput to High(TStageType) do
     Begin
      If I in BeginSet
       then
        Begin
         htSearch(@fSrchList,TmpSab2,Handle,stType_Key,Equal,@I,Nil);
         Try
...
begin
 ...
 htTransferToPhoto(aDocs, fSrchList, stDocID_Key);
