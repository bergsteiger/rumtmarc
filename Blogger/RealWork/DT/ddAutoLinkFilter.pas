...
{ $Id: ddAutoLinkFilter.pas,v 1.11 2013/08/08 10:33:09 fireton Exp $ }
...
type
 ...
 public
  ...
  procedure FindAndSetLinks(aPara           : Ik2Tag;
                            aMasterDocID    : TDocID;
...
procedure TddAutoLinkFilter.DoFlushBuffer(const aLeaf: Ik2Tag; aTagId: Integer; aNeedCloseBracket : Boolean);
begin
 if TypeTable.TypeByHandle[-aTagID].InheritsFrom(k2_idLeafPara) and f_SetLinks then
 begin
...
begin
 ...
 if not Result then
  Result := (TypeTable.TypeByHandle[-aID] <> nil) and TypeTable.TypeByHandle[-aID].InheritsFrom(k2_idLeafPara);
end;
...
procedure TddAutoLinkFilter.StartChild(TypeID: Integer);
var
 I: Integer;
begin
...
procedure TddDocumentLinker.FindAndSetLinks(aPara           : Ik2Tag;
                                            aMasterDocID    : TDocID;
                                            aMasterDocDate  : TStDate;
                                            const aCaseCode : Il3CString);
var
...
var
 l_Layer: Ik2Tag;
begin
 l_Layer := f_Para.cAtomEx([k2_tiSegments, k2_tiChildren, k2_tiHandle, ev_slHyperlinks], nil);
...
begin
 ...
 begin
  with f_Para.cAtomEx([k2_tiSegments, k2_tiChildren, k2_tiHandle, ev_slHyperlinks], nil) do
  begin
...
begin
 ...
 begin
  ...
  l_Sab := GetExpandDictIDValueSab(dlSources, f_AllowedSources);
 ...
 with DictServer(CurrentFamily).Dict[dlSources] do
 begin
 ...
 with DictServer(CurrentFamily).Dict[dlTypes] do
 begin
 ...
 if l_TempID >= 0 then
 begin
  DictServer(CurrentFamily).DictScanner.AddToDict(l3PCharLen('информационное письмо'), l_TempID, Ord(dlTypes));
...
var
 ...
 begin
  ...
  if (l_LastChainEndPos > 0) and (l_LastFoundType >= 0) and (l_FoundType < 0) and
     ((l_FoundDate > 0) or (l_FoundNum.S <> nil)) then
  begin
     ...
     begin
      ...
      begin
       ...
       try
        ...
        // дата и номер
        if (l_FoundDate > 0) or (l_FoundNum.S <> nil) then
        begin
         // если документ - постановление арбитражного суда, то надо отпилить у него суффикс "-ГК" или "-АК"
         if (l_FoundNum.S <> nil) and (l_FoundType = f_ResolutionType) and
            (f_ArbCourtNode <> nil) and (FoundSources.Count = 1) then
         begin
          l_SNode := DictServer(CurrentFamily).Dict[dlSources].FindNodeByID(FoundSources.Items[0]);
          if (l_SNode <> nil) and (l_SNode.ParentNode = f_ArbCourtNode) then
          begin
           if l3Ends(l3PCharLen('-ГК'), l_FoundNum, True) or l3Ends(l3PCharLen('-АК'), l_FoundNum, True) then
            Dec(l_FoundNum.SLen, 3);
          end;
         end;
         ...
         f_DataSource.FilterByDateNum(l_FoundDate, l_FoundNum);
        end;
        ...
        if ((l_DocID.rIntDocID = TDocID(cUndefDocID)) or (l_DocID.rIntDocID = TDocID(cUndefDocID))) and
           ((FoundSources.Count > 0) or (l_FoundDate > 0) or (l_FoundNum.S <> nil)) and
           (l_FurtherSeg.rOffs = 0) then
        begin
         l_HistoryIdx := f_DocHistory.FindBySubEntry(TddAutolinkDocEntry.Make(l_FoundType, l_FoundDate, l_FoundNum, f_FoundSources, l_DocID));
  ...
  begin
   ...
   if l_HistoryIdx < 0 then // мы нашли этот документ не в хистори
    // добавляем его туда
    f_DocHistory.AddEntry(TddAutolinkDocEntry.Make(l_FoundType, l_FoundDate, l_FoundNum, f_FoundSources, l_DocID))
   else
  ...
  case aDictItemID.rDictionaryID of
   ...
   cNumDictID :
    begin
     if l_FoundNum.S <> nil then
      WorkoutChain;
     l_FoundNum := l3PcharLen(PAnsiChar(aText) + aSegment.rOffs, aSegment.rLen);
     l_NumPos := aSegment;
    end;
   ...
   cGostDictID :
    begin
     ...
     begin
      ...
      l_FoundNum := l3PcharLen(PAnsiChar(aText) + aSegment.rOffs, aSegment.rLen); // и она же используется как номер документа
      lp_UpdateChain;
   ...
   case TDLType(aDictItemID.rDictionaryID) of
     dlSources: FoundSources.Add(aDictItemID.rID);
     dlTypes  :
      begin
...
var
 ...
 begin
  Result := True;
  l_ID := DictServer(CurrentFamily).Dict[dlTypes].FindIDByFullPath(l3Trim(aStr));
...
begin
 while NextLine do
  ...
  begin
   l_ID := DictServer(CurrentFamily).Dict[dlTypes].FindIDByFullPath(ValuesW[0]);
   if l_ID <> cUndefDictID then
...
var
 ...
 begin
  Result := True;
  DictServer(CurrentFamily).DictScanner.AddToDict(aStr, l_ID, Ord(dlTypes));
 end;
