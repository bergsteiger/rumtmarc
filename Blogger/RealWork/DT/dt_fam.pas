...
{ $Id: dt_fam.pas,v 1.39 2013/04/19 13:07:39 lulin Exp $ }
...
Begin
 ...
 If fSrchList.gFoundCnt=0 then
  exit;
 Ht(htOpenResults(fSrchList,ROPEN_READ,@FamFldArr,FldCount));
 Try
  While htReadResults(fSrchList,@FamRec,SizeOf(FamRec))<>0 do
   Begin
    TmpStr:=l3ArrayToString(FamRec.Path,SizeOf(FamRec.Path));
    PathArr^[FamRec.ID]:=NewStr(GlobalHtServer.ConvertAliasPath(TmpStr));
    AttribArr^[FamRec.ID]:=FamRec.Attrib;
   end;
 finally
  htCloseResults(fSrchList);
 end;
end;
...
Begin
 If aFamilyID = MainTblsFamily then
  ...
  Begin
   ...
   AbsN:=Ht(htRecordByUniq(@fSrchList,Handle,fmId_Key,@aFamilyID,@RecH));
   If AbsN=0 then
    raise EHtErrors.CreateInt(ecNotFound);
   Ht(htReadRecord(@fSrchList,Handle,AbsN,@FamRec));
   TmpStr:=l3ArrayToString(FamRec.Path,SizeOf(FamRec.Path));
   CurPath:=NewStr(GlobalHtServer.ConvertAliasPath(TmpStr));
...
Begin
 ...
 Ht(htCopyResults(TmpSab,fSrchList));
 Try
  Ht(htOpenResults(TmpSab,ROPEN_READ,@FamFldArr,FldCount));
  Try
   While htReadResults(TmpSab,@FamRec,SizeOf(FamRec))<>0 do
    Begin
...
Begin
 ...
 If WithCreate
  then
   CreateEmptyFamily(GlobalHtServer.ConvertAliasPath(aPath),FamRec.ID,aType);
 ...
 f_GlobalFamPaths^[FamRec.ID]:=NewStr(GlobalHtServer.ConvertAliasPath(aPath));
  ...
  Try
    If StartTA then
     Try
      ...
      f_GlobalFamPaths^[aID]:=NewStr(GlobalHtServer.ConvertAliasPath(aPath));
     except
      RollBackTA;
...
Begin
  ...
  AbsNum:=Ht(htRecordByUniq(@fSrchList,Handle,fmId_Key,@aID,@RecH));
  If l_Ok<>0
    then
      Begin
        DelRec(AbsNum);
...
Begin
  RefreshSrchList;
  Result:=0;
  HT(htKeyMaximum(fSrchList,fmId_Key,@Result));
end;
