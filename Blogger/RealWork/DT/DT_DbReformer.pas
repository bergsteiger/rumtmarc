...
{ $Id: DT_DbReformer.pas,v 1.6 2013/04/09 12:37:31 narry Exp $ }
...
destructor TDbReformer.Destroy;
begin
...
begin
 htDropTable(PChar(aFullTableName), nil, nil);
end;
   ...
   try
    ...
    try
     ...
     begin
      ...
      l_Filer.Write(#9);
      l_Filer.Write(lFileTbl.GenRecStr(l_Rec));
     end;
    finally
     FreeMem(l_pBuf);
    end;
...
var
 ...
 l_Name: String;
begin
 l_Table:= TAbsHtTbl.CreateWithOpen(FamilyFolder, TablePassword(CurrentFamily, ord(cDLPassports[dlAccGroups].rDict)), TableName(CurrentFamily, Ord(cDLPassports[dlAccGroups].rDict)), 0);
 try
...
begin
 if not (dlAccGroups in aDicts) then
  AddAccessGroups;
    ...
    try
     ...
     begin
      ...
      begin
       ...
       begin
        // Заточка под USERS
        if theTableName = MainTblNames[mtUsers] then
        begin
         l_List.Sorted:= False;
...
destructor TTableStruct.Destroy;
begin
