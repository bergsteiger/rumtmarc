...
{ $Id: dt_Sab.pas,v 1.116 2013/07/31 11:23:57 voba Exp $ }
...
// - add overload function MakeValueSet
//
...
// - add function MakeValueSet  overload;
...
type
 ...
 TSab = class(Tl3Base, ISab)
  private
   ...
   function GetFieldSizeArray : TFieldArray;
    overload;
   function GetFieldSizeArray(const aFldArr : array of ThtField) : TFieldArray;
    overload;
  ...
  public
   ...
   procedure Distinct(const aElemNum : array of ThtField);
    {* - убирает повторяющиеся значения для поля/группы полей}
...
//фабрики
function MakeSab(const aTable : ITblInfo; aSab : Sab) : ISab; overload;
...
function GetValueSAB(aFullSAB : SAB; aFld : Word): SAB;

function dtCleanSab(var aSab : SAB) : SAB;
  {* - зачистка саба перед использованием}

procedure dtFreeSab(aSab : SAB);
  {* - удаление данных  Sab`а}

function dtMakeSabCopy(aSab : SAB) : SAB;
  {* - делает копию Sab`а}
...
procedure dtCopyValuesSabToList(aValuesSab : SAB; aList : Tl3LongintList); overload;
...
function dtMakeRecListBySab(aSab : SAB;
...
function TSab.GetFieldSizeArray : TFieldArray;
begin
 Result := GetFieldSizeArray([]);
end;

function TSab.GetFieldSizeArray(const aFldArr : array of ThtField) : TFieldArray;
var
 lIsOpened : Boolean;
 I : Integer;
 lFldCount : Integer;
...
begin
 ...
 begin
  ...
  Result[I] := Sign(aFields[I].rField) * (lTblOffs + Abs(aFields[I].rField));
 end;
end;
...
begin
 ...
 Result := Sign(aField.rField) * (lFldOffset + Abs(aField.rField));
end;
...
begin
 ...
 Ht(htIncludeResultsItem(fSab.fSab, Pred(fSab.Count), aData));
 PurgeCache;
end;
...
procedure dtCopyValuesSabToList(aValuesSab : SAB; aList : Tl3LongintList);
var
 lValue    : Longint;
 lItemSize : Cardinal;
begin
...
function dtGetSabFieldSizeArray(aSab : SAB) : TFieldArray;
var
 I : Integer;
 lFldCount : Integer;
 lOPELArr : array of OPEL;
...
begin
 ...
 if not (lResultsType in [RES_SORTED, RES_SORTJOIN]) then
  Result.Sort
 else
  Result.Sorted := True;
 ...
 //aSab.CopyRecordsToBuffer(Result.ItemSlot(0));
end;
