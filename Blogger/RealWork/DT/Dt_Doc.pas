...
(* Структура данных документа при выводе на экран *)
 ...
 TFileTbl = Class(TPrometTbl)
  ...
  public
   ...
   function    GetDocIdOnVerLink(aLinkID : TDocID) : TDocID; { TODO -oВованыч -cОптимизация : .CheckVerLink точно такой же кроме exception }
 ...
 TDocumentServer = Class(TL3Base)
  ...
  public
   ...
   { Удаляет документ с aDelDocID.
     Если на документ имеются гиперсылки, то при удалении остаются записи в таблице Renum,
   ...
   { Удаляет документ с aDelDocID.
     Если на документ имеются гиперсылки или aPreventNumFree, то при удалении остаются записи в таблице Renum,
     не добавляется элемент во Free и не чистятся ссылки на удаляемый документ}
 ...
 TDocumentData = class(TL3Base)
  ...
  public
   ...
   {* - перенабирает список кореспондентов. }

   procedure RefreshVersionsList;
   {* - перенабирает список версий. }
 ...
 try
  ...
  for I := 0 to Pred(l_List.Count) do
   try
    ...
    l3System.Msg2Log(Format('Verlink на отсутствующий документ Id = #%d',[TDocId(l_List[I])]));
   end;
 finally
  l_List.Free;
 end;
...
var
 ...
 TmpPChar : PAnsiChar;
const
 NewName : Array [1..19] Of Char = '* Новый Документ *'#0;
begin
...
begin
 ...
 // оставляем только доки c VerLink <> 0
 if aOnlyWithEdition then
 begin
  lVal := 0;
  aDocSet.SubSelect(fVerLink_fld, lVal, GREAT);
  aDocSet.AndSab(lSab1);
  //из тех, у кого VerLink = 0 нужно оставить те, у кого будущая редакция есть
...
begin
 ...
 if Result then
  // в таблице FILE есть запись, но это ничего не значит, надо ещё проверить логи
  // чтобы понять, что документ был создан в результате заливки или в Архивариусе
