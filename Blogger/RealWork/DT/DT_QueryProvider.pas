...
{ $Id: DT_QueryProvider.pas,v 1.35 2012/08/07 11:03:18 voba Exp $ }
...
// - bug fix неправильно сортировалось по Name. После исправления выяснилось что по такому длинному полю вообще не может быть отсортированноно
//
...
type
 ...
 TdtQueryProvider = class(_l3LockedChange_, Il3ChangeRecipient)
  ...
  public
   ...
   {* - Сбрасывает результат, при следуещем требовании результат переоткрывается заново}
   procedure RefreshResult(aWithReSearch : Boolean = false);
   {* - Обновляет Photo для результата}
 ...
 // QueryProvider c расширенными возможностями для манипуляций со списком документов
 // см TDocListQuery
  private
   procedure MakeQuery;
...
{$Endif}
 ...
 // Нижеследующее - для того, чтобы избежать дубликатов, если периодов действия несколько
 // на один документ.
...
{$ifdef NeedInclInDocRec}
 ...
 // Нижеследующее - для того, чтобы избежать дубликатов, если документ подключен
 // несколько раз.
...
begin
 ...
 try
  ...
  // для скорости удалим сами + взведем fPreventReset
