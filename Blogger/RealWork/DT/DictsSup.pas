...
begin
 l_Res := '';
 CheckGroup(romNameAttr,  'Атрибуты');
 CheckGroup(romText,      'Текст и справка');
 CheckGroup(romHyperLink, 'Гиперссылки');
 CheckGroup(romBlock,     'Блоки');
 CheckGroup(romSub,       'Метки');
 CheckGroup(romKeyWord,   'Ключевые слова');
 CheckGroup(romClass,     'Классы');
 CheckGroup(romJurOp,     'Юридические операции');
 CheckGroup(romAnno,      'Аннотация');
 CheckGroup(romDocGroup,  'Группы документов');
 Result := l_Res;
