...
{ $Id: DT_DbInfo.pas,v 1.1 2011/12/12 13:58:35 fireton Exp $ }
// $Log: DT_DbInfo.pas,v $
...
begin
 ...
 f_FamiliesData := TObjectList.Create(True);
...
begin
 Result := ConcatDirName(TFamilyInfo(f_FamiliesData[f_ActiveFamilyIndex]).Path, 'base.ini');
end;
...
begin
 ...
 for I := 0 to f_FamiliesData.Count - 1 do
  aList.Add(TFamilyInfo(f_FamiliesData[I]).Name);
end;
   ...
   begin
    ...
    begin
     ...
     begin
      ...
      l_FamilyInfo := TFamilyInfo.Create;
...
begin
 Result := TFamilyInfo(f_FamiliesData[f_ActiveFamilyIndex]).Path;
end;
...
begin
 Result := TFamilyInfo(f_FamiliesData[f_ActiveFamilyIndex]).Name;
end;
...
begin
 l_Found := -1;
 for I := 0 to f_FamiliesData.Count - 1 do
  if AnsiCompareText(aFamilyName, TFamilyInfo(f_FamiliesData[I]).Name) = 0 then
  begin
   l_Found := I;
   Break;
  end;
