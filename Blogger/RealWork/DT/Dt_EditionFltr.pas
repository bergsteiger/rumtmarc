unit Dt_EditionFltr;
{ $Id: Dt_EditionFltr.pas,v 1.29 2011/06/10 11:53:44 voba Exp $ }
...
begin
 ...
 lValue:=Value;
 try
  if TopType[0].InheritsFrom(k2_idDocument) then {изменение элементарных полей документа}
  begin
   if (AtomIndex = k2_tiName) then
   begin
    if StripName then lValue.AsString.AsString := StripNameSuffix(lValue.AsString.AsString);
   end
   else
   ...
   begin
    ...
    begin
     ...
     lValue.asInteger:=fIntEditionID;
   ...
   if TopType[0].InheritsFrom(k2_idHyperLink) then {фильтрация идентификатора гипертекстовой ссылки}
   begin
    if (AtomIndex = k2_tiHandle) then
     lFltrAtom :=True;
   end
   else
    if TopType[1].InheritsFrom(k2_idHyperLink) and  {изменение записи: гипертекстовая ссылка}
     ...
     begin
      ...
      begin
       ...
       if fReverseHlinkRenum and
        ...
        lValue.AsInteger := fIntDocumentID;
      end
      else
       ConvertErrorEx(lValue.Kind);
     end;
