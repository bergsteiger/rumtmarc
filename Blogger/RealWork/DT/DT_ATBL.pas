...
{ $Id: DT_ATBL.pas,v 1.163 2013/04/19 13:07:39 lulin Exp $ }
...
{+------------------------------------------------------------------------+
 ...
 // SAB связанный с Сервером таблиц (GlobalHtServer - модуль Dt_Serv)
 // умеет отвязываться от таблицы переводя свои данные
 // в список значений уникального ключа
 //!! похоже это все никому не надо, потому что отключением "на лету" клиента от БД никто не пользуется

  TPrometSAB = class(Tl3Base)
   protected
    fTmpKeySab  : Sab;
    fTblObj     : TPrometTbl;
    fResElem    : SmallInt;
    ...
    fOnClose      : TPromProc;
    fOnOpen       : TOpenSabProc;

    procedure SetCurSab(aSab : Sab);
    procedure SetSabInfo;
    procedure DelSabInfo;
    procedure BeforeRelease; override;
   ...
   public
    ...
    property PSAB : Sab read fCurSab write SetCurSab;
    property OnClose : TPromProc write fOnClose;
    property OnOpen : TOpenSabProc write fOnOpen;
  end;

  // Класс для чтения данных (записи aRecs, поля aFieldNums)
  // Данные получает пачками и держит их в своем буфере (64k).

  // Примеры использования :
  // VI := TValuesIterator.Create(l_Recs, [1,3]);
  // try
  //  while not VI.EOF do
  //  begin
  //   VI.Read(1, l_Id);
  ...
  // VI := TValuesIterator.Create(l_Values);
  // try
  //  while not VI.EOF do
  //  begin
  //   VI.Read(l_Id);
  ...
  TCustomValuesIterator = class(Tl3Base)
  private
   f_Buf : PAnsiChar;
   f_BytesInBuf: Integer;
   f_Sab: Sab;
   f_RecLen: Integer;
   f_BufLen: Integer;
   f_NextRecPos: PAnsiChar;

   f_FieldsCount: Integer;
   f_FieldNums: TFieldArray;

   procedure LoadDataToBuf;
   procedure CheckOpen;
   function GetCurrentRecord : Pointer;
  public
   function EOF: Boolean;
   procedure Next;
   procedure Sort(const aSortFields: array of Smallint); virtual;

   procedure ReadFullRec(out aRecord); virtual;

   property  RecordSize: Integer read f_RecLen;
   property  CurrentRecord: Pointer read GetCurrentRecord;
  end;

  TValuesIterator = class(TCustomValuesIterator)
  private
   f_Table: TAbsHtTbl;
   function FieldOffset(aFieldNum: Integer): Integer;
  protected
   procedure Cleanup; override;
  public
   constructor Create(var aRecs: Sab; // Sab должен содержать список записей
                      const aFieldNums: array of SmallInt;
                      aTable: TAbsHtTbl = nil); overload;
   constructor Create(var aFieldValues: Sab; // Sab должен содержать список значений поля
                      aTable: TAbsHtTbl = nil); overload;
   function  ReadStr(aFieldNum: SmallInt): AnsiString;
   procedure Read(aFieldNum: SmallInt; var aValue); overload;
   procedure Read(var aValue); overload; // только для случаев, когда в данных всего одно поле
  end;

  TValuesIteratorForJoinedRecs = class(TCustomValuesIterator)
  private
   //f_FieldsCount: Integer; // кол-во полей asked_rec
   //f_FirstTableRecSize : Integer; //размер части записи относящейся к первой таблице
   //f_SabFieldNums: array of SmallInt; // мапа asked_field --> joined_rec_field
   f_FieldOffsets: array of SmallInt; // позиции полей в asked_rec
   f_FieldLengths: array of SmallInt; // размеры полей в asked_rec

   fFieldsCountInTbl1 : Word;   // общее количество полей в первой таблице
   fFieldsCountInTbl2 : Word;   // общее количество полей во второй таблице
   fReverseJoinOrder : Boolean; // обратный порядок склеивания таблиц

  protected
   procedure Cleanup; override;
  public
   constructor Create(var aJoinedRecs: Sab;
                      aTable1, aTable2 : TAbsHtTbl;
                      const aFieldNums : array of Smallint); overload;
                       // aFieldNums - номера полей, которые нужно читать,
                       //номера из второй таблицы сдвинуты на общее количество полей (включая суррогатный) в первой

   constructor Create(var aJoinedRecs: Sab; // Sab должен содержать список "joined" записей
                      aTable1: TAbsHtTbl;
                      const aFieldNums1: array of Smallint; // номера полей из табл.1, составляющие первую часть asked_rec
                      aTable2: TAbsHtTbl;
                      const aFieldNums2: array of Smallint // номера полей из табл.2, составляющие вторую часть asked_rec
                     ); reintroduce; overload;

   procedure Read(aRecFieldNum: SmallInt; // номер поля в полученной asked_rec (отсчет от 1)
                  out aValue);

   procedure Sort(const aSortFields: array of Smallint); override;
   //procedure ReadFullRec(out aRecord); override;
   //property FullRec: PAnsiChar read f_NextRecPos;
  end;
...
begin
 ...
 aOpenMode := GlobalHtServer.TblShare;
 if aWithFlush then
...
constructor TPrometSAB.Create(aTblObj : TPrometTbl);
begin
 if not Assigned(GlobalHtServer) then Abort;
 if not Assigned(aTblObj) then Abort;

 inherited Create(nil);
 fTblObj := aTblObj;
 fUniKeyFld := fTblObj.UniqKeyElemNum;
 l3FillChar(fCurSab,SizeOf(Sab));
 l3FillChar(fTmpKeySab,SizeOf(Sab));
 fSabType:=RES_NPROC;
 SetSabInfo;
end;

procedure TPrometSAB.BeforeRelease;
begin
 DataClear;
 DelSabInfo;
 inherited;
...
procedure TPrometSab.DataClear;
begin
 if (htResultsType(fCurSab)<>Res_NProc) then
  htClearResults(fCurSab);
 if (htResultsType(fTmpKeySab)<>Res_NProc) then
  htClearResults(fTmpKeySab);
 l3FillChar(fCurSab,SizeOf(Sab));
 l3FillChar(fTmpKeySab,SizeOf(Sab));
 fSabType:=RES_NPROC;
end;

procedure TPrometSab.SetSabInfo;
var
 TmpOSRec : TOpenSAbRec;
begin
 l3FillChar(TmpOSRec,SizeOf(TmpOSRec));
 with TmpOSRec do
  begin
   Tbl:=fTblObj;
   Obj:=Self;
  end;
 GlobalHtServer.AddSabObj(TmpOSRec);
end;

procedure TPrometSab.DelSabInfo;
begin
 GlobalHtServer.DelSabObj(Self);
end;

procedure TPrometSAB.SetCurSab(aSab : Sab);
var
 TmpTblH : HT_Const.THandle;
begin
 DataClear;
 fSabType:=htResultsType(aSab);
 if not (fSabType in [RES_RECORD, RES_VALUE]) then
 begin
  l3System.Msg2Log('TPrometSAB.SetCurSab: htResultsType, fSabType = ' + IntToStr(fSabType));

  raise EHtErrors.CreateInt(ecNotValidSab);
 end;
 fCurSab := aSab;
end;

procedure TPrometSAB.OpenSab;
begin
 case fSabType of
  RES_RECORD: begin
               if (htResultsType(fTmpKeySab)<>Res_NProc) then
                begin
                 htTransferToPhoto(fTmpKeySab,fTblObj.fSrchList,fUniKeyFld);
                 htRecordsByKey(fCurSab, fTmpKeySab);
  ...
  RES_VALUE : begin
               htTransferToPhoto(fCurSab, fTblObj.fSrchList, fResElem);
               if Assigned(fOnOpen) then
                fOnOpen(fCurSab);
              end;
 end;
...
procedure TPrometSAB.CloseSab;
begin
 if Assigned(fOnClose) then
  fOnClose;
 case fSabType of
  RES_RECORD :
   if fUniKeyFld>0 then
    begin
     if (htResultsType(fTmpKeySab)<>Res_NProc) then
      htClearResults(fTmpKeySab);
     htValuesOfKey(fTmpKeySab,fUniKeyFld,fCurSab);
     htClearResults(fCurSab);
    end
   else
    DataClear;
  RES_VALUE : Ht(htResultsElement(fCurSab,fResElem));
 end;
end;
...
{ TCustomValuesIterator }

procedure TCustomValuesIterator.CheckOpen;
begin
 if not htIsResultOpen(f_Sab) then
 begin
  if htResultsType(f_Sab) in [RES_VALUE] then
   Ht(htOpenResults(f_Sab, ROPEN_READ, nil, 0))
  else
   Ht(htOpenResults(f_Sab, ROPEN_READ, @f_FieldNums[0], f_FieldsCount));

  Next;
 end;
end;

function TCustomValuesIterator.GetCurrentRecord : Pointer;
begin
 CheckOpen;
 Result := f_NextRecPos;
end;

function TCustomValuesIterator.EOF: Boolean;
begin
 CheckOpen;
 Result := f_BytesInBuf = 0;
end;

procedure TCustomValuesIterator.LoadDataToBuf;
begin
 f_BytesInBuf := htReadResults(f_Sab, f_Buf, f_BufLen);
 if f_BytesInBuf > MAX_BUF_LEN then
  raise EHtErrors.CreateInt(ecCanNotRead);

 f_NextRecPos := f_Buf;
end;

procedure TCustomValuesIterator.Next;
begin
 CheckOpen;
 if f_NextRecPos = nil then
  LoadDataToBuf
 else
  Inc(f_NextRecPos, f_RecLen);

 if f_NextRecPos - f_Buf > f_BytesInBuf - f_RecLen then
  LoadDataToBuf;
end;

procedure TCustomValuesIterator.ReadFullRec(out aRecord);
begin
 l3Move(CurrentRecord^, aRecord, f_RecLen);
end;

procedure TCustomValuesIterator.Sort(const aSortFields: array of Smallint);
var
 lSortSub : Sab;
begin
 if f_Sab.gFoundCnt <= 1 then exit;
 dtCleanSab(lSortSub);
 Ht(htSortResults(lSortSub, f_Sab, @aSortFields[0], Length(aSortFields)));
...
constructor TValuesIterator.Create(var aRecs: Sab;
                                   const aFieldNums: array of SmallInt;
                                   aTable: TAbsHtTbl = nil);
var
 l_BytesRead: Word;
 I: Integer;
begin
 inherited Create;
 if not (htResultsType(aRecs) in [RES_RECORD, RES_SORTED, RES_JOIN, RES_SORTJOIN]) then
  raise EHtErrors.CreateInt(ecNotValidSrchType);

 if aTable = nil then
  f_Table := GlobalHtServer.GetTblObject(aRecs)
 else
  f_Table := aTable;

 Ht(htCopyResults(f_Sab, aRecs));

 f_FieldsCount := Length(aFieldNums);

 f_FieldNums := MakeFieldArray(aFieldNums);

 (*SetLength(f_FieldNums, f_FieldsCount);
 for I := 0 to f_FieldsCount - 1 do
  f_FieldNums[I] := aFieldNums[I];
 *)

 f_RecLen := 0;
 for I := 0 to f_FieldsCount - 1 do
  Inc(f_RecLen, f_Table.fldLength[f_FieldNums[I]]);

 f_BufLen := Min(MAX_BUF_LEN, f_Sab.gFoundCnt * f_RecLen);
 g_l3System.GetLocalMem(f_Buf, f_BufLen);

end;

constructor TValuesIterator.Create(var aFieldValues: Sab;
                                   aTable: TAbsHtTbl = nil);
begin
 inherited Create;
 if htResultsType(aFieldValues) <> RES_VALUE then
  raise EHtErrors.CreateInt(ecNotValidSrchType);

 if aTable = nil then
  f_Table := GlobalHtServer.GetTblObject(aFieldValues)
 else
  f_Table := aTable;

 Ht(htCopyResults(f_Sab, aFieldValues));

 f_FieldsCount := 1;
 SetLength(f_FieldNums, f_FieldsCount);
 Ht(htResultsElement(f_Sab, f_FieldNums[0]));
 f_RecLen := f_Table.fldLength[f_FieldNums[0]];
...
function TValuesIterator.FieldOffset(aFieldNum: Integer): Integer;
var
 I: Integer;
begin
 Result := 0;
 for I := 0 to f_FieldsCount do
 begin
  Assert(I < f_FieldsCount, 'Задан неверный номер поля.');
  if aFieldNum = f_FieldNums[I] then
   Break;

  Inc(Result, f_Table.fldLength[f_FieldNums[I]]);
 end;
end;

procedure TValuesIterator.Read(var aValue);
begin
 if f_FieldsCount <> 1 then
  raise Exception.Create('Не указан номер поля.');
 Read(f_FieldNums[0], aValue);
end;


procedure TValuesIterator.Read(aFieldNum: SmallInt; var aValue);
begin
 l3Move((PAnsiChar(CurrentRecord) + FieldOffset(aFieldNum))^,
      aValue,
      f_Table.fldLength[aFieldNum]);
end;

procedure TValuesIterator.Cleanup;
begin
 g_l3System.FreeLocalMem(f_Buf);
 f_FieldNums := nil;

 htCloseResults(f_Sab);
 htClearResults(f_Sab);

 inherited;
end;

function TValuesIterator.ReadStr(aFieldNum: SmallInt): AnsiString;
var
 l_StrLen: Integer;
 l_Str: AnsiString;
begin
 l_StrLen := f_Table.fldLength[aFieldNum];
 if f_Table.ElementType[aFieldNum] = ET_CHAR then
 begin
  SetLength(l_Str, l_StrLen);
  l3Move((PAnsiChar(CurrentRecord) + FieldOffset(aFieldNum))^, l_Str[1], l_StrLen);
  Result := Trim(l_Str);
 end
 else
  raise EHtErrors.Create(SysUtils.Format('Поле %d таблицы %s не является строковым.',
                                         [aFieldNum, f_Table.TblName]));
end;

{ TValuesIteratorForJoinedRecs }

procedure TValuesIteratorForJoinedRecs.Cleanup;
begin
 g_l3System.FreeLocalMem(f_Buf);
 //f_SabFieldNums := nil;
 f_FieldOffsets := nil;
...
constructor TValuesIteratorForJoinedRecs.Create(var aJoinedRecs: Sab;
                                                aTable1: TAbsHtTbl;
                                                const aFieldNums1: array of SmallInt;
                                                aTable2: TAbsHtTbl;
                                                const aFieldNums2: array of SmallInt);
var
 lFieldNums: TFieldArray;
 I : Integer;
begin
 SetLength(lFieldNums, Length(aFieldNums1) + Length(aFieldNums2));

 for I := 0 to Pred(Length(aFieldNums1)) do
  lFieldNums[I] := aFieldNums1[I];
 for I := 0 to Pred(Length(aFieldNums2)) do
  lFieldNums[I + Length(aFieldNums1)] := aFieldNums2[I] + aTable1.fldCount + 1;

 Create(aJoinedRecs, aTable1, aTable2, lFieldNums);
end;
...
constructor TValuesIteratorForJoinedRecs.Create(var aJoinedRecs: Sab;
 ...
 procedure CalcFieldParams;
  (* строит f_FieldNums, f_FieldOffsets, f_FieldLengths, вычисляет f_FieldsCount ,f_RecLen *)
 var
  I: Integer;
  l_TablesCount: Integer;
  l_TableHandles: array[1 .. MAX_TAB_LINK] of HT_Const.THandle;
 begin
  f_FieldNums := MakeFieldArray(aFieldNums);
  f_FieldsCount :=  Length(f_FieldNums);

  SetLength(f_FieldOffsets, f_FieldsCount);
  SetLength(f_FieldLengths, f_FieldsCount);

  fFieldsCountInTbl1 := aTable1.fldCount + 1;
  fFieldsCountInTbl2 := aTable2.fldCount + 1;

  l_TablesCount := Ht(htTellJoinOrder(aJoinedRecs, @l_TableHandles[1]));
  if l_TablesCount <> 2 then
   raise EHtErrors.CreateInt(ecNotValidSrchType);

  if (l_TableHandles[1] = aTable1.Handle) and (l_TableHandles[2] = aTable2.Handle) then
   // прямой порядок таблиц в Sab'е
   fReverseJoinOrder := False
  else
  if (l_TableHandles[1] = aTable2.Handle) and (l_TableHandles[2] = aTable1.Handle) then
   // обратный порядок таблиц в Sab'е
   fReverseJoinOrder := True
  else
   raise EHtErrors.CreateInt(ecNotValidSrchType);

  // размеры полей
  for I := 0 to Pred(Length(f_FieldNums)) do
   if f_FieldNums[I] < fFieldsCountInTbl1 then
    f_FieldLengths[I] := aTable1.fldLength[f_FieldNums[I]]
   else
    f_FieldLengths[I] := aTable2.fldLength[f_FieldNums[I] - fFieldsCountInTbl1];

  // длина записи и офсеты полей
  f_RecLen := 0;
  for I := 0 to f_FieldsCount - 1 do
  begin
   f_FieldOffsets[I] := f_RecLen;
   Inc(f_RecLen, f_FieldLengths[I]);
  end;

  // теперь учет расположения в JOIN
  if fReverseJoinOrder then
  begin // обратный порядок таблиц в Sab'е
   for I := 0 to Pred(Length(f_FieldNums)) do
    if f_FieldNums[I] < fFieldsCountInTbl1 then
     Inc(f_FieldNums[I], fFieldsCountInTbl2)
    else
     Dec(f_FieldNums[I], fFieldsCountInTbl1);
  end;

  (*
  // теперь учет смещения в объединен. записи
  if fReverseJoinOrder then
  begin // обратный порядок таблиц в Sab'е
   for I := 0 to Length(aFieldNums1) - 1 do
    Inc(f_FieldNums[I], fFieldsCountInTbl2);
  end
  else
  begin // прямой порядок таблиц в Sab'е
   for I := Length(aFieldNums1) to Length(aFieldNums1) + Length(aFieldNums2) - 1 do
    Inc(f_FieldNums[I], fFieldsCountInTbl1);
  end;
  *)

  //f_FirstTableRecSize := 0;
  (*
  // размеры полей
  for I := 0 to Length(aFieldNums1) - 1 do
  begin
   f_FieldLengths[I] := aTable1.fldLength[aFieldNums1[I]];
   //Inc(f_FirstTableRecSize, f_FieldLengths[I]);
  end;
  for I := 0 to Length(aFieldNums2) - 1 do
   f_FieldLengths[I + Length(aFieldNums1)] := aTable2.fldLength[aFieldNums2[I]];
  *)
...
begin
 ...
 if not (htResultsType(aJoinedRecs) in [RES_JOIN, RES_SORTJOIN]) then
  raise EHtErrors.CreateInt(ecNotValidSrchType);

 CalcFieldParams;

 // буфер
 f_BufLen := Min(MAX_BUF_LEN, aJoinedRecs.gFoundCnt * f_RecLen);
 g_l3System.GetLocalMem(f_Buf, f_BufLen);

 // копируем
 Ht(htCopyResults(f_Sab, aJoinedRecs));
end;

procedure TValuesIteratorForJoinedRecs.Sort(const aSortFields: array of Smallint);
var
 I : Integer;
 lSortFields: array of Smallint;
begin
 if fReverseJoinOrder then
 begin
  SetLength(lSortFields, Length(aSortFields));
  for I := 0 to Pred(Length(aSortFields)) do
   if aSortFields[I] < fFieldsCountInTbl1 then
    lSortFields[I] := aSortFields[I] + fFieldsCountInTbl2
   else
    lSortFields[I] := aSortFields[I] - fFieldsCountInTbl1;
  inherited Sort(lSortFields);
 end
 else
  inherited Sort(aSortFields);
end;

procedure TValuesIteratorForJoinedRecs.Read(aRecFieldNum: Smallint; out aValue);
begin
 l3Move((PAnsiChar(CurrentRecord) + f_FieldOffsets[aRecFieldNum])^,
      aValue,
      f_FieldLengths[aRecFieldNum]);
end;
