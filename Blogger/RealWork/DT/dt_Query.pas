...
{ $Id: dt_Query.pas,v 1.17 2013/07/15 13:19:44 voba Exp $ }
...
type
 ...
 TdtDictQuery = class(TdtCustomQuery)
  ...
  protected
   ...
   procedure Cleanup; override;
  public
   constructor Create(aDictType    : TDLType;
   ...
   property DictType    : TDLType        read fDictType;
 ...
 TdtTextQuery = class(TdtCustomQuery)
  ...
  protected
   ...
   procedure Cleanup; override;
  public
   constructor Create(const aTextFormula : String; aIndexType : TIndexType; const aPhoto : ISab = nil);
   procedure DoQuery; override;
 ...
 TdtDateNumQuery = class(TdtCustomQuery)
  ...
  protected
   class function GetRegNumber : Integer; override;
  public
   constructor Create(aFromDate : TStDate; aToDate : TStDate;const aNumberMask : String; aDNType : TDNType; const aPhoto : ISab = nil);
   procedure DoQuery; override;
...
uses
  ...
  Dt_Hyper, Dt_User,
  DT_Active, Dt_Log, Dt_BBLog, DT_Alarm, DT_SrchQueries,
  dt_LinkServ,
...
begin
 if fMainTblPhoto = aPhoto then Exit;
...
{TdtDictQuery}
constructor TdtDictQuery.Create(aDictType    : TDLType;
                                aDictIDList  : Tl3LongintList;
                                aWithEquals  : Boolean = False;
                                aWithSubTree : Boolean = False;
                                aWholeDocOnly: Boolean = False;
...
begin
 ...
 begin
  ...
  begin
   ...
   if lLinkDictType = dlNone then lLinkDictType := fDictType;
...
begin
 if fQuery <> nil then
 begin
  fQuery.MainTblPhoto := MainTblPhoto;
 ...
 if (fAddList <> nil) then
 begin
  fAddList.MainTblPhoto := MainTblPhoto;
 ...
 if (fDelList <> nil) then
 begin
  fDelList.MainTblPhoto := MainTblPhoto;
...
{TdtTextQuery}
constructor TdtTextQuery.Create(const aTextFormula : String; aIndexType : TIndexType; const aPhoto : ISab = nil);
begin
 inherited Create(aPhoto);
...
{TdtDateNumQuery}
constructor TdtDateNumQuery.Create(aFromDate : TStDate; aToDate : TStDate;const aNumberMask : String; aDNType : TDNType; const aPhoto : ISab = nil);
begin
 inherited Create(aPhoto);
 fFromDate   := aFromDate;
 fToDate     := aToDate;
...
procedure TdtDateNumQuery.DoQuery;
 ...
 begin
  ...
  if aNumberMask[lLen] = '*' then
   ...
   begin
    Result := aNumberMask;
    lFldSize := DictServer(CurrentFamily).GroupTbl[dlDateNums].FldLength[dnNumFld];
...
begin
 ...
 begin
  lSabDate := MakeSab(DictServer(CurrentFamily).GroupTbl[dlDateNums]);
  if (fToDate <> 0) then
  begin
  ...
  lSabDate.TransferToPhoto(lnkDictIDFld, LinkServer[dlDateNums]);
 ...
 if fNumberMask <> '' then
 begin
  lSabNum := MakeSab(DictServer(CurrentFamily).GroupTbl[dlDateNums]);
