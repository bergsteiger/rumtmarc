...
{ $Id: Dt_acces.pas,v 1.35 2013/04/19 13:07:39 lulin Exp $ }
...
Interface
Uses Dt_ATbl,Dt_Types,l3Base, csClient;
const
 mskUsGr_fld  = 1;
 mskFamID_fld = 2;
 mskDocGr_fld = 3;
...
begin
 ...
 SrchRec.UsGr:=aUG_ID;
 RefreshSrchList;
 htSearch(@fSrchList,tmpSab,Handle,mskUG_F_key,EQUAL,@SrchRec,nil);
 Try
  SortFld:=mskF_DG_key;
  ...
  Try
   ...
   Try
    ReadRes:=htReadResults(Masks,@ReadRec,SizeOf(ReadRec))<>0;
...
begin
 ...
 RefreshSrchList;
 AbsNum:=htRecordByUniq(@fSrchList,Handle,mskThree_key,fFullRecord,@RecH);
 if AbsNum=0 then
  begin
...
begin
 ...
 htSearch(@fSrchList,TmpList,Handle,mskUsGr_fld,EQUAL,@aUG_ID,nil);
 Try
...
begin
 ...
 begin
  MaxItem:=DictServer(aFamily).DictTbl[dlAccGroups].GetMaxID+1;
 end
 else
...
begin
 if GlobalHtServer.CurUserID = usServerService then
