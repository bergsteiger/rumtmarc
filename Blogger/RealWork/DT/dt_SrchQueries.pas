...
{ $Id: dt_SrchQueries.pas,v 1.14 2013/01/17 06:19:49 narry Exp $ }
  ...
  case aDictItemID.rDictionaryID of
   ...
   cNumDictID, cGostDictID:
    begin
     if l_NumMask <> '' then
      Exit;
     l_NumMask := l3Str(l3PcharLen(lText.S + aSegment.rOffs, aSegment.rLen, lText.SCodePage));
   ...
   cLawcaseNumDictID:
    begin
     if l_LawCase <> '' then
      Exit;
     l_LawCase := l3Str(l3PcharLen(lText.S + aSegment.rOffs, aSegment.rLen, lText.SCodePage));
    end;
...
begin
 ...
 try
  DictServer(CurrentFamily).DictTbl[dlSources].GetDelNumbers(l_DelSources);
  DictServer(CurrentFamily).DictTbl[dlTypes].GetDelNumbers(l_DelTypes);
  ...
  begin
   ...
   if (l_Sources <> nil) then
   begin
    l_Q := TdtDictQuery.Create(dlSources, l_Sources);
    AndQuery(l_Q);
   end;
 
   if l_Types <> nil then
   begin
    l_Q := TdtDictQuery.Create(dlTypes, l_Types, False, False, True);
    AndQuery(l_Q);
   end;
  end;
