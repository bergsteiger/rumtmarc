...
{ $Id: dt_LinkServ.pas,v 1.12 2013/04/19 13:07:39 lulin Exp $ }
...
type
 ...
 TLinkServer = Class(Tl3Base)
  protected
   ...
   function  GetLinkObj(aLinkType : TDLType) : TLinkTbl;
  ...
  public
   ...
   procedure  GetAttributeList(LT    : TDLType;
                               ID    : TDocID;
                               aList : TPersistent;
                               ShortName : Boolean = False);

   procedure GetDataList(LT : TDLType;ID : LongInt; aDataList : Tl3CustomDataList; Const aSortOrder : TSortRec);
   ...
   //procedure SetDataList(LT : TDLType;ID : LongInt; aAddList,aDelList : Tl3CustomDataList);
   ...
   procedure  AddNodeEx(aDictType: TDLType; aNewRec: PAnsiChar);
   procedure  DelSubNode(aAttr: TdtAttribute; aDoc : TDocID; aSub : TSubID; aDictID : Longint);

   procedure  GetHistogramOfDictElementUsing(LT : TDLType; ResList : Tl3DataList; MinCount : LongInt = 1);
   function   LinkExists(aDict: TDLType;
                         aDocId: TDocID;
                         aDictId: Longint{;
                         aSubId: TSubID = 0}): Boolean;

   //procedure  AddLinksToGroupDict(aDictType: TDLType; aData: TStream);
   //procedure ReplaceLinks(aDictID: TDLType; aOldValue, aNewValue: PAnsiChar; const aLevelSlash: AnsiString); overload;
   ...
   procedure  VerifyLink(aDictType : TDLType; aReportFile : PText; aWithCorrect : Boolean);

   property   Family : TFamilyID read fFamily write SetFamily;
   property   Links[LinkType : TDLType] : TLinkTbl read GetLinkObj;  //default;
...
function TLinkServer.GetLinkObj(aLinkType : TDLType) : TLinkTbl;
begin
...
procedure TLinkServer.GetAttributeList(LT    : TDLType;ID : LongInt;
...
begin
 ...
 Links[LT].GetNodes(ID,Bodies,True);
 try
  if ShortName and
...
procedure TLinkServer.GetDataList(LT : TDLType;ID : LongInt;
...
begin
 ...
 Assert(lDictType <> dlNone, 'Не словарный атрибут');

 // старые DictIds
 if lDictType in sSubLinkDicts then
  TSubLinkTbl(Links[lDictType]).GetSubNodes(aDocId, aSubId, l_OldDictIds)
 else
  Links[lDictType].GetNodes(aDocId, l_OldDictIds, True);
 ...
 try
  ...
  try
   ...
   if l_OldDictIds.gFoundCnt > 0 then
   begin
    l_ValIter := TValuesIterator.Create(l_OldDictIds);
    try
     while not l_ValIter.EOF do
     begin
      l_ValIter.Read(l_DictId);
  ...
  htClearResults(l_OldDictIds);
 end;

 // запись в лог
 GlobalHtServer.BigBrother.LogEditDoc(fFamily, aDocID, MakeDocEditRec(detDiction, lDictType));
end;
...
begin
 ...
 GlobalHtServer.BigBrother.LogEditDoc(fFamily,
                                      aDocId,
                                      MakeDocEditRec(detDiction, lDictType));
end;

function TLinkServer.LinkExists(aDict: TDLType;
                                aDocId: TDocID;
                                aDictId: Longint{;
                                aSubId: TSubID = 0}): Boolean;
var
...
begin
 ...
 GlobalHtServer.BigBrother.LogEditDoc(fFamily, ID, MakeDocEditRec(detDiction, lDictType));
end;
...
begin
 ...
 Links[dlDateNums].GetNodes(ID,Bodies,True);
 try
  DictServer(Family).GroupTbl[dlDateNums].RefreshSrchList;
  htTransferToPhoto(Bodies,DictServer(Family).GroupTbl[dlDateNums].fSrchList,dtIDFld);
  htRecordsByKey(AllIDs,Bodies);
  try
   SrchVal:=dnPublish;
   DictServer(Family).GroupTbl[dlDateNums].GetItemsOnKeyValue(TypeIDs,@SrchVal,dnTypFld,False);
   try
    htAndResults(ResSab,AllIDs,TypeIDs);
    try
     HT(htKeyMinimum(ResSab,dnDateFld,@MinDate));
     DocumentServer(fFamily).FileTbl.UpdSortDate(ID, MinDate);
    finally
     htClearResults(ResSab);
    end;
   finally
    htClearResults(TypeIDs);
   end;
  finally
   htClearResults(AllIDs);
  end;
 finally
  htClearResults(Bodies);
 end;
  ...
  try
   if lDictID in sSubLinkDicts
    then
     ...
     Links[lDictID].GetNodes(ID,Bodies,True);
...
var
 ...
 TypeVal  : TDictID;
begin
 Links[dlSources].GetNodes(ID,SourSab,True);
 Links[dlTypes].GetNodes(ID,TypeSab,True);
 try
...
begin
 ...
 GrMask:=0;
 Links[dlAccGroups].GetNodes(aDoc,Bodies,True);
 if Bodies.gFoundCnt<>0 then
  begin
   Ht(htOpenResults(Bodies,ROPEN_READ,nil,0));
   try
...
begin
 ...
 GlobalHtServer.BigBrother.LogEditDoc(fFamily, aDoc, MakeDocEditRec(detDiction, lDictType));
end;

procedure TLinkServer.AddNodeEx(aDictType: TDLType; aNewRec: PAnsiChar);
...
procedure TLinkServer.GetHistogramOfDictElementUsing(LT : TDLType; ResList : Tl3DataList;
                                                     MinCount : LongInt);
var
 HistoSab : SAB;
...
begin
 ...
 // запись в лог
 GlobalHtServer.BigBrother.LogEditDoc(fFamily, aDstId, MakeDocEditRec(detDiction, lDictType));
end;
...
procedure TLinkServer.VerifyLink(aDictType : TDLType; aReportFile : PText; aWithCorrect : Boolean);
