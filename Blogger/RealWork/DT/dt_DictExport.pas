...
procedure ExportDictionary(aDictServ : TDictionaryServer; aDictType : TDLType; const aDir : TFileName);
...
procedure ExportDictLink(aDictServ: TDictionaryServer; aDict: TDLType; aFile: TFileName; aRenum: Boolean; aProgress:
...
begin
 lAnsiCP := GlobalHtServer.BaseLanguage[CurrentFamily].AnsiCodePage{CP_ANSI};
 lOemCP  := GlobalHtServer.BaseLanguage[CurrentFamily].OEMCodePage {CP_OEM};
 ...
 try
  ...
  try
   ...
   lRootNode := aDictServ.DictRootNode[dlNewClasses];
   lRootNodeOfClasses := aDictServ.DictRootNode[dlClasses];
   l3IterateSubTreeF(lRootNode, l3L2NA(@IterHandler));
  finally
   l3Free(lMoveList);
  end;
...
begin
 ...
 ExportDictTree(aDictServ, dlNewClasses, ConcatDirName(aDir, 'clax.new'), false,
                nil, '', {aProgress : Tl3ProgressProc; aProgTitle: ShortString;}
                         False {WithID}, True {WithCommands}, True {WithNumbers}, True {WithEquals});
 ExportDictTree(aDictServ, dlNewClasses, ConcatDirName(aDir, 'gar.cla'), false,
                nil, '', {aProgress : Tl3ProgressProc; aProgTitle: ShortString;}
 ...
 begin
  ...
  begin
   ...
   begin
    ...
    lNeedOut := (aDictServ.Dict[dlCorSources].DictTbl.GetIsPrivate((lFolderNode as Il3HandleNode).Handle) = 0);

    if lNeedOut then
     lSaveFolderNodeName := l3GetFullPathStr(aDictServ.DictRootNode[dlCorSources{dlPublisheds}], lSaveFolderNode as Il3Node, '\', '', False, True);
   end;
...
begin
 ...
 l3IterateSubTreeF(aDictServ.DictRootNode[dlCorSources{dlPublisheds}], l3L2NA(@IterHandler));
 FileClose(F);
end;
 ...
 begin
  ...
  begin
   lStr := l3ChangeCodePage(lStr,
                    GlobalHtServer.BaseLanguage[CurrentFamily].ANSICodePage,
                    GlobalHtServer.BaseLanguage[CurrentFamily].OEMCodePage);
...
begin
 ...
 case aDictType of
  ...
  //dlKeyWords : ; // отдельная песня

  dlBases       : ; //belongs.tbl + ID, Code, вес базы 2 языка

  dlWarnings    :  //warning.txt
  ...
  (*dlCorSources  : ExportDictTree(aDictServ, aDictType, ConcatDirName(aDir, 'CorSources'), True {AppendCurrent},
                  nil {aProgress}, '' {aProgTitle},
                  False {WithID}, True {WithCommands}, false {WithNumbers}, false {WithEquals});
  *)
  dlPublisheds  :  //publ.sta
  ...
  //dlCorrects    : ; //не сливается
  //dlDateNums    : ; //не сливается
  //dlAccGroups   : ; //не сливается
  //dlAutoClasses : ; //не сливается
 end;
...
var
 lDict : TDLType;
begin
 for lDict := Succ(Low(TDLType)) to High(TDLType) do
  ...
  Try
   ...
   try
    ...
    if aWithDocLinks then
     begin
      //LinkServer[dlKeyWords].RefreshSrchList;
      ...
      if WithoutAbolished and (aDict = dlKeywords) then
      begin
       TmpSab := LinkServer(CurrentFamily).LogBook.GetDocsOnAction(acAbolished);
       htTransferToPhoto(TmpSab,LinkServer(CurrentFamily).Links[aDict].Table.fSrchList,lnkDocIDFld);
       ...
       htClearResults(TmpSab);
       htClearResults(TmpSab1);
      end; // WithoutAbolished and (aDict = dlKeywords) then
