...
// Generated from UML model, root element: <<UtilityPack::Class>> Shared Delphi TFW::DUnitTuning::Source::KTestRunner
//
//
   ...
   try
    ...
    begin
     ...
     begin
      ...
      if f_AllIsOk AND ((l_MaxDate < Date) OR (l_Max <= 100){((l_Min = 0) AND (l_Max = 0))}) then
      ...
      l_F.Write{Ln}(Chart +
                  ...
                  '|width=' + IntToStr(Max(Max(400, l_GraphicsCount * 75), l_MaxSeriesCount * 80)) +
...
//#UC START# *4B6C40530304_4B2F565002FD_impl*
 Result.rDate := aDate;
 Result.rID.rName := aName;
 Result.rTime := aTime;
 if (aSubName = '') then
  Result.rID.rSubName := 'Time'
 else
  Result.rID.rSubName := aSubName;
...
//#UC START# *4B2B6B800265_4B2A6CEB0377_impl*
 ...
 try
  CheckTimeInfo;
  f_TimeInfo.Add(TTimeInfo_C('_ Sys Local Memory Used', Norm(l3System.AllLocalMemUsed), 'Left'));
  f_TimeInfo.Add(TTimeInfo_C('_ Local Memory Used', Norm(Max(l3System.LocalMemUsed, 0)), 'Left'));
  f_TimeInfo.Add(TTimeInfo_C('_ Global Memory Used', Norm(l3System.GlobalMemUsed), 'Left'));
  f_TimeInfo.Add(TTimeInfo_C('_ Object Memory Used', Norm(l3System.ObjectMemUsed), 'Left'));
  f_TimeInfo.Add(TTimeInfo_C('_ Objects Used', Norm(l3System.ObjectCount), 'Left'));

  f_TimeInfo.Add(TTimeInfo_C('_ Sys Local Memory Used', Norm(l3System.MaxAllLocalMemUsed), 'Max'));
  f_TimeInfo.Add(TTimeInfo_C('_ Local Memory Used', Norm(l3System.MaxLocalMemUsed), 'Max'));
  f_TimeInfo.Add(TTimeInfo_C('_ Global Memory Used', Norm(l3System.MaxGlobalMemUsed), 'Max'));
  f_TimeInfo.Add(TTimeInfo_C('_ Object Memory Used', Norm(l3System.MaxObjectMemUsed), 'Max'));
  f_TimeInfo.Add(TTimeInfo_C('_ Objects Used', Norm(l3System.MaxObjectCount), 'Max'));
  if (f_TimeInfo <> nil) then
   PutTimesToK;
 except
  on E : Exception do
...
//#UC START# *4C44691E00D2_4B2A6D4303A9_impl*
 ...
 begin
  ...
  if (l_Path <> '') AND
     ...
     not ANSISameText(l_Path, 'Scripts\') AND
     not ANSISameText(l_Path, 'Auto\') AND
     not ANSISameText(l_Path, 'Dictionary\') then
   Result := aStr
  else
  begin
   l_Path := g_CVSPath + '\' + TBaseTest.TestSetFolderName;
   if DirectoryExists(ExtractFilePath(l_Path + '\' + aStr)) and not g_IgnoreCVS then
   begin
    Result := l_Path + '\' + aStr;
    if FileExists(Result) then
     Exit;
   end;//DirectoryExists(l_Path)
   Result := ExtractFilePath(ParamStr(0)) + TBaseTest.TestSetFolderName + '\' + aStr;
