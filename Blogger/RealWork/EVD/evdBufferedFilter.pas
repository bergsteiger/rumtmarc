...
{ $Id: evdBufferedFilter.pas,v 1.15 2011/09/15 12:43:55 voba Exp $ }
...
type
  ...
  {*- Умеет буфферизовывать нужные теги (задается в NeedStartBuffering),
    ...
    // internal methods
      ...
      function  NeedFlushBuffer(const aLeaf: Ik2Tag; aTagId: Integer): Boolean;
        virtual;
      ...
      procedure FlushBuffer(const aLeaf: Ik2Tag; aTagId: Integer; aNeedCloseBracket : Boolean);
        {-}
  ...
  protected
    function ChildTypeForFiltering: Integer; virtual;
...
function  TevdBufferedFilter.NeedFlushBuffer(const aLeaf: Ik2Tag; aTagId: Integer): Boolean;
  //virtual;
  {-}
begin
 Result := true;
...
procedure TevdBufferedFilter.DoFlushBuffer(const aLeaf: Ik2Tag; aTagId: Integer; aNeedCloseBracket : Boolean);
  //virtual;
  {-}
begin
...
procedure TevdBufferedFilter.FlushBuffer(const aLeaf: Ik2Tag; aTagId: Integer; aNeedCloseBracket : Boolean);
  {-}
begin
...
procedure TevdBufferedFilter.StartChild(TypeID: Integer);
begin
 if (SkipLevel = 0) then
 begin
  if NeedStartBuffering(-TypeID) then
   IncSkipTags;
 end;//SkipLevel = 0
 if SkipLevel > 0 then
...
begin
 ...
 begin
  ...
  // - открываем коробку для тегов (K 93260874)
   f_Buffer.StartChild(CurrentType.ID);
...
function TevdCustomChildBufferedFilter.ChildTypeForFiltering: Integer;
  //virtual;
  {-}
begin
...
begin
 Result := aID = -ChildTypeForFiltering;
end;
...
procedure TevdTagKeeperFilter.StartChild(TypeID: Integer);
begin
 if NeedKeepThisTag(-TypeID) then
 begin
  KeepComplete;
  fStarted := True;
 end;
