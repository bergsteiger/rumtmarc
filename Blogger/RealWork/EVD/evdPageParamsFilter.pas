...
{ Библиотека "EVD"    }
{ Начал: Дудко Д.В.       }
{ Модуль: evdPageParamsFilter - добавляет в "трубу" параметры страницы документа }
{ Начат: 20.01.2005 13:20 }
{ $Id: evdPageParamsFilter.pas,v 1.10 2011/06/17 10:16:48 lulin Exp $ }
...
// - new: Проверяем печатаемую область и бросаем исключение (cq27281)
//
 ...
 begin
  ...
  begin
   ...
   if (l_TT = nil) OR l_TT.IsNull then
    // - срезаем отступы - только для основного документа (для автореферата)
    Exit;
...
procedure TevdPageParamsFilter.DoStartChild(TypeID: Long);
...
begin
 ...
 begin
  l_TT := TopType[1];
  if (l_TT = nil) OR l_TT.IsNull then
  // - учитываем, что это не вложенный документ (для автореферата)
   if (Generator <> nil) then
   begin
    { Передаются только Ширина, Левый и Правый отступы. Для Альбомной ориентации
      вместо ширины передается высота }
