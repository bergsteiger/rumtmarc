...
var
 ...
 function mEnum(i : Integer;
                ...
                aTo         : String = ''): String;
 begin//mEnum
  Result := AnsiReplaceStr(GetEnumName(aType, i), aFrom, aTo);
 end;//mEnum

 function mType(i : Integer): String;
  overload;
 begin//mType
  if (i > Ord(High(Tk2TypeID))) then
   Result := 'id_Empty'
  else
  begin
   Result := mEnum(Ord(i), TypeInfo(Tk2TypeID), '_k2_id', 'id_');
   Assert(Result <> '');
  end;//i > Ord(High(Tk2TypeID))
 end;//mType

 function mType(i : Tk2TypeID): String;
  overload;
 begin
  Result := mType(Ord(i));
 end;

 function mTypeX(i : Tk2TypeID): String;
 begin//mTypeX
  Result := mEnum(Ord(i), TypeInfo(Tk2TypeID), '_k2_id', '');
 end;//mTypeX
 ...
 procedure PutDefaultChildren;
  ...
  begin//PutEmpty
   Put('%s // %s %d -> no default %d', [mType(_k2_idEmpty), aType, i, Ord(_k2_idEmpty)]);
  end;//PutEmpty
 ...
 begin//PutDefaultChildren
  ...
  l_First := true;
  for i := Low(Tk2TypeID) to High(Tk2TypeID) do
  begin
   PutComma;
   l_Type := l_Table[Ord(i)];
   if (l_Type = nil) then
   ...
   begin
    ...
    begin
     ...
     Put('%s // %s %d -> %s %d', [mType(Tk2TypeID(l_CT.ID)), l_Type.AsString, Ord(i), l_CT.AsString, l_CT.ID]);
    end;//l_Prop = nil..
   end;//l_Type = nil
  end;//for i
 ...
 begin//PutInherrits
  ...
  Put;
  Put('const %s %s[%d][%d] = {', ['bool', c_Inherits, Ord(High(Tk2TypeID))+1, Ord(High(Tk2TypeID))+1]);
  l_First := true;
  for i := Low(Tk2TypeID) to High(Tk2TypeID) do
  begin
   PutComma;
   ...
   l_First := true;
   l_A := l_Table[Ord(i)];
   for j := Low(Tk2TypeID) to High(Tk2TypeID) do
   begin
    PutComma;
    if (l_A = nil) then
    ...
    begin
     if l_A.InheritsFrom(Ord(j)) then
 ...
 procedure PutBaseChildren;
  ...
  procedure PutEmpty(const aType: String; i: Integer);
  begin//PutEmpty
   Put('%s // %s %d -> no children %d', [mType(_k2_idEmpty), aType, i, Ord(_k2_idEmpty)]);
  end;//PutEmpty
 ...
 procedure PutAttrTypes;
  ...
  begin//PutEmpty
   Put('%s // %s %d -> no attr %s %d', [mType(_k2_idEmpty), aType, i, mTagX(j), Ord(j)]);
  end;//PutEmpty
 ...
 begin//PutAttrTypes
  ...
  Put;
  Put('const %s %s[%d][%d] = {', [c_TypeID, c_AttrType, Ord(High(Tk2TypeID))+1, Ord(High(Tk2TagID))+1]);
  l_First := true;
  for i := Low(Tk2TypeID) to High(Tk2TypeID) do
  begin
   PutComma;
   ...
   begin
    ...
    begin
     ...
     if (l_Prop = nil) then
      ...
      Put('%s // %s %d -> %s %d', [mType(Tk2TypeID(l_Prop.AtomType.ID)), l_Type.AsString, Ord(i), l_Prop.AsString, l_Prop.TagIndex]);
    end;//for j
   end;//l_Type = nil
   Put('}');
  end;//for i
 ...
 const
  ...
  begin//PropValueName
   ...
   begin
    ...
    begin
     try
      Result := Tk2Type(aProp.TagType).IntToTag(aProp.DefaultValue).AsString;
     except
     end;//try..except
 ...
 begin//PutAttrDefaultValues
  ...
  try
   ...
   Put;
   Put('const %s %s[%d][%d] = {', ['long', c_AttrDefaultValue, Ord(High(Tk2TypeID))+1, Ord(High(Tk2TagID))+1]);
   l_First := true;
   for i := Low(Tk2TypeID) to High(Tk2TypeID) do
   begin
    PutComma;
    ...
    begin
     ...
     begin
      ...
      begin
       if l_Table[Ord(i)].InheritsFrom(k2_idLeafPara) AND
          l_Prop.AtomType.InheritsFrom(k2_idStyle) then
 ...
 begin//PutAttrChilds
  ...
  Put;
  Put('const %s %s[%d][%d] = {', [c_TypeID, c_AttrChild, Ord(High(Tk2TypeID))+1, Ord(High(Tk2TagID))+1]);
  l_First := true;
  for i := Low(Tk2TypeID) to High(Tk2TypeID) do
  begin
   PutComma;
    ...
    begin
     ...
     begin
      ...
      if (l_CProp = nil) then
       ...
       Put('%s // %s %d -> %s %d', [mType(Tk2TypeID(l_CProp.ChildType.ID)), l_Type.AsString, Ord(i), l_Prop.AsString, l_Prop.TagIndex]);
     end;//l_Prop = nil
    end;//for j
   end;//l_Type = nil
   Put('}');
 ...
 procedure PutIsType(const aName: String; aType: Integer; aComment: String);
 var
(*  i       : Tk2TypeID;
  l_Type  : Ik2Type;*)
  l_Name  : String;
 begin//PutIsType
  ...
  Put('// %s', [aComment]);
  Put(' return %s[%s][%s];', [c_Inherits, 'type', mType(Tk2TypeID(aType))]);
(*  Put(' if (');
  l_First := true;
  for i := Low(Tk2TypeID) to High(Tk2TypeID) do
  begin
 ...
 begin//WriteTypes
  ...
  Put;
  for l_T := Low(l_T) to High(l_T) do
   Put('CONST evd::%s %d', [mType(l_T), Ord(l_T)]);
 end;//WriteTypes

 procedure WriteStyles;
 
  function OutStyle(aStyle: PObject; Index: Long): Bool;
  var
   l_Tag : Ik2Tag;
   l_H   : Integer;
  begin//OutStyle
   Result := true;
   if Supports(aStyle^, Ik2Tag, l_Tag) then
    try
  ...
  function OutStyle1(aStyle: PObject; Index: Long): Bool;
  var
   l_Tag : Ik2Tag;
   l_H   : Integer;
  begin//OutStyle1
 ...
 begin//WriteControlParaAttrValues
  ...
  Put;
  l_CP := k2_typControlPara.MakeTag;
  try
   for l_TID := Low(l_TID) to High(l_TID) do
   begin
    l_Prop := Tk2CustomProperty(l_CP.TagType.Prop[Ord(l_TID)]);
    if (l_Prop <> nil) then
    begin
 ...
 try
  ...
  try
   ...
   begin
    ...
    //PutVect('TypeName', TypeInfo(Tk2TypeID), 'имена типов', '_k2_id', '');
    ...
    Put;
    PutIsType('leaf_para', k2_idLeafPara, 'Определяет, что параграф листьевой');
    PutIsType('para_list', k2_idParaList, 'Определяет, что параграф является списком параграфов');
    PutIsType('para', k2_idPara, 'Определяет, что тег является параграфом');
    PutIsType('text_para', k2_idTextPara, 'Определяет, что параграф является текстовым');
    PutIsType('styled_leaf_para', k2_idStyledLeafPara, 'Определяет, что параграф является листьевым и имеет стиль');
