...
{ $Id: evdReader.pas,v 1.79 2013/04/08 10:16:40 lulin Exp $ }
...
{$IfNDef Nemesis}
  TevdFramesStatus = (evd_fsCheck, evd_fsNewFrames, evd_fsOldFrames);
    {* - Устанвока рамок: автоматическая по дате и версии, принудительно старые, принудительно новые. }
{$EndIf  Nemesis}
  ...
  TevdNativeReader = class(Tk2CustomFileParser)
   {* Читатель тегов в формате evd. }
    ...
    // internal fields
      ...
      {$IfNDef Nemesis}
      f_OldFrames           : Bool;
      f_CheckFrames         : TevdFramesStatus;
      {$IFDEF evdCheckFrames}
      f_DocumentDate        : TDateTime;
      {$ENDIF evdCheckFrames}
      {$EndIf  Nemesis}
      {$IFDEF evdCollTechComm}
      f_CellStarted         : Bool;
      {$ENDIF evdCollTechComm}
      {$IF Defined(evdCollect132) or Defined(evdCollTechComm)}
      f_Stat    : Tl3CustomFiler;
      f_WasStat : Boolean;
      {$IFEND}
    private
    // property fields
    ...
    // public properties
      ...
      property NotReadAsText: Bool
        ...
        {-}
{$IfNDef Nemesis}
      property CheckFrames: TevdFramesStatus
        read f_CheckFrames
        write f_CheckFrames
        default evd_fsCheck;
        {* - Поле режима устанвоки рамок. }
      property OldFrames: Bool
        read f_OldFrames;
        {* - Работали со старыми или новыми рамками. }
{$ENDIF Nemesis}        
  end;//TevdNativeReader

{$IF Defined(evdCollect132) or Defined(evdCollTechComm)}
var
 g_AllowRaiseAbort : Boolean = false;
{$IFEND}

{$IFDEF evdCheckFrames}
var
 g_OldFramesDate : TDateTime = 0;
{$ENDIF evdCheckFrames}
...
{$IfNDef Nemesis}
{$IFDEF evdCheckFrames}
 f_CheckFrames := evd_fsCheck;
{$ELSE}
 f_CheckFrames := evd_fsOldFrames;
{$ENDIF evdCheckFrames}
{$ENDIF Nemesis}
...
{$Else  evdTabReadOrd}
function TevdNativeReader.ReadOrd(aFiler : Tl3CustomFiler;
                                  ID     : TevTypeID): sInt32;
  {-}
var
 sI32 : sInt32;
 uI32 : uInt32 absolute sI32;
 uI8  : uInt8  absolute sI32;
 sI8  : sInt8  absolute sI32;
 uI16 : uInt16 absolute sI32;
 sI16 : sInt16 absolute sI32;
begin
 Case ID of
  ev_idSInt8  :
  begin
   aFiler.Read(@sI8, SizeOf(sI8));
   Result := sI8;
  end;
  ev_idUInt8  :
  begin
   aFiler.Read(@uI8, SizeOf(uI8));
   Result := uI8;
  end;
  ev_idSInt16 :
  begin
   aFiler.Read(@sI16, SizeOf(sI16));
   Result := sI16;
  end;
  ev_idUInt16 :
  begin
   aFiler.Read(@uI16, SizeOf(uI16));
   Result := uI16;
  end;
  ev_idSInt32 :
  begin
   aFiler.Read(@sI32, SizeOf(sI32));
   Result := sI32;
  end;
  ev_idUInt32 :
  begin
   aFiler.Read(@uI32, SizeOf(uI32));
   Result := uI32;
  end;
  else
  begin
   Error(Format('Invalid type ID: %d', [Ord(ID)]), false);
   Result := 0;
  end;//else
 end;//Case ID
end;
  ...
  Case CurType of
   ...
   begin
    ...
    if CheckFontNames then
     AddStringAtom(TagID, Tl3CustomString(f_FontNames.Items[l_ID]));
   end;//ev_idLong
...
begin
 ...
 {$IfNDef Nemesis}
 {$IFDEF evdCheckFrames}
 f_DocumentDate := FlexStr2Date(evEVDCurRevisionDate);
 f_OldFrames := False;
 {$ELSE}
 f_OldFrames := Bool(Pred(f_CheckFrames));
 {$ENDIF evdCheckFrames}
 {$EndIf  Nemesis}
end;
...
begin
 {$IF Defined(evdCollect132) or Defined(evdCollTechComm)}
 if not g_AllowRaiseAbort then
 begin
  if (f_Stat <> nil) then
   f_Stat.Close;
  FreeAndNil(f_Stat);
 end;//not g_AllowRaiseAbort
 {$IFEND}
 if (f_FontNames <> nil) then
  f_FontNames.Clear;
...
begin
 {$IfNDef Nemesis}
 {$IfDef evdCollect132}
 if not f_WasStat AND
    g_AllowRaiseAbort AND
    (CurrentVersion <> evNonPackedVer) AND
    (CurrentVersion <> 135) then
 begin
  f_WasStat := true;
  raise EAbort.Create('');
 end;//CurrentVersion <> evNonPackedVer
 {$EndIf evdCollect132}
 {$IFDEF evdCollTechComm}
 CT := CurrentType;
 if (CT <> nil) AND (CT.ID = k2_idTableCell) then
  f_CellStarted := False;
 if f_WasStat then
  CheckStat;
 {$ENDIF evdCollTechComm}
 if (CurrentVersion <= evNonPackedVer)
    {$IfDef evdCollect132}
    OR (CurrentVersion = 135)
    {$EndIf evdCollect132} then
 begin
  CT := CurrentType;
  if (CT <> nil) AND (CT.ID = k2_idTableCell) then
  begin
   {$IfDef evdCollect132}
   if (CurrentVersion = 135) OR
      ((CurrentVersion = evNonPackedVer) AND not f_OldFrames) then
    if not f_WasStat then
    begin
     f_WasStat := true;
     CheckStat;
    end; // if not f_WasStat then
   {$EndIf evdCollect132}
   if (CurrentVersion < evNonPackedVer) or f_OldFrames then // Проверка нужна только для 132 версии...
   begin
    {$IfDef evdCollect132}
    if not f_WasStat AND g_AllowRaiseAbort then
    begin
     f_WasStat := true;
     raise EAbort.Create('');
    end;//not f_WasStat 
    {$EndIf evdCollect132}
    AddIntegerAtom(k2_tiFrame, 0);
    // - тут восстанавливаем скрытые рамки - http://mdp.garant.ru/pages/viewpage.action?pageId=158336759
   end;//if (CurrentVersion < evNonPackedVer) or f_OldFrames then
  end;//TypeID = k2_idTableCell
 end;//CurrentVersion <= evNonPackedVer
 {$EndIf  Nemesis}
 inherited;
...
begin
 if (CurrentVersion <= evFormatOldInch) then
 begin
  l_CT := CurrentType;
  if (l_CT <> nil) then
  begin
   l_Prop := l_CT.Prop[aTagID];
   if (l_Prop <> nil) AND l_Prop.AtomType.InheritsFrom(k2_idInch) then
    aValue := l3MulDiv(aValue, l3Inch, evdOldInchMul);
  end;//l_TT <> nil
  AddIntegerAtom(aTagID, aValue);
 end//CurrentVersion <= evFormatOldInch
 else
  AddIntegerAtom(aTagID, aValue);
end;
         ...
         try
          ...
          try
           ...
           begin
            ...
            try
             AddStringAtom(k2_tiText, l_Stream._String);
            finally
             SafeFinish;
            end;//try..finally
           end;
...
begin
  ...
  begin
   ...
   begin
    try
         ...
         begin
          Case l_Parser.KeyWord.StringID of
           ...
           begin
            ...
            {$IFDEF evdCheckFrames}
            try
             f_DocumentDate := FlexStr2Date(l_Parser.TokenString);
             if (f_CheckFrames = evd_fsCheck) then
             begin
              f_OldFrames := false;
              if (f_DocumentDate <= g_OldFramesDate) then
               f_OldFrames := True;
              if (Date < g_Magic) and (f_DocumentDate >= Date) then // Типа пытаемся перестраховаться от будущих дат...
               f_OldFrames := True;
             end // if (f_CheckFrames = evd_fsCheck) then
             else
              f_OldFrames := Bool(Pred(f_CheckFrames));
            except
             on EConvertError do; // Если дату не переварили, то оставляем как было...
            end;
            {$ENDIF evdCheckFrames}
            {$EndIf  Nemesis}
            l_Parser.NextToken;
     ...
     begin
  {$IFDEF evdCollTechComm}
      if g_AllowRaiseAbort or NotReadAsText then
  {$ELSE}
 ...
 {$IF Defined(evdCollect132) or Defined(evdCollTechComm)}
  ...
  on EAbort do CheckException;
  on El3ReadError do CheckException;
 end;//try..except
 {$IFEND}
end;

procedure TevdNativeReader.StartChild(TypeID: Long);
  //override;
  {-}
var
 l_CT : Tk2Type;
begin
 ...
 begin
  ...
  if (l_CT <> nil) then
  begin
   if (TypeID = k2_idTableRow) AND (l_CT.ID = k2_idSBS) then
    TypeID := k2_idSBSRow
   else
   if (TypeID = k2_idTableCell) AND (l_CT.ID = k2_idSBSRow) then
    TypeID := k2_idSBSCell;
  end;//l_CT <> nil
 end;//CurrentVersion <= evPackedVer
 {$IFDEF evdCollTechComm}
 ...
 {$ENDIF evdCollTechComm}
 inherited;
 if not f_DocumentWasOpened AND CurrentType.InheritsFrom(k2_idDocument) then
...
{$IF Defined(evdCollect132) or Defined(evdCollTechComm)}
procedure TevdNativeReader.AddAtom(AtomIndex: Long; TK: Tk2TypeKind;
  const Value);
begin
 if f_CellStarted and CurrentType.InheritsFrom(k2_idTextPara) and (AtomIndex = k2_tiStyle) then
  f_WasStat := Integer(Value) = ev_saTxtComment;
 inherited;
end;

procedure TevdNativeReader.CheckStat;
begin
 if (f_Stat = nil) then
 begin
  f_Stat := Tl3DOSFiler.Make(ChangeFileExt(ParamStr(0), '.132')  + '.csv', l3_fmWrite);
  f_Stat.Open;
 end;//f_Stat = nil
 f_Stat.WriteLn(IntToStr(Parser.Filer.Handle));
 f_Stat.Flush;
 f_WasStat := False;
 if g_AllowRaiseAbort then
  raise EAbort.Create('');
end;

procedure TevdNativeReader.CheckException;
begin
 if g_AllowRaiseAbort then
 begin
  while StreamOpened do
   Finish(true);
  f_WasStat := false;
 end;//g_AllowRaiseAbort
end;
{$IFEND}

{$IFDEF evdCheckFrames}
initialization
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\Garant\EVD\evdReader.pas initialization enter'); {$EndIf}
 g_OldFramesDate := EncodeDate(2009,07,23);
 g_Magic := EncodeDate(2009,08,08);
{$ENDIF evdCheckFrames}

{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\Garant\EVD\evdReader.pas initialization leave'); {$EndIf}
end.
