...
EeGenerator_i::EeGenerator_i (TeeWriter writer, void* user_data, long buf_size)
	: m_buffer (0)
...
long EeGenerator_i::flush_buffer () {
	long ret = 0;
	if (m_writer) {
		m_writer ((unsigned char*) m_buffer, m_offset, m_user_data);
		ret = m_offset;
		m_offset = 0;
	}
	return ret;
...
long EeGenerator_i::write_buffer (const void* buf, long len) {
 	if (m_buffer) {
		...
		return m_writer ((unsigned char*) buf, len, m_user_data);
	}
	return 0;
}
