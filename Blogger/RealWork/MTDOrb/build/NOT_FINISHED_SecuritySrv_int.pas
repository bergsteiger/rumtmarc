unit NOT_FINISHED_SecuritySrv_int;

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
// Библиотека "MTDOrb"
// Модуль: "w:/common/components/rtl/external/MTDOrb/build/NOT_FINISHED_SecuritySrv_int.pas"
// Родные Delphi интерфейсы (.pas)
// Generated from UML model, root element: <<UtilityPack::Class>> Shared Delphi TFW::MTDOrb::build::SecuritySrv_int
//
//
// Все права принадлежат ООО НПП "Гарант-Сервис".
//
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

// ! Этот файл используется только для моделирования, а не для компиляции. !

interface

{$If defined(MTDORB) AND defined(nsTest) AND not defined(NotTunedDUnit)}
type
 TCoreSrv_SecuritySrv_Token = class
 end;//TCoreSrv_SecuritySrv_Token

 ICoreSrv_SecuritySrv_SessionManager = interface(IUnknown)
   ['{577D0C09-6ECE-4F9E-96D7-279B2A0B4576}']
 end;//ICoreSrv_SecuritySrv_SessionManager
{$IfEnd} //MTDORB AND nsTest AND not NotTunedDUnit

implementation

{$If defined(MTDORB) AND defined(nsTest) AND not defined(NotTunedDUnit)}
{$IfEnd} //MTDORB AND nsTest AND not NotTunedDUnit
end.