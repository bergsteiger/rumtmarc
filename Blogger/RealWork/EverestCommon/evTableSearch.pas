...
type
 ...
 protected
 // protected methods
   function CheckCellType(const aCell: InevTag): Boolean; virtual;
 ...
 // overridden protected methods
   function CheckCellType(const aCell: InevTag): Boolean; override;
...
function TevTableTextSearcher.CheckCellType(const aCell: InevTag): Boolean;
...
function TevTableTextSearcher.DoCheckText(const aPara: InevTag;
  aText: Tl3CustomString;
  const aSel: TevPair;
...
//#UC START# *50751B680376_50751AB90061_impl*
 ...
 if Result then
  if evInPara(aPara, k2_idTableCell, [k2_idSBSCell], l_Cell) then
...
function TevFirstTableCellSearcher.DoCheckText(const aPara: InevTag;
  aText: Tl3CustomString;
  const aSel: TevPair;
  out theSel: TevPair): Boolean;
...
//#UC START# *4D553AC103A3_50751B080168_impl*
 if evInPara(aBlock.BottomChildBlock(aView).Obj^, k2_idTable, [k2_idSBS], l_Table) then
 begin
...
//#UC START# *4D553AC103A3_50751B12004F_impl*
 if evInPara(aBlock.BottomChildBlock(aView).Obj^, k2_idTableCell, [k2_idSBSCell], l_Cell) then
 begin
  Result := true;
...
function TevTextInContinueCellSearcher.CheckCellType(const aCell: InevTag): Boolean;
