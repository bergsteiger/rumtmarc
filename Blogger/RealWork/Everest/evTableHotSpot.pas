...
{ $Id: evTableHotSpot.pas,v 1.50 2011/09/13 10:48:06 lulin Exp $ }
...
begin
 ...
 if (l_Map <> nil) then
  ...
  begin
   {$IfDef Nemesis}
   if not aChild.InheritsFrom(k2_idReqRow) then
   {$EndIf Nemesis}
   begin
    if aPt.HasInner then
    begin
     Result := True;
     theSpot := TevTableHotSpot.Make(aChild.OwnerObj, Processor, aPt.Inner.Obj.PID);
     Exit;
    end;//aPt.HasInner
   end;//not aChild.InheritsFrom(k2_idReqRow)
...
constructor TevTableHotSpot.Create(const aTag       : Ik2Tag;
                                   const aProcessor : Ik2Processor;
                                   aColumnIndex     : Integer);
  //reintroduce;
  {-}
...
class function TevTableHotSpot.Make(const aTag       : Ik2Tag;
                                    const aProcessor : Ik2Processor;
                                    aColumnIndex     : Integer): IevHotSpot;
  //reintroduce;
  {-}
 ...
 case aButton of
  ...
  begin
   case anAction of
    ...
    begin
(*     {$IFDEF Nemesis}
     Result := aView.Control.Selection.Select(ParaX.Range, false);
     {$ElSE}*)
     // http://mdp.garant.ru/pages/viewpage.action?pageId=219125149&focusedCommentId=220202298#comment-220202298
