...
{ $Id: evMarker.pas,v 1.81 2012/01/27 07:45:59 dinishev Exp $ }
...
// - rename method: Tk2AtomR._rAtom -> rAtom.
//
...
type
  ...
  // internal methods
    ...
    procedure DoSetValue(aValue: Long; const Context: InevOp);
      virtual;
      {-}
    function  GetLevel(const aPara: Ik2Tag): Long;
      {-}
  ...
  // public methods
    ...
    procedure IevMarker.SetValue = IevMarker_SetValue;
      {-}
    function  CompareLevel(const aTagWrap: Ik2Tag): Long;
...
begin
 ...
 f_View := aView;
 //Para := aPara;
 f_Type := Tk2Prop(aPara.TagType.Prop[aTag]);
 Style := aStyle;
 f_Acc  := Value;
 Hint := aHint;
end;
...
function TevMarker.GetLevel(const aPara: Ik2Tag): Long;
  {-}
var
 l_Parent : Ik2Tag;
begin
 ...
 while l_Parent.IsValid AND l_Parent.InheritsFrom(k2_idPara) do
 begin
  Inc(Result);
...
begin
 if (HitType <> nil) then
 begin
  ParaInst.IntW[HitType.TagIndex, aContext] := aValue;
...
procedure TevMarker.Assign(P: TPersistent);
  {override;}
  {-}
begin
...
procedure TevMarker.TagChanged(const anOld, aNew: Ik2Tag);
  //override;
  {-}
begin
 inherited;
 ...
 begin
  if (HitType <> nil) then
   HitType := Tk2Prop(aNew.TagType.Prop[HitType.TagIndex]);
 end;//aNew <> nil
end;
...
function TevMarker.CompareLevel(const aTagWrap: Ik2Tag): Long;
  {* - Сравнить уровени вложенности маркера и тега. }
begin
...
begin
 if (HitType = nil) then
  ...
  Result := ParaInst.IntA[HitType.TagIndex];
end;
