...
{$Define evParaPainter_imp}
 ...
 // protected methods
   ...
   procedure DrawDecorObject(const aDecorObj: InevTag;
     aCollapsed: Boolean;
   ...
   procedure GetFramePart(const aFrame: InevTag;
 ...
 public
 // public methods
   class function Make(const aTag: InevTag): IevPainter; reintroduce;
 protected
...
{$IfDef ParaPainter_IsParaListPainter}
 ...
 if f_TopAnchor.AtStart then
  evDrawSub(ParaX.IntA[k2_tiLayerID], ParaX, ParaX)
 {$ELSE}
  ...
  begin
   ...
   if (l_Para <> nil) AND l_Para.IsValid then
    if (l_Para.PID < 10) then
     if l_Para.Owner.IsSame(ParaX) then
     begin
      if (l_MI.VertPosition(Area.rView.As_InevView, nil) <= 0) then
       while (l_Para <> nil) AND l_Para.IsValid AND not l_Para.IsList do
       begin
...
procedure _evParaPainter_.DrawDecorObject(const aDecorObj: InevTag;
  aCollapsed: Boolean;
...
class function _evParaPainter_.Make(const aTag: InevTag): IevPainter;
var
...
//#UC START# *4804B77A039C_4804A2C101A5_impl*
 l_FrameTag := ParaX.Attr[k2_tiFrame];
 l_Bounds := Map.Bounds;
...
procedure _evParaPainter_.GetFramePart(const aFrame: InevTag;
  anIndex: Tl3FramePartIndex;
  var thePart: TnevFramePart);
  ...
  l3_fpiUp :
   ...
   begin
    ...
    // !!! ещё бы вспомнить - зачем
     if ParaX.InheritsFrom(k2_idTableCell) then
      Exit;
...
procedure _evParaPainter_.GetFramePartPrim(const aFrame: InevTag;
  anIndex: Tl3FramePartIndex;
  var thePart: TnevFramePart);
...
var
 ...
 function DrawSubLayer(const aLayer: InevTag; Index: Integer): Boolean;

  function l_DrawSub(const aSub: InevTag; Index: Integer): Boolean;
  begin//l_DrawSub
   Result := true;
   evDrawSub(aLayer.IntA[k2_tiHandle], aSub, l_Prev);
  end;//l_DrawSub

 begin//DrawSubLayer
  aLayer.IterateChildrenF(k2L2TIA(@l_DrawSub));
  Result := true;
 end;//DrawSubLayer
...
//#UC START# *4804BC570301_4804A2C101A5_impl*
 ...
 begin
  l_Prev := ParaX;
  l_Prev.Attr[k2_tiSubs].IterateChildrenF(k2L2TIA(@DrawSubLayer));
  ...
  // http://mdp.garant.ru/pages/viewpage.action?pageId=232095992
   ...
   begin
    ...
    begin
     ...
     if (l_Prev <> nil) AND l_Prev.IsValid then
     begin
...
var
 l_Rect          : Tl3Rect;
 {$If Defined(evUseTreePara) OR Defined(Nemesis)}
...
//#UC START# *4804BC640021_4804A2C101A5_impl*
 assert(aSub <> nil);
 if aSub.InheritsFrom(k2_idDocumentSub) then
  Exit;
 if not aSub.Attr[k2_tiHandle].IsValid then
 begin
  {$IfDef Nemesis}
  if aSub.InheritsFrom(k2_idMark) then
  begin
 ...
 {$IfDef Nemesis}
 if (aLayerID = ev_sbtMark) AND aPara.InheritsFrom(k2_idLeafPara) AND
    evInPara(aPara, k2_idCommentPara) then
 ...
 {$IfDef evUseTreePara}
 if (aLayerID = ev_sbtSub) then
 begin
  l_PrevParaIndex := Pred(aPara.PID);
  if (l_PrevParaIndex >= 0) then
  begin
   with aPara.OwnerPara[l_PrevParaIndex] do
    if InheritsFrom(k2_idTreePara) then
    begin
     if Supports((Attr[k2_tiObject].AsObject As Tl3Tree).RootNode, InevTag, l_TagWrap) then
      try
       if l_TagWrap.IsSame(aSub) then
       begin
        l_Rect.OffsetPt(l3PointY({-Shape.Height}0)); // <stub> - Height
        l_Rect.OffsetPt(l3PointY(-Spaces.Top));
       end;//l_TagWrap.IsSame(aSub)
      finally
       l_TagWrap := nil;
      end;//try..finally
    end;//..k2_idTreePara..
  end;//l_PrevParaIndex >= 0
 end//aLayerID = ev_sbtSub
 {$IfDef Nemesis}
 else
 {$Else  Nemesis}
 ;
 ...
 {$IfDef Nemesis}
 if f_Hidden AND
    (aLayerID = ev_sbtMark) then
 begin
  if ((aSub.IntA[k2_tiFlags] = ev_cUserCommentFlags) OR
     (aPara.rAtomEx([k2_tiSubs,
                 k2_tiChildren, k2_tiHandle, ev_sbtSub]).ChildrenCount = 0)) then
  begin
   ...
   begin
    ...
    begin
     ...
     end;//Para.InheritsFrom(k2_idLeafPara)
     Dec(l_PrevParaIndex);
     if (l_PrevParaIndex >= 0) AND
        (aSub.IntA[k2_tiFlags] <> ev_cUserCommentFlags) AND
        InheritsFrom(k2_idTextPara) AND IsEmpty then
     begin
  ...
  begin
   ...
   begin
    with aPara.OwnerPara[l_PrevParaIndex] do
     if InheritsFrom(k2_idTextPara) AND IsEmpty then
     begin
   ...
   if (l_PrevParaIndex < aPara.OwnerPara.ParaCount) then
    ...
    with aPara.OwnerPara[l_PrevParaIndex] do
     l_Rect.OffsetPt(l3PointY(IntA[k2_tiSpaceBefore]));
  end;//aSub.IntA[k2_tiFlags] = ev_UserCommentFlags
 end;//f_Hidden..
 {$EndIf Nemesis}
 {$IfDef ParaPainter_IsParaListPainter}
 if aSub.InheritsFrom(k2_idBlock) then
  l_Rect.OffsetPt(l3PointY(aSub.IntA[k2_tiSpaceBefore]));
 {$EndIf ParaPainter_IsParaListPainter} 
 Area.rCanvas.DrawSub(aPara, l_Rect, aLayerID, aSub);
   ...
   try
    ...
    try
     ...
     try
      ...
      try
       f_Collapsed := ParaX.BoolA[k2_tiCollapsed];
       if f_Hidden then
        Result := True
       else
        Result := DoDraw;
       ...
       if (f_TopAnchor.VertPosition(Area.rView.As_InevView, FormatInfo) <= 0) then
        DrawSubs;
       {$IfDef ParaPainter_IsParaListPainter}
       if ParaX.InheritsFrom(k2_idBlock) then
        DrawBlockSub;
       {$EndIf ParaPainter_IsParaListPainter}
       if (Caret <> nil) then
       begin
...
function _evParaPainter_.Get_ParentTool: InevTool;
//#UC START# *4E25BC580284_4804A2C101A5get_var*
//#UC END# *4E25BC580284_4804A2C101A5get_var*
begin
//#UC START# *4E25BC580284_4804A2C101A5get_impl*
 Result := f_ParentPainter;
//#UC END# *4E25BC580284_4804A2C101A5get_impl*
end;//_evParaPainter_.Get_ParentTool
...
//#UC START# *50A5EEEA0040_4804A2C101A5_impl*
 ...
 begin
  ...
  if l_Start.HasInner and l_Start.Inner.IsSame(aPara) and (l_Start.Inner.Diff(Area.rView.As_InevView, l_Point, FormatInfo) = 0) then
  begin
   l_Point.SetAtEnd(Area.rView.As_InevView, True);
   Result := l_Finish.HasInner and l_Finish.Inner.IsSame(aPara) and (l_Finish.Inner.Diff(Area.rView.As_InevView, l_Point, FormatInfo) = 0);
