...
{ $Id: evHyperlink.pas,v 1.83 2013/05/31 04:58:21 lulin Exp $ }
  ...
  TevHyperlink = class(Tk2ProcTagTool,
    ...
    // property methods
      ...
      procedure pm_SetAddress(const Value: TevAddress);
        {-}
      function  IevAddressHolder_pm_GetAddress: Ik2Tag;
    ...
    // public methods
      ...
      class function Make(const aLoc       : InevLocation;
                          const aHyperlink : Ik2Tag;
                          const aPara      : Ik2Tag): IevHyperlink;
        reintroduce;
        overload;
        {-}
      class function Make(const aPara      : Ik2Tag;
                          const aHyperlink : Ik2Tag): IevHyperlink;
        reintroduce;
        overload;
        {-}
      procedure Init(const aSeg: Ik2Tag);
        {-}
...
class function TevHyperlink.Make(const aPara      : Ik2Tag;
                                 const aHyperlink : Ik2Tag): IevHyperlink;
  //overload;
  {-}
begin
...
procedure TevHyperlink.Init(const aSeg: Ik2Tag);
  {-}
begin
 f_Hyperlink := aSeg;
 with aSeg do
 ...
 begin
  ...
  try
   ...
   begin
    ...
    l_OpPack.MarkModified(Para);
    ResetInfo;
   end//Exists
   else
   begin
    {* - Надо подумать о том что делать, если ссылка уже есть. }
    if not f_Loc.Obj.QT(InevTextPara, ATextPara) then
     Assert(false);
    try
...
var
 l_Document : Ik2Tag;
begin
 if evInPara(f_Loc.Obj^, k2_idDocument, l_Document) then
 begin
...
begin
 ...
 begin
  ...
  try
   ...
   with Hyperlink do
   begin
    l_NewAddress := Tk2Type(TagType).ArrayProp[k2_tiChildren].ChildType.MakeTag;
    try
...
procedure TevHyperlink.Modify(const anOldAddress, aNewAddress: TevAddress);
 ...
 function CheckModify(const theAddress: Ik2Tag; anIndex: Long): Bool;
 var
  l_OpPack  : InevOp;
...
begin
 if Exists then
  Hyperlink.IterateChildrenF(k2L2TIA(@CheckModify));
end;
...
begin
 ...
 begin
  l_OpPack := Processor.StartOp;
  try
   Hyperlink.Attr[k2_tiChildren] := nil;
   l_OpPack.MarkModified(Para);
   ResetInfo;
  finally
   l_OpPack := nil;
...
procedure TevHyperlink.AddressList_Delete(const anAddress: TevAddress);
 ...
 function CheckDelete(const theAddress: Ik2Tag; anIndex: Long): Bool;
 var
  l_OpPack  : InevOp;
  l_Address : TevAddress;
 begin//CheckDelete
...
begin
 if Exists then
  Hyperlink.IterateChildrenF(k2L2TIA(@CheckDelete));
end;

function TevHyperlink.IevAddressHolder_pm_GetAddress: Ik2Tag;
  {-}
begin
 if Exists then
...
begin
 ...
 if (Para <> nil) then
 begin
  if evInPara(Para, k2_idDocument, l_Doc) then
...
begin
 ...
 if (Result = 0) AND
  ...
  with f_Para.Attr[k2_tiHandle] do
  if IsValid then
   Result := AsLong
  else
...
function TevHyperlink.GetHyperlink: InevTag;
begin
 Result := f_Hyperlink;
end;
