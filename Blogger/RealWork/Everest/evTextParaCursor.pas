...
type
 ...
 // overridden protected methods
   ...
   function DoSplit(const aView: InevView;
    ...
    const anOp: InevOp): InevTag; override;
   ...
   function DoGetStyle(Stop: PInteger): InevTag; override;
...
//#UC START# *4A391212035B_49DF7D98029A_impl*
 l_Segments := ParaX.Attr[k2_tiSegments];
...
var
 ...
 procedure DoMoveSubs(const aFrom : Ik2Tag;
                      const aTo   : Ik2Tag);
 var
  l_SubList : InevSubList;
  l_AddSubs : Boolean;

  function AddSubLayer(const aSubLayer: Ik2Tag; Index: Integer): Boolean; far;
  var
   l_Handle : Integer;
  begin//AddSubLayer
   Result := True;
   ...
   begin
    ...
    if l_AddSubs OR (l_Handle <> ev_sbtSub) then
  ...
  with aFrom.Attr[k2_tiSubs] do
   ...
   begin
    ...
    if (l_SubList <> nil) then
     try
      IterateChildrenF(k2L2TIA(@AddSubLayer));
     finally
      l_SubList := nil;
     end;//try..finally
   end;//IsValid
...
//#UC START# *49DEFB410161_49DF7D98029A_impl*
 ...
 if aSecondPara.InheritsFrom(k2_idFormula) OR
    aSecondPara.InheritsFrom(k2_idBitmapPara) then
 begin
  Result := l_FirstPara.PCharLenA[k2_tiText].SLen;
  ...
  try
   ...
   Result := l_FirstPara.PCharLenA[k2_tiText].SLen;
   l_Seg := k2_typObjectSegment.MakeTag;
   try
    l_Seg.IntA[k2_tiStart] := Pred(Result);
    l_Seg.IntA[k2_tiFinish] := Pred(Result);
    l_NextPara := aSecondPara;
    l_Seg.AddChild(l_NextPara, anOp);
  ...
  Exit;
 end;//aSecondPara.InheritsFrom(k2_idFormula)
 if aSecondPara.InheritsFrom(k2_idTextPara) then
 begin
  with l_FirstPara.Attr[k2_tiText] do
  begin
   if IsValid then
  ...
  if evHasText(aSecondPara) then
  begin
   l_Text := aSecondPara.Attr[k2_tiText].AsObject As Tl3CustomString;
   {$IFNDEF Nemesis}
   ...
   if (Result = 0) then
   begin
    if aSecondPara.InheritsFrom(l_FirstPara.TagType.ID) then
     l_FirstPara.AssignCloneParams(aSecondPara, k2_amAll, anOp)
    else
     l_FirstPara.AssignCloneParams(aSecondPara, k2_amAll - [k2_amNull], anOp);
    with l_FirstPara do
   ...
   begin
    evSegments_AddSegments(l_FirstPara,
                           ...
                           0, Result,
                           anOp);
    OevJoinParas.ToUndo(anOp, l_FirstPara, aSecondPara);
  ...
  DoMoveSubs(aSecondPara, l_FirstPara);
  if aSecondPara.Owner.IsValid then
   ...
   {-нет коррекция нужна, но ее надо переделать на случай нетекстового параграфа
     (а может это бред)}
 end//aSecondPara.InheritsFrom(k2_idTextPara)
 else
 // Закомментировано т.к. http://mdp.garant.ru/pages/viewpage.action?pageId=234363615
(* if aSecondPara.InheritsFrom(k2_idContentsElement) then
 begin
 ...
 end//aSecondPara.InheritsFrom(k2_idContentsElement)
 else*)
 begin
  if not evHasText(l_FirstPara) then
  begin
   if MoveSubs then
   begin
    // - надо тогда переставить Sub на листьевой параграф
    if evInPara(aSecondPara, k2_idStyledLeafPara, l_NextPara) then
     DoMoveSubs(l_FirstPara, l_NextPara);
   end;//MoveSubs
   Result := -2;
...
//#UC START# *49DEFB770015_49DF7D98029A_impl*
 ...
 begin
  ...
  l_Anchor := Position;
  l_OldPara := ParaX;
  Result := Tk2Type(l_OldPara.TagType).MakeTag;
  try
   Result.AssignCloneParams(l_OldPara);
   with l_OldPara.Attr[k2_tiText] do
    if IsValid then
   ...
   begin
    ...
    begin
     ...
     begin
      l_OldSegments := l_OldPara.Attr[k2_tiSegments];
      ...
      with (Result.cAtom(k2_tiText).AsObject As Tl3String) do begin
  ...
  if l_SaveBreak then
   OevBreakPara.ToUndo(anOp, l_OldPara, Result);
 end;//ReadOnly
...
//#UC START# *49DF4C6E0101_49DF7D98029A_impl*
 try
  l_SecondPara := Self.pm_GetParentPoint.Obj.Child[Self.pm_GetParentPoint.Position];
 except
  on EListError do
  begin
 ...
 begin
  if (l_Pos >= 0) then
   if l_SecondPara.InheritsFrom(k2_idBitmapPara) then
   begin
...
//#UC START# *49E2F8F80062_49DF7D98029A_impl*
 ...
 if (Value = ev_cpAtEnd) then
  f_ParaEnd := true;
 Value := Max(0, Min(Value, ParaX.PCharLenA[k2_tiText].SLen));
...
//#UC START# *49E31657038E_49DF7D98029A_impl*
 ...
 case aCode of
  ...
  ev_ocWordFinish,
  ev_ocWholeWordRight :
   if not l_Tag.QT(Il3WordsSeq, l_String, k2Proc(anOp)) then
    Exit;
 end;//case aCode of
 try
...
//#UC START# *49E34DFB0259_49DF7D98029A_impl*
 Result := f_ParaEnd and (Position >= ParaX.PCharLenA[k2_tiText].SLen);
...
//#UC START# *49E3662D028E_49DF7D98029A_impl*
 Result := evTextLength(ParaX);
//#UC END# *49E3662D028E_49DF7D98029A_impl*
end;//TevTextParaCursor.DoGetBlockLength

function TevTextParaCursor.DoGetStyle(Stop: PInteger): InevTag;
...
//#UC START# *4A38AA5C019F_49DF7D98029A_var*
 ...
 var
  ...
  l_TabStop : InevTag;
 begin
  if evGetDecimalTabIndentTag(ParaX, l_TabStop) then
  begin
...
//#UC START# *4A38AA5C019F_49DF7D98029A_impl*
 ...
 if (ParaX.IntA[k2_tiStyle] <> ev_saTxtNormalOEM) then
...
//#UC START# *4A38F52201BA_49DF7D98029A_impl*
 Result := True;
 l_TextPara := ParaX;
 l_Segs := l_TextPara.Attr[k2_tiSegments];
 with l_Segs do
  if IsValid then
   with evSegments_GetSegmentOnPos(l_Segs, ev_slHyperlinks, Position, l_Si) do
    if IsValid AND InheritsFrom(k2_idHyperlink) then
    begin
    ...
    if (PLC <> nil) then
     try
      ...
      begin
       ...
       if not l_BrokenPara.Owner.QT(InevPara, l_BrokenParaOwner) then
        Assert(false);
       l_List := Obj^;
       if l_List.IsSame(l_BrokenParaOwner) then
       ...
       begin
        ...
        if HasInner then
        begin
         if Inner.InheritsFrom(k2_idLeafPara) then
         // - Вставили листьевой параграф
         begin
          Inner.SetEntryPoint(OEP - l_Pos, l_Context);
          Inner.Refresh;
         end//Inner.InheritsFrom(k2_idLeafPara)
         else
       ...
       begin
        if not l_BrokenParaOwner.Owner.QT(InevPara, l_BrokenParaOwnerOwner) then
         Assert(false);
        if l_List.IsSame(l_BrokenParaOwnerOwner) then
        begin
         l_Inn := l_BrokenParaOwner.MakePoint;
         if not l_BrokenPara.Box.QT(InevPara, l_BrokenPara) then
...
//#UC START# *4A38F59B0234_49DF7D98029A_impl*
 ...
 begin
  ...
  try
   if (l_NewPara = nil) OR not l_NewPara.IsValid then
   begin
    Result := False;
    Exit;
   end//l_NewPara = nil
   else
    evInsertPara(anOp, Self.pm_GetParentPoint, l_NewPara, [ev_ipfAtEnd]);
  finally
   l_NewPara := nil;
  end;//try..finally
  evIterateCursorsF(l_OldPara, l3L2IA(@_ModifyCursor));
 end;//aDrawLines
 ...
 l_InsString : Tl3InterfacedString;
  ...
  begin
   if not(misfLockCursors in aFlags) then
    evIterateCursorsF(l_TextPara.Box, l3L2IA(@_ModifyCursor));
  end;
 ...
 l_TextProcessor : IevTextOpProcessor;
  ...
  begin//DoInsert
   ...
   begin
    ...
    if (misfLockSegments in aFlags) then
     OevInsertString1.ToUndo(anOp, l_TextPara, l_Pos, l_InsString)
    else
     OevInsertString.ToUndo(anOp, l_TextPara, l_Pos, l_InsString);
   end;//_DoInsert..
  end;//DoInsert
   ...
   var
    ...
    begin//_IntoNext
     ...
     begin
      ...
      begin
       DoInsert;
       if l_TextPara.QT(Il3WordsSeq, l_StringTool, k2Proc(anOp)) then
        try
   ...
   begin//InsertIntoNext
    ...
    begin
     ...
     begin
      ...
      if (l_OldParentPos <= GetBlockLength) then
      begin
       // - У родителя еще есть следующий параграф
       l_Next := Obj.Child[Position];
       // - Берем следующий
       if l_Next.InheritsFrom(k2_idTextPara) then 
       begin
     ...
     begin
      l_Next := Tk2Type(l_TextPara.TagType).MakeTag;
      try
       l_Next.AssignCloneParams(l_TextPara);
       l_String := l_Text.Clone;
       try
        with l_String do
        begin
        ...
        l_Next.ObjW[k2_tiText, nil] := l_String;
   ...
   try
    try
     ...
     begin
      ...
      with l_TextPara.Attr[k2_tiText] do
       if IsValid then
       begin
        ...
        // - Здесь делаем копию текста, которую мы знаем как редактировать
         l_Str := Tl3String.Make(l_Str);
         try
          l_TextPara.ObjW[k2_tiText, anOp] := l_Str;
...
var
 l_UndoString : Tl3String;
  ...
  begin//SaveDeleted
   if (misfLockSegments in aFlags) then
    OevDeleteString1.ToUndo(anOp, l_TextPara, l_Pos, l_UndoString)
   else
    OevDeleteString.ToUndo(anOp, l_TextPara, l_Pos, l_UndoString);
  end;//SaveDeleted
  ...
  begin
   ...
   begin
    ...
    begin
     ...
     try
      l_Text := l_TextPara.Attr[k2_tiText].AsObject As Tl3CustomString;
    ...
    if not (misfLockCursors in aFlags) then
     evIterateCursorsF(l_TextPara, l3L2IA(@_ModifyCursor));
  ...
  begin
   ...
   begin
    ...
    begin
     ...
     if not evSegmentHasSomeFormat(l_Segment) then
    ...
    begin
     Result := true;
     l_Segment.AttrW[anIndex, anOp] := nil;
     if not l_Segment.InheritsFrom(k2_idHyperlink) AND
        not evSegmentHasSomeFormat(l_Segment) then
...
var
 ...
 function SegmentID(const Seg: Ik2Tag; Index: Integer): Boolean;
 begin//SegmentID
  Result := true;
  if Seg.InheritsFrom(k2_idHyperlink) then
  begin
  ...
  end;//Seg.InheritsFrom(k2_idHyperlink)
 end;//SegmentID
...
//#UC START# *4A60B23E00C3_49DF7D98029A_impl*
 ...
 begin
  ...
  try
   ...
   if l_B.Len > 0 then
   begin                                                         
    ResultHL := TevHyperlink.Create(Self, ParaX);
    try
     evSegmentsLayer_IterateSegmentsF(ParaX,
                                      ParaX.rAtomEx([k2_tiSegments, k2_tiChildren, k2_tiHandle, ev_slHyperLinks]),
                                      Succ(l_B.rStart),
                                      l_B.rFinish,
                                      k2L2TIA(@SegmentID));
