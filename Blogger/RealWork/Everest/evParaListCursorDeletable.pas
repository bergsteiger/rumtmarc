...
//#UC START# *48D0C21201B1_4A3A43F6003A_impl*
 ...
 if l_AsUsual then
 begin
  evIterateCursorsF(l_Parent, l3L2IA(@_ModifyCursor1));
  k2.SignalEvent(Self, k2_eidChildrenDeleted, l_Parent, anOpPack, Pred(PID), 1, l_Parent.ParaCount);
 end; // if not (ev_dpfForce in aFlags) then
 l_Parent.DeleteChild(Pred(PID), anOpPack, l_DeletedPara);
 l_DeletedPara := nil;
 if l_AsUsual then
  evIterateCursorsF(l_Parent, l3L2IA(@_ModifyCursor2));
