...
procedure TevControlContainerEX.DoDeleteOnUndo(const aTag: InevTag);
...
//#UC START# *47D011870105_47D01F9801D8_impl*
 if (f_CurrPara <> nil) then
  f_CurrPara.QT(IevEditorControl, l_OldControl);
 if (aCurPara <> nil) then
  aCurPara.QT(IevEditorControl, l_NewControl);
 ...
 if (l_OldControl <> nil) and
  ...
  if not l_CalEdit.IsFieldEmpty then 
   ...
   l_CalEdit.AnalysText;//"Расширение" года, если введены только день и месяц!
 ...
 if (f_DropTree <> nil) and (f_CurrPara <> nil) then
  if aCurPara.InheritsFrom(k2_idLeafPara) then
   if not f_CurrPara.IsSame(aCurPara) then
    HideControl(true);
 inherited;  
    ...
    if Supports(f_DropTree, IevDropCalendar, l_Calendar) then
     try 
      ...
      if l_SS = [] then
       ...
       begin
        f_Calendar.KeyDown(Msg.CharCode, l_SS);
        Msg.CharCode := 0;
        Msg.Result := 0;
        Result := True;
       end;
