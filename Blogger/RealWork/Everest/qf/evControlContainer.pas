...
type
 TevControlContainer = class(TevCustomControlTool, InevControlListner, IevQueryCard)
 private
 // private fields
...
Ссылка на внешнюю модель.}
 ...
 // realized methods
   ...
   function GetFocusedControl(const aCurPara: InevTag): IevEditorControl;
   ...
   procedure LinkListner(const aListner: IevAdapterModel);
...
procedure TevControlContainer.DoDeleteOnUndo(const aTag: InevTag);
...
//#UC START# *47D011870105_47CFEE9E02E1_impl*
 if (aCurPara <> nil) {and aCurPara.InheritsFrom(k2_idLeafPara)} then
   f_CurrPara := aCurPara
 else
  f_CurrPara := nil;
 if f_CurrPara <> nil then
 begin
  if f_CurrPara.InheritsFrom(k2_idControlPara) then
  begin
   l_Control := evGetEditorControl(f_CurrPara);
...
procedure TevControlContainer.InsertOnUndo(const aPrev: InevTag;
  const aChild: InevTag;
  anAdd: Boolean);
...
procedure TevControlContainer.DeleteOnUndo(const aTag: InevTag);
...
function TevControlContainer.NeedKey(const aPara: InevTag;
  var aKeyCode: Word): Boolean;
...
//#UC START# *47C7CB490100_47CFEE9E02E1_impl*
 ...
 begin
  with pm_GetEditor do
   if (Selection <> nil) and not Selection.Point.MostInner.Obj.IsSame(f_StartFocusControl.Para.MakePoint.MostInner.Obj^) then
...
function TevControlContainer.IsLastField(const aPara: InevTag): Boolean;
...
//#UC START# *47C7CB5D0311_47CFEE9E02E1_impl*
 if aPara.TagType.ID = k2_idControlPara then
 begin
...
function TevControlContainer.IsFirstField(const aPara: InevTag): Boolean;
...
function TevControlContainer.GetFocusedControl(const aCurPara: InevTag): IevEditorControl;
...
//#UC START# *47CD83B10329_47CFEE9E02E1_impl*
 ...
 if f_CurrPara <> nil then
 begin
  Result := evGetEditorControl(f_CurrPara);
  if Result <> nil then
...
//#UC START# *47CD83C8037F_47CFEE9E02E1_impl*
 ...
 begin
  LinkListner(Self);
  Assert(Obj.AsPara.ChildrenCount <> 0, 'Нет дочерних параграфов!!!');
  with Obj do
   if IsValid then
   begin
...
procedure TevControlContainer.ReleaseListners;
...
end;//TevControlContainer.ReleaseListners
...
//#UC START# *47CD84CC027E_47CFEE9E02E1get_impl*
 Result := TevQueryType(Para.IntA[k2_tiCardType]);
...
//#UC START# *479731C50290_47CFEE9E02E1_impl*
 ...
 if (f_QueryContainer <> nil) then
  f_QueryContainer.UnlinkListner(Self);
