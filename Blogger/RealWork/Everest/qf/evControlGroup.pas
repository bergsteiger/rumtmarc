...
//#UC START# *48DB8D660301_48DB819600D8_impl*
 ...
 aValue.IntA[k2_tiModelControl] := Long(IevQueryGroup(Self));
//#UC END# *48DB8D660301_48DB819600D8_impl*
end;//TevControlGroup.Create

function TevControlGroup.IsMultyLine: Boolean;
...
end;//TevControlGroup.IsMultyLine
...
//#UC START# *47CD7F53034B_48DB819600D8_impl*
 (Get_QueryCard As InevControlListner).HideDroppedControl(True);
...
//#UC START# *47CD802F02AE_48DB819600D8_impl*
 ...
 if Get_Expanded and not CanCollapsed then
  Exit;
 with Para do
  BoolW[k2_tiCollapsed, k2StartOp(aView.Control.Processor)] :=
   not BoolA[k2_tiCollapsed];
 AfterCollapsed;
 Para.BoolW[k2_tiChecked, nil] := False;
 Set_Upper(False);
...
var
 ...
 begin
  Result := True;
  with aChild do
   if InheritsFrom(k2_idReqRow) then
   begin
    l_Name := aChild.Attr[k2_tiReqID].AsString;
    l_ReqKind := TevReqKind(aChild.IntA[k2_tiReqKind]);
    l_Enabled := l_ReqKind <> ev_rkDescription;
    aChild.BoolW[k2_tiEnabled, nil] := l_Enabled;
    if l_Enabled then
    begin
...
//#UC START# *47CD8184009F_48DB819600D8get_impl*
 Result := l3CStr(Para.PCharLenA[k2_tiShortName]);
...
//#UC START# *47CD819D0144_48DB819600D8get_impl*
 Result := not Para.BoolA[k2_tiCollapsed];
...
//#UC START# *47CD819D0144_48DB819600D8set_impl*
 Para.BoolW[k2_tiCollapsed, nil] := not aValue;
