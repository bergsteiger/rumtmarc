...
{ $Id: evDropCalendar.pas,v 1.1 2010/09/28 08:07:22 dinishev Exp $ }
 ...
 if (Button = mbLeft) and (Shift = []) then
  begin
   if (THackCalendarPanel(FCalendar).GetDayFromPoint(Point(X,Y)) > 0) and
      (THackCalendarPanel(FCalendar).GetDayFromPoint(Point(X,Y)) = THackCalendarPanel(FCalendar).OldDay)
    then
      if Assigned(FOnChooseDate) then
       FOnChooseDate(Self);
  end;
...
begin
 ...
 FCalendar.OnMouseDown := CalendarMouseDown;
 FWasSelected := False;
end;
