...
type
 ...
 // realized methods
   ...
   function Get_ModelListner: IevModelListner;
...
//#UC START# *48D3CD560338_48D3BD0A02D7_impl*
 ...
 begin
  ...
  begin
   ...
   try
    TevTableImplementation.Make(aView, l_Para.MakePoint.PointToTypedParent(k2_idTable), l_Para.DocumentContainer.Processor).InsertRows(1, True);
   finally
    Get_QueryCard.InsertRowMode := False;
   end;
   l_Para := l_Para.MakePoint.PointToTypedParent(k2_idReqRow).Obj.AsPara;
  ...
  begin
   for i := 0 to aPara.ChildrenCount - 1 do
    if TevControlType(aPara.Child[i].Child[0].IntA[k2_tiType]) = ev_ctCombo then
    begin
     with aPara.Child[i].Child[0] do
   ...
   if not aPara.Child[i].Child[0].QT(IevEditorControlField, l_NewField) then
    Assert(false);
...
//#UC START# *48D3CD950327_48D3BD0A02D7_impl*
 ...
 begin
  try
   ...
   if f_FieldsList.IndexOf(l_Field) = 0 then //этот if - костыль для [$274843876]
   begin
    l_Field.Text := f_FieldsList[1].Text;
    DeleteValue(aView, f_FieldsList[1], ToModel);
    Exit;
   end;
   ...
   if Supports(l_Field, IevEditorFieldWithTree, l_TreeCont) then
    if l_TreeCont.Value <> nil then
     l_Field.Para.IntA[k2_tiNodeVisibleIndex] := l_TreeCont.GetNodeIndex(l_TreeCont.Value)
    else
     l_Field.Para.IntA[k2_tiNodeVisibleIndex] := -1
   else
    l_Field.Para.IntA[k2_tiNodeVisibleIndex] := -1;
   if not ToModel then
    TevTableImplementation.Make(aView, l_Field.Para.MakePoint.PointToTypedParent(k2_idTable), l_Field.Para.DocumentContainer.Processor).DeleteRow;
...
//#UC START# *48D3D1C10394_48D3BD0A02D7_impl*
 Result := Para.HasSubAtom(k2_tiOperations);
...
//#UC START# *48D3D1E8007E_48D3BD0A02D7_impl*
 Result := (l3BitCountF(Para.IntA[k2_tiOperations]) > 1);
...
function TevReq.AnalysString(const aValue: Il3CString;
  out aRslt: Il3CString): Boolean;
...
//#UC START# *47CD666F01DC_48D3BD0A02D7_impl*
 ML := Get_ModelListner;
 if (ML <> nil) then
  Result := ML.AnalysString(aValue, aRslt)
 else
  Result := False;
//#UC END# *47CD666F01DC_48D3BD0A02D7_impl*
end;//TevReq.AnalysString
...
//#UC START# *47CD670E02FC_48D3BD0A02D7_impl*
 Result := Para.BoolA[k2_tiRequired]; 
...
//#UC START# *47CD677302C2_48D3BD0A02D7_impl*
 if IsMulty and AddValue(aView, LastField) then 
 begin
  Result := LastField;
...
var
 ...
 begin
  ...
  try
   with l_ContTag do
    if InheritsFrom(k2_idControlPara) then
    begin
     if Attr[k2_tiType].IsValid then
      l_ControlType := TevControlType(l_ContTag.IntA[k2_tiType])
     else
     ...
     begin
      ...
      l_ContTag.BoolW[k2_tiEnabled, nil] := False;
    end;//if InheritsFrom(k2_idControlBlock)
  finally
   l_ContTag := nil;
  end;//try..finally
 end;//GetChildLine
      ...
      Case l_BTNControl.GetButtonType of
       ...
       ev_btAdd:
        l_BTNControl.Visible := CheckEdit(aField) and (l_Index = f_FieldsList.Count - 1)
         and not aField.IsFieldEmpty and IsMulty  and aField.Valid;
       ev_btLogical:
       begin
...
function TevReq.Get_ModelListner: IevModelListner;
...
{$IFNDEF TEST_MODEL}
 if (l_QC <> nil) AND (l_QC.AdapterModel <> nil) then
  Result := l_QC.AdapterModel.ModelListner(Self)
 else
{$ENDIF TEST_MODEL} 
  Result := nil; 
//#UC END# *47CD6E5E00F6_48D3BD0A02D7get_impl*
end;//TevReq.Get_ModelListner
...
//#UC START# *47CD6EB4017D_48D3BD0A02D7get_impl*
 Supports(Para[k2_tiReqID], Il3CString, Result);
...
//#UC START# *47CD6F3B0317_48D3BD0A02D7get_impl*
 Result := nil;
 if (f_Group = nil) then
  if not evInPara(Para, k2_idControlsBlock, f_Group) then
   Exit;
...
//#UC START# *47CD74D802DF_48D3BD0A02D7_impl*
 Result := Para.BoolA[k2_tiNumList];
...
//#UC START# *47CD75160008_48D3BD0A02D7_impl*
 Get_ModelListner.HyperLinkClick;
...
//#UC START# *47CD75890059_48D3BD0A02D7get_impl*
 Result := Para.IntA[k2_tiStateIndex];
...
//#UC START# *47CD75890059_48D3BD0A02D7set_impl*
 Para.IntA[k2_tiStateIndex] := aValue;
...
//#UC START# *47CD759702B8_48D3BD0A02D7_impl*
 Result := TevReqKind(Para.IntA[k2_tiReqKind]) = ev_rkContext;
...
//#UC START# *479731C50290_48D3BD0A02D7_impl*
 l_Cleared := false;
 if (Para <> nil) and Para.IsValid then
 begin
  with Para.Owner do
   if IsValid then
   begin
...
//#UC START# *47CFE07602FE_48D3BD0A02D7_impl*
 ...
 aPara.IntA[k2_tiModelControl] := Integer(IevReq(Self));
