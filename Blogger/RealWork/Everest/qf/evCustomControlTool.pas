...
//#UC START# *479731C50290_47CFDFAB0197_impl*
 if (f_Para <> nil) and f_Para.IsValid then
 begin
  if f_Para.HasSubAtom(k2_tiModelControl) then
  // - проверка, чтобы в WevReqRow.GetAtomData не попадать
   if l3IEQ(Self, IUnknown(f_Para.IntA[k2_tiModelControl])) then
    f_Para.Attr[k2_tiModelControl] := nil;
 end;//f_Para <> nil..
 f_Para := nil;
 inherited;
