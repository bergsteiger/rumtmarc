...
{ $Id: evTableRowHotSpot.pas,v 1.65 2013/04/24 09:35:36 lulin Exp $ }
...
begin
 ...
 begin
  ...
  begin
   ...
   if (l_X < 0) then
    theSpot := TevHotSpotWrap.Make(TevTableRowHotSpot.Make(aChild.OwnerObj, Processor))
   else
   begin
    l_Delta := l_X - l_Map.FI.Width; 
    if (Abs(l_Delta) <= 4 * evEpsilon) then
     theSpot :=  TevHotSpotWrap.Make(
                       GetTableColumnHotSpot.Make(aView, aChild.OwnerObj,
                                                  aChild.PID + 1,
...
begin
 ...
 theInfo.rHint := l3CStr('Строка таблицы');
end;
