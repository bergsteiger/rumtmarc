...
begin
 ...
 if not fGenerator.Root.QT(InevPara, l_Para) then
  Assert(false);
 TevDocumentPreview.Make(l_Para, evDefaultPreviewCacheKey, fHeader).Print;
end;
...
begin
 ...
 begin
  fIsStarted := True;
  fGenerator.Start;
  fGenerator.StartChild(k2_idDocument);
 end;
end;
...
begin
 ...
 begin
  ...
  try
   AddPCharLenAtom(k2_tiText, l3PCharLen(PAnsiChar(aText), -1, CP_ANSI));
  finally
   Finish;
  end;//try..finally
...
begin
 ...
 begin
  evSBS_LeftPara(aHeader).Attr[k2_tiText] := nil;
  evSBS_RightPara(aHeader).Attr[k2_tiText] := nil;
 end;//aTop
end;
