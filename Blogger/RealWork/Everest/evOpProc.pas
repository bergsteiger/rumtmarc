...
{ $Id: evOpProc.pas,v 1.123 2012/04/20 14:56:33 lulin Exp $ }
...
// change: тип свойства DefaultStyle теперь TevStyleId;
//
   ...
   function IsCheckOff: Boolean;
    ...
    // internal methods
      ...
      procedure BeepOnParaLimit;
        virtual;
        {-}  
      procedure DoCheckParaOp(const aParent  : Ik2Tag;
                              var aChild     : Ik2Tag;
                              anOp           : Tl3Operation;
                              anIndex        : TnevParaIndex);
        virtual;
        {* - проверить операцию с параграфом. }
      procedure DoParaOp(const Parent : Ik2Tag;
                         const Child  : Ik2Tag;
                         Op           : Tl3Operation);
        virtual;
    ...
    // public methods
      ...
      procedure DenyModify;
        {-}
      procedure CheckParaOp(const aParent  : Ik2Tag;
                            var aChild     : Ik2Tag;
                            anOp           : Tl3Operation;
      ...
      procedure CheckTextOp(Text: Tl3CustomString; Op: Tl3Operation);
        {-}
      procedure ParaOpEx(const Parent    : Ik2Tag;
                         const aChild    : Ik2Tag;
                         Op              : Tl3Operation;
                         IterateChildren : Boolean);
        {-}
      procedure ParaOp(const Parent : Ik2Tag;
                       const Child  : Ik2Tag;
                       Op           : Tl3Operation); 
        {* - завершить операцию с параграфом. }
...
procedure TevCustomProcessor.DoCheckParaOp(const aParent  : Ik2Tag;
                                           var aChild     : Ik2Tag;
                                           anOp           : Tl3Operation;
                                           anIndex        : TnevParaIndex);
  //virtual;
...
begin
 ...
 begin
  ...
  begin
   with aChild.Attr[k2_tiText] do
    if IsValid then
     S := AsObject As Tl3CustomString
    else
     S := nil;
   if (S = nil) then DCC := 0 else DCC := S.Len;
   Inc(DCC, cc_EOL_Len);
...
procedure TevCustomProcessor.CheckParaOp(const aParent  : Ik2Tag;
                                         var aChild     : Ik2Tag;
                                         anOp           : Tl3Operation;
                                         var anIndex    : TnevParaIndex);
  {-}
...
procedure TevCustomProcessor.ParaOpEx(const Parent    : Ik2Tag;
                                      const aChild    : Ik2Tag;
                                      Op              : Tl3Operation;
                                      IterateChildren : Boolean);
  {-}
  
 function ParaOpPrim(const Child     : Ik2Tag;
                     Op              : Tl3Operation;
                     IterateChildren : Boolean): Boolean;
   {-}

  procedure DoLeaf(const aLeaf : Ik2Tag);
  var
   l_DCC : Long;
  begin//DoLeaf
   if aLeaf.InheritsFrom(k2_idBitmapPara) then
   begin
    Case Op of
     l3_opDelete:
     begin
   ...
   end;//aLeaf.InheritsFrom(k2_idBitmapPara)
   if Tk2Type(aLeaf.TagType).HasProperty[k2_tiText] then
   begin
    if true{aWasText} then
    begin
     with aLeaf.Attr[k2_tiText] do
      if IsValid then
       l_DCC := AsPCharLen.SLen
      else
       l_DCC := 0;
    end//aWasText
    else
  ...
  procedure DoList(const aList : Ik2Tag);
 
   function SubParaOp(const aSubChild: Ik2Tag; Index: Long): Bool;
   var
    l_Inc : Integer; 
   begin//SubParaOp
    Result := true;
    if aSubChild.InheritsFrom(k2_idLeafPara) then
    begin
     DoLeaf(aSubChild);
     l_Inc := 1;
    end//aSubChild.InheritsFrom(k2_idLeafPara)
    else
    begin
     DoList(aSubChild);
  ...
  var
   ...
   function TableRowSubParaOp(const aSubChild: Ik2Tag; Index: Long): Bool;
   begin//TableRowSubParaOp
    Result := true;
    Assert(aSubChild.InheritsFrom(k2_idParaList));
    DoList(aSubChild);
    l_Max := Max(l_Max, aSubChild.IntA[k2_tiAllChildrenCount]);
   end;//TableRowSubParaOp

  begin//DoList
   if aList.InheritsFrom(k2_idTableRow) then
   begin
    l_Max := 0;
    aList.IterateChildrenF(k2L2TIA(@TableRowSubParaOp));
    aList.IntA[k2_tiAllChildrenCount] := l_Max;
   end//aList.InheritsFrom(k2_idTableRow)
   else
    aList.IterateChildrenF(k2L2TIA(@SubParaOp));
  end;//DoList

 begin//ParaOpPrim
  Result := false;
  if Child.InheritsFrom(k2_idLeafPara) then
  begin
   Result := true;
   DoLeaf(Child);
  end//Child.InheritsFrom(k2_idLeafPara)
  else
  if IterateChildren then
   if Child.InheritsFrom(k2_idParaList) then
    DoList(Child);
 end;//ParaOpPrim
...
var
 ...
 function GetMax(const aTag: Ik2Tag; anIndex: Long): Boolean;
 begin//
...
procedure TevCustomProcessor.DoParaOp(const Parent : Ik2Tag;
                                      const Child  : Ik2Tag;
                                      Op           : Tl3Operation);
  //virtual;
  {* - завершить операцию с параграфом. }
...
procedure TevCustomProcessor.ParaOp(const Parent : Ik2Tag;
                                    const Child  : Ik2Tag;
                                    Op           : Tl3Operation);
  {-}
begin
...
procedure TevCustomUndoProcessor.Cleanup;
  ...
  {-}
begin
 if (f_Container <> nil) AND (f_Container.GetOwner = Self) then
...
begin
 {$IfDef Nemesis}
 if Supports(aTarget, Ik2Tag, l_Tag) then
  try
   if l_Tag.MarkModified then
    Exit;
  finally
   l_Tag := nil;
  end;//try..finally
...
begin
 {$IfOpt D+}
 l_Tag := aParent As Ik2Tag;
 {$Else}
 l_Tag := Ik2Tag(aParent);
 {$EndIf}
 if l_Tag.InheritsFrom(k2_idParaList) then
  CheckParaOp(l_Tag, Ik2Tag(aChild), l3_opInsert, TnevParaIndex(anIndex));
end;
...
begin
 {$IfOpt D+}
 l_Tag := aParent As Ik2Tag;
 l_Child := aChild As Ik2Tag;
 {$Else}
 l_Tag := Ik2Tag(aParent);
 l_Child := Ik2Tag(aChild);
 {$EndIf}
 if l_Tag.InheritsFrom(k2_idParaList) then
  CheckParaOp(l_Tag, l_Child, l3_opDelete, TnevParaIndex(anIndex));
end;
...
begin
 {$IfOpt D+}
 l_Tag := aList As Ik2Tag;
 {$Else}
 l_Tag := Ik2Tag(aList);
 {$EndIf}
 if l_Tag.InheritsFrom(k2_idParaList) then
  ParaOpEx(l_Tag, Ik2Tag(aChild), l3_opInsert, false);
end;
