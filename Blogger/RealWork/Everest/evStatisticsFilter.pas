...
{ $Id: evStatisticsFilter.pas,v 1.15 2010/11/30 11:47:02 lulin Exp $ }
...
procedure TevStatisticsFilter.StartChild(TypeID: Long);
  //override;
  {-}
var
 CT : Tk2Type;
begin
 ...
 if ClearForEachDocument then begin
  if CT.InheritsFrom(k2_idDocument) then begin
   ClearStatistics;
   Exit;
  end;//..k2_idDocument..
 ...
 if CT.InheritsFrom(k2_idTable) AND not CT.InheritsFrom(k2_idSBS) then
  Inc(f_TablesOpened)
 else if InTable AND CT.InheritsFrom(k2_idTableCell) then
  Inc(f_CellCount);
end;
