...
{ $Id: evParaFrame.pas,v 1.55 2010/07/13 10:45:27 dinishev Exp $ }
  ...
  TevUsialParaFramePart = class(Tl3CacheableBase, IedFramePart)
    ...
    // internal methods
      procedure Cleanup;
        override;
        {-}
      function  FramePart(Check: Bool = false): Ik2Tag;
        {-}
  ...
  TevTableFramePart = class(Tl3CacheableBase, IedFramePart)
    ...
    {internal methods}
      procedure Cleanup;
        override;
        {-}
      procedure IterateCells(anAction    : Ik2Tag_IterateChildrenF_Action;
                             aCellAction : TevCellAction = ev_caRead;
                             const anOp  : InevOp = nil);
        {-}
      procedure IterateCellsF(anAction    : Ik2Tag_IterateChildrenF_Action;
                              aCellAction : TevCellAction = ev_caRead;
                              const anOp  : InevOp = nil);
        {-}
...
function TevUsialParaFramePart.FramePart(Check: Bool = false): Ik2Tag;
  {-}
var
 l_Op : InevOp;
begin
...
constructor TevTableFramePart.Create(aTableFrame : TevTableFrame;
 ...
 function CalcVisible(const FramePart: Ik2Tag; Index: Long): Bool; far;
 begin
  with FramePart do
 ...
 function CalcParam(const FramePart: Ik2Tag; Index: Long): Bool; far;
 begin
  with FramePart do
   if IsValid then begin
    if (f_Width <> 0) then
...
begin
 ...
 f_Color := -2;
 f_Style := -1;
 IterateCellsF(k2L2TIA(@CalcVisible));
 if f_Visible then
  IterateCellsF(k2L2TIA(@CalcParam));
...
procedure TevTableFramePart.IterateCells(anAction    : Ik2Tag_IterateChildrenF_Action;
                                         aCellAction : TevCellAction = ev_caRead;
                                         const anOp  : InevOp = nil);
  {-}

 function SetParam(const Cell: Ik2Tag; FramePartIndex: Long): Bool; far;
 var
  l_FramePart : Ik2Tag;
 var
  l_Frame : Ik2Tag;
 ...
 function SetUpParam(const Cell: Ik2Tag; Index: Long): Bool; far;
 begin
  Result := SetParam(Cell, k2_tiFrameUp);
 end;

 function SetDownParam(const Cell: Ik2Tag; Index: Long): Bool; far;
 begin
  Result := SetParam(Cell, k2_tiFrameDown);
 end;

 function SetLeftParam(const Row: Ik2Tag; Index: Long): Bool; far;
 var
  l_CellIndex : Long;
 begin//SetLeftParam
 ...
 function SetRightParam(const Row: Ik2Tag; Index: Long): Bool; far;
 var
  l_CellCount : Long;
  l_CellIndex : Long;
 begin//SetRightParam
 ...
 function SetVertParam(const Row: Ik2Tag; Index: Long): Bool; far;   
 var
  l_StartCell  : Long;
  l_FinishCell : Long;

  function SetCellParam(const Cell: Ik2Tag; Index: Long): Bool; far;
  begin//SetCellParam
 ...
 begin//SetVertParam
  ...
  with Row do begin
   ...
   if (l_CellCount > 1) AND ((l_FinishCell - l_StartCell) > 0) then
    IterateChildrenF(k2L2TIA(@SetCellParam), l_StartCell, l_FinishCell);
  end;{with Row}
 end;//SetVertParam
...
var
 ...
 function SetHorzParam(const Row: Ik2Tag; Index: Long): Bool; far;

  function SetFirstRowCellParam(const Cell: Ik2Tag; Index: Long): Bool; far;
  begin//SetFirstRowCellParam
   Result := SetParam(Cell, k2_tiFrameDown);
   if not Result then SetHorzParam := false;
  end;//SetFirstRowCellParam

  function SetLastRowCellParam(const Cell: Ik2Tag; Index: Long): Bool; far;
  begin//SetLastRowCellParam
   Result := SetParam(Cell, k2_tiFrameUp);
   if not Result then SetHorzParam := false;
  end;//SetLastRowCellParam

  function SetInnerRowCellParam(const Cell: Ik2Tag; Index: Long): Bool; far;
 ...
 begin//SetHorzParam
  ...
  with TableFrame do
   if (Index = l_StartRow) then
    Row.IterateChildrenF(k2L2TIA(@SetFirstRowCellParam),
                         Max(StartCell, l3MinIndex),
                         l_FinishCell)
   else if (Index = l_FinishRow) then
    Row.IterateChildrenF(k2L2TIA(@SetLastRowCellParam),
                         l_StartCell,
                         Min(FinishCell, l3MaxIndex))
   else
    if Child[l_StartRow].ChildrenCount = Row.ChildrenCount then
     Row.IterateChildrenF(k2L2TIA(@SetInnerRowCellParam),
                          Max(StartCell, l3MinIndex),
                          l_FinishCell)
    else
     Row.IterateChildrenF(k2L2TIA(@SetInnerRowCellParam),
...
begin//IterateCells
 ...
 begin
  ...
  begin
   ...
   begin
    l_OffsetF := l_ParaF.OffsetX + l_ParaF.IntA[k2_tiWidth] - 1;
  ...
  case f_Index of
   ed_fpiUp : begin
    Child[l_StartRow].
     IterateChildrenF(k2L2TIA(@SetUpParam),
                      Max(StartCell, l3MinIndex),
                      l_FinishCell);
    if (l_StartRow > 0) then
     Child[Pred(l_StartRow)].
      IterateChildrenF(k2L2TIA(@SetDownParam),
                      Max(StartCell, l3MinIndex),
                      l_FinishCell);
   end;//ed_fpiUp
   ed_fpiDown : begin
   ...
   ed_fpiLeft :
    IterateChildrenF(k2L2TIA(@SetLeftParam), l_StartRow, l_FinishRow);
   ed_fpiRight :
    IterateChildrenF(k2L2TIA(@SetRightParam), l_StartRow, l_FinishRow);
   ed_fpiVert :
    IterateChildrenF(k2L2TIA(@SetVertParam), l_StartRow, l_FinishRow);
   ed_fpiHorz : begin
    if (l_RowCount > 1) AND ((l_FinishRow - l_StartRow) > 0) then
     IterateChildrenF(k2L2TIA(@SetHorzParam), l_StartRow, l_FinishRow);
   end;{ed_fpiHorz}
  end;{case f_Index}
 end;//with TableFrame
end;//IterateCells

procedure TevTableFramePart.IterateCellsF(anAction    : Ik2Tag_IterateChildrenF_Action;
                                          aCellAction : TevCellAction = ev_caRead;
                                          const anOp  : InevOp = nil);
  {-}
begin//IterateCellsF
...
var
 ...
 function SetParam(const FramePart: Ik2Tag; Index: Long): Bool; far;
 begin//SetParam
  Result := true;
...
begin//SetLongProp
 if f_Visible AND (OldValue <> Value) then begin
  ...
  with TableFrame do begin
   ...
   try
    IterateCellsF(@k2L2TIA(@SetParam), ev_caCheck, l_Op);
   finally
    l_Op := nil;
   end;{try..finally}
  end;{with TableFrame}
...
constructor TevTableFrame.Create(const aTagWrap   : Ik2Tag;
                                 const aProcessor : InevProcessor;
                                 aStartRow        : Long = 0;
                                 aFinishRow       : Long = l3MaxIndex;
                                 aStartCell       : Long = 0;
...
function TevTableFrame.Get_FramePart(Index: TedFramePartIndex): IedFramePart;
 ...
 function DoRow(const aRow: Ik2Tag; anIndex: Integer): Boolean;
 begin//DoRow
...
begin
 Case Index of
  ...
  ed_fpiVert : begin
   ...
   begin
    Result := nil;
    Self.TagInst.IterateChildrenF(k2L2TIA(@DoRow), StartRow, FinishRow);
...
constructor TevCellFrame.Create(const aTagWrap    : Ik2Tag;
                                const aProcessor  : InevProcessor;
                                const aNeighbours : TevCellNeighbours);
  {reintroduce;}
  {-}
...
begin
 ...
 for i := Low(f_Neighbours) to High(f_Neighbours) do begin
  if (aNeighbours[i] = nil) then
   f_Neighbours[i] := nil
  else
   Pointer(f_Neighbours[i]) := Pointer(IedFrame(TevUsialParaFrame.Create(aNeighbours[i], aProcessor)));
 end;{for i}
end;
...
begin
 ...
 if (l_Start <> nil) then
  with l_Start.MostInner.Obj^ do
   if IsValid AND not InheritsFrom(k2_idParaList) then
...
var
 ...
 function IterateBlock(const aBlock: InevRange): Bool;
  ...
  function GetBlockFramePart(const aChild: Ik2Tag; Index: Long): Bool; far;
  var
   l_Frame     : IedFrame;
   l_FramePart : IedFramePart;
  begin{GetBlockFramePart}
   ...
   begin
    ...
    IterateBlock := Result;
   end//aChild.InheritsFrom(k2_idParaList)
   else
   begin
    Pointer(l_Frame) := Pointer(IedFrame(TevUsialParaFrame.Create(aChild, l_Processor)));
    try
