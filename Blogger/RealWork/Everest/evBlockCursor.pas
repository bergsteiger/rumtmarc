...
type
 ...
 // overridden protected methods
   ...
   procedure DoAssignSel(const aView: InevView;
    const aSource: InevRange;
    AllowAddToMulty: Boolean); override;
      ...
      try
       case aCode of
        ...
        ev_ocWordRight:
         ...
         Result := false;
       end;//case TevOperationCode(aCode)
       if Result then
        Exit;
      finally
       l_DrawLineTool := nil;
     ...
     if Result OR
      ...
      Case aCode of
       ...
       //   направление движения
        with Cursor.MostInner do
         if (View <> nil) AND
            View.FormatInfoByPara(Obj).IsHidden(false, true) then
          // проверка видимости параграфа для http://mdp.garant.ru/pages/viewpage.action?pageId=89106155
          Move(View, {ev_ocParaDown}ev_ocNextParaTopLeft);
...
//#UC START# *48E2719C019C_487B5D8D00ED_impl*
 ...
 try
  ...
  begin
   f_Start.AssignPoint(View, aStart);
   f_Finish.AssignPoint(View, aFinish);
  end//aStart.Compare(aFinish) <= 0
  else
  begin
   f_Start.AssignPoint(View, aFinish);
   f_Finish.AssignPoint(View, aStart);
  end;//aStart.Compare(aFinish) <= 0
  f_Finish.Refresh;
 finally
  l_Pack := nil;
...
procedure TevBlockCursor.DoSetDocument(const OldDocument: InevTag;
  const Value: InevTag);
