...
{ $Id: evTableCellHotSpot.pas,v 1.71 2011/09/13 10:48:06 lulin Exp $ }
...
// - изменен тип параметра Parent у методов IevVisualShapedTag._ParentToClient, _ClientToParent с IUnknown на Ik2Tag.
//
...
constructor TevTableCellHotSpot.Create(const aTagWrap   : Ik2Tag;
                                       const aProcessor : Ik2Processor);
  //override;
  {-}
var
 l_Tag : InevPara;
begin
 if not aTagWrap.QT(InevPara, l_Tag) then
  Assert(False);
 inherited;
end;
   ...
   case anAction of
    ev_maDown :
     ...
     begin
      ...
      f_StartCell := f_StartCell.PointToTypedParent(k2_idTableRow);
      ...
      begin
       ...
       if (ParaX.ChildrenCount <= 1) AND l_Finish.AtStart then
     ...
     begin
      ...
      begin
       if l_Point.InheritsFrom(k2_idTableCell) then Break;
       l_Point := l_Point.Inner;
      end;//while l_Point.HasInner
      if l_Point <> nil then
       l_TableHotSpot := TevTableHotSpot.Create(SelectionTable,
                                                Processor,
                                                l_Point.Obj.PID)
      else
       Assert(False);
...
begin
 ...
 begin
  if l_Point.InheritsFrom(k2_idTableRow) then Break;
  l_Point := l_Point.Inner;
