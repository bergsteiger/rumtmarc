...
{$Define nevTableRowBaseLineCommon_imp}
 ...
 // protected methods
   ...
   procedure DoCompareWithOtherInner(const aView: InevView); virtual;
 ...
 public
 // public methods
   constructor Create(const aRow: InevTag); reintroduce; virtual;
...
//#UC START# *4DDC92F301BC_4DD77B7C0163_impl*
 ...
 if TevMergeStatus(anObject.IntA[k2_tiMergeStatus]) = ev_msContinue then
 begin
  anObject.QT(InevTableCell, l_TableCell);
  l_TableCell := l_TableCell.GetMergeHead;
  if l_TableCell = nil then Exit;
  Result := l_TableCell.MakePoint;
  Result.SetEntryPoint(1);
  lp_GetIncDelta;
  while not l_RowPara.IsSame(ParaX) do
  begin
  ...
  begin
   ...
   begin
    ...
    if l_Point = nil then
     ...
     f_TopCells[i] := l_Point.ClonePoint(aView);
...
constructor _nevTableRowBaseLineCommon_.Create(const aRow: InevTag);
...
//#UC START# *47C5C25C022F_4DD77B7C0163_impl*
 ...
 begin
  ...
  if l_Point = nil then
   ...
   f_Children.Add(l_Point.ClonePoint(aView));
  ...
  begin
   if l_RealPoint = nil then
    ...
    f_HeadCells.Add(l_RealPoint.ClonePoint(aView));
   f_TopCells.Add(l_TopReal.ClonePoint(aView));
  end;
...
//#UC START# *47F5FDA303BC_4DD77B7C0163_impl*
 ...
 begin
  ...
  if (aRealTop <> nil) and not aRealTop.IsValid then
   aRealTop := nil;
...
procedure _nevTableRowBaseLineCommon_.DisableListner;
...
end;//_nevTableRowBaseLineCommon_.DisableListner

procedure _nevTableRowBaseLineCommon_.EnableListner;
...
end;//_nevTableRowBaseLineCommon_.EnableListner
...
function _nevTableRowBaseLineCommon_.Get_Listner: InevPointListner;
...
end;//_nevTableRowBaseLineCommon_.Get_Listner
...
//#UC START# *4AA76AE00147_4DD77B7C0163_impl*
 ...
 begin
  ...
  begin
   if (TevMergeStatus(Result.IntA[k2_tiMergeStatus]) = ev_msContinue) then
