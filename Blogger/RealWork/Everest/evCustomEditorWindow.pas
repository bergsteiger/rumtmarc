...
{ $Id: evCustomEditorWindow.pas,v 1.78 2013/08/06 14:32:55 dinishev Exp $ }
...
// - присваиваем Owner'а каретке напрямую, а не через Selection.
//
...
// - уменьшаем количество получения необходимых интерфейсов косвенно (через QI).
//
...
// - new method: TevStyleInterface.MakeList.
//
...
// - fix: заплатка : после обновления, если на экране был автореферат, в DoDrawSub получался Sub = nil
//
// Revision 1.349  2005/04/11 13:46:28  dinishev
// Bug fix: нельзя было увидеть компонент в Delphi, происходило AV
//
...
// - свойство TevSelection.Document стало Ik2Tag.
//
// Revision 1.332  2005/03/23 16:34:07  lulin
// - свойство TextSource.Document стало Ik2Tag.
//
...
// - bug fix: не очищался Owner курсора.
//
...
// - delete prop: удалено свойство TevSelection._VisualFormatting.
//
  ...
  TevCustomEditorWindow = class(TevCustomEditorWindowPrim,
   ...
   {* Реализует базовый оконный элемент для отображения и редактирования содержимого документа. }
    ...
    // property fields
      ...
      f_CurStyle         : array [Boolean] of Ik2Tag;
      f_InScroll         : Boolean;
      f_WebStyle         : Boolean;
   ...
   {$IF not Defined(Archi) and not Defined(EverestLite)}
    ...
    // event methods
      ...
      function  RightIndentMul: Integer;
        virtual;
        {-}
      function VScrollWidth(anIC: Il3InfoCanvas): Integer; virtual; 
    private
    // private methods
    ...
    // public methods
      ...
      function  TextCodePage(aCheckBlock: Boolean = false): Long;
        {-}
      function  InPara(ID                : Long;
                       out theParaCursor : InevBasePoint;
                       anExcludeType     : Long = k2_idEmpty): Boolean;
        overload;
  ...
  IevEditorWindow = class(Ik2Unknown)
    ...
    // internal methods
      procedure SetParam(Index: Long; const Source: Ik2Tag);
        override;
        {-}
      procedure DoSetOwner(aValue: TObject);
        override;
        {-}
      function COMQueryInterface(const IID: Tl3GUID; out Obj): Tl3HResult;
        override;
...
// class IevEditorWindowFont
  ...
  IevEditorWindowFont = class(TevCustomFont)
    ...
    public
    // public methods
      class function Make(anOwner : TObject): Il3Font;
        {-}
  ...
  IevEditorWindowStyle = class(IevEditorWindowStylePrim, IedStyle)
    ...
    public
    // public methods
      class function Make(anOwner : TObject): IedStyle;
        {-}
  ...
  IevEditorWindowTextPara = class(IevEditorWindow, IedParagraphFormatting, IedTextParagraph)
    ...
    public
    // public methods
      class function Make(anOwner : TObject): IedTextParagraph;
        {-}
    public
    // public properties
...
function IevEditorWindow.pm_GetProcessor: InevProcessor;
  {-}
begin
...
begin
 ...
 if IID.EQ(IedTextParagraph) then
  IedTextParagraph(Obj) := IevEditorWindowTextPara.Make(Self)
 else
  Result := inherited COMQueryInterface(IID, Obj);
end;
  
procedure IevEditorWindow.SetParam(Index: Long; const Source: Ik2Tag);
  {override;}
  {-}
begin
...
procedure IevEditorWindow.DoSetOwner(aValue: TObject);
  {override;}                                                                 
  {-}
begin
 inherited;
 if (aValue Is TevCustomEditorWindow) then
  f_EditorWindow := TevCustomEditorWindow(aValue)
 else
 if (aValue Is IevEditorWindow) then
  f_EditorWindow := IevEditorWindow(aValue).f_EditorWindow
 else
  f_EditorWindow := nil;
end;
...
class function IevEditorWindowFont.Make(anOwner : TObject): Il3Font;
  {-}
var
...
procedure IevEditorWindowFont.DoSetOwner(aValue: TObject);
  {override;}
  {-}
begin
 inherited;
 if (aValue Is IevEditorWindow) then
  f_EditorWindow := IevEditorWindow(aValue)
 else
  f_EditorWindow := nil;
end;
...
procedure IevEditorWindowFont.SetParam(Index: Long; const Source: Ik2Tag);
  {override;}
  {-}
begin
 with f_EditorWindow do
 begin
  SetParam(Index, Source);
  with f_EditorWindow do
  begin
   if CursorInSelection then
   begin
    if not CheckBlockFont(false) then
     Exit;
...
var
 l_InSel : Boolean;
begin
 with f_EditorWindow.f_EditorWindow do
 begin
  l_InSel := f_EditorWindow.f_EditorWindow.CursorInSelection;
  if l_InSel AND (f_CurFont[l_InSel] = nil) then
   CheckBlockFont(true);
  Result := f_CurFont[l_InSel];
 end;//with f_EditorWindow.f_EditorWindow
end;
...
begin
 ...
 if (f_EditorWindow.CurPara.TagType <> nil) then
  with k2.TypeTable[f_EditorWindow.CurPara.TagType.ID] do
  begin
...
class function IevEditorWindowStyle.Make(anOwner : TObject): IedStyle;
  {-}
var
...
procedure IevEditorWindowStyle.pm_SetID(Value: Long);
  {-}
begin
 SetLongParam(k2_tiHandle, Value);
end;
 ...
 begin
  ...
  begin
   if Locked AND (ev_uwfPara in fl_UpdateFlags) then
    CheckCurStyle;
   Long(Dest) := f_CurStyle[CursorInSelection].IntA[k2_tiHandle];
  end;//with f_EditorWindow
...
procedure IevEditorWindowStyle.SetParam(Index: Long; const Source: Ik2Tag);
  {override;}
  {-}
var
 TT   : Tk2Type;
 ...
 begin
  ...
  begin
   ...
   if (TT = nil) then
    Exit;
   try
    if (Index = k2_tiName) then
     f_CurStyle[CursorInSelection] := TT.StrToTag(Source.AsString)
    else
     f_CurStyle[CursorInSelection] := TT.IntToTag(Source.AsLong);
   except
    on Ek2ConversionError do Exit;
   end;//try..except
   if f_CurStyle[CursorInSelection].IsNull then
    f_CurStyle[CursorInSelection] := Tk2TypeTable(TT.TypeTable).ObjToTag(TT.ValueTable.Default);
    //f_CurStyle[CursorInSelection] := k2Tag(TT.ValueTable.Default);
   l_Op := StartOp;
   try
    if l_Op.DeleteMapped then
     Selection.SetAtom(k2_tiStyle, f_CurStyle[CursorInSelection])
    else
    begin
...
class function IevEditorWindowTextPara.Make(anOwner : TObject): IedTextParagraph;
  {-}
var
...
begin
 if (Value = nil) then
  SetParam(k2_tiFont, k2_typIUnknown.IUnknownToTag(Value))
 else
...
begin
 ...
 if IID.EQ(IedStyle) then
  IedStyle(Obj) := IevEditorWindowStyle.Make(Self)
 else
...
begin
 Result := false;
 if Document.IsValid AND (View <> nil) then
 begin
...
function TevCustomEditorWindow.ObjectXOffset: Long;
  {-}
begin
 Result := - Document.IntA[k2_tiLeftIndent];
end;
...
function TevCustomEditorWindow.InPara(ID                : Long;
                                      out theParaCursor : InevBasePoint;
                                      anExcludeType     : Long = k2_idEmpty): Boolean;
  //overload;
  {* - курсор в параграфе который наследуется от типа ID? }
...
begin
 ...
 if (aSearcher <> nil) AND Document.IsValid AND (Selection <> nil) then
 begin
  {
    ...
    try
     try
      ...
      if (ev_soDocumentPart in anOptions) AND
         InPara(k2_idBlock, l_Cursor, k2_idDocument) then
...
begin
 ...
 begin
  ...
  begin
   ...
   if Supports(l_Para.Attr[k2_tiObject], Il3Bitmap, l_Bitmap) then
    try
...
function TevCustomEditorWindow.DoProcessCommand(Cmd    : TevOperationCode;
                                                aForce : Boolean;
                                                aCount : Integer): Boolean;
  //override;
  {-process the specified command, return true if processed}
...
//  DeleteFoundRgn;
  ...
  begin
    Case Cmd of
     ...
     begin
      TextSource.DocumentContainer.SubList.SubEx[Cmd - ev_ocSetMarker0, ev_sbtMarker].SetTo(Selection.Cursor.MostInner.Obj^);
      Result := true;
     end;//ev_ocSetMarker0..ev_ocSetMarker9

     ev_ocGotoMarker0..ev_ocGotoMarker9 :
     begin
      TextSource.DocumentContainer.SubList.SubEx[Cmd - ev_ocGotoMarker0, ev_sbtMarker].Select(Selection);
      Result := true;
...
begin
 if HasDocument then
 begin
  if (f_Document <> nil) AND not f_Document.IsSame(TextSource.Document) then
  begin
   //f_Document := nil;
   FreeView;
  end;//f_Document <> nil
...
begin
 ...
 if (l_Document <> nil) AND l_Document.IsValid then
 begin
  MakeView;
  if TextSource.Document.IsSame(l_Document) then
...
var
 l_Map : InevMap;
begin
 if not afw.IsActionUpdateLocked then
 begin
...
begin
 ...
 begin
  ...
  begin
   ...
   l_Map := View.RootMap;
   if (l_Map <> nil) AND Document.IsValid then
   begin
    Result := true;
    if Document.QT(InevHotSpotTester2, l_HotSpotTester, Processor) then
     try
...
function TevCustomEditorWindow.VScrollWidth(anIC: Il3InfoCanvas): Integer;
begin
...
begin
 ...
 begin
  Result := Document.IntA[k2_tiWidth];
...
begin
 ...
 begin
  ...
  try
   if not Document.IsValid OR (TextSource = nil) then
   begin
    f_NeedUpdateRange := true;
    Exit;
   end;//not Document.IsValid..
...
function TevCustomEditorWindow.pm_GetCurPara: Ik2Tag;
  {-}
begin
 if (f_Cursor = nil) OR (f_Cursor.Cursor = nil) then
  Result := k2NullTag
 else
  Result := f_Cursor.Cursor.MostInner.Obj^;
end;
...
begin
 ...
 begin
  ...
  begin
   ...
   begin
    if aNow then
     f_CurStyle[true] := InevSelection(Self.Selection).GetBlock.Formatting.GetStyle(@fl_BlockStyleCalls);
    if (fl_BlockStyleCalls = l3NilLong) then
    begin
     fl_BlockFontCalls := 0;
     if (TextSource <> nil) then
...
begin
 ...
 begin
  if (f_Cursor = nil) OR (f_Cursor.Cursor = nil) then
   f_CurStyle[false] := k2NullTag
  else
   f_CurStyle[false] := f_Cursor.Cursor.Formatting.GetStyle;
 end;//not fl_Capture
 {$EndIf evNeedGetBlockStyle}
end;
...
procedure TevCustomEditorWindow.DoDrawSub(const aSubTarget : IUnknown;
                                          const Canvas     : Il3Canvas;
                                          const R          : Tl3Rect;
                                          SubType          : Tl3Handle;
                                          const aSub       : IUnknown);
...
var
 l_B : Ik2Tag;
begin
 if evInPara(aPoint.MostInner.Obj^, k2_idBlock, l_B) then
 begin
  if l_B.InheritsFrom(k2_idDocument) then
   Result := TevSubImplementation.Make(l_B, Processor, 0, ev_sbtSub) As IevDocumentPart
  else 
