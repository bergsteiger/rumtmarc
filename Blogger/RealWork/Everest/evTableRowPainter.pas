  ...
  begin
   ...
   if (l_Point <> nil) then
    ...
    begin
     ...
     if l_Point.HasBaseLine then // Указатель на верхнюю строку дочерней таблицы
     begin
      l_Point := l_Point.ParentPoint;
      l_ChildID := l_Point.Obj.PID; // ID дочерней таблицы.
      l_Point := l_Point.ParentPoint;
 ...
 begin
  ...
  with ParaX do
   for l_Index := 0 to Pred(ChildrenCount) do
    Assert(Child[l_Index].InheritsFrom(k2_idTableCell), 'Строка таблицы должна состоять из ячеек, а не из ' + Child[l_Index].TagType.AsString);
  {$EndIf}
  {$EndIf Nemesis}
