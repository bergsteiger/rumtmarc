...
{ $Id: evDrawLineTool.pas,v 1.60 2013/04/04 11:18:51 lulin Exp $ }
...
begin
 ...
 begin
  l_Pos := Position;
  with (Obj.cAtom(k2_tiText, anOpPack).AsObject As Tl3String) do
  begin
...
begin
 ...
 begin
  with aCursor do
  begin
{    if (rAtom(k2_tiStyle).AsLong <> ev_saTxtNormalOEM) then
    Exit;}
   with (Obj.cAtom(k2_tiText, anOpPack).AsObject As Tl3String) do begin
    l_Delta := aPos - Len;
    if (l_Delta > 0) then
...
begin
 ...
 begin
  ...
  begin
   if (l_PID > 0) then
    with Child[Pred(l_PID)].Attr[k2_tiText] do
     if IsValid then
      with (AsObject As Tl3String) do
       if IsOEM then
        l_N[ev_ldUp] := Ch[l_Pos];
   if (l_PID < Pred(ChildrenCount)) then
    with Child[Succ(l_PID)].Attr[k2_tiText] do
     if IsValid then
      with (AsObject As Tl3String) do
       if IsOEM then
...
var
 ...
 var
  l_NewPara : Ik2Tag;
 begin
  l_NewPara := Tk2Type(l_Cursor.Obj.TagType).MakeTag;
  try
...
begin
 ...
 if (l_CursorParent <> nil) then begin
  Result := true;
   ...
   begin
    l_Style := IntA[k2_tiStyle];
    if (l_Style <> ev_saTxtNormalOEM) then
    begin
     if Attr[k2_tiText].IsValid then
     begin
      ...
      IntW[k2_tiStyle, anOp] := ev_saTxtNormalOEM;
   ...
   Case aDirection of
    ev_ldUp :
     with l_CursorParent do
      if (Position <= 1) OR
         not Obj.Child[Position - 2].InheritsFrom(k2_idTextPara) then begin
       if (aStyle = ev_lsClear) then Exit;
       InsertNewPara(false);
      end;//Position <= 1
    ev_ldDown :
     with l_CursorParent, Obj^ do
      if (Position >= ChildrenCount) OR
         not Child[Position].InheritsFrom(k2_idTextPara) then
      begin
       if (aStyle = ev_lsClear) then Exit;
       InsertNewPara(true);
