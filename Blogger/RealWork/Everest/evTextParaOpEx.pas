...
{ $Id: evTextParaOpEx.pas,v 1.18 2013/04/04 11:18:52 lulin Exp $ }
...
// - сделано объединение двух операций OevTextConvert.
//
...
constructor OevTextConvert.Create(const aPara : Ik2Tag;
                                  aPos        : Long;
                                  const anOld : Tl3PCharLen;
                                  const aNew  : Tl3PCharLen);
  //reintroduce;
...
function OevTextConvert.GetStart: PAnsiChar;
  {-}
begin
 with f_P.Attr[k2_tiText] do
  if IsValid then
   Result := (AsObject As Tl3CustomString).St + f_Pos
  else
   Result := nil; 
end;
