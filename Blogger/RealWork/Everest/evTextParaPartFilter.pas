...
{ $Id: evTextParaPartFilter.pas,v 1.31 2009/06/02 17:23:16 lulin Exp $ }
...
begin
 ...
 try
  ...
  try
   ...
   try
    anAtom.Write(l_TPPF, aFlags, anExclude);
   finally
...
procedure TevTextParaPartFilter.StartChild(TypeID: Long);
  {override;}
  {-}
begin
 inherited;
 if (f_SideGenerator = nil) then
 begin
  if CurrentType.InheritsFrom(k2_idSegment) then
  begin
  ...
  end//CurrentType.InheritsFrom(k2_idSegment)
  else
  ...
  begin
   ...
   begin
    ...
    try
     ...
     Generator.AddStringAtom(k2_tiText, l_New);
    finally
     l3Free(l_New);
    end;{try..finally}
   end else
...
begin
 Result := true;
 if Atom.InheritsFrom(k2_idSegment) then
 begin
