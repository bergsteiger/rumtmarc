...
{ $Id: evInterface.pas,v 1.12 2009/07/23 13:42:11 lulin Exp $ }
   ...
   {* Базовый клас для инструментов работающих с тегами. }
    ...
    // property methods
      function  pm_GetInheritance(Index: Long): TevInheritance;
        {-}
      function  pm_GetOwnParam(Index: Long): Ik2Tag;
        {-}
...
(*      procedure pm_SetTypeTable(const Value: Tk2TypeTable);*)
    ...
    // public properties
      ...
      property OwnParam[Index: Long]: Ik2Tag
        read pm_GetOwnParam;
...
function TevInterface.pm_GetOwnParam(Index: Long): Ik2Tag;
  {-}
begin
 with Tag do
  Result := rOwnAtom(Index);
end;
...
function TevInterface.pm_GetIsTransparent(Index: Long): Bool;
  ...
  {-}
begin
 with OwnParam[Index] do Result := not IsNull AND IsTransparent;
end;
...
begin
 Changing;
 try
  Tag.IntW[anIndex, Context] := aValue;
 finally
  Changed;
 end;//try..finally
end;
...
function TevInterface.pm_GetTag: Ik2Tag;
  {-}
begin
 Result := f_Tag;
end;

procedure TevInterface.pm_SetTag(const aValue: Ik2Tag);
  {-}
begin
 ...
 begin
  ...
  if (aValue <> nil) AND (TagType = nil) then
   if (aValue.TagType = nil) then
    ...
    TagType := TypeTable[aValue.TagType.ID];
 end;//f_Tag <> aValue
end;
