...
{ $Id: evTextFormatter.pas,v 1.172 2013/04/26 13:23:46 dinishev Exp $ }
...
type
  ...
  // interface methods
    ...
    procedure CorrectParentWidth(anSBS     : Boolean;
                                 ...
                                 var aWidth  : Integer);
      {-}
    procedure CheckSBSTable(const aText      : Ik2Tag;
                            const aRow       : TevTableRowAtom;
                            var aParentWidth : Integer);
      {* - Проверяем и корректируем ширину _SBS. }
    function CheckSubList(const aTag: Ik2Tag): Tk2TagList;
      {-}
  protected
  // property methods
  ...
  // internal methods
    ...
    procedure DoFinishTableRow(const TR: Tk2StackAtom);
      {-}
    procedure DoStartTable(const T: Ik2Tag);
      {-}
    ...
    procedure DoFinishTextPara(const TP: Tk2StackAtom);
      {-}
    procedure ValidateSegments(const aPara : Ik2Tag;
                               const aText : Tl3PCharLen);
      {* - Проверяет список сегментов и склеивает соседние. }
    procedure FormatLines(const TP    : Ik2Tag;
  ...
  protected
  // property methods
    procedure pm_SetSubs(Value: Tk2TagList);
      {-}
  ...
  //property methods
    ...
    procedure pm_SetCurrentRow(Value: TevTableRowAtom);
      {-}
    procedure pm_SetTable(const Value: Ik2Tag);
      {-}
  protected
  //internal methods
    procedure Cleanup;
  ...
  {internal methods}
    ...
    procedure Release;
      override;
      {-}
    procedure SetFrame(const Value: Ik2Tag);
      {-}
  public
  {public methods}
    class function IsCacheable: Bool;
   ...
   try
    ...
    begin
     ...
     begin
      StartTag(k2_tiSubs);
      try
       StartChild(k2_idSubLayer);
       try
        AddIntegerAtom(k2_tiHandle, ev_sbtSub);
        for l_Index := 0 to Pred(TevSubString(S).Subs.Count) do
        begin
         StartChild(TevSubString(S).Subs[l_Index].TagType.ID);
         try
          TevSubString(S).Subs[l_Index].Write(Self.Generator);
         finally
          Finish;
         end;//try..finally
        end;//for l_Index
...
procedure TevTableAtom.pm_SetTable(const Value: Ik2Tag);
  {-}
begin
 f_Table := Value;
 if (Value = nil) then
  ...
  f_SBS := Value.InheritsFrom(k2_idSBS);
end;
...
procedure TevSubString.pm_SetSubs(Value: Tk2TagList);
  {-}
begin
 l3Set(f_Subs, Value);
end;
...
procedure TevCell.SetFrame(const Value: Ik2Tag);
  {-}
begin
 f_Borders := [];
...
begin
 ...
 begin  
  ...
  if not aTable.SBS AND (l_Row.CurrentCell < 0) then 
    Dec(f_Width); {!!!}
  ...
  begin
   ...
   begin
    ...
    try
     ...
     AddStringAtom(k2_tiText, aComment);
    finally
     Finish;
    end;//try..finally
   end;//with Generator
...
procedure TevCustomTextFormatter.DoStartTable(const T: Ik2Tag);
  {-}
var
 TA : TevTableAtom;
begin
...
procedure TevCustomTextFormatter.ValidateSegments(const aPara : Ik2Tag;
                                                  const aText : Tl3PCharLen);
  {* - Проверяет список сегментов и склеивает соседние. }
  
 function CheckLayer(const aLayer: Ik2Tag; anIndex: Long): Bool;

  function CheckSegment(const aSegment: Ik2Tag; anIndex: Long): Bool;
  begin//CheckSegment
   Result := True;
   with aSegment do
...
(*     if HasSubAtom(k2_tiStart) AND
 ...
 function CheckNeigbours(const aLayer: Ik2Tag; anIndex: Long): Bool;
 var
  l_Index   : Long;
  l_PrevSeg : Ik2Tag;
  l_Start   : Long;
    ...
    with l_Boundaries[i] do
     ...
     begin
      if aHyperLink and not rHyperLink then
       Ik2Tag(rLayer).DeleteChild(rIndex)
      else
  ...
  begin
   ...
   begin
    ...
    begin
     ...
     l_PrevSeg := l_Seg;
    if l_PrevSeg.InheritsFrom(k2_idHyperlink) then
     CheckSegment(True)
    else
...
begin
 with aPara do
 begin
  Attr[k2_tiSegments].IterateChildrenF(k2L2TIA(@CheckLayer));
  // - Исправляем сегменты с неправильными концами
  l_BoundArrayL := 0;
  Attr[k2_tiSegments].IterateChildrenF(k2L2TIA(@CheckNeigbours));
  // - Объединяем соседние сегменты
  SetLength(l_Boundaries, 0);
 end;//with aPara
end;

procedure TevCustomTextFormatter.FormatLines(const TP    : Ik2Tag;
                                             Strings     : Tl3StringList;
                                             Subs        : Tk2TagList;
                                             InTable     : Bool;
                                             ParentWidth : Long);
...
var
 ...
 procedure ShiftSegments(aShiftCount: Long; aDeltaWhiteSpace : Integer);
  ...
  function ShiftSegmentsLayer(const Layer: Ik2Tag; Index: Long): Bool; far;

   function ShiftSegment(const Segment: Ik2Tag; Index: Long): Bool; far;
...
(*       if (aShiftCount = l_Pos) and not Segment.InheritsFrom([k2_idHyperlink, k2_idObjectSegment]) and OneLineSelection then
       HL2HLCont
      else
       if (l_Pos = aShiftCount) and Segment.InheritsFrom([k2_idHyperlink]) then
  ...
  begin//ShiftSegmentsLayer
   if (Layer.IntA[k2_tiHandle] > ev_slSuperposition) then
    Layer.IterateChildrenF(k2L2TIA(@ShiftSegment));
   Result := True;
  end;//ShiftSegmentsLayer

 begin//ShiftSegments
  TP.Attr[k2_tiSegments].IterateChildrenF(k2L2TIA(@ShiftSegmentsLayer));
 end;//ShiftSegments
...
var
 ...
 procedure Paint(First, aLast: Bool);
  ...
  procedure PaintSegments;
   ...
   function PaintLayer(const Layer: Ik2Tag; Index: Long): Bool; far;

    function PaintSegment(const Segment: Ik2Tag; Index: Long): Bool; far;
    begin{PaintSegment}
     PaintLine(l_ParaVisible, l_ParaStyle, l_Formatted, Segment, First, aLast);
     Result := True;
    end;{PaintSegment}

    function PaintTableSegment(const Segment: Ik2Tag; Index: Long): Bool; far;
    var
     l_OF  : Long;
     l_Len : Long;
   ...
   begin{PaintLayer}
    if (Layer.IntA[k2_tiHandle] > ev_slSuperposition) then
    begin
     l_PVList := evSegmentsLayer_GetSegments(TP, Layer, 1, l_Formatted.Len);
     try
      if OneLineSelection then
       l_PVList.ForEachF(k2L2TIA(@PaintTableSegment))
      else
       l_PVList.ForEachF(k2L2TIA(@PaintSegment));
     finally
      l_PVList := nil;
     end;//try..finally
  ...
  begin{PaintSegments}
   TP.Attr[k2_tiSegments].IterateChildrenF(k2L2TIA(@PaintLayer));
...
var
 ...
 var
  ...
  l_SS : TevSubString;
 begin
  l_S := TP.Attr[k2_tiText].AsObject As Tl3CustomString;
 ...
 begin
  ...
  begin
   ...
   begin
    ...
    if (l_ParentWidth > 0) AND not l_Table.SBS AND (l_Row.CurrentCell < 0) then
     Dec(l_TabIndent);
    Inc(l_FirstIndent, l_TabIndent);
   end;//l_TabIndent > 0
   //if l_ParentWidth - l_FirstIndent < l_Text.SLen then
...
(*    if (l_SoftEnterIndx = 0) then
   ...
   begin
    l_LineLength := l3FindNextLine(l_Text, l_CharCount);
...
procedure TevCustomTextFormatter.ValidateLine(aLine : Tl3String);
  //virtual;
  {* - процедура для проверки корректности строки. Для перекрытия в потомках. }
var
 l_Pnt : IevTextPainter;
begin
 if Supports(Generator, IevTextPainter, l_Pnt) then
  try
   l_Pnt.ValidateLine(aLine);
  finally
   l_Pnt := nil;
  end;//try..finally
end;

procedure TevCustomTextFormatter.AddAtom(AtomIndex: Long; TK: Tk2TypeKind; const Value);
  {override;}
  {-}
var
 l_O : TObject;
 l_S : Tl3TempFileStream;
 l_Pos : Int64;
begin
 if (AtomIndex = k2_tiData) then
 begin
  if (TK = k2_tkStream) AND
     Tags.Top.InheritsFrom(k2_idBitmapPara) AND
     Tags.Top.Parent.InheritsFrom(k2_idObjectSegment) then
  begin
   l_O := TObject(Value);
   ...
   begin
    ...
    try
     ...
     try
      ...
      {$EndIf nsTest}
      Tags.Top.Box.ObjW[AtomIndex, nil] := l_S;
     finally
      FreeAndNil(l_S);
     end;//try..finally
    finally
    ...
    Tags.Top.Box.ObjW[AtomIndex, nil] := l_O;
  end//TK = k2_tkStream
  else
...
begin
 ...
 begin
  ...
  if l_Parent.InheritsFrom(k2_idSBSCell) then
  begin
  ...
  end;//l_Parent.InheritsFrom(k2_idSBSCell)
   ...
   try
    ...
    begin
     ...
     begin
      ...
      TP.Box.Write(Generator, l3_spfAll, l_Exclude);
     end;//i = 0
    end;//l_OutStyle
...
procedure TevCustomTextFormatter.NotifyInsertion(aTarget: TObject; aPos, aLen: Long);
 ...
 function ShiftSegments(const Layer: Ik2Tag; Index: Long): Bool;

  function ShiftSegment(const Segment: Ik2Tag; Index: Long): Bool; 
  var
   l_Pos : Long;
  begin
   l_Pos := Segment.IntA[k2_tiStart];
...
begin
 with TevSubString(aTarget) do
  if (TP <> nil) then
   TP.Attr[k2_tiSegments].IterateChildrenF(k2L2TIA(@ShiftSegments));
end;
...
procedure TevCustomTextFormatter.DoStartAtom(var Atom: Tk2StackAtom);
  ...
  {override;}
begin
 if Atom.InheritsFrom(k2_idPara) AND
    ((Atom.Parent = nil) OR
     not Atom.Parent.InheritsFrom(k2_idObjectSegment)) then 
 begin
  ...
  begin
   ...
   if Atom.InheritsFrom(k2_idTableRow) then
    StartRow
   else 
    if Atom.InheritsFrom(k2_idTableCell) then
     StartCell
    else 
     if Atom.InheritsFrom(k2_idTable) then
      ...
      if Atom.InheritsFrom(k2_idTextPara) then
       ...
       begin
        ...
        if Atom.InheritsFrom(k2_idPageBreak) then
        begin
...
begin
 with anAtom, Box do
  if InheritsFrom(k2_idPara) AND
     ((Parent = nil) OR
      not Parent.InheritsFrom(k2_idObjectSegment)) then
  begin
   Result := False;
   if InheritsFrom(k2_idTableRow) then
...
begin
 l_Del := false;
 l_DelPos := 0;
 if IsMarkStyle(ParaVisible, ParaStyle, Obj, l_Open, l_Close, l_Del) then 
 begin
  l_Seg := Obj.InheritsFrom(k2_idSegment);
  if l_Seg then 
  begin
...
procedure TevCustomTextPainter.ValidateLine(aLine : Tl3String);
  //virtual;
  {* - Процедура для проверки корректности строки. Для перекрытия в потомках. }
begin
end;
...
procedure TevCustomTextFormatter.CheckSBSTable(const aText      : Ik2Tag;
                                               const aRow       : TevTableRowAtom;
                                               var aParentWidth : Integer);
var
 i             : Integer;
 ...
 begin
  ...
  begin
   ...
   begin
    ...
    FormatLines(l_CellText, l_PrevCell.f_Lines, CheckSubList(l_CellText), True, l_NewWidth);
...
begin
 ...
 if (aWidth > 0) AND not anSBS AND (aCurrenCell < 0) then
  Dec(aWidth); {!!!}
end;

function TevCustomTextFormatter.CheckSubList(
  const aTag: Ik2Tag): Tk2TagList;
var
 SA     : Tk2TagList;
 l_Subs : Ik2Tag;
begin
 Result := nil;
 with aTag.rAtomEx([k2_tiSubs, k2_tiChildren, k2_tiHandle, ev_sbtSub]) do
  if IsValid then
