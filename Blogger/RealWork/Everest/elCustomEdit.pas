...
//#UC START# *483445F4032B_482ACC330173_impl*
 ...
 with Document.Child[0].Attr[k2_tiSegments] do
  if IsValid then
  begin
   if rAtomEx([k2_tiChildren, k2_tiHandle, ev_slMistakes], @l_Layer).IsValid then
    DeleteChild(l_Layer);
  end;//IsValid
...
// сегмента надо будет убрать. К-105578765
 ...
 Find(TevStyleSearcher.Make(ev_saMistake, ev_slMistakes, ev_spSegments), nil, [ev_soGlobal, ev_soNoException, ev_soNoSelection, ev_soNoProgress]);
  ...
  try
   ...
   with Document do
    if (ChildrenCount > 0) then
     l_X := CN.LP2DP(l3PointX(Child[0].IntA[k2_tiFirstIndent])).X
    else
     l_X := 0;
