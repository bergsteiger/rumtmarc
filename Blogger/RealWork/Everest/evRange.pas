...
var
 ...
 procedure IterateRange;
  ...
  begin
   ...
   if anItem.Obj.InheritsFrom(k2_idParaList) then
    anItem.
 ...
 try
  ...
  begin                                                     
   ...
   if l_Cursor <> nil then
    l_Cursor := l_Cursor.PointToTypedParent(k2_idTable);
  ...
  if (l_Cursor = nil) then
   ...
   Pointer(Result) := Pointer(IedTable(TevTable.Create(f_View, l_Cursor.Obj^, Processor, l_CellRange)));
 finally
  l_CellRange := nil;
 end;//try..finally
...
//#UC START# *4BBCB2AD02B2_48E3859101D5_impl*
 ...
 begin
  with f_Loc.AsPoint.MostInner.Obj^ do
   Result := IsValid AND not InheritsFrom(k2_idParaList);
 end
 else
 if (f_Loc.Range <> nil) then
 begin
  ...
  begin
   ...
   l_Finish := l_Finish.MostInner;
   with l_Start.Obj^ do
    Result := IsValid AND not InheritsFrom(k2_idParaList) AND IsSame(l_Finish.Obj^);
  end;//Result
 end//f_Loc.Range <> nil
 else
  Result := false;
