...
{ $Id: evEvdWriter.pas,v 1.44 2013/04/05 12:02:15 lulin Exp $ }
...
var
 l_Storage : IStorage;
begin
 l_Storage := m2COMOpenStorage(aStorage,
                               PWideChar(aName),
                               m3_saReadWrite,
                               true);
 try
  OleCheck(l_Storage.QueryInterface(Im3IndexedStorage, Result));
 finally
  l_Storage := nil;
 end;//try..finally
end;
...
begin
 Result := m2COMCreateStream(aStorage,
                             PWideChar(aName),
                             m3_saCreate,
                             false);
end;
...
function TevCustomEvdStorageWriter.SaveInStorage(const aType: Tk2Type): Bool;
  {-}
begin
 Result := (aType.InheritsFrom(k2_idBlock));
// Result := (aType.InheritsFrom(k2_idDocument));
end;

procedure TevCustomEvdStorageWriter.DoStartChild(TypeID: Long);
  //override;
  {-}
var
 CT : Tk2Type;
 ...
 if f_IsPlain then
  ...
  if SaveInStorage(CT) then begin
   ...
   if
     ...
     Binary AND
     {$EndIf evdNeedEverestTxt}
     CT.InheritsFrom(k2_idDocument) then
    OutBinary;
  end else
   inherited;
 end;
...
begin
 with aBlock do begin
  if not TagType.InheritsFrom(k2_idDocument) then begin
   {$IfDef evdNeedEverestTxt}
   ...
   {$EndIf evdNeedEverestTxt}
    ...
    {$Else  evdPack}
    WriteWord(unpack_idLink);
    WriteWord(TagType.ID);
    {$EndIf evdPack}
    WriteLong(ID);
   {$IfDef evdNeedEverestTxt}
   end else begin
...
begin
 if f_IsPlain then
  ...
  CT := CurrentType;
  if InBlock AND CT.InheritsFrom(Block.TagType.ID) then begin
   with f_Stack do 
   begin
    DeleteLast;
    with Block do begin
...
begin
 ...
 l3Set(f_Type, aType);
 if aType.InheritsFrom(k2_idDocument) then
  ID := 0
 else
  ID := -1; 
end;
       ...
       {$EndIf evdNeedEverestTxt}
        ...
        begin
         ...
         if l3IOk(Storage.QueryInterface(Im3IndexedStorage, l_IndexedStorage)) then
          try
           l_IndexedStorage.SetIndexParam(8, 8 * SizeOf(Tk2TypeID));
          finally
