...
procedure TevControlsBlockPainter.GetFramePartPrim(const aFrame: InevTag;
  anIndex: Tl3FramePartIndex;
...
//#UC START# *4804B9BB0383_49DB469F01B8_impl*
 ...
 if (anIndex = l3_fpiUp) AND
    l3IsNil(ParaX.PCharLenA[k2_tiShortName]) then
  Exit;
...
//#UC START# *4804C35B00B2_49DB469F01B8_impl*
 if not aTop then
  Exit;
 l_Name := ParaX.PCharLenA[k2_tiShortName]{evGetBlockName(ParaX)};
 ...
 try
  PushWO;
  try
   l_IsCloak := ParaX.InheritsFrom(k2_idCloak);
   l_IsNodeGroup := ParaX.InheritsFrom(k2_idNodeGroup);
   ...
   try 
    ...
    if (l_Extent.X <= 0) then
     l_Extent.X := ParaX.IntA[k2_tiWidth];
...
(*      Area.rCanvas.FillRect(l3SRectBnd(l3SPoint(l_ExtentS.X,
    ...
    if not l3IsNil(l_Name) then
     SmartCanvas.DrawCollapsedPanel(l_Name{.AsPCharLen},
                                    l_Rect,
                                    ParaX.BoolA[k2_tiEnabled],
                                    Collapsed,
                                    ParaX.BoolA[k2_tiUpper],
