...
//#UC START# *4B1D18650208_4B1CFCBA0170_impl*
 ...
 if (l_ChildPoint <> nil) AND l_ChildPoint.IsValid then
  if not l_ChildPoint.BoolA[k2_tiCollapsed] then
...
(*           l_HalfChildren := (l_CI.rCountMinus1 + 1) div 2;
     ...
     begin
      ...
      begin
       ...
       begin
        ...
        if (l_FI <> nil) and not l_FI.IsHidden(false, true) then
         IncLine;
        SetPID(l_Child.PID);
        Assert(GetInner.Obj^.IsSame(l_Child));
        if (l_MoveDistance > 0) and
           (l_ChildPoint <> nil) and
           l_ChildPoint.IsValid and
      ...
      begin
       l_ChildPoint := Self.GetInner;
       if (l_ChildPoint <> nil) AND l_ChildPoint.IsValid and
          not l_ChildPoint.BoolA[k2_tiCollapsed] then
