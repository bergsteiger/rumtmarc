...
{ $Id: evExportGenerator.pas,v 1.27 2013/04/24 09:35:36 lulin Exp $ }
...
begin
 ...
 f_InternalGenerator.Root.Write(Generator, l3_spfAll, [k2_tiSubs]);
...
begin
 if AccumulateHeader AND
    not NeedUndo AND
    CurrentType.InheritsFrom(k2_idDocument) then OutHeader;
 inherited;
end;

procedure TevCustomExportGenerator.StartChild(TypeID: Long);
  {override;}
  {-}
var
 l_CT : Tk2Type;  
begin
 if AccumulateHeader AND TypeTable[TypeID].InheritsFrom(k2_idPara) then OutHeader;
 inherited;
 if Transparent then
 begin
  l_CT := CurrentType;
  if l_CT.InheritsFrom(k2_idDocument) then
  begin
   {-надо вылить шапку документа}
   AccumulateHeader := true;
   with f_InternalGenerator do PushChild(l_CT, Root);
   Generator := f_InternalGenerator;
   f_InternalGenerator.Root.Write(Generator, l3_spfAll, [k2_tiSubs]);
  end;{..k2_idDocument..}
 end;{..Transparent..}
