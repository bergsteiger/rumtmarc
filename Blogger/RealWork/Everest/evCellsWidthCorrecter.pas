...
type
 ...
 TevCellsWidthCorrecter = class(TevAlignWithOverlapInfo)
  {* Инструмент для выравнивания границ ячеек. }
 private
 // private fields
 ...
 protected
 // realized methods
   function GetCellsOffsetPairByIndex(anIndex: Integer): TevCellsOffsetsPair; override;
   ...
   function GetNumericData: TevCellsOffsetsPair; override;
 protected
...
var
 ...
 procedure lp_CheckWidthList;
 const
  csSet4Convertion = [{ed_Continue, ed_HeadText, ed_HeadNumber, ed_HeadInteger, ed_HeadFloat, ed_HeadEmpty}];
 var
  l_Offset: Integer;
 begin
  if (l_WidthList.CellsType[l_CellIndex] = ed_Empty) and (l_PrevList <> nil)  then
  begin
   l_Offset := l_WidthList.GetOffsetByIndex(l_CellIndex);
   if l_PrevList.CellTypeByOffset(l_Offset) in csSet4Convertion then
    l_Cell.MakeCellContinue(anOp);
  end; // if (l_WidthList.CellsType[l_CellIndex] = ed_NeedDelete) and (l_PrevList <> nil)  then
 end;
...
//#UC START# *4FA52C7A027C_4F2B767D015E_impl*
 ...
 begin
  ...
  SaveRowType(Result);
  SaveCellsTypeList(GetIterator, l_Index, Result);
...
//#UC START# *4FC5D31C000E_4F2B767D015E_impl*
 ...
 begin
  ...
  try
   ...
   if FoundPriorityTemplate and (f_PriorityTemplateRow = Result) and (aFoundType <> ev_fstInGroup) then
...
//#UC START# *4FC5D4D2033A_4F2B767D015E_impl*
 ...
 try
  if (aFoundType <> ev_fstInGroup) or not CopyWithGrop(aList, l_WidthList, l_CellIterator) then
...
//#UC START# *4F2FA52C00EC_4F2B767D015E_impl*
 ...
 begin
  ...
  if FoundPriorityTemplate then
  begin
   // Здесь анализируем группы строк...
   l_CellIteraor := GetIterator;
   Result := FindInGroups(l_CellIteraor, f_CellsOffsetPairList.Count);
   if Result <> nil then
   begin
    aFoundType := ev_fstInGroup;
    Exit;
   end; // if Result <> nil then
  end; // if FoundPriorityTemplate then
  {$ENDIF EverestLite}
  if not f_HeadAlignment then
  begin
...
function TevCellsWidthCorrecter.GetCellsOffsetPairByIndex(anIndex: Integer): TevCellsOffsetsPair;
//#UC START# *511342AC0272_4F2B767D015E_var*
//#UC END# *511342AC0272_4F2B767D015E_var*
begin
//#UC START# *511342AC0272_4F2B767D015E_impl*
 Result := f_CellsOffsetPairList[anIndex];
//#UC END# *511342AC0272_4F2B767D015E_impl*
end;//TevCellsWidthCorrecter.GetCellsOffsetPairByIndex
...
function TevCellsWidthCorrecter.GetNumericData: TevCellsOffsetsPair;
//#UC START# *51541B7802ED_4F2B767D015E_var*
//#UC END# *51541B7802ED_4F2B767D015E_var*
begin
//#UC START# *51541B7802ED_4F2B767D015E_impl*
 Result := f_NumbericData;
//#UC END# *51541B7802ED_4F2B767D015E_impl*
end;//TevCellsWidthCorrecter.GetNumericData
