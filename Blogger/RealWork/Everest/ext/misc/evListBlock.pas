...
{ $Id: evListBlock.pas,v 1.80 2013/04/24 09:35:37 lulin Exp $ }
...
begin
 ...
 begin
  if k2_typList.MakeTag.QT(InevObject, l_Tag) then
  try
   with l_Tag do
   begin
    l_Prop := TagType.Prop[k2_tiObject];
    Attr[l_Prop.TagIndex] := Tk2Type(l_Prop.AtomType).IUnknownToTag(aList);
    Result := Range;
   end;//with l_Tag
  finally
   l_Tag := nil;
...
function TevListInterfaceFactory.COMQueryTool(const aTag: Ik2Tag;
...
function TevListInterfaceFactory.QueryTool(const aTag: Ik2Tag; const ID: Tl3IID; out I: Tk2ToolPrim): Bool;
  //override;
  {-}
begin
 Result := True;
...
function TevListBlock.DoIterateChildrenF(Action : Ik2Tag_IterateChildrenF_Action;
                                       aStart : Tl3Index;
                                       aHi    : Tl3Index;
                                       aLoadedOnly : Boolean): Long;
  //override;
 ...
 with GetRedirect^ do
  if l3IOk(Attr[k2_tiObject].QueryInterface(Il3CStringList, l_List)) then
   try
    ...
    begin
     l_Tag := k2_typTextPara.MakeTag;
     try
      l_Tag.Owner := k2.TypeTable.ObjToTag(Self);
