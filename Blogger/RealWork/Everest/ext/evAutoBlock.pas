...
// $Id: evAutoBlock.pas,v 1.67 2013/04/19 13:05:40 lulin Exp $
...
type
 ...
 TddBlockNotify = procedure (const WasText : Boolean;
  ...
  function CheckColorSelection(aStyle: Long; const Sender : Ik2Tag; S :
      Tl3CustomString; const aSel  : TevPair; aText : AnsiString; out theSel  :
      TevPair): Bool;
  function CheckComment(const Sender : Ik2Tag; S: Tl3CustomString; aSel:
      TevPair; out theSel: TevPair): Bool;
  function CheckContentTable(const Sender : Ik2Tag; S: Tl3CustomString; aSel:
      TevPair; out theSel: TevPair): Bool;
  function CheckDigits(aText: Tl3CustomString; var Num: Integer): TddBlockType;
  function CheckFootnote(const Sender : Ik2Tag; S: Tl3CustomString; aSel:
      TevPair; out theSel: TevPair): Bool;
  function CheckHeader(const Sender : Ik2Tag; S: Tl3CustomString; aSel: TevPair;
      const aText: AnsiString; out theSel: TevPair): Bool;
  function CheckParagraph(const Sender : Ik2Tag; S: Tl3CustomString; aSel:
      TevPair; out theSel: TevPair): Bool;
  ...
  function DoCheckText(const Sender : Ik2Tag; S            : Tl3CustomString;
...
begin
 ...
 begin
  l_InBlock:= DocEditor.InPara(k2_idBlock, k2_idDocument) and not DocEditor.HasSelection;
    ...
    begin
     ...
     try
      try
       ...
       // Selection.Assign(l_Cursor);
       end // Editor.HasSelection
       else
       ...
       try
        ...
        begin
         ...
         begin
          ...
          Assert(l_Guard > 0);
          {$EndIf nsTest}
         end;//while not Replacer.AllDone
        end
        else
         {$IfNDef nsTest}
...
function EnableAppendix(const Atom: Ik2Tag): Boolean;
begin
...
function IsAtomInSBS(const Atom: Ik2Tag): Boolean;
var
 R: Ik2Tag;
begin
 R:= Atom.Owner;
 while (R.IsValid) and not (R.InheritsFrom(k2_idSBS) or R.InheritsFrom(k2_idDocument)) do
  R:= R.Owner;

 if not (R.IsValid) or R.InheritsFrom(k2_idDocument) then
  Result:= False
 else
  Result:= True;
end;

function IsAtomInBlock(const Atom: Ik2Tag): Boolean;
var
 R: Ik2Tag;
begin
 R:= Atom.Owner;
 while (R.IsValid) and not (R.InheritsFrom(k2_idBlock) or R.InheritsFrom(k2_idDocument)) do
  R:= R.Owner;
...
function IsAtomInTable(const Atom: Ik2Tag): Boolean;
var
 R: Ik2Tag;
begin
 ...
 begin
  R:= Atom.Owner;
  while (R.IsValid) and not (R.InheritsFrom(k2_idTable) or R.InheritsFrom(k2_idDocument)) do
   R:= R.Owner;
...
function TddBlockStructureSearcher.CheckColorSelection(aStyle: Long; const
    Sender : Ik2Tag; S : Tl3CustomString; const aSel  : TevPair; aText :
...
begin
 ...
 begin
  l_St:= TrimLeft(aText);
  if Sender.InheritsFrom(k2_idTextPara) then
...
function TddBlockStructureSearcher.CheckComment(const Sender : Ik2Tag; S:
...
function TddBlockStructureSearcher.CheckContentTable(const Sender : Ik2Tag;
...
function TddBlockStructureSearcher.CheckFootnote(const Sender : Ik2Tag; S:
    Tl3CustomString; aSel: TevPair; out theSel: TevPair): Bool;
begin
 if f_WaitForFootnote then
 begin
  // тут может быть формула!
  if (S.First = '*') and not EvIsParaFormula(Sender) then       
  begin
...
function TddBlockStructureSearcher.CheckHeader(const Sender : Ik2Tag; S:
...
function TddBlockStructureSearcher.CheckParagraph(const Sender : Ik2Tag; S:
    Tl3CustomString; aSel: TevPair; out theSel: TevPair): Bool;
var
 l_Num: Integer;
 i: Integer;
...
function TddBlockStructureSearcher.DoCheckText(const Sender : Ik2Tag; S
  ...
  begin
   ...
   begin
    Result := {$IFDEF nsTest}
               ...
               {$ENDIF}
              {$ELSE}
               MessageDlg('Является ли данный заголовок началом блока?',
                          mtConfirmation, mbYesAllNoAllCancel, 0)
              {$ENDIF};
  ...
  begin
   ...
   begin
    Result:= {$IFDEF nsTest}
              ...
              {$ENDIF}
             {$ELSE}
              MessageDlg('Является ли данный текст подписью?',
                         mtConfirmation, mbYesAllNoAllCancel, 0)
             {$ENDIF};
...
var
 ...
 l_Obj          : InevObject;  
begin
 if not (BlockInfo.RealType in [dd_cbBlock, dd_cbOutSide]) then
  l_AddBlock:= True
 else
 begin
  l_AddBlock:= False;
...
begin
 ...
 begin
  ...
  begin
   F_Dialog.Hide;
   l3FillChar(f_BlockNumbers, 0, SizeOf(f_BlockNumbers));
