...
//#UC START# *49D218B9032C_49D251E600DA_impl*
 ...
 if (aThisTarget = nil) OR aThisTarget.IsNull then
  l_ThisTarget := l_CursorTarget.OwnerObj
 else
  l_ThisTarget := aThisTarget;
 while not l_CursorTarget.IsSame(l_ThisTarget) do
 begin
  {-курсор нижнего уровня}
  l_ParentCursor := nil;
  try
   l_ParentCursor := Result.ParentPoint;
   if (l_ParentCursor = nil) OR not l_ParentCursor.Obj.IsValid then
   begin
