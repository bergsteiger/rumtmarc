...
{ $Id: evHndFlt.pas,v 1.16 2009/03/04 16:25:52 lulin Exp $ }
...
procedure TevCustomDocIDFilter.StartChild(TypeID: Long);
  {override;}
  {-}
begin
 inherited;
 ...
 begin
  f_DocID := GenerateID;
  NewID(f_DocID);
  Generator.StartChild(TopType[1].ID);
  Generator.StartChild(TypeID);
 end//f_Locked
 else
 begin
  if not Transparent AND CurrentType.InheritsFrom(k2_idDocument) then
  begin
   f_Locked := true;
   if CurrentType.InheritsFrom(k2_idAnnoTopic) then
...
begin
 Generator.Finish;
 IsDone := CurrentType.InheritsFrom(k2_idDocument);
 inherited;
...
begin
 if f_Locked then begin
  ...
  begin
   ...
   f_DocID := GenerateID;
  NewID(f_DocID);
  Generator.StartChild(CurrentType.ID);
