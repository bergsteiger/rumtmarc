  ...
  Assert(not HasInner OR
         (
          ((Self.GetInner.Obj.PID = Pred(Position)) AND
           ...
           (Self.GetInner.Obj.Box.IsSame(ParaX.ToList.Obj[Pred(Position)])))
         ));
 except
  SetEntryPoint(Position + Delta, l_CursorContext);
...
var
 l_Listner : InevPointListner;
begin
//#UC START# *4A3A39CA022C_4A3A237A01C0_impl*
 l_Listner := pm_GetListner;
 if (l_Listner <> nil) then
 begin
  l_Listner.ParaChanged(Self);
   ...
   begin
    ...
    begin
     ...
     begin
      ...
      {$IFNDEF Nemesis}
      //if not ((anEvent.Point = 0) and HasInner and GetInner.InheritsFrom(k2_idBlock)) then // Вставлили блок, сформировали цепочку детей - позиция поменялась.
...
//#UC START# *49DE40D30260_4A3A237A01C0_impl*
 ...
 begin
  ...
  begin
   ...
   begin
    ...
    if (l_Inn = nil) then
     ...
     begin
      l_Prev := Obj^;
      if NeedWindowsCaret AND not l_Prev.InheritsFrom(k2_idControlPara) then
       l_Prev := nev.NullPara;
     end;//l_Inn
   end;//f_Child = nil
  end;//l_Listner <> nil
  ...
  begin
   {$IfDef Nemesis}
   if (f_Child <> nil) AND (l_Listner <> nil) AND
      f_Child.InheritsFrom(k2_idParaList) then
    f_Child.ForceStore;
   {$EndIf Nemesis}
   f_Child.Outer := nil;
  end;//f_Child <> nil
  ...
  if (l_Listner <> nil) AND
     ((f_Child = nil) OR not f_Child.IsSame(l_Prev)) then
  begin
   if l_Prev.IsValid then
    if l_Prev.InheritsFrom(k2_idControlPara) then
   ...
   begin
    l_Inn := f_Child.MostInner;
    if l_Inn.InheritsFrom(k2_idControlPara) then
    ...
    begin
     ...
     begin
      with TagInst.Owner do
       if InheritsFrom(k2_idBlock) OR not InheritsFrom(k2_idPara) then
       begin
        SetEntryPoint(ev_cpNull, l_Ctx);
        Result := ev_dcDoneOk;
       end;//TagInst.InheritsFrom(k2_idBlock)
...
//#UC START# *49E343F60057_4A3A237A01C0_impl*
 if (f_Child = nil) or f_Child.AfterEnd then
  Result := false
 else
...
//#UC START# *49E3568201B9_4A3A237A01C0_impl*
 ...
 begin
  ...
  Assert(f_Child.Obj^.Owner.IsSame(ParaX), 'http://mdp.garant.ru/pages/viewpage.action?pageId=255980313&focusedCommentId=259888626&#comment-259888626');
  // - http://mdp.garant.ru/pages/viewpage.action?pageId=255980313&focusedCommentId=259888626&#comment-259888626
...
//#UC START# *4A38F59B0234_4A3A237A01C0_impl*
 ...
 begin
  ...
  if evInsertPara(anOp, Self, Tk2Type(TagInst.TagType).ArrayProp[k2_tiChildren].DefaultChildType.MakeTag) then
  begin
...
//#UC START# *4BBB10FB015B_4A3A237A01C0_impl*
 ...
 if HasInner and aPoint.HasInner then
 begin
  l_Inner := GetInner;
