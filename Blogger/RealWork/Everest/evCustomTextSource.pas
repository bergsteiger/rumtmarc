...
{ $Id: evCustomTextSource.pas,v 1.91 2013/04/16 11:23:23 dinishev Exp $ }
...
type
  ...
  TevDocumentChangedEvent = procedure (aSender             : TObject;
                                       ...
                                       const aNewDocument  : Ik2Tag) of object;
    {-}
...
(*      function  pm_GetTypeTable: Ik2TypeTable;
    ...
    // public methods
      ...
      procedure New(aDocumentType : Long);
        overload;
...
function TevCustomTextSource.Get_Document: Ik2Tag;
  {-}
begin
 Result := Document;
end;
       ...
       begin
        ...
        begin
         ...
         if evIsPointTextInCell(l_StartBC) then
          l_NewCursor := l_NewCursor.PointToTypedParent(k2_idTable);
         l_FinishTable := l_FinishMI.PointToTypedParent(k2_idTable);
         if l_FinishTable.ParentPoint.AtEnd(aView) and not l_NewCursor.IsSame(l_FinishTable) then
         ...
         if not l_FinishMI.IsSame(l_StartBC) then
         begin
...
procedure TevCustomTextSource.New(aDocumentType : Long);
  {-создать новый документ}
var
 G : Ik2TagGenerator;
 
 procedure l_WriteTextPara(aType: Long);
 begin//l_WriteTextPara
  G.StartChild(k2.TypeTable[aType].ArrayProp[k2_tiChildren].DefaultChildType.ID);
  try
   ;
  finally
   G.Finish;
...
begin
 ...
 begin
  ...
  try
   l_DocType := k2.TypeTable[aDocumentType];
   G.Start;
   try
    try
     if (aDocumentType = k2_idDocument) then
      if HasDocument then
       aDocumentType := DocumentContainer.DefaultDocumentType;
     G.StartChild(aDocumentType);
     try
      if l_DocType.InheritsFrom(k2_idDictEntry) then
      begin
       l_ParaType := l_DocType.ArrayProp[k2_tiChildren].DefaultChildType.ID;
      ...
      end//l_DocType.InheritsFrom(k2_idDictEntry)
      else
      if l_DocType.InheritsFrom(k2_idXML) then
      begin
       l_ParaType := l_DocType.ArrayProp[k2_tiChildren].DefaultChildType.Id;
       G.StartChild(l_ParaType);
       try
        G.StartChild(k2_idTagName);
        try
         ;
        finally
         G.Finish;
        end;//try..finally
        G.StartChild(k2_idTagBody);
        try
         ;
        finally
         G.Finish;
...
(*        G.StartChild(k2_idAttr);
      ...
      end//l_DocType.InheritsFrom(k2_idXML)
      else
...
procedure TevCustomTextSource.New(aDocumentType : TevDocumentType = ev_dtDocument);
  ...
  {* - создать новый документ. }
const
 l_DocTypes : array [TevDocumentType] of Integer = (k2_idDocument, k2_idDictEntry);
begin
...
function  TevCustomTextSource.pm_GetDocument: Ik2Tag;
  {-}
begin
 try
  if (f_DocumentContainer = nil) then
  ...
  begin
   Result := InevDocumentContainerInternal(f_DocumentContainer.Internal).Document;
   if (Result = nil) then
    Result := k2NullTag
...
procedure TevCustomTextSource.pm_SetDocumentContainer(const aValue: InevDocumentContainer);
  {-}
var
 l_OldDocument : Ik2Tag;
 l_NewDocument : Ik2Tag;
 //l_ReadOnly    : Boolean;
begin
 {$IfDef l3ProgressComponentIsComponent}
...
(*   if (f_DocumentContainer = nil) OR not f_DocumentContainer.HasProcessor then
   ...
   if (f_DocumentContainer = nil) then
    ...
    l_OldDocument := f_DocumentContainer.ExistingDocument; 
   FreeDocumentContainer;
   Changed(nev_cpView);
...
(*   if (f_DocumentContainer <> nil) AND f_DocumentContainer.HasProcessor then
    Processor.ReadOnly := l_ReadOnly;*)

   if not l3IEQ(l_OldDocument, l_NewDocument) then
    DocumentChanged(l_OldDocument, l_NewDocument);
...
begin
 ...
 begin
  f_ShowLevel := aValue;
  evDocumentPartSetShowLevel(Document, aValue, Processor);
...
procedure TevCustomTextSource.DocumentChanged(const anOldDocument, aNewDocument : Ik2Tag);
  {-}
begin
...
procedure TevCustomTextSource.ReloadSubFlags;
 ...
 function _GetLayerFlags(const Layer: Ik2Tag; Index: Long): Bool;
...
begin
 ...
 Document.Attr[k2_tiSubs].IterateChildrenF(k2L2TIA(@_GetLayerFlags));
end;
{$EndIf  Nemesis}
  ...
  try
   try
    ...
    begin
     ...
     while (l_Next <> nil) do
     begin
      if l_Next.HasDocument AND not l_Next.Document.Owner.InheritsFrom(k2_idPara) then
       l_Next.Save(G, true);
      l_Next := l_Next.Next;
...
procedure TevCustomTextSource.PropChanged(Prop      : Tk2Prop;
  ...
  {-}
begin
 if V.rTag.InheritsFrom(k2_idPara) then
 begin
  if (Prop.TagIndex in [k2_tiLeftIndent, k2_tiWidth, k2_tiFirstIndent,
                        k2_tiRightIndent, k2_tiFirstLineIndent, k2_tiTabStops]) then
   NotifyMarkerChange;
 end//Atom.InheritsFrom(k2_idPara)
 else
 if V.rTag.InheritsFrom(k2_idTabStop) then
 begin
  if (Prop.TagIndex in [k2_tiStart, k2_tiType]) then
   NotifyMarkerChange;
 end//if V.rTag.InheritsFrom(k2_idTabStop) then
 else
 if V.rTag.InheritsFrom(k2_idFrame) OR
    V.rTag.InheritsFrom(k2_idFramePart) then
  Invalidate;
end;
