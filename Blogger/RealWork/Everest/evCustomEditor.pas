...
{ $Id: evCustomEditor.pas,v 1.146 2013/05/17 12:40:18 morozov Exp $ }
...
// - evGraph и _Preview полностью переведены с класса TPrinter на интерфейс Il3Printer.
//
   ...
   {* Базовый класс окна редактора с кучей "полезных" функций. Рекомендуется для конечного пользователя (программиста). }
    ...
    // message handlers
      ...
      procedure CMEnter(var Message: TCMGotFocus);
        ...
        {-}
    private
      function  DoSearchHyperLink(const aSearchDown: Boolean): Boolean;
        {-}
    public
    ...
    // public methods
      ...
      procedure TextBufConvertF(TBCP: TevTextBufConvertProc);
        {* - конвертировать выделенный блок путем вставки/удаления. }
      function  MoveParaTypeCursor(anOp: Long; aTypeID: Long): Bool;
      ...
      function  CanUserModify: Boolean;
        {-}
      function  NextHyperlink: Boolean;
        {-}
      function  PrevHyperlink: Boolean;
        {-}
    public
    // interface methods
      { Il3ActionProcessor }
...
begin
 ...
 begin
  ...
  try
   l_TabletTag := k2_typTable.MakeTag;
   try
    l_TargetPoint := Selection.Cursor.MostInner.ParentPoint;
    l_ParaList := l_TargetPoint.Obj^;
    if l_ParaList.InheritsFrom(k2_idDocument) then
     l_ColWidth := def_inchSBSWidth
    else
    begin
     with l_ParaList do
      l_ColWidth := IntA[k2_tiWidth] - 2 * IntA[k2_tiLeftIndent];
     if (l_ColWidth <= 0) then
      l_ColWidth := def_inchSBSWidth; 
    end;//l_ParaList.InheritsFrom(k2_idDocument)
...
var
 ...
 l_InTable : Boolean;
  ...
  begin
   ...
   l_InTable := l_SubRange.InheritsFrom([k2_idTableRow, k2_idTable]);
   if l_SubRange.InheritsFrom(k2_idTableCell) then
   begin
    l_SubRange.GetBorderPoints(l_BS, l_BF);
    l_InTable := l_BS.AtStart and l_BF.AtEnd(View);
   end; // if l_SubRange.InheritsFrom(k2_idTableCell] then
  end;
...
begin
 ...
 begin
  Result := true;
  P := CurPara;
  if (P.InheritsFrom(k2_idTextPara)) AND
     (P.Owner.InheritsFrom(k2_idBlock)) then
  begin
   l_Pack := StartOp(ev_msgInsertPara);
   try
    try
     ...
     try
      ...
      try
       ...
       if MoveParaTypeCursor(ev_ocTopLeft, k2_idSBS) then
 ...
 begin
  ...
  begin
   ...
   try
    ...
    try
     ...
     try
      ...
      try
       ...
       begin
        G.StartChild(k2_idSectionBreak);
    ...
    if MoveParaTypeCursor(ev_ocTopLeft, k2_idPageBreak) then
     MoveLeafCursor(ev_ocParaRight, true);
   finally
    Pack := nil;
   end;//try..finally
...
begin
 ...
 begin
  ...
  try
   ...
   if MoveParaTypeCursor(ev_ocTopLeft, k2_idBitmapPara) then
    MoveLeafCursor(ev_ocParaRight, true);
  finally
   Pack := nil;
  end;//try..finally
...
begin
 ...
 begin
  ...
  try
   ...
   if MoveParaTypeCursor(ev_ocTopLeft, k2_idFormula) then
    MoveLeafCursor(ev_ocParaRight, true);
  finally
   Pack := nil;
  end;//try..finally
...
function TevCustomEditor.MoveParaTypeCursor(anOp: Long; aTypeID: Long): Bool;
  {* - переместить курсор для типа aTypeID. (см. evOp: ev_ocCharLeft, ev_ocCharRight etc.) }
var
 l_Cursor : InevBasePoint;
begin
...
begin
 ...
 if (PrintDialog = nil) AND (TextSource <> nil) then
  PrintDialog := TextSource.PrintDialog;
 if (PrintDialog = nil) then
  Print(nil, false)
 else
 begin
  P := Tl3Printer.Create;
  try
   ...
   try
    ...
    if PrintDialog.Execute then
     Print(P, PrintDialog.PrintRange = prSelection, aMacroReplacer);
   finally
    SetPrinter(OP);
   end;//try..finally
  finally
...
function TevCustomEditor.DoSearchHyperLink(const aSearchDown: Boolean): Boolean;
var
 l_Opt: TevSearchOptionSetEx;
 l_Op: InevOp;
begin
 ...
 begin
  ...
  try
   l_Op := Processor.StartOp;
   try
    Find(TevCustomHyperlinkSearcher.Make, nil, l_Opt);
    Result := true;
   finally
    l_Op := nil;
   end;//try..finally
...
function TevCustomEditor.NextHyperlink: Boolean;
  {-}
begin
 Result := DoSearchHyperLink(True);
end;

function TevCustomEditor.PrevHyperlink: Boolean;
  {-}
begin
 Result := DoSearchHyperLink(False);
end;
...
var
 ...
 function CheckRow(const aSelection : InevRange;
  ...
  function CheckCell(const aSelection : InevRange;
   ...
   begin
    aSelection.Obj.QT(InevTextPara, l_TextPara);
...
begin
 if f_InCheckAACCursor then
  Exit;
 if (AACLike > nev_aacNone) and not HasSelection then
 begin
  f_InCheckAACCursor := true;
  try
