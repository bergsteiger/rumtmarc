...
type
 ...
 // overridden protected methods
   ...
   procedure ValidateSegFont(const aSeg: InevTag); override;
...
//#UC START# *49DB2E950297_4804CDA403CE_impl*
 ...
 case aType of
  ev_ctButton, ev_ctStateButton:
   begin
    SmartCanvas.DrawPictureButton(aEditRect, ParaX.BoolA[k2_tiUpper],
      ParaX.BoolA[k2_tiEnabled], evChecked2State(ParaX), TopBC,
  ...
  ev_ctEllipsesEdit, ev_ctCalEdit:
    SmartCanvas.DrawElipsisEdit(aEditRect, aBtnWidth, ParaX.BoolA[k2_tiEnabled],
      Caret <> nil, false, not ParaX.BoolA[k2_tiFlat], aPartialDraw,
...
//#UC START# *4804B76803D5_4804CDA403CE_impl*
 if ParaX.BoolA[k2_tiVisible] and
 ...
 begin
  ...
  l_Type := TevControlType(ParaX.IntA[k2_tiType]);
  l_Height := 0;
(*  if (l_Type = ev_ctPictureLabel) then
   if ParaX.Owner.InheritsFrom(k2_idTableCell) then
   begin
...
(*      //   т.к. УЖЕ увеличили при расчёте Map.Bounds в DrawLeaf см. l_FillParent
   ...
   end;//ParaX.Owner.InheritsFrom(k2_idTableCell)*)
  Area.rCanvas.PushWO;
  try
...
(*    Dec(l_EditRect.R.Right, 2);
   ...
   l_Enabled := ParaX.BoolA[k2_tiEnabled];
   ...
   case l_Type of
    ev_ctEdit, ev_ctEmailEdit, ev_ctPhoneEdit:
     SmartCanvas.DrawEdit(l_EditRect, l_Enabled, (Caret <> nil), false,
       not ParaX.BoolA[k2_tiFlat], false, l_BorderWidth);
    ev_ctMemoEdit:
    begin
     l_EditRect.Left := l_EditRect.Left +
                        Area.rCanvas.LP2DP(l3PointX(ParaX.IntA[k2_tiFirstIndent])).X - 2;
     SmartCanvas.DrawEdit(l_EditRect, l_Enabled, (Caret <> nil), false,
       not ParaX.BoolA[k2_tiFlat], false, l_BorderWidth);
    end;//ev_ctMemoEdit
    ev_ctCombo:
     SmartCanvas.DrawComboBox(l_EditRect, l_ButtonWidth, l_Enabled,
       Caret <> nil, False, not ParaX.BoolA[k2_tiFlat], false,
       ParaX.BoolA[k2_tiChecked]);
    ev_ctButton,
    ev_ctPictureLabel,
    ev_ctStateButton,
    ev_ctEllipsesEdit,
    ...
    ev_ctSpinedit:
      SmartCanvas.DrawSpinEditBox(l_EditRect, l_ButtonWidth, l_Enabled,
        Caret <> nil, false, false, not ParaX.BoolA[k2_tiFlat]);
    ev_ctLabel:
     if l_Enabled then
      l_EditRect.Inflate1(1);
    ev_ctCheckEdit, ev_ctRadioEdit:
    begin
     l_FirstIndent := Area.rCanvas.LP2DP(l3PointX(ParaX.IntA[k2_tiFirstIndent])).X;
...
//#UC START# *4804C81000B9_4804CDA403CE_impl*
 ...
 l_Visible := ParaX.BoolA[k2_tiVisible];
 l_Type := TevControlType(ParaX.IntA[k2_tiType]);
 l_Spaces := Spaces;
  ...
  begin
   ...
   try
    ...
    begin
     ...
     if not ParaX.BoolA[k2_tiValid] then
   ...
   case l_Type of
    ...
    ev_ctLabel:
     if l_Enabled then
      l_EditRect.Inflate1(1);
    ev_ctCheckEdit, ev_ctRadioEdit:
     l_EditRect.Left := l_EditRect.Left + Area.rCanvas.LP2DP(l3PointX(ParaX.IntA[k2_tiFirstIndent])).X - 3;
   end;//case l_Type
...
procedure TevControlParaPainter.ValidateSegFont(const aSeg: InevTag);
...
//#UC START# *4D664903006E_4804CDA403CE_impl*
 ...
 if aSeg.InheritsFrom(k2_idHyperlink) then
  if not ParaX.BoolA[k2_tiEnabled] then
...
//#UC START# *4D725A550218_4804CDA403CE_impl*
 inherited;
 if l3IsNil(ParaX.PCharlenA[k2_tiText]) then
  if (ParaX.IntA[k2_tiType] = Ord(ev_ctMemoEdit)) then
   Area.rCanvas.Font.ForeColor := clSilver;
