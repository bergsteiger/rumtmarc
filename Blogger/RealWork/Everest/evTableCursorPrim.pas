...
var
 ...
 l_MarkerCellWidth     : Integer;
  ...
  var
   l_Pt : TnevPoint;
  begin
   if not theMarker.Para.QT(InevParaList, l_MarkerRow) then
   ...
   l_MarkerCellWidth := l_MarkerRow[l_MarkerCellPID].IntA[k2_tiWidth];
  end;

  function lp_ChildFunc(const aChild: Ik2Tag; anIndex: Long): Bool;
  var
   l_CellPID    : Long;
   l_LeftBorder : Long;
   l_PrevBorder : Long;       

   function lp_CellFunc(const aChild: Ik2Tag; anIndex: Long): Bool;
   begin
   ...
   function lp_FindMergeCellFunc(const aChild: Ik2Tag; anIndex: Long): Bool;
   var
  ...
  begin//ChildFunc
   ...
   try
    l_Value := aValue;
    l_WasRecalc := False;
    if theMarker.Para.IsSame(aChild) then
    begin
     l_Marker := theMarker;
    ...
    begin
     ...
     begin
      ...
      l_LeftBorder := l_Row.IntA[k2_tiLeftIndent];
      l_Row.IterateChildrenF(k2L2TIA(@lp_CellFunc));
    ...
    begin
     l_Row.IterateChildrenF(k2L2TIA(@lp_FindMergeCellFunc), l_CellPID + 1);
...
//#UC START# *4A26260A00E6_4EF9B83D010D_impl*
 ...
 if Supports(aMarker, IevColumnBorderMarker, theMarker) then
  try
   ...
   GetRedirect.IterateChildrenBackF(k2L2TIA(@lp_ChildFunc), aMaxIndex, aMinIndex);
