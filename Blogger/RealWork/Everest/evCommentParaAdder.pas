...
{ $Id: evCommentParaAdder.pas,v 1.14 2009/03/04 13:32:45 lulin Exp $ }
...
procedure TevCommentParaAdder.StartChild(TypeID: Long);
  //override;
  {-}
var
 l_Type      : Tk2Type;
begin
 ...
 if not f_WasComment AND not l_Type.InheritsFrom(k2_idDocument) AND
    l_Type.InheritsFrom(k2_idPara) then
 begin
  f_WasComment := true;
  Assert(not l_Type.InheritsFrom(k2_idCommentPara));
  inherited StartChild(k2_idCommentPara);
...
begin
 if (AtomIndex = k2_tiHandle) AND
    CurrentType.InheritsFrom(k2_idCommentPara) AND
    (f_SubList <> nil) then
 begin
  if f_SubList.SubEx[VariantAsInteger(AtomIndex, Value), ev_sbtMark].Exists then
   Exit;
