...
{ $Id: evCursorContext.pas,v 1.30 2009/07/23 13:42:10 lulin Exp $ }
...
begin
 ...
 begin
  ...
  begin
   if l_C.Obj.QT(IevSavedCursor, OC) then
    try
     OC.Store(l_C);
...
begin
 Result := true;
 if PevSavedCursor(SC)^.Cursor.Obj.QT(IevSavedCursor, NC) then
  try
