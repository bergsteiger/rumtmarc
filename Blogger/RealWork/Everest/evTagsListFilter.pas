...
{ $Id: evTagsListFilter.pas,v 1.22 2009/03/03 12:46:04 lulin Exp $ }
...
type
  ...
  TevTagsListFilter = class(Tk2TagFilter)
   {* Фильтр списка тегов. }
    protected
    // property fields
      f_ExcludeChildList   : TLongArray;
      f_IncludeChildList   : TLongArray;
    ...
    // internal methods
      ...
      procedure DoAddAtomEx(AtomIndex: Long; const Value: Tk2Variant);
        override;
        {-}
      function  IsChildForExclude(aTypeID: Long): Bool;
        {-}
    ...
    public
    // public methods
      constructor Make(const anExcludeChildList : array of Long);
        reintroduce;
        overload;
        {-}
      constructor Make(const anExcludeChildList : array of Long;
                       const anIncludeChildList : array of Long);
        reintroduce;
        overload;
        {-}
      constructor Make(const anExcludeChildList   : array of Long;
      ...
      constructor MakeAttr(const anExcludeTagAttrList : array of TevTagAttr);
        {-}
      class function SetTo(const anExcludeTagAttrList : array of TevTagAttr;
                           var theGenerator           : Tk2TagGenerator): Tk2TagGenerator;
        reintroduce;
        overload;
        {-}
      class function SetTo(const anExcludeTagAttrList : array of TevTagAttr;
                           var theGenerator           : Ik2TagGenerator): Ik2TagGenerator;
        reintroduce;
        overload;
        {-}
      class function MakeAttrList(aTypeID       : Long;
                                  const anAttrs : array of Long): TevTagAttrArray;
        {-}
    public
    // public properties
      property ExcludeChildList: TLongArray
        read f_ExcludeChildList
        write f_ExcludeChildList;
        {* - список ID типов тегов для исключения. }
      property IncludeChildList: TLongArray
...
function evTagAttr(aTypeID, aTagID : Long): TevTagAttr;
...
constructor TevTagsListFilter.Make(const anExcludeChildList : array of Long);
  //overload;
  {-}
begin
 Create;
...
constructor TevTagsListFilter.MakeAttr(const anExcludeTagAttrList : array of TevTagAttr);
  {-}
begin
 Make(PLong(nil)^, PLong(nil)^, anExcludeTagAttrList);
end;
...
class function TevTagsListFilter.SetTo(const anExcludeTagAttrList : array of TevTagAttr;
                                       var theGenerator           : Tk2TagGenerator): Tk2TagGenerator;
  //reintroduce;
  {-}
begin
...
class function TevTagsListFilter.MakeAttrList(aTypeID       : Long;
                                              const anAttrs : array of Long): TevTagAttrArray;
  {-}
var
 l_Index : Integer;
...
procedure TevTagsListFilter.DoStartChild(TypeID: Long);
  //override;
  {-}
begin
...
function TevTagsListFilter.IsChildForExclude(aTypeID: Long): Bool;
  {-}
var
 i  : Long;
 TT : Tk2TypeTable;//Ik2TypeTableEx;
begin
 Result := false;
 ...
 begin
  ...
  for i := Low(ExcludeChildList) to High(ExcludeChildList) do
   if TT[aTypeID].InheritsFrom(ExcludeChildList[i]) then
   begin
    Result := true;
    break;
   end;//aTypeID..
  ...
  begin
   for i := Low(IncludeChildList) to High(IncludeChildList) do
    if TT[aTypeID].InheritsFrom(IncludeChildList[i]) then
    begin
     Result := false;
     break;
    end;//aTypeID..
 ...
 begin
  ...
  for i := Low(ExcludeTagAttrList) to High(ExcludeTagAttrList) do
   with ExcludeTagAttrList[i] do
    if (aTagID = TagID) AND CT.InheritsFrom(TypeID) then
    begin
     Result := true;
     break;
    end;//aTagID = TagID
