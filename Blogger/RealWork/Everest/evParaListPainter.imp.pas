...
//#UC START# *4A8122600201_481D6C300084_impl*
 ...
 if WasInit and (BottomAnchor <> nil) and TopAnchor.HasBaseLine then
  if not TopAnchor.AfterEnd and BottomAnchor.Obj^.IsSame(TopAnchor.Obj^) then
  begin
   ...
   begin
    ...
    begin
     l_CheckTop := not l_Point.Obj.OwnerObj.IsSame(l_Para);
 ...
 var
  ...
  var
   ...
   l_Top                : InevBasePoint;
    ...
    begin//DrawChild
     ...
     if (l_ChildCaret = nil) OR not l_ChildCaret.IsSame(aChild) then
      ...
      begin
       ...
       begin
        ...
        begin
         l_Points.rTop := l_Top;
         l_Points.rFake := True;
         if (l_ChildCaret <> nil) and l_ChildCaret.IsSame(RealTopAnchor.Obj^) then
          l_Points.rCaret := l_ChildCaret;
        end
        else
         l_Points.rTop := l_Top;
  ...
  begin//DrawChild
   ...
   begin
    ...
    try
     if aChild.QT(IevPainter, l_ChildPainter) then
     // - Получаем интерфейс к парню, который умеет рисовать текущий параграф
      try
       l_ChildPainter.ParentPainter := Self;
       CheckChild(aChild);
       l_DrawFakeChild := (ParentPainter <> nil) and (ParentPainter.TablePainter <> nil) and not ParentPainter.TablePainter.WasPainted(aChild, aChild.PID, False);
       if RealTopAnchor <> nil then // Ячейку мы рисуем объединенную, а её начало не видно...
       begin
 ...
 begin//DrawPara
  ...
  try
   ...
   try
    ...
    if l_Para.IsSame(TopAnchor.Obj^) or (RealTopAnchor <> nil) then // В случае объединенной ячейки условие выполняться не будет...
    begin
...
//#UC START# *4804BC800172_481D6C300084_impl*
 ...
 begin
  ...
  if (l_Pos = theBottom.Obj.ChildrenCount) then
