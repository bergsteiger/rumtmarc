...
{ $Id: evSearch.pas,v 1.299 2013/05/31 04:58:21 lulin Exp $ }
...
// - new behavior: операция DoCheckText возвращает False всегда, если f_SrchMashine.SearchPattern = ''
//
...
// bugfix: TevRegularExpressionSearcher.DoCheckText;
//
...
// - bug fix: TevRegularExpressionSearcher.DoCheckText - корректируем конечный курсор
...
// - new event: TevSearchTool.OnTextChanged.
//
...
(*      f_TypeTable : Ik2TypeTable;*)
   ...
   {* Базовый класс для инструментов поиска. }
    ...
    // public methods
      ...
      class function Make(const aText : AnsiString;
        ...
        reintroduce;
        {-}
      function DoCheckTextEx(const aPara    : Ik2Tag;
   ...
   {* Инструмент для поиска стиля, примененного к параграфу или его части. }
    ...
    // internal methods
      ...
      procedure Cleanup;
        override;
        {-}
     function IsStyleFound(const aStyle : Ik2Tag): Boolean;
       virtual;
       {-}
    protected
  ...
  TevOneLevelSearcher = class(TevBaseSearcher)
    ...
    public
    // public methods
      constructor Create(const aParent : Ik2Tag);
        reintroduce;
        {-}
      class function Make(const aParent : Ik2Tag;
                          aNext         : IevSearcher = nil): IevSearcher;
        reintroduce;
        {-}
  ...
  TevParaByIDSearcher = class(TevBaseSearcher)
    ...
    // public methods
      ...
      class function Make(anID           : Integer;
        ...
        {-}
      function  DoCheckText(const Sender : Ik2Tag;
                            S            : Tl3CustomString;
                            const aSel   : TevPair;
                            out theSel   : TevPair): Bool;
        override;
...
function TevBaseSearcher.DoCheckTextEx(const aPara    : Ik2Tag;
                                       aText          : Tl3CustomString;
                                       const aSel     : TevPair;
                                       const aPrevSel : TevPair;
                                       out theSel     : TevPair): Bool;
...
begin
 ...
 Result := DoCheckTextEx(aTag, l_Text, l_Sel, l_PrevSel, theSel);
 if (l_Sel.rFinish = ev_cpAtEnd) AND (theSel.rFinish <> ev_cpAtEnd) then
  ...
  begin
   if (l_Text.Len = theSel.rStart) then
   begin
...
function TevBMTextSearcher.DoCheckText(const aPara : Ik2Tag;
                                       aText       : Tl3CustomString;
                                       const aSel  : TevPair;
                                       out theSel : TevPair): Bool;
  {override;}
...
function TevStrictWordSearcher.DoCheckText(const aPara : Ik2Tag;
                     aText       : Tl3CustomString;
                     const aSel  : TevPair;
                     out theSel  : TevPair): Bool;
//        override;
...
function TevWordFromBeginningSearcher.DoCheckText(const aPara : Ik2Tag;
                     aText       : Tl3CustomString;
                     const aSel  : TevPair;
                     out theSel  : TevPair): Bool;
//        override;
...
begin
 ...
 begin
  ...
  f_Style.SelectStyledObjectType(k2_idTextPara);
 end;//f_Style = nil
...
function TevStyleSearcher.IsStyleFound(const aStyle : Ik2Tag): Boolean;
begin
 Result := aStyle.IsSame(f_Style.Tag);
end;

function TevStyleSearcher.DoCheckText(const aPara : Ik2Tag;
                                      aText       : Tl3CustomString;
                                      const aSel  : TevPair;
                                      out theSel : TevPair): Bool;
  {override;}
...
var
 ...
 function _SearchLayer(const SL: Ik2Tag; Index: Long): Bool;

  function _SearchSegment(const Seg: Ik2Tag; Index: Long): Bool; 
 ...
 begin//_SearchLayer
  ...
  if (Layer = l3NilLong) OR (Layer = SL.IntA[k2_tiHandle]) then
   evSegmentsLayer_IterateSegmentsF(aPara, SL, l_Start, l_Finish,
                                    k2L2TIA(@_SearchSegment), l_Back);
 end;//_SearchLayer
...
begin
 ...
 begin
  ...
  if (aText.Len = aSel.rFinish) then
   theSel.rFinish := ev_cpAtEnd
  else
 ...
 begin
  ...
  begin
   ...
   aPara.Attr[k2_tiSegments].IterateChildrenF(k2L2TIA(@_SearchLayer));
...
function TevNormalWordSearcher.DoCheckText(const aPara : Ik2Tag;
                                           aText       : Tl3CustomString;
                                           const aSel  : TevPair;
                                           out theSel : TevPair): Bool;
  {override;}
 ...
 begin
  ...
  begin
   ...
   if (l_SubList <> nil) then
    try
     l_SubList.SubEx[l_Handle, Layer].SetTo(Start.MostInner.Obj^);
    finally
     l_SubList := nil;
    end;//try..finally
   Result := true;
...
begin
 ...
 try
  l_S.SelectStyledObjectType(k2_idTextPara);
...
begin
 ...
 begin
  ...
  aBlock.Formatting.Modify(aView).SetAtom(aView, k2_tiStyle, f_Style.Tag, Container);
 end;//f_Style = nil
end;
...
function TevRegularExpressionSearcher.DoCheckText(const aPara : Ik2Tag;
                                                  aText       : Tl3CustomString;
                                                  const aSel  : TevPair;
                                                  out theSel : TevPair): Bool;
var
...
class function TevCustomHyperlinkSearcher.Make(anOptions : TevSearchOptionSet = [];
  ...
  {-}
begin
 Result := inherited Make(ev_saHyperLink, ev_slHyperlinks, ev_spSegments, anOptions, aNext);
end;
...
function TevCustomHyperlinkSearcher.DoCheckText(const aPara : Ik2Tag;
                                                aText       : Tl3CustomString;
                                                const aSel  : TevPair;
                                                out theSel : TevPair): Bool;
  //override;
...
begin
 ...
 repeat
  ...
  begin
   try
    Result := DoHyperlinkSearch(TevHyperlink.Make(aPara, f_CurrentSeg));
   except
    Result := false;
   end;//try..except
   if Result then
 ...
 begin
  ...
  try
   ...
   if lFound and Assigned(FFoundStart) and (not FMultiblock) then // проверяем, относится ли найденное к тому же родителю
    lFound := FFoundStart.MostInner.Obj.Owner.IsSame(lFinish.MostInner.Obj.Owner);
   if lFound then
   begin
...
function TevEmptyTextSearcher.DoCheckText(const aPara : Ik2Tag;
                                          aText       : Tl3CustomString;
                                          const aSel  : TevPair;
                                          out theSel  : TevPair): Bool;
begin
...
function TevAnyParagraphSearcher.DoCheckText(const aPara : Ik2Tag;
                                             aText       : Tl3CustomString;
                                             const aSel  : TevPair;
                                             out theSel  : TevPair): Bool;
begin
...
function TevSpecialParagraphSearcher.DoCheckText(const aPara : Ik2Tag;
                                                 aText       : Tl3CustomString;
                                                 const aSel  : TevPair;
                                                 out theSel  : TevPair): Bool;
begin
...
function TevContinuousTextSearchController.DoCheckTextEx(const aPara    : Ik2Tag;
                                                         aText          : Tl3CustomString;
                                                         const aSel     : TevPair;
                                                         const aPrevSel : TevPair;
                                                         out theSel     : TevPair): Bool;
...
constructor TevOneLevelSearcher.Create(const aParent : Ik2Tag);
  //reintroduce;
  {-}
begin
 inherited Create;
 f_Parent := aParent.Box;
end;

class function TevOneLevelSearcher.Make(const aParent : Ik2Tag;
                                        aNext         : IevSearcher = nil): IevSearcher;
  //reintroduce;
  {-}
var
...
begin
 if (f_Parent <> nil) then
  if not f_Parent.IsSame(aTag.Owner) then
...
function TevParaPlaceSearcer.DoCheckText(const aPara : Ik2Tag;
                                         aText       : Tl3CustomString;
                                         const aSel  : TevPair;
                                         out theSel : TevPair): Bool;
  //override;
...
function TevParaByIDSearcher.DoCheckText(const Sender : Ik2Tag;
                                         S            : Tl3CustomString;
                                         const aSel   : TevPair;
                                         out theSel   : TevPair): Bool;
  //override;
