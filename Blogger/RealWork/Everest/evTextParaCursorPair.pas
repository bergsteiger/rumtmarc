unit evTextParaCursorPair;

{ Библиотека "Эверест"    }
{ Автор: Люлин А.В. ©     }
{ Модуль: evTextParaCursorPair - }
{ Начат: 16.12.2002 15:23 }
{ $Id: evTextParaCursorPair.pas,v 1.147 2013/04/24 09:35:36 lulin Exp $ }

// $Log: evTextParaCursorPair.pas,v $
  ...
  TevTextCursorPair = class(_nevTextParaTool_)
   {* Выделение для текстового параграфа. }
    ...
    // internal methods
      ...
      {$EndIf Nemesis}
      function  DeleteSegmentsEx(aLayerID       : Integer;
                                 const anOpPack : InevOp): Bool;
...
function TevTextCursorPair.DoSetAtom(const aView : InevView;
 ...
 var
  ...
  procedure ClearLayer(aLayer: Integer);
  var
   l_Index : Integer;
   l_Layer : Ik2Tag;
   l_Seg   : Ik2Tag;
  begin//ClearLayer
   ...
   begin
    ...
    begin
     ...
     if not l_Seg.InheritsFrom(k2_idHyperlink) AND
        not evSegmentHasSomeFormat(l_Seg) then
   ...
   begin
    ...
    begin
     ...
     if (pSource.IsNull) OR
      ...
      evSegments_PaintSegment(ParaX, ParaX.cAtom(k2_tiSegments, anOpPack),
    ...
    begin
     ...
     evTextParaSetFontParam(ParaX, anIndex, pSource, anOpPack);
     l_Segs := ParaX.Attr[k2_tiSegments];
...
var
 ...
 function _AddSegmentFont(const aSegment: Ik2Tag; Index: Integer): Bool;
 var
...
begin
 ...
 evSegmentsLayer_IterateSegmentsF(ParaX, aMap.ViewSegments, l_Start, f_B.rFinish, k2L2TIA(@_AddSegmentFont));
end;
{$EndIf evNeedGetBlockFont}
...
var
 ...
 function SegmentID(const Seg: Ik2Tag; Index: Integer): Bool; 
 begin//SegmentID
  Result := true;
...
begin
 ...
 begin
  l_Atom := GetRedirect;
  G.StartChild(l_Atom.TagType.ID);
  try
   ...
   begin
    if Solid(aView) then
     ...
     l_Atom.Write(G, l3_spfAll, [k2_tiSubs]);
   end
   else
...
function TevTextCursorPair.DeleteSegmentsEx(aLayerID       : Integer;
                                            const anOpPack : InevOp): Bool;
  {* - удаляет сегменты на слое aLayerID. }
var
 l_Segments : Ik2Tag;
...
begin
 ...
 begin
  ...
  begin
   ...
   if l_Layer.IsValid then
   begin
    l_List := evSegmentsLayer_GetSegments(ParaX, l_Layer, Succ(f_B.rStart), Succ(f_B.rFinish), false);
    try
     ...
     begin
      ...
      begin
       ...
       if (aLayerID <> ev_slHyperlinks) OR l_Seg.InheritsFrom(k2_idHyperlink) then
