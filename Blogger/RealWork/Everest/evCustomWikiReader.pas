     ...
     try
      ...
      try
       ...
       begin
        ...
        begin
         AddPCharLenAtom(k2_tiURL, l3PCharLen(l_SA[1]));
       ...
       begin
        ...
        try
         ...
         if (Length(l_SA) > 2) then
          AddPCharLenAtom(k2_tiShortName, l3PCharLen(l_SA[2]));
        finally
         Finish;
        end;//try..finally
       end//Length(l_SA) > 2
...
var
 ...
 function IsOpenParam(const aSt: AnsiString; var aList : Tl3ProtoIntegerList): Boolean;
  ...
  begin
   ...
   begin
    l_TagID := GetEnumValue(TypeInfo(Tk2TagID), '_k2_ti' + aSt);
 ...
 begin//TryTag
  ...
  begin
   ...
   begin
    StartChild(aChildID);
    if (anAtom > 0) then
     AddPCharLenAtom(anAtom, Filer.ReadLn);
    if (aChildID = k2_idCloak) then
...
//#UC START# *4E0B6520020B_4D6BB1E0012D_impl*
 l_S := aText.AsString;
 Result := false;
 if not TryTag('cloak', f_CloakLevel, k2_idCloak, k2_tiShortName) then
