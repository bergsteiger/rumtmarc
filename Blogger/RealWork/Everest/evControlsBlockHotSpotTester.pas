...
{ $Id: evControlsBlockHotSpotTester.pas,v 1.10 2012/07/06 09:31:04 dinishev Exp $ }
...
begin
 ...
 thisMap := aMap;
 if l3IsNil(ParaX.PCharLenA[k2_tiShortName]){ AND
    not ParaX.InheritsFrom(k2_idCloak)} then
 ...
 begin
  if (thisMap <> nil) AND
     ParaX.InheritsFrom(k2_idCloak) AND
  ...
  begin
   ...
   theSpot := TevHotSpotWrap.Make(Self);
   Result := true;
  end//..ParaX.InheritsFrom(k2_idCloak)..
  else
   Result := inherited DoGetAdvancedHotSpot(aView, aState, aPt, aMap, theSpot);
  ...
  begin
   case anAction of
    ...
    begin
      ...
      if (l_CL = nil) then
      begin
       if ParaX.InheritsFrom(k2_idCloak) then
       begin
    ...
    ev_maUp: begin
     ...
     begin
      ...
      begin
       Effect.rNeedAsyncLoop := false;
       Result := true;
       ParaX.BoolW[k2_tiCollapsed, aView.Control.Processor.StartOp] :=
        not ParaX.BoolA[k2_tiCollapsed]; 
      end//ParaX.InheritsFrom(k2_idCloak)
      else
       Result := False;
     end//l_CL = nil
