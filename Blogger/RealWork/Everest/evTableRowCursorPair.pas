...
{ $Id: evTableRowCursorPair.pas,v 1.25 2013/01/30 11:32:09 dinishev Exp $ }
...
function TevTableRowCursorPair.GetChildSel(const aView: InevView;
  ChildIndex: Long;
  aForDrawing: Boolean): InevRange;
var
...
begin
 Result := inherited GetChildSel(aView, ChildIndex, aForDrawing);
 if aForDrawing and (Result <> nil) then
 begin
  l_CellObj := Result.Obj^;
  if TevMergeStatus(l_CellObj.IntA[k2_tiMergeStatus]) = ev_msContinue then
  begin
   l_CellObj.QT(InevTableCell, l_TableCell);
