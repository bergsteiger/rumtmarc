...
//#UC START# *48E22736003D_48E22669037D_impl*
 ...
 begin 
  ...
  if (l_Point.TagType.InheritsFrom(k2_idControlPara)) and 
    (TevControlType(l_Point.Obj.IntA[k2_tiType]) = ev_ctCalEdit) then
  begin
 ...
 begin
  with Selection.Caret do
   with Selection.Cursor.MostInner.Obj^ do
    if InheritsFrom(k2_idControlPara) AND (IntA[k2_tiType] = Ord(ev_ctCalEdit)) then
    begin
    ...
    end//InheritsFrom(k2_idControlPara) AND (IntA[k2_tiType] = Ord(ev_ctCalEdit))
    else
    ...
    try
     ...
     try
      ...
      begin
       ...
       if not l_Control.IsMultyLine then
       begin
        SetCursorToPara(l_Control.Para, False, False);//Чтобы курсор не терялся
        Result := True;
       end; //if not l_Control.IsMultyLine then
     finally//try..finally
      l_VisCtrl := nil;
     end;
    finally//try..finally
...
//#UC START# *48E2488202D5_48E22669037D_impl*
 Selection.Cursor.MostInner.Obj.QT(IevCustomEditorControl, Result);
...
function TevQueryCardEditor.GetCurrPara: InevTag;
...
//#UC START# *482BFD1802D3_48E22669037D_impl*
 ...
 try
  ...
  if not Selection.Cursor.MostInner.Obj.InheritsFrom(k2_idParaList) then
...
var
 l_ContorlListner     : InevControlListner;
 l_QueryCardContainer : InevQueryDocumentContainer;
...
//#UC START# *48C7C1A2010C_48E22669037D_impl*
 ...
 if Assigned(l_QueryCardContainer) then
  l_ContorlListner := l_QueryCardContainer.ModelNotify
 else
  l_ContorlListner := nil;
 if l_ContorlListner <> nil then  
  l_ContorlListner.StartPaste;   
 try
  Result := inherited Paste;
 finally
  if l_ContorlListner <> nil then  
   l_ContorlListner.FinishPaste;   
 end; 
...
//#UC START# *48E228DB0194_48E22669037D_impl*
 ...
 begin
  aParams.Op.Flag[vcm_ofEnabled] := InPara(k2_idControlPara, l_Point) and
                                   not l_Point.ReadOnly and
                                   evHasText(l_Point.Obj^);
