...
type
 ...
 public
 // public methods
   class function Make(const aTag: InevTag): InevAnchor; reintroduce;
 protected
...
class function TnevAnchor.Make(const aTag: InevTag): InevAnchor;
var
 l_Inst : TnevAnchor;
begin
 l_Inst := Create(aTag);
...
end;//TnevAnchor.LinkListner

procedure TnevAnchor.UnlinkListner(const aControl: InevScrollListner);
...
//#UC START# *47FF3F1800FE_49DE393900D3_impl*
 ...
 begin
  ...
  begin
   l_CA := Self.GetInner;
   if (l_CA <> nil) AND l_CA.IsValid AND not l_CA.AtEnd(aView) then
    Result := False;
  end;//Result
...
//#UC START# *47FF414E039D_49DE393900D3_impl*
 ...
 begin
  l_CA := Self.GetInner;
  if (l_CA <> nil) AND l_CA.IsValid AND not l_CA.AtStart then
   Result := False;
 end;//Result
...
procedure TnevAnchor.DisableListner;
...
end;//TnevAnchor.DisableListner

procedure TnevAnchor.EnableListner;
...
function TnevAnchor.Get_Listner: InevPointListner;
...
//#UC START# *4B1D08980098_49DE393900D3_impl*
 ...
 begin
  ...
  begin
   f_NeedSignal := false;
   IafwScrollListner(f_View).Signal;
  end//f_LockScroll = 0
  else
   f_NeedSignal := True; 
 end;//f_View <> nil
