...
type
 ...
 // property methods
   ...
   function pm_GetViewSegmentsPlace: InevTag; virtual;
 ...
 // overridden property methods
   ...
   function Get_GreatestObjPID: Integer; override;
 ...
 // overridden public methods
   function ViewSegments: InevTag; override;
   ...
   function DecorObj(aType: TnevDecorType): InevTag; override;
   ...
   function InPara(aTypeID: Integer): Boolean; override;
 ...
 // protected methods
   ...
   procedure TuneFrameText(const aText: InevTag); virtual;
   ...
   function GetGreatestObjPID: Integer; virtual;
 ...
 // protected properties
   ...
   property Prepared: TnevRenderingInfoParts
     read f_Prepared
     write f_Prepared;
   property ViewSegmentsPlace: InevTag
...
procedure TnevFormatInfo.TuneFrameText(const aText: InevTag);
...
//#UC START# *4E5F8B2601DC_4810DFE100B4_impl*
 if (Obj.IntA[k2_tiStyle] = ev_saChangesInfo) then
 begin
 ...
 begin
  ...
  if Obj.Attr[k2_tiStyle].BoolA[k2_tiHeaderHasOwnSpace] then
  begin
   if l_IsLegalComment then
   begin
    with Tk2Type(aText.TagType.Prop[k2_tiStyle].TagType) do
     l_Font := TevStandardStyles.
                Instance.
                 GetStyleFont(ObjToTag(ValueTable.DRByID[ev_saTxtNormalANSI]),
...
//#UC START# *4E6DE6D800F4_4810DFE100B4_impl*
 ...
 l_Collapsed := aView.IsTagCollapsed(Obj);
 ...
 begin
  ...
  begin
   if Obj.Attr[k2_tiStyle].BoolA[k2_tiHeaderHasOwnSpace] then
   begin
    if l3Same(Obj.Attr[k2_tiStyle].PCharLenA[k2_tiShortName], sp_HardSpace[false]) then
   ...
   if l3IsNil(Obj.PCharLenA[k2_tiText]) then
    Dec(Result, Obj.IntA[k2_tiSpaceBefore]);
  end//not l_Collapsed
  else
   Dec(Result, Obj.IntA[k2_tiSpaceBefore]);
...
function TnevFormatInfo.pm_GetViewSegmentsPlace: InevTag;
...
//#UC START# *4E5F3D1102B8_4810DFE100B4get_impl*
 Result.Left := Obj.IntA[k2_tiLeftIndent];
 Result.Right := Obj.IntA[k2_tiRightIndent];
 Result.Top := Obj.IntA[k2_tiSpaceBefore];
 Result.Bottom := Obj.IntA[k2_tiSpaceAfter];
 with Obj.Attr[k2_tiFrame] do
  if IsValid then
  begin
...
//#UC START# *481DDAAE014B_4810DFE100B4_impl*
 ...
 begin
  Result := true;
  Self.DoPrepare;
  Assert(aParts - f_Prepared = [], Format('%s : "%s" - not prepared', [Obj.TagType.AsString, Obj.StrA[k2_tiText]]));
  //wForce(false);
...
function TnevFormatInfo.GetGreatestObjPID: Integer;
//#UC START# *4B3B20420144_4810DFE100B4_var*
//#UC END# *4B3B20420144_4810DFE100B4_var*
begin
//#UC START# *4B3B20420144_4810DFE100B4_impl*
 Result := -1;
 Assert(False);
//#UC END# *4B3B20420144_4810DFE100B4_impl*
end;//TnevFormatInfo.GetGreatestObjPID
...
function TnevFormatInfo.ViewSegments: InevTag;
...
//#UC START# *4810665E02DC_4810DFE100B4_impl*
 if Obj.BoolA[k2_tiFixedWidth] then
...
function TnevFormatInfo.Get_GreatestObjPID: Integer;
//#UC START# *4B3B1D2C03D2_4810DFE100B4get_var*
//#UC END# *4B3B1D2C03D2_4810DFE100B4get_var*
begin
//#UC START# *4B3B1D2C03D2_4810DFE100B4get_impl*
 Result := GetGreatestObjPID;
//#UC END# *4B3B1D2C03D2_4810DFE100B4get_impl*
end;//TnevFormatInfo.Get_GreatestObjPID
...
function TnevFormatInfo.DecorObj(aType: TnevDecorType): InevTag;
...
//#UC START# *4E5E57E503E4_4810DFE100B4get_var*
 ...
 var
  ...
  l_CharWidth : Integer;
 begin
  Result := Obj.IntA[k2_tiFirstIndent];
...
//#UC START# *4E5E77070150_4810DFE100B4get_impl*
 ...
 begin
  l_FrameText := k2_typTextPara.MakeTag;
  try
...
var
 ...
 begin//GetParaOwnFont
  ...
  begin
   CheckOwnObjectFont;
   if Obj.HasSubAtom(k2_tiStyle) then
    l_ObjectOwnFont.AddStyledTag(Obj)
   else
    l_ObjectOwnFont.AddTag(Obj);
 ...
 procedure ApplyPrintFontSize;
  ...
  procedure DoApply;
  begin//DoApply
   if (TevIndentType(Obj.IntA[k2_tiJustification]) = ev_itPreformatted) then
    ApplySize(Obj.IntA[k2_tiPrintFontSize], not l_Fixed)
   else
   if l_Fixed then
   begin
    if Obj.HasSubAtom(k2_tiFont) then
    begin
     // - убиваем шрифт параграфа, зато исправляем ситуацию для f_FrameText
     Assert(l_OwnFont);
     Result.SetItem(k2_tiSize, TevDefaultStylesFontSizes.Instance.FontSizeForTag(Obj));
 ...
 begin//ApplyPrintFontSize
  ...
  begin
   if l_Fixed then
    ...
    begin
     if (Obj.Attr[k2_tiFont].IntA[k2_tiSize] = k2_TransparentValue) then
     ...
     end//Obj.Attr[k2_tiFont].IntA[k2_tiSize] = k2_TransparentValue
     else
      Result.SetItem(k2_tiSize, TevDefaultStylesFontSizes.Instance.FontSizeForTag(Obj));
    end;//Obj.HasSubAtom(k2_tiFont)
   //Assert(false);
...
var
 ...
 begin//ApplyZoom
  ...
  if l_Fixed AND Obj.HasSubAtom(k2_tiFont) then
   l_FontSize := l3MulDiv(TevDefaultStylesFontSizes.Instance.FontSizeForTag(Obj), l_Zoom, 100)
  else
...
//#UC START# *4E5FB65D02AA_4810DFE100B4_impl*
 ...
 try
  ...
  if TevStyleInterface.DocumentUseFixedFontForPrintAndExportSetting then
   if l_Printing then
    l_Fixed := true;
  if Obj.IsValid then
  begin
   ...
   begin
    if not TevStandardStyles.Instance.GetStandardStyleFont(Obj, l_Fixed, Result) then
     GetParaOwnFont;
   end;//Obj.HasSubAtom(k2_tiFont)
  end//Obj.IsValid
  else
  ...
  if not l_InTable then
   ApplyPrintFontSize;
  Result.AsFont(aCorrectItalic, l_OwnFont OR IsSegment);
...
//#UC START# *4E689B2A003C_4810DFE100B4get_impl*
 Result := Obj.IntA[k2_tiLineSpacing];
...
//#UC START# *4E6F79C702EC_4810DFE100B4_impl*
 ...
 if not Result AND aNeedCheckCollapsed AND Obj.TreatCollapsedAsHidden then
  if Metrics.IsTagCollapsed(Obj) then
   Result := true;
...
function TnevFormatInfo.InPara(aTypeID: Integer): Boolean;
...
//#UC START# *4E70759C00AC_4810DFE100B4_impl*
 ...
 while (l_F <> nil) do
 begin
  if l_F.Obj.InheritsFrom(aTypeID) then
  begin
   Result := true;
   break;
  end//l_F.Obj.InheritsFrom(aTypeID)
  else
   l_F := l_F.Parent;
 end;//l_F <> nil
  ...
  case l_Style of
   ...
   begin
    ...
    case l_Style of
     ...
     ev_saHLE1:
      f_ImageInfo.rInfo.rFirstIndex := 3;
     ev_saHLE2:
      f_ImageInfo.rInfo.rFirstIndex := 6;
     ev_saHLE3:
      f_ImageInfo.rInfo.rFirstIndex := 2;
     ev_saHLE4:
      f_ImageInfo.rInfo.rFirstIndex := 1;
     ev_saHLE5:
      f_ImageInfo.rInfo.rFirstIndex := 5;
     ev_saHLE6:
      f_ImageInfo.rInfo.rFirstIndex := 4;
    end;//case l_Style
   end;//evd_saAttention
  end;//case l_Style
  if not l_InStyle then
   if Metrics.AACLike > nev_aacNone then
   begin
