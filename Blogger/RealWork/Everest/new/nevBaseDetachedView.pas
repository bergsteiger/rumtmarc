...
//#UC START# *48121CB0013E_48120C1C0060_impl*
 ...
 begin
  if (f_LimitWidth = 0) then
   f_LimitWidth := {InevTag}InevObject(f_Text).IntA[k2_tiWidth];
  Result := f_LimitWidth;
 end;//f_Text = nil
...
function TnevBaseDetachedView.IsTagCollapsed(const aTag: InevTag): Boolean;
...
(* if (f_Text = nil) then
 ...
 if not InevTag(f_Text).QT(InevObject, Result) then
   Assert(False);*)
