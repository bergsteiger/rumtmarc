...
type
 ...
 TnevBaseDrawView = class(_nevDocumentContainerDecorationRules_, InevScrollListner, InevControlView, InevDrawView, InevViewMetrics)
 private
 // private fields
   f_Control : Pointer;
...
//#UC START# *5011297D01AA_4811E4BA020A_var*
 ...
 begin
  Result := aValue;
  while Result.HasInner and not Result.BoolA[k2_tiCollapsed] do
   Result := Result.Inner;
 end;
  ...
  try
   ...
   begin
    ...
    if (f_Control <> nil) AND
       l_Shape.QT(InevDrawingShape, l_DrawShape, Processor) then
    try
     with Canvas do
     begin
...
//#UC START# *467FAE0C02F3_4811E4BA020A_impl*
 ...
 begin
  ...
  Assert(l_Map <> nil);
  if f_PrevTop.Obj.Owner.IsValid then
   // - параграф не удалили
...
//#UC START# *47C5C40002AD_4811E4BA020Aget_impl*
 ...
 if (f_Top <> nil) AND not f_Top.Obj.Owner.IsValid then
 begin
 ...
 begin
  l_Shape := Shape;
  if (l_Shape <> nil) AND l_Shape.IsValid then
  begin
   f_Top := l_Shape.MakeAnchor;
   f_Top.LinkListner(Self);
  end;//l_Shape <> nil..
 end;//f_Top = nil
 Result := f_Top;
...
function TnevBaseDrawView.IsTagCollapsed(const aTag: InevTag): Boolean;
...
//#UC START# *50123B01029B_4811E4BA020A_impl*
 ...
 begin
  if not l_Point.AtEnd(Self) then
   if l_Move then
   begin
    l_Line := 1;
