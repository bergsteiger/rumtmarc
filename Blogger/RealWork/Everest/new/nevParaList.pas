...
type
 ...
 public
 // public methods
   class function Make(const aTag: InevTag): InevParaList; reintroduce;
 end;//TnevParaList
...
class function TnevParaList.Make(const aTag: InevTag): InevParaList;
var
 l_Inst : TnevParaList;
begin
 l_Inst := Create(aTag);
...
//#UC START# *48CFA8920170_48CE0B85029C_impl*
 ...
 begin
  if l_Child.InheritsFrom(k2_idPara) then
   l_Para := TnevPara.Make(l_Child)
  else
  begin
   Result := nev.NullPara;
   Exit;
  end;//l_Child.InheritsFrom(k2_idPara)
...
//#UC START# *48CFA8C301F4_48CE0B85029C_impl*
 Result := TagInst.IndexOfChild(aPara.Box);
  ...
  begin
   ...
   begin
    ...
    try
     ...
     begin
      if l_Tag.IsSame(l_InsPara) then
      begin
       InsertPara := false;
       Exit;
...
//#UC START# *47C6ACC90087_48CE0B85029C_impl*
 ...
 if not aPara.QT(InevPara, l_InsPara) then
  Assert(false);
 if not l_InsPara.IsValid then Exit;
 l_ParaList := TagInst;
 ...
 l_ParaList.InsertChild(l_PID, l_Para, anOp);
 FireInsertion;
...
//#UC START# *47C6ACE6026E_48CE0B85029C_impl*
 l_Para := Tk2Type(TagInst.TagType).ArrayProp[k2_tiChildren].DefaultChildType.MakeTag;
 try
  if aNeedFire then
...
function TnevParaList.DoIterateChildrenF(Action: Ik2Tag_IterateChildrenF_Action;
  aLo: Tl3Index;
  aHi: Tl3Index;
...
//#UC START# *4BB0751F00A4_48CE0B85029C_var*
 ...
 function DoChild(const aChild: Ik2Tag; anIndex: Integer): Boolean;
 var
  l_Para : InevParaInternal;
 begin//DoChild
...
//#UC START# *4BB0751F00A4_48CE0B85029C_impl*
 aLo := Max(0, aLo);
 Result := TagInst.IterateChildrenF(k2L2TIA(@DoChild), aLo, aHi, aLoadedOnly);
