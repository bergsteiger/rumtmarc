...
type
 ...
 InevScrollListner = afwInterfaces.IafwScrollListner;
 ...
 InevTagGenerator = k2Prim.Ik2TagGenerator;
...
*)
 ...
 InevViewMetrics = interface(InevMetrics)
   ...
   function IsTagCollapsed(const aTag: InevTag): Boolean;
...
(*
 MnevShapeTools = PureMixIn
   function InPara(aTypeID: Integer): Boolean; virtual; abstract;
...
(*
 MnevFormatInfoForMergedTablesMayBe = PureMixIn
   ...
   function Get_GreatestObjPID: Integer; virtual; abstract;
   ...
   property GreatestObjPID: Integer
     read Get_GreatestObjPID;
     {* Используется только для строк с объединенными ячейкайми. Для остальных - всегда -1 }
...
(*
 MnevFormatInfoForTextPara = PureMixIn
   ...
   function ViewSegments: InevTag; virtual; abstract;
...
(*
 MnevDecor = PureMixIn
   ...
   function DecorObj(aType: TnevDecorType): InevTag; virtual; abstract;
 end;//MnevDecor
*)

 TnevFormatInfoPrim = {abstract} class(Tl3CProtoObject)
