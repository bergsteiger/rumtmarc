...
type
 ...
 // overridden protected methods
   ...
   function GetGreatestObjPID: Integer; override;
...
//#UC START# *4B9A319D01D2_4848EBFB00B0_impl*
 if f_AllCellsIsMerged and (f_HeightList.Count = 0) and (Obj.ChildrenCount > 0) then
...
var
 ...
 begin
  ...
  if (TevMergeStatus(anItem.IntA[k2_tiMergeStatus]) = ev_msNone) then
  begin
   l_MergedOnly := False;
   lp_CheckLineCount;
   //{$IFOPT D+}
   ...
   if (TevMergeStatus(anItem.IntA[k2_tiMergeStatus]) = ev_msHead) then
   begin
    //{$IFOPT D+}
    f_AllCellsIsMerged := False;
    //{$ENDIF}
    if not anItem.QT(InevTableCell, l_Cell) then
     Assert(false);
    ...
    if (TevMergeStatus(anItem.IntA[k2_tiMergeStatus]) = ev_msContinue) then
     if l_IsSingle then
     begin
      f_AllCellsIsMerged := anItem.ChildrenCount = 0;
      if not f_AllCellsIsMerged then
      anItem.IntA[k2_tiMergeStatus] := Ord(ev_msNone);
      l_MergedOnly := False;
      lp_CheckLineCount;
     end
     else
     begin
      ...
      begin
       ...
       begin
        ...
        l_ContCell := l_Head;
        l_LineCount := 0;
        while not l_Row.IsSame(Obj.Box) do
        begin
         l_RowInfo := l_TableInfo.GetInfoForChild(l_Row);
         Assert(l_RowInfo <> nil);
         if l_Row.IsSame(l_StartRow.Box) then
         begin
...
function TnevTableRowRenderInfo.GetGreatestObjPID: Integer;
//#UC START# *4B3B20420144_4848EBFB00B0_var*
//#UC END# *4B3B20420144_4848EBFB00B0_var*
begin
//#UC START# *4B3B20420144_4848EBFB00B0_impl*
 CheckDeltaHeight;
 Result := f_GreatestObjPID;
//#UC END# *4B3B20420144_4848EBFB00B0_impl*
end;//TnevTableRowRenderInfo.GetGreatestObjPID
...
//#UC START# *4B9A10070246_4848EBFB00B0_impl*
 ...
 if l_ItemH = evUndefHeight then
  ...
  Result := f_HeightList[anIndex] - f_HeightList[GetGreatestObjPID];
