...
//#UC START# *4E709676009E_4815B8510359_impl*
 l_FixedWidth := Obj.BoolA[k2_tiFixedWidth];
 if AllowTotalRecalc OR
    InPara(k2_idTable) then
 begin
 ...
 begin
  ...
  begin
   ...
   begin
    ...
    if (l_FI <> nil) then
    begin
     Assert(l_FI.Obj.IsSame(l_Point.Obj^));
...
//#UC START# *4820985D0238_4815B8510359_impl*
 ...
 with aChild.Obj do
  if IsList then
  begin
   if BoolA[k2_tiFixedWidth] then
    if not TagType.Prop[k2_tiWidth].ReadOnly then
    begin
 ...
 begin
  if not Metrics.IsWebStyle AND
     not Obj.BoolA[k2_tiFixedWidth] AND
   ...
   begin
    ...
    begin
     ...
     begin
      ...
      if (not l_Break.IsValid OR
          (l_Break.OwnerObj.IsValid AND (l_Break.PID >= 0))) then
       l_Inited := true
      else
       l_Break := nil;
     ...
     begin
      ...
      begin
       l_Inited := true;
       if not aChild.Obj.IsSame(l_Break) then
...
//#UC START# *48172A690313_4815B8510359_impl*
 ...
 begin
  l_FI := nil;
  if Obj.IsSame(aShape.OwnerObj) then
...
//#UC START# *4815C94A027A_4815B8510359_impl*
 ...
 begin
  if not aChild.Owner.IsSame(Obj) then
  ...
  Assert(aChild.Owner.IsSame(Obj));
  if (f_Children = nil) then
   f_Children := TnevFormatInfoArray.Create;
  l_Child := f_Children[l_PID];
  Result := l_Child;
  if (Result <> nil) AND not Result.Obj.IsSame(aChild) then
  ...
  begin
   Assert(Result.Obj.IsSame(aChild));
   l_Child.LimitWidth := ChildWidth(l_Child);
...
var
 ...
 begin//lp_IncLinesCount
  Result := True;
  Inc(l_LinesCount, InfoForChild(anItem).MaxLinesCount);
