...
{ $Id: evDocumentsCache.pas,v 1.5 2011/05/17 14:54:34 lulin Exp $ }
...
// - new method: TevDocumentContainer.AddUserComment (пока ничего не делает). TevDocumentsCache.NotifyUserCommentAdded теперь вызывает его.
//
...
// - реализация-заглушка для TevDocumentsCache.NotifyUserCommentDeleted.
//
...
// - new method version: TevDocumentsCache._NotifySubDeleted.
//
// Revision 1.20  2004/10/12 17:22:51  lulin
// - new method version: TevDocumentsCache._NotifySubAdded(
...
begin
 if (aSub.LayerID = ev_sbtMark) then
  Exit;
 ...
 begin//DoContainer
  ...
  begin
   ...
   begin
    ...
    begin
     if (aSubID = 0) AND (aLayerID = ev_sbtBookmark) then
     // http://mdp.garant.ru/pages/viewpage.action?pageId=263293374
     begin
      if not InevDocumentContainer(aDoc^).FindObjByID(0, l_Para) then
       Exit;
      if (l_Para.IntA[k2_tiStyle] = ev_saTechComment) then
      begin
       l_Para := evFindNumberedPara(l_Para, true);
       if (l_Para = nil) then
        Exit;
      end;//aParaID = 0
      l_ST := l_Para.rAtomEx([k2_tiSubs,
