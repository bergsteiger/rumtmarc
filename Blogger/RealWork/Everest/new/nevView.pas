...
{ $Id: nevView.pas,v 1.110 2013/04/29 10:18:58 dinishev Exp $ }
...
begin
 ...
 begin
  ...
  begin
   l_TA := pm_GetTopAnchor;
   if l_TA.Obj.Owner.IsValid then
   begin
...
function TnevVScroller.Pos: Integer;
 ...
 var
  ...
  l_PCC  : Integer;
 begin//AbsIndex
  if aParent.IsValid then
  begin
   l_PID := aPara.PID;
   if not aParent.AlmostLinear then
   begin
    l_PCC := aParent.ChildrenCount;
  ...
  begin
   ...
   begin
    ...
    begin
     ...
     if (l_D <> nil) then
     begin
      if not l_TA.Obj.IsSame(l_D) then
...
{$IfOpt D+}
  ...
  begin
   if l_OldMI.IsSame(l_NewMI.MostInner) then
   begin
    ...
    if l_OldMI.IsSame(l_NewMI) then
    begin
