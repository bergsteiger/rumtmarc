...
{ $Id: nevShapesPainted.pas,v 1.230 2013/08/07 11:37:30 dinishev Exp $ }
...
// - теперь в PaintedShapes хранятся не _InevShape, а Ik2Tag - чтобы избежать циклических ссылок между _View и _Shape.
//
...
type
  ...
  // public methods
    ...
    procedure Clear;
      override;
      {-}
    procedure MoveBy(const aDelta: TnevPoint); virtual;
      {-}
  ...
  public
    function NeedIncHeight: Boolean; override;
      {* - Нужно ли учитывать высоту элемента при расчете родительcкого. }
    procedure MoveBy(const aDelta: TnevPoint); override;
      {-}
  ...
  // private methods
    ...
    function CheckRowShape(const aView: InevView;
      ...
      {-}
    function CorrectShape(const aView: InevView; const aShape: TnevShape): TnevShape;
     {-}
...
begin
 ...
 begin
  ...
  begin
   ...
   if (l_Inn <> nil) and not l_Inn.Obj^.Owner.IsSame(Result.f_Obj) then
   begin
 ...
 begin
  if (l_Point <> nil) AND not l_Point.Obj.IsSame(aCheck) then
   Result := nil;
  if (Result <> nil) AND not Result.f_Obj.IsSame(aCheck) then
   Result := nil;
 end;//aCheck <> nil
end;
...
var
 ...
 l_RowAnchor : InevBaseLine4Anchor;
  ...
  begin//SetTemp
   ...
   if (l_Point <> nil) then
   begin
    if l_Temp.f_Obj.IsSame(l_Point.Obj^) or (aTop <> nil) {or CheckFake} then
    // ^ - http://mdp.garant.ru/pages/viewpage.action?pageId=86479273&focusedCommentId=90442637#comment-90442637
    begin
...
begin
 ...
 begin
  l_Top := aView.Data;
  if (l_Top = nil) OR not l_Top.IsValid then
   Result := nil
  else
  begin
   l_StartIndex := 0;
...
(*      if aNeedAnchor then
     ...
     begin
      l_PrevList := nil;
      if (l_Temp <> nil) and (l_Temp.f_Parent <> nil) and l_Temp.f_Parent.IsRowShape and not l_Temp.f_Obj.IsSame(l_Top) then
       if l_SaveList <> nil then
        RestroreSavedData;
      l_SaveList := nil;
     ...
     if l_NeedBottom then
      ...
      if l_Top.IsList then
       ...
       begin
        ...
        Break;
     if l_Parent.InheritsFrom(k2_idTable) then
...
begin
 ...
 begin
  ...
  begin
   ...
   Assert(l_RealShape.f_Obj.IsSame(aShape.Owner));
  ...
  begin
   ...
   Assert(l_Decor OR
          f_Current.f_Obj.IsSame(aShape.Owner) OR
          (aShape.Owner.InheritsFrom(k2_idObjectSegment){ AND
 ...
 Assert(l_Shape.f_Obj.IsSame(aShape));
 theMap := l_Shape;
...
(*  if f_Current.f_Obj.InheritsFrom(k2_idTextPara) then
   // - заточка конечно же*)
...
function TnevShapesPainted.VertScroll(const aView : InevView;
 ...
 begin//ProcessDelta
  ...
  begin
   l_Para := aPara.Obj^;
   l_Item := anItem;
   while l_Para.IsValid AND (aFromBaseLine or (l_Para.PID <= 0)) do
   begin
    ...
    if not l_Para.OwnerObj.BoolA[k2_tiCollapsed] then
     if aInc then
...
begin
 ...
 begin
  ...
  if l_FromA.Obj.IsSame(l_ToA.Obj^) then
  begin
...
begin
 ...
 Assert((aParent = nil) OR
        f_Obj.Owner.IsSame(aParent.f_Obj) OR
        f_Obj.IsDecorationElement OR
        (f_Obj.Owner.InheritsFrom(k2_idObjectSegment){ AND
...
begin
 ...
 begin
  if aShape.IsVertical then
  begin
   if aShape.InheritsFrom(k2_idControlsBlock) then
    Result := TnevControlsBlockShape.Create(f_AllShapes,
                                              aShape.ToList,
                                              Self,
                                              aTopLeft)
   else
   if aShape.InheritsFrom(k2_idTable) then
    Result := TnevLinesStoredListShape.Create(f_AllShapes,
                                              aShape.ToList,
                                              Self,
                                              aTopLeft)
   else
   if aShape.InheritsFrom(k2_idTableCell) then
   ...
   begin
    if aShape.InheritsFrom(k2_idDocument) then
     ...
     Result := TnevListShape.Create(f_AllShapes,
                                    ...
                                    Self,
                                    aTopLeft);
   end;//aShape.InheritsFrom(k2_idTableCell)
  end//aShape.IsVertical
  else
   if aShape.InheritsFrom(k2_idTableRow) then
 ...
 if aShape.InheritsFrom(k2_idTextPara) then
...
begin
 ...
 if (Result <> nil) and not Result.f_Obj.IsSame(aShape) then
  Result := nil;
 if (Result = nil) then
 begin
 ...
 begin
  ...
  begin
   ...
   begin
    ...
    {$Else  XE4}
    Tl3Rect(Result.Bounds).TopLeft := Tl3Point(aTopLeft);
    Tl3Rect(Result.Bounds).BottomRight := Tl3Point(aTopLeft);
    {$EndIf XE4}
...
begin
 ...
 Bounds := l_Rect;
 if IsVertical then
  if (f_BottomShape = nil) or ((f_BottomShape.f_Obj.PID <= aChild.f_Obj.PID) and IsBottomShape(l_Rect.Bottom, f_BottomShape)) then
...
begin
 Assert(TopObject.IsSame(anObject));
...
begin
 ...
 begin
  l_Item := Items[l_Index];
  if l_Item.f_Obj.IsSame(aShape) then
  begin
   Result := l_Item;
   Exit;
...
{ TnevLinesStoredListShape }

procedure TnevLinesStoredListShape.CheckStoredLines(
...
procedure TnevLinesStoredListShape.Cleanup;
begin
 f_LinesArray := nil;
 inherited;
end;

procedure TnevLinesStoredListShape.RestoreFromLineList;
begin
 if f_WasScrolled and (f_LinesArray <> nil) and (f_LinesArray.Count > 0) then
  with InevView(f_AllShapes.f_View).Metrics.InfoCanvas do
   if not Printing and (FrameLines <> nil) then
    FrameLines.WriteAlignData(Integer(f_Obj.Box.AsObject), f_LinesArray);
end;

procedure TnevLinesStoredListShape.SaveLinesList;
begin
 with InevView(f_AllShapes.f_View).Metrics.InfoCanvas do
  if not Printing and (FrameLines <> nil) then
  begin
   f_LinesArray := FrameLines.ReadAlignData(Integer(f_Obj.Box.AsObject));
   {$IFDEF nsTest}
   FrameLines.ClearFrameObject(Integer(f_Obj.Box.AsObject));
   {$ENDIF nsTest}
...
begin
 Result := (Parent.f_FI.GreatestObjPID = f_Obj.PID) or
           ((f_Obj.IntA[k2_tiVerticalAligment] = Ord(ev_valBottom)) and (f_Obj.OverlapType = otNone));
end;
...
begin
 ...
 if (f_Obj.IntA[k2_tiJustification] = Ord(ev_itPreformatted)) and (f_Bounds.Right < Abs(evShapeNil)) then
 ...
 begin
  ...
  begin
   ...
   begin
    ...
    l_Parent.Inner := l_Anchor;
    aNeedMoveDown := aShape.Bounds.Bottom < (l_Bottom div 2);
    Exit;
  ...
  l_Shape := CorrectShape(aView, l_Shape);
...
begin
 ...
 begin
  ...
  if (l_Item <> nil) and not lp_CheckItem then
   l_MaxItem := l_Item;
  if l_MaxItem = nil then
   for l_Index := 0 to aShape.Count - 1 do
   begin
    l_Item := aShape.Items[l_Index];
    if (l_Item <> nil) then
    begin
     if lp_CheckItem then Continue;
  ...
  begin
   ...
   anAnchor.AddInner(aView, l_Anchor, l_MaxItem.f_Obj.PID, l_Fisrt, l_AddMode);
   if not aSimple then
    anAnchor.CompareWithOtherInner(aView);
...
begin
 ...
 begin
  for l_Index := 0 to f_TablePoints.Count - 1 do
   if f_TablePoints[l_Index].IsSame(aRowObj.Owner) then
   begin
...
begin
 ...
 l_Found := False;
 for l_Index := 0 to f_TablePoints.Count - 1 do
  if f_TablePoints[l_Index].IsSame(aPoint.Obj^) then
  begin
   l_Found := True;
   Break;
...
function TnevShapesPainted.CorrectShape(const aView: InevView; const aShape: TnevShape): TnevShape;
var
 l_Bottom  : Integer;
 l_IDShape : Integer;
begin
 ...
 begin
  if not aShape.IsBottomShape(l_Bottom, aShape) then
  begin
   l_Bottom := aView.Metrics.Extent.Y - evEpsilon;
...
procedure TnevCellShape.MoveBy(const aDelta: TnevPoint);
(*var
 l_Delta: Integer;*)
begin
 //l_Delta := VerticalAlignmentMargin;
 inherited;
end;
