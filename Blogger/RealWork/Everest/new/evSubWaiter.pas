...
{ $Id: evSubWaiter.pas,v 1.4 2011/10/25 13:53:47 dinishev Exp $ }
...
begin
 if (aControl = nil) OR (aSubType = ev_sbtNone) then
  Result := nil
 else
 begin
...
begin
 ...
 begin
  ...
  begin
   ...
   end;//aChild.InheritsFrom(k2_idLeafPara)
  end//f_WasSub
  else
  begin
   case f_Type of
    ev_sbtSub .. ev_sbtMark :
    begin
     if aChild.InheritsFrom(k2_idSub, [k2_idDocumentSub]) then
     begin
     ...
     end;//aChild.InheritsFrom(k2_idSub, [k2_idDocumentSub])
    end;//ev_sbtSub
    ev_sbtPara :
    begin
     if aChild.InheritsFrom(k2_idLeafPara) AND (aChild.IntA[k2_tiHandle] = f_ID) then
     begin
      if aChild.QT(InevObject, l_Para) then
       ...
       Assert(false);
     end;//aChild.InheritsFrom(k2_idLeafPara) AND ..
    end;//ev_sbtPara
    else
    begin
     Assert(false);
