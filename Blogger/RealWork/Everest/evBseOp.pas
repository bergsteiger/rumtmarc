...
{ $Id: evBseOp.pas,v 1.64 2011/04/27 14:23:55 lulin Exp $ }
...
// - new method: _Ik2Tag.InsertChild.
//
...
// - избавляемся от знания о _TevBasePara и TevBaseParaList.
//
...
type
  TevParaOperation = class(Tk2Op)
    ...
    public
    // public methods
      constructor Create(const aPara : Ik2Tag);
        reintroduce;
        {-}
      procedure Cleanup;
        override;
  ...
  OevDeletePara = class(TevParaOperation)
    ...
    // internal fields
      f_PID         : Long;
      f_DeletedPara : Ik2Tag;
    protected
    // property methods
      function pm_GetDeletedPara: Ik2Tag;
        {-}
    protected
    // internal methods
    ...
    // public methods
      class procedure ToUndo(const anOpPack  : InevOp;
                             const aParaList : Ik2Tag;
                             aPID            : Long;
                             const aPara     : Ik2Tag);
        {-}
      constructor Create(const aParaList : Ik2Tag;
                         aPID            : Long;
                         const aPara     : Ik2Tag);
        reintroduce;
        {-}
...
constructor TevParaOperation.Create(const aPara: Ik2Tag);
  {virtual;}
  {-}
begin
 inherited Create;
...
constructor OevDeletePara.Create(const aParaList : Ik2Tag;
                                 aPID            : Long;
                                 const aPara     : Ik2Tag);
  {virtual;}
  {-}
...
function OevDeletePara.pm_GetDeletedPara: Ik2Tag;
  {-}
begin
...
function  OevDeletePara.OpRedo(const Container: InevOp): Bool;
  ...
  {-}
begin
 if not TagInst.DeleteChild(Pred(f_PID), Container, f_DeletedPara) then
