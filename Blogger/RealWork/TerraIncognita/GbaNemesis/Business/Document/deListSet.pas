...
type
 ...
 // public methods
   ...
   class function Make(const aList: IDynList;
     ...
     const aSearchInfo: IdeSearchInfo = nil): IdeDocumentListSet; reintroduce;
 end;//TdeListSet
{$IfEnd} //not Admin AND not Monitorings
...
class function TdeListSet.Make(const aList: IDynList;
  ...
  aTimeMachineOff: Boolean = True;
  const aNodeForPositioning: Il3SimpleNode = nil;
  const aSearchInfo: IdeSearchInfo = nil): IdeDocumentListSet;
var
 l_Inst : TdeListSet;
begin
 l_Inst := Create(aList, aWhatDoingIfOneDoc, aOpenFrom, aTimeMachineOff, aNodeForPositioning, aSearchInfo);
 try
  Result := l_Inst;
 finally
  l_Inst.Free;
