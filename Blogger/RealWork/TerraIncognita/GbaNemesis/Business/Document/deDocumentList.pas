...
type
 ...
 // public methods
   ...
   class function Make(const aList: IDynList;
     ...
     const aSearchInfo: IdeSearchInfo = nil): IdeDocumentList; reintroduce;
 end;//TdeDocumentList
{$IfEnd} //not Admin AND not Monitorings
...
//#UC START# *4B1F7A850082_492FF76202CA_impl*
 Create(aList, aOpenFrom, aTimeMachineOff,
   aWhatDoingIfOneDoc, aNodeForPositioning, aSearchInfo);
...
class function TdeDocumentList.Make(const aList: IDynList;
  ...
  aWhatDoingIfOneDoc: TbsWhatDoingIfOneDoc = wdAlwaysOpen;
  const aNodeForPositioning: Il3SimpleNode = nil;
  const aSearchInfo: IdeSearchInfo = nil): IdeDocumentList;
var
 l_Inst : TdeDocumentList;
begin
 l_Inst := Create(aList, aOpenFrom, aTimeMachineOff, aWhatDoingIfOneDoc, aNodeForPositioning, aSearchInfo);
 try
  Result := l_Inst;
 finally
  l_Inst.Free;
