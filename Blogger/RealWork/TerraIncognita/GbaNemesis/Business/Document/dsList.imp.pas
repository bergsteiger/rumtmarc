...
Возвращает признак того, что чего-то сохранили }
   ...
   function ApplyLogicOperation(const aList: IUnknown;
 ...
 // protected methods
   ...
   function DoOpenDocument(out theDataInfo: IdeDocInfo): Boolean; virtual;
     {* - возвращает SDS для открытия сборки документ. }
   function DataForNewList(const aNewList: IDynList): IdeList; virtual;
   ...
   function DoApplyLogicOperation(const aList: IUnknown;
...
function _dsList_.DoApplyLogicOperation(const aList: IUnknown;
  aOperation: TListLogicOperation): Boolean;
//#UC START# *47FB05B90024_47E9EDA800FE_var*
var
 l_OtherList: IDynList;
//#UC END# *47FB05B90024_47E9EDA800FE_var*
begin
//#UC START# *47FB05B90024_47E9EDA800FE_impl*
 Result := False;
 if (ImpList <> nil) and Supports(aList, IDynList, l_OtherList) and
  (ImpList.GetContentType = l_OtherList.GetContentType) then
 try
  try
   case aOperation of
    LLO_OR:
     ImpList.MergeTree(l_OtherList);
    LLO_AND_NOT:
     ImpList.MinusTree(l_OtherList);
    LLO_AND:
     ImpList.IntersectTree(l_OtherList);
...
//#UC START# *47F32E5D01B0_47E9EDA800FE_impl*
 ...
 try
  Result := DataForNewList(l_NewList);
 finally
  l_NewList := nil;
 end;{try..finally}
...
function _dsList_.ApplyLogicOperation(const aList: IUnknown;
  aOperation: TListLogicOperation): Boolean;
...
//#UC START# *492ACF630072_47E9EDA800FE_var*
  ...
  procedure lp_LoadActiveFilters(const aFilters: IFiltersFromQuery);
  var
   l_Index  : Integer;
   l_Filter : IFilterFromQuery;
  begin
  ...
  begin
   ...
   //l_Filters := TnsFiltersContainer.Instance.Filters[pm_GetListType].PermanentFilters;
   l_Filters := GetPermanentFilters(pm_GetListType);
   ...
   begin
    ...
    begin
     lp_LoadActiveFilters(l_Filters);
     try
...
//#UC START# *492ACF630072_47E9EDA800FE_impl*
 ...
 if (f_SearchInfo = nil) or f_SearchInfo.NeedApplyPermanentFilters then
  lp_ApplyPermanentFilters
 // Загрузим активные фильтры:
 else
  if Supports(ImpList, IFilterable, l_Filterable) then
  try
   l_Filterable.GetActiveFilters(l_ActiveFilters);
   lp_LoadActiveFilters(l_ActiveFilters);
  finally
   l_Filterable := nil;
  end;
