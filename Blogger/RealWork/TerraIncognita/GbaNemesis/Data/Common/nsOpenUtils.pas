...
// $Id: nsOpenUtils.pas,v 1.48 2013/06/24 10:23:55 morozov Exp $
...
class procedure TnsOpenDocumentFromFoldersEvent.Log(const aDoc: IDocument);
var
 ...
 if Supports(aBaseEntity, IQuery, l_Query) then
  try
   case l_Query.GetType of
    QT_KEYWORD :
     TdmStdRes.OpenQuery(lg_qtKeyWord, l_Query);
    //QT_OLD_ATTRIBUTE,
    QT_ATTRIBUTE :
...
(*    QT_OLD_FILTER :
     TdmStdRes.OpenQuery(lg_qtAttribute, l_Query);*)
    QT_PUBLISHED_SOURCE :
     TdmStdRes.OpenQuery(lg_qtPublishedSource, l_Query);
    QT_COMMENTS:
    ...
    QT_REVIEW:
     if not defDataAdapter.LegislationReviewAvailable then
      ...
      TdmStdRes.OpenQuery(lg_qtLegislationReview, l_Query);
    QT_CONSULT:
     Assert(false);
    QT_MAIL_LIST:
     Exit;
    QT_BASE_SEARCH:
     TdmStdRes.OpenQuery(lg_qtBaseSearch, l_Query);
    QT_PHARM_SEARCH:
     TdmStdRes.OpenQuery(lg_qtInpharmSearch, l_Query);
    else
     Assert(false); 
...
begin
 ...
 try
  ...
  try
   if inNewWindow then
    l_Cont := nsOpenNewWindowParams(aForm.NativeMainForm)
   else
    l_Cont := nil;
     ...
     begin
      try
       ...
       if (l_Document <> nil) and
        ...
        TnsOpenDocumentFromFoldersEvent.Log(l_Document);
      except
 ...
 begin
  case TNavigatorMenuItemType(aNode.GetType) of
   ...
   NM_SEARCH_WITH_LAW_SUPPORT:
    Assert(false);
   NM_LAW_SUPPORT:
    TdmStdRes.OpenQuery(lg_qtSendConsultation, nil);
   // Толковый словарь.
