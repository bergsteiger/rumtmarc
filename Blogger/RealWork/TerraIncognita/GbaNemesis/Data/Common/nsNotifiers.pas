...
// $Id: nsNotifiers.pas,v 1.15 2013/06/19 09:48:33 morozov Exp $
...
begin
 if not Assigned(g_Dispatcher) then
  Exit;
 Assert(GetCurrentThreadID = MainThreadID);
 // Рассылка сообщения об изменении статуса объектов на контроле всем активным формам
 Op_Common_OnControlChanged.Broadcast;
end;
{$IfEnd}
