...
// $Id: nsUtils.pas,v 1.28 2013/04/24 09:35:56 lulin Exp $
// $Log: nsUtils.pas,v $
...
function nsNewWindow(const aMainForm : IvcmContainer): IvcmContainer;
  {* - открыть новое окно. }
{$If not Defined(Admin) AND not Defined(Monitorings)}
...
begin
 ...
 begin
  ...
  Result := nsOpenDocumentByNumber(l_DocId, l_PosID, l_PosType, aFaultMessage);
 end;
end;
{$IfEnd}
...
function nsNewWindow(const aMainForm: IvcmContainer): IvcmContainer;
  {* - открыть новое окно. }
begin
 Result := nsOpenNewWindowParams(aMainForm);
end;
