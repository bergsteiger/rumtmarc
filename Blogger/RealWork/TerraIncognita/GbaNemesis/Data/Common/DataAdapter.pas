...
// $Id: DataAdapter.pas,v 1.72 2013/08/14 14:10:31 kostitsin Exp $
...
type
  ...
  private
    ...
    {$If not (defined(Monitorings) or defined(Admin))}
    f_BuildingDefferedTreesList  : TvcmInterfaceList;
    {$IfEnd}
...
(*    function pm_UseUserATPrefix: Boolean;
  ...
  // internal methods
    ...
    {$If not (defined(Monitorings) or defined(Admin))}
    function RememberPassword(aSender: TnsRememberPasswordForm): TModalResult;
      {-}
    function RegisterAction(aSender: TnsRegistrationForm): TModalResult;
      {-}
...
var
 {$IfNDef nsWithoutLogin}
 l_Break: Boolean;
 l_LongProcessMessage: TenLongProcessMessage;
...
//  f_OldOnExceptionEvent := nil;
 ...
 l_LongProcessMessage := TenLongProcessMessage.Create(
...
begin
 ...
 begin
  ...
  vcmShowMessageDlg(Tl3Message_C(nsCStr(anException.Message),
                                 'TDataAdapter.HandleGlobalException',
                                 mtWarning));
 end;//Result
end;
...
{$If not (defined(Monitorings) or defined(Admin))}
function TDataAdapter.RememberPassword(aSender: TnsRememberPasswordForm): TModalResult;
begin
 result := mrNone;
 if not Assigned(aSender) then
  exit;
 try
  f_Authorization.SendUserInfoByEmail(nsAStr(aSender.EMail).S);
...
function TDataAdapter.RegisterAction(aSender: TnsRegistrationForm): TModalResult;
var
 l_Data           : Il3CString;
begin
 Result := mrNone;
 ...
 if not Assigned(aSender) or
    not TestLoginName(aSender.Login) then
  Exit;
 //
 ...
 try
  try
   f_Authorization.Autoregistration(nsAStr(aSender.UserName).S,
                                    nsAStr(aSender.Login).S,
                                    nsAStr(aSender.Password).S,
                                    nsAStr(aSender.EMail).S);
   f_CommonInterfaces := NativeAdapter.MakeCommon;
  except
   on E: EStorageLocked do
   begin
 ...
 {$IfDef nsAutoLoginDisable}
 if (aSender.Login = cMonitoringLogin) and
   (aSender.Password = cMonitoringPassword) then
 begin
  RegistryLogin := aSender.Login;
  RegistryPassword := aSender.Password;
  RegistryIsAutoLogin := True;
 end;  //if (aSender.Login = cMonitoringLogin)...
 {$else}
  RegistryLogin := l3Str(aSender.Login);
  RegistryPassword := l3Str(aSender.Password);
  RegistryIsAutoLogin := aSender.IsAutoLogin;
 {$EndIf}
  Result := mrOk;
...
var
 l_RememberPasswordForm: TnsRememberPasswordForm;
begin
 l_RememberPasswordForm := TnsRememberPasswordForm.Create(nil);
 try
...
begin
 l_RegistrationForm := TnsRegistrationForm.Create(nil, aIsAutoLogin, CanRelogin);
 try
...
begin
 ...
 begin
  ...
  begin
   f_BuildingDefferedTreesList := TvcmInterfaceList.Make;
...
begin
 ...
 if Assigned(f_BuildingDefferedTreesList) then
  while f_BuildingDefferedTreesList.Count > 0 do
   InsDeferredTree(f_BuildingDefferedTreesList.Items[0]).WaitForReady;
 {$IfEnd}  
end;
