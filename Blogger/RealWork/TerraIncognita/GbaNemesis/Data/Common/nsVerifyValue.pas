...
(*-----------------------------------------------------------------------------
 ...
 ¬ерси€:     $Id: nsVerifyValue.pas,v 1.2 2010/09/10 11:29:30 lulin Exp $
 »стори€:
...
begin
 ...
 if Result then
 begin
  if (l3Len(aValue) > 32) or (l3Len(aValue) < 1) then
   Result := False
  else
   Result := l3AllCharsInCharSet(aValue, cValidLoginChars);
  if not Result then
