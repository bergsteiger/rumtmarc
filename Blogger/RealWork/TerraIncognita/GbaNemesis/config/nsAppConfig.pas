...
(*-----------------------------------------------------------------------------
 ...
 $Id: nsAppConfig.pas,v 1.304 2013/09/03 09:35:30 kostitsin Exp $
...
var
 g_ConfigurationProperties: IvcmStrings = nil;
...
var
 ...
 begin
  ...
  Result.Left := ConfigItemLeft;
  Result.Top := lTop;
  lTop := lTop + Result.Height + ConfigItemTop;
...
begin
 if Assigned(f_ConfName) then
  f_ConfName.Width := f_Frame.Width - ConfigItemRight - f_ConfName.Left;
 if Assigned(f_ConfHint) then
  f_ConfHint.Width := f_Frame.Width - ConfigItemRight - f_ConfHint.Left;
end;
 ...
 try
  ...
  try
   ...
   begin
    ...
    if (f_UserCR1SheetType.IntegerValue = f_UserCR2SheetType.IntegerValue) and
       (f_UserCR1Type.StringValue = f_UserCR2Type.StringValue) then
    begin
...
var
 ...
 l_Idx: Integer;
begin
 if not Assigned(g_ConfigurationProperties) then
 begin
  f_MakeSettings := true;
  try
   g_ConfigurationProperties := TvcmStringList.Make;
    ...
    for l_Idx := 0 to Pred(Items.Count) do
     if Assigned(Items.Objects[l_Idx]) then
      if Items.Objects[l_Idx] is TstgNavigatorFormPosition then
       ...
       begin
        g_ConfigurationProperties.Add(FloatingSetting);
        g_ConfigurationProperties.Add(ZoneSetting);
       end
      else
       g_ConfigurationProperties.Add(Items[l_Idx]);
   if l_NeedClear then
    DoneEditing;
  finally
 ...
 Result := g_ConfigurationProperties;
end;
...
begin
 if TvcmCustomizeToolsForm.Execute(g_MenuManager, nil) then
  f_DialogsCalled := True;
end;
 ...
 begin
  ...
  begin
   ...
   AddGroupItem(GetAliasName, vcmConstString(str_pui_NavigatorSheets));
     ...
     AddItem(l_Node, l_Item);
   CloseGroup;
   AddGroupItem(GetAliasName, vcmConstString(str_pui_Filters));
     //
   ...
   CloseGroup;
   // Печать и экспорт
   AddGroupItem(pi_Document_PrintAndExport, str_PrintAndExport.AsStr);
     // Использовать для экспорта и печати размер шрифта, отображаемого на экране
     l_Item := TddRadioConfigItem.CreateItem(pi_Document_PrintAndExport + '/0', str_PrintAndExport_Default.AsStr, False);
     AddItem(l_Node, l_Item);
     LabelTop := False;
     // Использовать для экспорта и печати следующий размер шрифта
     l_Item := TddRadioConfigItem.CreateItem(pi_Document_PrintAndExport + '/1', str_PrintAndExport_Custom.AsStr, True);
     AddItem(l_Node, l_Item);
     LabelTop := False;

     AddItem(l_Node, TstDocumentPrintAndExportFontSizeItem.Create(l_Item));
...
function TnsConfigSettingsInfo.SettingChanged(
