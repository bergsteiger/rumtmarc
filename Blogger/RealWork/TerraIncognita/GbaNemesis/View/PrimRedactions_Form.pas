...
type
 ...
 // private methods
   ...
   procedure RedactionTreeCurrentChanged(Sender: TObject;
     NewCurrent: Integer;
     OldCurrent: Integer);
 ...
 public
 // public properties
   property RedactionTree: TnscTreeView
...
type
  Tkw_PrimRedactions_Control_RedactionTree = class(TtfwControlString)
...
[code] }
  ...
  // overridden protected methods
   ...
   function GetString: AnsiString; override;
    {$IfEnd} //not NoScripts
  end;//Tkw_PrimRedactions_Control_RedactionTree

// start class Tkw_PrimRedactions_Control_RedactionTree

{$If not defined(NoScripts)}
function Tkw_PrimRedactions_Control_RedactionTree.GetString: AnsiString;
 {-}
begin
 Result := 'RedactionTree';
end;//Tkw_PrimRedactions_Control_RedactionTree.GetString
{$IfEnd} //not NoScripts

type
  Tkw_PrimRedactions_Control_RedactionTree_Push = class(TkwBynameControlPush)
...
[code] }
  ...
  // overridden protected methods
   ...
   procedure DoDoIt(const aCtx: TtfwContext); override;
    {$IfEnd} //not NoScripts
  end;//Tkw_PrimRedactions_Control_RedactionTree_Push

// start class Tkw_PrimRedactions_Control_RedactionTree_Push

{$If not defined(NoScripts)}
procedure Tkw_PrimRedactions_Control_RedactionTree_Push.DoDoIt(const aCtx: TtfwContext);
 {-}
begin
 aCtx.rEngine.PushString('RedactionTree');
 inherited;
end;//Tkw_PrimRedactions_Control_RedactionTree_Push.DoDoIt
{$IfEnd} //not NoScripts
...
procedure TPrimRedactionsForm.RedactionTreeCurrentChanged(Sender: TObject;
  NewCurrent: Integer;
  OldCurrent: Integer);
...
//#UC START# *4EC3D9E50143_497A12850078_impl*
 if not IsLocalPositionning and (NewCurrent <> -1) then
 begin
  l_CurNode := RedactionTree.GetNode(NewCurrent);
  try
...
//#UC START# *4ED613000183_497A12850078_impl*
 if Supports(RedactionTree.GetNode(aItemIndex), InevTag, l_Tag) then
  try
   if l_Tag.InheritsFrom(k2_idNodeGroup) then
...
begin
 inherited;
 f_RedactionTree := TnscTreeView.Create(Self);
...
{$IfEnd} //not Admin AND not Monitorings
{$If not defined(Admin) AND not defined(Monitorings)}
// Регистрация Tkw_PrimRedactions_Control_RedactionTree
 Tkw_PrimRedactions_Control_RedactionTree.Register('контрол::RedactionTree', TnscTreeView);
{$IfEnd} //not Admin AND not Monitorings
{$If not defined(Admin) AND not defined(Monitorings)}
// Регистрация Tkw_PrimRedactions_Control_RedactionTree_Push
 Tkw_PrimRedactions_Control_RedactionTree_Push.Register('контрол::RedactionTree:push');
{$IfEnd} //not Admin AND not Monitorings
