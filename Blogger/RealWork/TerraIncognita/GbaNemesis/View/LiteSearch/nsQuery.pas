...
// $Id: nsQuery.pas,v 1.13 2013/08/12 07:10:30 morozov Exp $
...
// - в TQueryFormState.pm_setFilter если фильтр был установлен, то сначала освобождаем интерфейс;
//
...
function TnsQueryContainer.pm_getQuery : IQuery;
begin
 Result := f_Query;
end;

procedure TnsQueryContainer.pm_setQuery(const aValue : IQuery);
begin
 f_Query := aValue;
end;
...
function TnsQueryFormState.pm_GetTag : Il3CString;
begin
 Result := f_Tag;
end;

procedure TnsQueryFormState.pm_SetTag(const aValue : Il3CString);
begin
 f_Tag := aValue;
end;

function TnsQueryFormState.pm_GetFilter : IFilterFromQuery;
begin
 Result := f_Filter;
end;

procedure TnsQueryFormState.pm_SetFilter(const aValue : IFilterFromQuery);
begin
