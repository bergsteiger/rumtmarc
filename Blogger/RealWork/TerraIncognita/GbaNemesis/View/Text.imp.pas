...
//#UC START# *4A854E340357_4A853E260311_impl*
 if _Instance_R_(Self).Text.GetTopCursor(l_TopCursor) then
  try
   Result := TeePara.Make(l_TopCursor.MostInner.Obj.AsPara, _Instance_R_(Self).Text As IeeDocumentEx);
  finally
   l_TopCursor := nil;
  end//try..finally
 else
 ...
 begin
  ...
  begin
   if View.IsCaretVisible then
    l_Para := TeePara.Make(Selection.Cursor.MostInner.Obj^, _Instance_R_(Self).Text)
   else
    l_Para := GetTopParaID;
  end;//View <> nil
 end;//with Text
