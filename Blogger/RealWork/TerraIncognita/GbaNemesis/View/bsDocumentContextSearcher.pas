...
//#UC START# *4952664C03BC_495137C70135_var*
 ...
 function lp_MakeFound(const aDoc   : InevObject;
  ...
  function FindInProvider(const aDoc      : InevTag;
                          aFromParaID     : Integer;
  ...
  begin//FindInProvider
   if Supports(aDoc.Box, IDocumentTextProvider, l_Provider) then
    try
     Result := true;
     try
  ...
  begin//FindViaEverestOwnSearcher
   ...
   try
    ...
    try
     ...
     if not aDoc.QT(InevPara, l_P) then
      Assert(false);
     l_C.Options := cOp;
     l_BM.Options := cOp;
  ...
  var
   ...
   var
    ...
    l_Pt        : InevBasePoint;
   begin//DoDoc
    if aDoc.InheritsFrom(k2_idDocument) then
    begin
     ...
     if aNeedID then
      ...
      l_ThisDocID := 0;
     if FindInProvider(aDoc, l_ThisDocID, l_It) then
     begin
    ...
    end;//aDoc.InheritsFrom(k2_idDocument)
   end;//DoDoc
  ...
  begin//TryFindInProvidersList
   ...
   try
    ...
    Assert(l_Median >= 0);
    l_ID := aPoint.MostInner.IntA[k2_tiHandle];
 ...
 var
  ...
  l_WasDoc     : Boolean;
 begin//lp_MakeFound
  l_FromParaID := aPoint.MostInner.IntA[k2_tiHandle];
  if FindInProvider(aDoc, l_FromParaID, Result) then
  // - нашли в "новом" документе
  begin
   Exit;
  end//FindInProvider(aDoc, l_FromParaID, Result)
  else
  if (aDoc.ChildrenCount = 1) then
  begin
   l_Child0 := aDoc.Child[0];
   if l_Child0.InheritsFrom(k2_idBlock) AND
     not l_Child0.InheritsFrom(k2_idDocument) then
   ...
   end;//l_Child0.InheritsFrom(k2_idBlock)
