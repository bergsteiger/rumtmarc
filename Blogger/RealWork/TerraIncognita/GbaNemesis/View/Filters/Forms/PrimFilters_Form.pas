...
событие для получения шрифта элемента. }
 ...
 public
 // public properties
   property FiltersList: TnscTreeView
     read f_FiltersList;
...
type
  Tkw_PrimFilters_Control_FiltersList = class(TtfwControlString)
...
[code] }
  ...
  // overridden protected methods
   ...
   function GetString: AnsiString; override;
    {$IfEnd} //not NoScripts
  end;//Tkw_PrimFilters_Control_FiltersList

// start class Tkw_PrimFilters_Control_FiltersList

{$If not defined(NoScripts)}
function Tkw_PrimFilters_Control_FiltersList.GetString: AnsiString;
 {-}
begin
 Result := 'FiltersList';
end;//Tkw_PrimFilters_Control_FiltersList.GetString
{$IfEnd} //not NoScripts

type
  Tkw_PrimFilters_Control_FiltersList_Push = class(TkwBynameControlPush)
...
[code] }
  ...
  // overridden protected methods
   ...
   procedure DoDoIt(const aCtx: TtfwContext); override;
    {$IfEnd} //not NoScripts
  end;//Tkw_PrimFilters_Control_FiltersList_Push

// start class Tkw_PrimFilters_Control_FiltersList_Push

{$If not defined(NoScripts)}
procedure Tkw_PrimFilters_Control_FiltersList_Push.DoDoIt(const aCtx: TtfwContext);
 {-}
begin
 aCtx.rEngine.PushString('FiltersList');
 inherited;
end;//Tkw_PrimFilters_Control_FiltersList_Push.DoDoIt
{$IfEnd} //not NoScripts
...
begin
 inherited;
 f_FiltersList := TnscTreeView.Create(Self);
...
{$IfEnd} //not Admin AND not Monitorings
{$If not defined(Admin) AND not defined(Monitorings)}
// Регистрация Tkw_PrimFilters_Control_FiltersList
 Tkw_PrimFilters_Control_FiltersList.Register('контрол::FiltersList', TnscTreeView);
{$IfEnd} //not Admin AND not Monitorings
{$If not defined(Admin) AND not defined(Monitorings)}
// Регистрация Tkw_PrimFilters_Control_FiltersList_Push
 Tkw_PrimFilters_Control_FiltersList_Push.Register('контрол::FiltersList:push');
{$IfEnd} //not Admin AND not Monitorings
{$If not defined(Admin) AND not defined(Monitorings)}
