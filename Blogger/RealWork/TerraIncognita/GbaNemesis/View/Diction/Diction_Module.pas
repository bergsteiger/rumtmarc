...
//#UC START# *4A9CEF360326_4A9CE5F2036F_impl*
 ...
 l_DictTree := TnsDictionTree.Make(aLanguage, l_Str, True);
 ...
 try
  ...
  try
   ...
   try
    OpenDictionary(
     TdeDiction.Make(l_Doc,
                     aLanguage,
                     l_FilterTree.CloneFilters.Context),
     DefaultContainer);
   finally
    l_Doc := nil;
   end;//try..finally
...
//#UC START# *4A9D55E901D6_4A9CE5F2036Fexec_impl*
 ...
 begin
  ...
  end//not l_Form.Entity.Operation(op_Document_FindExplanation).Done
  else
   Exit;
 end;//l_Cont <> nil
