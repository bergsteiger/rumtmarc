...
событие для получения шрифта элемента. }
 ...
 public
 // public properties
   property FoldersTree: TnscTreeView
     read f_FoldersTree;
...
type
  Tkw_PrimFoldersTree_Control_FoldersTree = class(TtfwControlString)
...
[code] }
  ...
  // overridden protected methods
   ...
   function GetString: AnsiString; override;
    {$IfEnd} //not NoScripts
  end;//Tkw_PrimFoldersTree_Control_FoldersTree

// start class Tkw_PrimFoldersTree_Control_FoldersTree

{$If not defined(NoScripts)}
function Tkw_PrimFoldersTree_Control_FoldersTree.GetString: AnsiString;
 {-}
begin
 Result := 'FoldersTree';
end;//Tkw_PrimFoldersTree_Control_FoldersTree.GetString
{$IfEnd} //not NoScripts

type
  Tkw_PrimFoldersTree_Control_FoldersTree_Push = class(TkwBynameControlPush)
...
[code] }
  ...
  // overridden protected methods
   ...
   procedure DoDoIt(const aCtx: TtfwContext); override;
    {$IfEnd} //not NoScripts
  end;//Tkw_PrimFoldersTree_Control_FoldersTree_Push

// start class Tkw_PrimFoldersTree_Control_FoldersTree_Push

{$If not defined(NoScripts)}
procedure Tkw_PrimFoldersTree_Control_FoldersTree_Push.DoDoIt(const aCtx: TtfwContext);
 {-}
begin
 aCtx.rEngine.PushString('FoldersTree');
 inherited;
end;//Tkw_PrimFoldersTree_Control_FoldersTree_Push.DoDoIt
{$IfEnd} //not NoScripts
  ...
  try
   ...
   try
    ...
    case l_FoldersItemType of
     ...
     begin
      l_NewWindow := (l_FoldersItemType <> FIT_FOLDER) and
                     afw.Settings.LoadBoolean(pi_OpenInNewWindow, dv_OpenInNewWindow);
      OpenFolderElement(self.as_IvcmEntityForm, l_eeNode, l_NewWindow);
     end;
    end;
   finally
    l_AdapterNode := nil;
...
begin
 inherited;
 f_FoldersTree := TnscTreeView.Create(Self);
...
initialization
{$If not defined(Admin) AND not defined(Monitorings)}
// Регистрация Tkw_PrimFoldersTree_Control_FoldersTree
 Tkw_PrimFoldersTree_Control_FoldersTree.Register('контрол::FoldersTree', TnscTreeView);
{$IfEnd} //not Admin AND not Monitorings
{$If not defined(Admin) AND not defined(Monitorings)}
// Регистрация Tkw_PrimFoldersTree_Control_FoldersTree_Push
 Tkw_PrimFoldersTree_Control_FoldersTree_Push.Register('контрол::FoldersTree:push');
{$IfEnd} //not Admin AND not Monitorings
{$If not defined(Admin) AND not defined(Monitorings)}
