...
uses
  ...
  kw_Folders_MyInformation,
  kw_Folders_MyConsultations,
  kw_Folders_UnderControlOpenFrmAct,
  kw_Folders_OpenFrmAct,
  StdRes {a},
  vcmModuleDef {a}
  ;
{$IfEnd} //not Admin AND not Monitorings
...
//#UC START# *4AC0AB11030C_4A96B68A03B1_impl*
 vcmDispatcher.FormDispatcher.Lock;
 try
  l_Container := TcfFoldersInfo.Make(vcmCheckAggregate(
                                      vcmMakeParams(nil, nil, Application)));
(*  TenFoldersElementInfo.Make(vcmParams(l_Container.Aggregate,
                                       l_Container.AsContainer));*)
  l_Form := TenFoldersElementInfo.Make(vcmMakeParams(l_Container.Aggregate,
                                                 l_Container.AsContainer));
