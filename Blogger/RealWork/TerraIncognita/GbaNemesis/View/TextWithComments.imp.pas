...
//#UC START# *4A8AACD503DC_4A8AACAE0356_impl*
 ...
 begin
  ...
  if aControl.rControl.InheritsFrom(k2_idBitmapPara) then
  begin
   ...
   begin
    if aControl.rMetrics.IsTagCollapsed(aControl.rFormatInfo.ParentInfo.ParentInfo.Obj) then
     theImageInfo.rFirstIndex := 3
    else
     theImageInfo.rFirstIndex := 2;
   end;//aControl.rFormatInfo <> nil
  end//aControl.rControl.InheritsFrom(k2_idBitmap)
  else
   Case aControl.rControl.IntA[k2_tiStyle] of
    ev_saTxtComment:
...
(*     theImageInfo.rImageList := vtMakeImageListWrapper(nsDocumentRes.DocumentItemsImageList);
     ...
     theImageInfo.rLeftIndent := aControl.rControl.IntA[k2_tiFirstIndent] div 2;
