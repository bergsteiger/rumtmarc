...
type
  Tkw_Rubricator_RubricatorList_ControlInstance = class(TtfwWord)
   {* Слово словаря для доступа к экземпляру контрола RubricatorList формы Rubricator }
  protected
  // realized methods
    {$If not defined(NoScripts)}
...
// Регистрация Tkw_Form_Rubricator
 Tkw_Form_Rubricator.Register('форма::Rubricator', TefRubricator);
{$IfEnd} //not Admin AND not Monitorings
{$If not defined(Admin) AND not defined(Monitorings)}
// Регистрация Tkw_Rubricator_RubricatorList_ControlInstance
