...
{$If not defined(Admin) AND not defined(Monitorings)}
 ...
 // protected methods
   ...
   procedure LftRCToPartQueryMaximized(aSender: TObject); virtual; abstract;
     {* Обработчик события lftRCToPart.OnQueryMaximized }
   procedure LftRCToPartQueryOpen(aSender: TObject); virtual; abstract;
...
var
   { Локализуемые строки lftRCToPartLocalConstants }
  str_lftRCToPartCaption : Tl3StringIDEx = (rS : -1; rLocalized : false; rKey : 'lftRCToPartCaption'; rValue : 'Ссылки из фрагмента/Ссылки на фрагмент');
   { Заголовок пользовательского типа "Ссылки из фрагмента/Ссылки на фрагмент" }
  str_lftRCToPartSettingsCaption : Tl3StringIDEx = (rS : -1; rLocalized : false; rKey : 'lftRCToPartSettingsCaption'; rValue : 'Список: Ссылки из фрагмента/Ссылки на фрагмент');
   { Заголовок пользовательского типа "Ссылки из фрагмента/Ссылки на фрагмент" для настройки панелей инструментов }
...
begin
 ...
 with AddUsertype(lftRCToPartName,
  str_lftRCToPartCaption,
  str_lftRCToPartSettingsCaption,
  false,
  -1,
  60,
  lftRespondentName,
  LftRCToPartQueryMaximized,
  LftRCToPartQueryOpen,
  nil,
  vcm_ccNone) do
 begin
 end;//with AddUsertype(lftRCToPartName
...
// Инициализация str_lftRCToPartCaption
 str_lftRCToPartCaption.Init;
// Инициализация str_lftRCToPartSettingsCaption
 str_lftRCToPartSettingsCaption.Init;
