...
type
   Tkw_WarningBaloon_Viewer_ControlInstance = class(TtfwWord)
    {* Слово словаря для доступа к экземпляру контрола Viewer формы WarningBaloon }
   protected
   // realized methods
    {$If not defined(NoScripts)}
...
// Регистрация Tkw_Form_WarningBaloon
 Tkw_Form_WarningBaloon.Register('форма::WarningBaloon', TWarningBaloonForm);
{$IfEnd} //not Admin AND not Monitorings
{$If not defined(Admin) AND not defined(Monitorings)}
// Регистрация Tkw_WarningBaloon_Viewer_ControlInstance
