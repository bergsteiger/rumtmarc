...
type
   Tkw_UserCR2_WarningBaloon_Viewer_ControlInstance = class(TtfwWord)
    {* Слово словаря для доступа к экземпляру контрола Viewer формы UserCR2_WarningBaloon }
   protected
   // realized methods
    {$If not defined(NoScripts)}
...
// Регистрация Tkw_Form_UserCR2_WarningBaloon
 Tkw_Form_UserCR2_WarningBaloon.Register('форма::UserCR2_WarningBaloon', TUserCR2_WarningBaloonForm);
{$IfEnd} //not Admin AND not Monitorings
{$If not defined(Admin) AND not defined(Monitorings)}
// Регистрация Tkw_UserCR2_WarningBaloon_Viewer_ControlInstance
