...
//#UC START# *4D8CA46002BE_4979E5520222_impl*
 ...
 begin
  ...
  begin
   ...
   begin
    l_Para := Child[l_ind];
    Assert(l_Para.InheritsFrom(k2_idTextPara)); // на случай, если в параграфе когда-то будет не просто текст
...
initialization
{$If not defined(Admin) AND not defined(Monitorings)}
// Регистрация Tkw_PrimWarning_Control_Viewer
 Tkw_PrimWarning_Control_Viewer.Register('контрол::Viewer', TnscEditor);
{$IfEnd} //not Admin AND not Monitorings
{$If not defined(Admin) AND not defined(Monitorings)}
// Регистрация Tkw_PrimWarning_Control_Viewer_Push
 Tkw_PrimWarning_Control_Viewer_Push.Register('контрол::Viewer:push');
{$IfEnd} //not Admin AND not Monitorings
