...
var
  ...
  str_utProgressIndicatorSettingsCaption : Tl3StringIDEx = (rS : -1; rLocalized : false; rKey : 'utProgressIndicatorSettingsCaption'; rValue : 'Ход выполнения');
   { Заголовок пользовательского типа "" для настройки панелей инструментов }
...
initialization
{$If not defined(Admin) AND not defined(Monitorings)}
// Регистрация Tkw_PrimProgressIndicator_Control_ProgressBar
 Tkw_PrimProgressIndicator_Control_ProgressBar.Register('контрол::ProgressBar', TProgressBar);
{$IfEnd} //not Admin AND not Monitorings
{$If not defined(Admin) AND not defined(Monitorings)}
// Регистрация Tkw_PrimProgressIndicator_Control_ProgressBar_Push
 Tkw_PrimProgressIndicator_Control_ProgressBar_Push.Register('контрол::ProgressBar:push');
{$IfEnd} //not Admin AND not Monitorings
{$If not defined(Admin) AND not defined(Monitorings)}
