...
type
  Tkw_MedicMainMenu_pnlMain_ControlInstance = class(TtfwWord)
   {* Слово словаря для доступа к экземпляру контрола pnlMain формы MedicMainMenu }
  protected
  // realized methods
    {$If not defined(NoScripts)}
...
type
  Tkw_MedicMainMenu_hfLastOpenDocs_ControlInstance = class(TtfwWord)
   {* Слово словаря для доступа к экземпляру контрола hfLastOpenDocs формы MedicMainMenu }
  protected
  // realized methods
    {$If not defined(NoScripts)}
   procedure DoDoIt(const aCtx: TtfwContext); override;
...
type
  Tkw_MedicMainMenu_tvLastOpenDocs_ControlInstance = class(TtfwWord)
   {* Слово словаря для доступа к экземпляру контрола tvLastOpenDocs формы MedicMainMenu }
  protected
  // realized methods
    {$If not defined(NoScripts)}
   procedure DoDoIt(const aCtx: TtfwContext); override;
...
type
  Tkw_MedicListSynchroView_ztChild_ControlInstance = class(TtfwWord)
   {* Слово словаря для доступа к экземпляру контрола ztChild формы MedicListSynchroView }
  protected
  // realized methods
    {$If not defined(NoScripts)}
   procedure DoDoIt(const aCtx: TtfwContext); override;
...
// Регистрация Tkw_Form_MedicMainMenu
 Tkw_Form_MedicMainMenu.Register('форма::MedicMainMenu', Ten_MedicMainMenu);
{$IfEnd} //not Admin AND not Monitorings
{$If not defined(Admin) AND not defined(Monitorings)}
// Регистрация Tkw_MedicMainMenu_pnlMain_ControlInstance
 TtfwScriptEngine.GlobalAddWord('.Ten_MedicMainMenu.pnlMain', Tkw_MedicMainMenu_pnlMain_ControlInstance);
{$IfEnd} //not Admin AND not Monitorings
{$If not defined(Admin) AND not defined(Monitorings)}
// Регистрация Tkw_MedicMainMenu_hfLastOpenDocs_ControlInstance
 TtfwScriptEngine.GlobalAddWord('.Ten_MedicMainMenu.hfLastOpenDocs', Tkw_MedicMainMenu_hfLastOpenDocs_ControlInstance);
{$IfEnd} //not Admin AND not Monitorings
{$If not defined(Admin) AND not defined(Monitorings)}
// Регистрация Tkw_MedicMainMenu_tvLastOpenDocs_ControlInstance
...
// Регистрация Tkw_Form_MedicListSynchroView
 Tkw_Form_MedicListSynchroView.Register('форма::MedicListSynchroView', Ten_MedicListSynchroView);
{$IfEnd} //not Admin AND not Monitorings
{$If not defined(Admin) AND not defined(Monitorings)}
// Регистрация Tkw_MedicListSynchroView_ztChild_ControlInstance
