...
// $Id: nsTagNodeTools.pas,v 1.14 2012/02/08 12:04:07 lulin Exp $
...
begin
 ...
 try
  ...
  begin
   ...
   if not nsTagByPath(aDocument, l_Pos.rNode, true).QT(InevPara, l_Para, aProc) then
    Assert(false);
  ...
  if not l_Selected.Empty then
   theFoundBlock := TevMultiSelectionBlock.Make(aDocument, l_Selected)
  else
   Result := false;  
 finally
  vcmFree(l_Selected);
...
var
 ...
 l_Para : InevTag;
begin
 if (aSub.LayerID <> ev_sbtSub) then
  Result := nil
 else
 begin
...
(*  if Supports(l_Para, IDocTagNode, l_DTN) then
  ...
  if Supports(l_Para, IDocumentTextProvider, l_DTP) then
   try
