...
//#UC START# *4C727B6A0242_4C7286FE0291_impl*
 ...
 try
  l_T := k2.TypeTable[Ord(aProvider.GetChildType(l_EP))];
 except
 ...
 l_P := nil;
 try
  if l_T.InheritsFrom(k2_idLeafPara) then
...
(*  if l_T.InheritsFrom(k2_idCommentPara) then
  ...
  if l_T.InheritsFrom(k2_idParaList) then
  ...
  end;//l_T.InheritsFrom(k2_idParaList)
  Result := l_P;
 finally
  FreeAndNil(l_P);
 end;//try..finally
