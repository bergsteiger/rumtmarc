...
// $Id: nsContentsNode.pas,v 1.4 2013/07/18 12:25:02 kostitsin Exp $
...
type
  TnsContentsNode = class(TnsINodeWrap, Il3HandleNode)
    ...
    // public methods
      ...
      class function Make(const aNode     : INodeBase;
                          const aDocument : Ik2Tag): Il3SimpleNode;
        reintroduce;
        {-}
  end;//TnsContentsNode
  ...
  TnsContentsTagNode = class(TnsContentsNode)
    ...
    // public methods
      ...
      class function Make(const aNode     : INodeBase;
                          const aDocument : Ik2Tag;
                          const aTag      : Ik2Tag): Il3SimpleNode;
        reintroduce;
        {-}
...
function TnsBookmarkNode.LayerID: Integer;
  ...
  {-}
begin
 Result := ev_sbtBookmark;
end;

// start class TnsSubImplementation

constructor TnsSubImplementation.Create(const aDocument  : Ik2Tag;
                                        anID             : Integer;
                                        aLayerID         : Integer;
                                        const aNode      : Il3SimpleNode);
  //reintroduce;
...
var
 l_Proc : Ik2Processor;
begin
 if not Supports(aDocument.Box.Owner, Ik2Processor, l_Proc) then
 begin
  if aDocument.Box.Owner.IsValid then
   Assert(false);
 end;//not Supports(aDocument.Box.Owner, Ik2Processor, l_Proc)
 inherited Create(aDocument, l_Proc, anID, aLayerID);
 f_Node := aNode;
end;

class function TnsSubImplementation.Make(const aDocument  : Ik2Tag;
                                         anID             : Integer;
                                         aLayerID         : Integer;
                                         const aNode      : Il3SimpleNode): IevSub;
  //reintroduce;
...
function TnsSubImplementation.Tag: Ik2Tag;
  {-}
begin
 Supports(f_Node, Ik2Tag, Result);
end;

function TnsSubImplementation.SubPara: Ik2Tag;
  {-}
...
begin
 Result := nil;
 if Tag.InheritsFrom(k2_idBlock) then
  DoFind(AdapterNode)
 else
 if Tag.InheritsFrom(k2_idCommentPara) then
  Result := nsFindTagByID(Document, k2_idCommentPara, ID)
 else
 if Tag.InheritsFrom(k2_idSub) then
  Result := nsFindTagByID(Document, k2_idSub, ID);
end;

// start class TnsDocumentPartImplementation

constructor TnsDocumentPartImplementation.Create(const aDocument  : Ik2Tag;
                                                 anID             : Integer;
                                                 aLayerID         : Integer;
                                                 const aNode      : Il3SimpleNode);
  //reintroduce;
...
class function TnsDocumentPartImplementation.Make(const aDocument  : Ik2Tag;
                                                  anID             : Integer;
                                                  aLayerID         : Integer;
                                                  const aNode      : Il3SimpleNode): IevDocumentPart;
  //reintroduce;
...
function TnsSubPrim.SubPara: Ik2Tag;
  //virtual;
  {-}
begin
 Result := nil;
...
begin
 Result := True;
 if (Self.pm_GetLayerID = ev_sbtSub) then
  if Supports(Self.pm_GetPara.Box, IDocumentTextProvider, l_Provider) then
   Result := l_Provider.ShowSubPanelIcon(ID);
end;

function TnsSubPrim.Tag: Ik2Tag;
  //virtual;
  {-}
begin
 Result := nil;
