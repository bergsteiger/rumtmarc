...
{$If not defined(Admin) AND not defined(Monitorings)}
 ...
 _dsEditionDiff_ = {abstract vac} class(_ucpHAFMacroReplacerFactory_, IdsEditionDiff, InsEditionListner {from IdsEditionDiff}, IucpFilterInfoFactory {from IdsEditionDiff})
  {* Разница редакций }
 private
 // private fields
 ...
 // Методы преобразования к реализуемым интерфейсам
   function As_InsEditionListner: InsEditionListner;
 ...
 if (NotifiedObjList <> nil) then
  for l_Index := NotifiedObjList.Hi downto 0 do
   try
    ...
    if Supports(l_Item, InsEditionListner, l_Intf) AND
       (l_Item = l_Intf) then
     try
...
//#UC START# *479731C50290_4A7188E40294_impl*
 UseCaseController.As_Il3ChangeNotifier.Unsubscribe(InsEditionListner(Self));
 inherited;
 f_Cont := nil;
...
//#UC START# *47A042E100E2_4A7188E40294_impl*
 inherited;
 UseCaseController.As_Il3ChangeNotifier.Subscribe(InsEditionListner(Self));
//#UC END# *47A042E100E2_4A7188E40294_impl*
end;//_dsEditionDiff_.InitFields

// Методы преобразования к реализуемым интерфейсам

function _dsEditionDiff_.As_InsEditionListner: InsEditionListner;
begin
 Result := Self;
end;
