...
{$If not defined(Admin) AND not defined(Monitorings)}
 _VScroll_ = {abstract mixin} class(_VScroll_Parent_)
  {* Реализация чистой примеси VScroll транслирующая вызовы к своим подписчикам в виде InsVScrollListner }
 public
 // realized methods
 ...
 if (NotifiedObjList <> nil) then
  for l_Index := NotifiedObjList.Hi downto 0 do
   try
    ...
    if Supports(l_Item, InsVScrollListner, l_Intf) AND
       (l_Item = l_Intf) then
     try
      l_Intf.Sync(aPara);
     finally
