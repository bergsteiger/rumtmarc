...
// $Id: nsFolderNodes.pas,v 1.20 2013/07/29 14:10:37 morozov Exp $
...
type
  ...
  // fields
    ...
    f_RootListener     : Pointer; // InsDefferedTreeListner
  private
  // InsFolderFilterInfoListener
  ...
  // methods
    ...
    constructor Create(const aNotifyObject: InsFolderNodeNotifyObject;
                       const aListener: InsDefferedTreeListner);
      reintroduce;
      {-}
    class function Make(const aNotifyObject: InsFolderNodeNotifyObject;
                        const aListener: InsDefferedTreeListner): Il3Tree;
      reintroduce;
      {-}
  end;//TnsFoldersTree
...
function TnsFoldersNode.CanAcceptData(const aData: Tl3TreeData): Boolean;
begin
...
begin
 Result := inherited CanAcceptData(aData) or
...
function TnsFoldersNode.CanMove: Boolean;
  // override;
  {* определяет возможность перемещения узла }
begin
 Result := inherited CanMove or AdapterNode.CanMove;
end;
...
function TnsFoldersNode.DropData(const aData: Tl3TreeData): Boolean;
  // override;
  {* - принять данные. }
...
var
 ...
 l_Self: INode;
begin
 Result := inherited DropData(aData);
...
procedure TnsFoldersNode.Delete;
var
 l_NotifyObject: InsFolderNodeNotifyObject;
...
begin
 ...
 if Assigned(f_RootListener) then
  InsDefferedTreeListner(f_RootListener).TreeLoaded(kdtFolders);
