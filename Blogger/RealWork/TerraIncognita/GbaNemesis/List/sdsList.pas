...
//#UC START# *493E7C24005A_493E78640269_impl*
 with SetData.UserCRListInfo[aId] do
  if KindOfList = bs_ulFullCR then
   case ListType of
    crtCorrespondents:
     pm_GetDSCorrespondentsRef.NeedMake := StateIfNeedShowFullCRList;
    crtRespondents:
     pm_GetDsRespondentsRef.NeedMake := StateIfNeedShowFullCRList;
   end;//case SetData.UserCRListInfo[ulFirst] of
...
//#UC START# *493E7EAC0171_493E78640269_impl*
 ...
 begin
  ...
  Refresh;
 end;//if SetData.refFilters.IsEmpty then
...
//#UC START# *D35F2F1E4DE1_493E78640269_impl*
 if (aSynchroForm <> SetData.List_SynchroView_Form) then
 begin
  ClearAllDS;
  SetData.UseDataProducer := False;
  SetData.List_SynchroView_Form := aSynchroForm;
  // При открытии документа сбрасываем типы корр/респ. CQ: 00018879
  if (SetData.List_SynchroView_Form = sva_List_SynchroView_Document) then
  begin
   ChangeCorrType(nil);
   ChangeRespType(nil);
...
//#UC START# *493957960321_493E78640269_impl*
 // Для случая когда пользователь нажал кноку "Показать документ", в этот момент
 // умничать и показывать аннотацию не нужно:
 if not SetData.UseDataProducer then
  Result := True
 else
 begin
  SetData.UseDataProducer := False;
  // См. в конфлюенс "Оптимизация через получение данных".
  // http://mdp.garant.ru/pages/viewpage.action?pageId=6979611&focusedCommentId=6979639#comment-6979639
  // http://mdp.garant.ru/pages/viewpage.action?pageId=7438511
  case SetData.List_SynchroView_Form of
   sva_List_SynchroView_Annotation:
   begin
    Result := TruthHasAnnotation;
    // Аннотации у документа нет, открываем документ:
    if not Result then
     ChangeSynchroForm(sva_List_SynchroView_Document);
   end;//bs_lsfAnnotation
   sva_List_SynchroView_Document:
   begin
    Result := not pm_GetHasAnnotation;
    // У документа есть аннотация, открываем аннотацию:
    if not Result then
 ...
 end;//if not SetData.UseDataProducer then
...
//#UC START# *4B1EA9069D74_493E78640269get_impl*
 Result := SetData.List_SynchroView_Form = sva_List_SynchroView_SimilarDocuments;
...
//#UC START# *500CFAE50070_493E78640269get_var*
 ...
 function lpNeedMake: Boolean;
 begin
  Result := 
   (afw.Settings.LoadBoolean(pi_List_Sheets_Filters, dv_List_Sheets_Filters) and
   (SetData.dsFiltersRef.NeedMake = vcm_nmYes)) or
   (SetData.dsFiltersRef.NeedMake = vcm_nmForce);
 end;//lpNeedMake
...
//#UC START# *6499B6E1DD77_493E78640269_impl*
 SetData.dsSimilarDocumentsRef.SetIfNeedMakeNo(vcm_nmForce);
...
//#UC START# *8BB87AADAF20_493E78640269get_impl*
 Result := SetData.List_SynchroView_Form = sva_List_SynchroView_UserCR2;
...
//#UC START# *9CCDF86F498F_493E78640269get_impl*
 Result := SetData.List_SynchroView_Form = sva_List_SynchroView_UserCR1;
...
//#UC START# *A6475CA40B39_493E78640269get_impl*
 Result := SetData.List_SynchroView_Form = sva_List_SynchroView_Document;
...
//#UC START# *AFD62513C0A3_493E78640269_impl*
 pm_GetDsRelatedDocRef.SetIfNeedMakeNo(vcm_nmForce);
...
//#UC START# *C79AA9D9E5A1_493E78640269get_impl*
 Result := SetData.List_SynchroView_Form = sva_List_SynchroView_RelatedDoc;
...
//#UC START# *D22CC2DAE849_493E78640269get_impl*
 Result := SetData.List_SynchroView_Form = sva_List_SynchroView_Attributes;
...
//#UC START# *E74EC4E20DB7_493E78640269_impl*
 SetData.dsAnnotationRef.SetIfNeedMakeNo(vcm_nmForce);
 ChangeSynchroForm(sva_List_SynchroView_Annotation);
...
//#UC START# *FB06C8ED7916_493E78640269get_impl*
 Result := SetData.List_SynchroView_Form = sva_List_SynchroView_Annotation;
...
//#UC START# *47FDDACC0101_493E78640269_impl*
 pm_GetdsAttributesRef.SetIfNeedMakeNo(vcm_nmForce);
...
//#UC START# *47FE03AE0225_493E78640269_impl*
 Result := (SetData.List_SynchroView_Form = sva_List_SynchroView_Attributes) AND
...
//#UC START# *4925B9370022_493E78640269_var*
 ...
 begin
  ...
  if f_disableAutoOpenAnnotation or
  ...
  try
   Result := (l_RefDS <> nil) and (l_RefDS.NeedMake <> vcm_nmForce);
  finally
   l_RefDS := nil;
  end;//try..finally
...
//#UC START# *4925B9370022_493E78640269_impl*
 ...
 begin
  ...
  begin
   if SetData.List_SynchroView_Form = sva_List_SynchroView_Annotation then
   begin
     // Даже если у формы нет Аннотации всё равно создадим бизнес объект,
     // определение формы которая должна быть показана в зоне синхронного
     // просмотра произойдет в методе IbsDataProducer_MakeData:
     if not TruthHasAnnotation then
      SetData.dsAnnotationRef.Referred := TdsAnnotation.Make(Self, nil);
   end;//case SetData.SynchroForm of
   if SetData.List_SynchroView_Form in
      [sva_List_SynchroView_Annotation, sva_List_SynchroView_Document] then
    SetData.UseDataProducer := True;
  end;//if lp_NeedAutoOpenAnnotation then
  Refresh;
 end;//Result
...
//#UC START# *493D1BE601B1_493E78640269_impl*
 Result := (SetData.List_SynchroView_Form = sva_List_SynchroView_Document) and
...
//#UC START# *493D51B00058_493E78640269_impl*
 Result := (SetData.List_SynchroView_Form = sva_List_SynchroView_UserCR1) and
...
//#UC START# *493D51BE03DC_493E78640269_impl*
 Result := (SetData.List_SynchroView_Form = sva_List_SynchroView_UserCR2) and
...
//#UC START# *493D6E7E0282_493E78640269_impl*
 Result := (SetData.List_SynchroView_Form = sva_List_SynchroView_RelatedDoc) AND
...
//#UC START# *493D6EAB0019_493E78640269_impl*
 Result := (SetData.List_SynchroView_Form = sva_List_SynchroView_Annotation) AND
...
//#UC START# *493D6EC40228_493E78640269_impl*
 Result := (SetData.List_SynchroView_Form = sva_List_SynchroView_SimilarDocuments) AND
