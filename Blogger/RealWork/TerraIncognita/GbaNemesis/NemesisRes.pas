...
f_RequestingForm.Entity.Operation(op_Loadable_Load, l_Params).Done -> Коллеги, кто может описать этот метод? }
   ...
   class function mod_opcode_WorkJournal_OpenJournal : TvcmMOpID;
     {  ->  }
   class function opcode_System_CommonDataChanged : TvcmOpID;
     { Система ->  }
   ...
   class function mod_opcode_Search_InpharmSearch : TvcmMOpID;
     { Поиск -> Поиск лекарственного средства }
...
begin
 ...
 g_module_opcode_Search_InpharmSearch := 
  PublishModuleOperation(Tmo_Search, 'InpharmSearch', 'Поиск лекарственного средства');
...
begin
 TBaseEditionsModule.MakeCompareEditions(aLeft, aRight, aParaForPositionning, aDocumentForReturn, aParaForReturn);
end;
...
class procedure TNemesisRes.MakeCompareEditions(const aLeft: IDocument;
  ...
  const aParaForReturn: IeeLeafPara);
begin
 TBaseEditionsModule.MakeCompareEditions(aLeft, aRight, aPosition, aDocumentForReturn, aParaForReturn);
end;
...
class function TNemesisRes.DocumentPrintAndExportSetting: Boolean;
begin
 Result := TDocumentModule.DocumentPrintAndExportSetting;
end;

class procedure TNemesisRes.WriteDocumentPrintAndExportSetting(aValue: Boolean);
begin
 TDocumentModule.WriteDocumentPrintAndExportSetting(aValue);
end;

class function TNemesisRes.DocumentPrintAndExportFontSizeSetting: Integer;
begin
 Result := TDocumentModule.DocumentPrintAndExportFontSizeSetting;
end;

class procedure TNemesisRes.WriteDocumentPrintAndExportFontSizeSetting(aValue: Integer);
begin
...
{$If defined(Monitorings) AND not defined(Admin)}
class function TNemesisRes.StartOpen(const aContainer: IvcmContainer): IvcmEntityForm;
begin
 Result := TCommonPostModule.StartOpen(aContainer);
end;
{$IfEnd} //Monitorings AND not Admin
...
class function TNemesisRes.ShowDayTipsAtStartup: Boolean;
begin
...
class procedure TNemesisRes.InpharmSearch(const aQuery: IQuery;
  const aList: IDynList);
begin
 TSearchModule.InpharmSearch(aQuery, aList);
end;
...
begin
 TSearchModule.OpenKeyWord(aQuery);
end;
...
class procedure TNemesisRes.AttributeSearch(const aQuery: IQuery;
  const aList: IDynList);
begin
 TSearchModule.AttributeSearch(aQuery, aList);
end;
...
begin
 TSearchModule.OpenOldSituationCard(aQuery);
end;
...
class function TNemesisRes.opcode_System_CommonDataChanged : TvcmOpID;
begin
 Result := g_opcode_System_CommonDataChanged;
 Assert((Result.rEnID > 0) AND (Result.rOpID > 0));
end;
