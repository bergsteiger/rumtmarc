...
(*  function FindAdapterNodeByType(const aType: TbsPrefixItemType): Il3SimpleNode;*)
 ...
 public
  constructor Create(const aID: TnsValueMapID{; aUIMapID: TnsValueMapID;
   NeedUseSpatialValue: Boolean});
   reintroduce;
  class function Make(const aID: TnsValueMapID{; aUIMapID: TnsValueMapID;
   NeedUseSpatialValue: Boolean}): InsStringValueMap;
...
begin
 ...
 //f_UseSpatialValue := NeedUseSpatialValue;
end;
...
begin
 Result := aDisplayName;
(* if f_UseSpatialValue and DefDataAdapter.UseUserATPrefix and l3Same(DefDataAdapter.UserATPrefixName, aDisplayName) then
  l_Type := c_SpatialValue
 else
 begin
...
(* for l_Idx := Low(TbsPrefixItemType) to High(TbsPrefixItemType) do
  if (l_Idx <> PIT_UNDEFINED) and ((l_IDX <> c_SpatialValue) or (f_UseSpatialValue and DefDataAdapter.UseUserATPrefix)) then
  begin
   if (l_Idx = c_SpatialValue) then
...
begin
 l_Map := Create(aID{, aUIMapID, NeedUseSpatialValue});
 try
  Result := l_Map;
 finally
  vcmFree(l_Map);
...
(* Result := f_UserMap.MapSize - 1;
 if not (f_UseSpatialValue and DefDataAdapter.UseUserATPrefix) then
  Dec(Result);*)
end;
...
(* l_Type := TbsPrefixItemType(f_SettingsMap.DisplayNameToValue(aValue));
 if f_UseSpatialValue and DefDataAdapter.UseUserATPrefix and (l_Type = c_SpatialValue) then
