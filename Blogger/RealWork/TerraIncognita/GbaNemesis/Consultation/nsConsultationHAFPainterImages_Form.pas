...
// Версия     : $Id: nsConsultationHAFPainterImages_Form.pas,v 1.1 2010/11/29 15:21:50 lulin Exp $
////////////////////////////////////////////////////////////////////////////////
...
procedure TnsConsultationHAFPainter.TuneHeader(const aCanvas : Il3Canvas;
 ...
 begin
  ...
  try
   l_Mem := Tl3MemoryStream.Make;
//   l_Mem := Tl3FileStream.Create('O:\haf\1.png', l3_fmRead);
   try
    THackImg(aPic).WriteData(l_Mem);
    l_Pic.IntA[k2_tiDPI] := 150;
    // - заплатка
    l_Pic.Attr[k2_tiData] := k2.TypeTable.ObjToTag(l_Mem);
    //l_Pic.Attr[k2_tiData] := k2Tag(l_Mem);
   finally
    l3Free(l_Mem);
   end;//try..finally
