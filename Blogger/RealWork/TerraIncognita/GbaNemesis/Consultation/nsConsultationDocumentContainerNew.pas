...
type _Instance_R_ = TnsConsultationDocumentContainerNew;

{$Include w:\garant6x\implementation\Garant\GbaNemesis\F1DocumentProcessing\nsDocumentContainer.imp.pas}

// start class TnsConsultationDocumentContainerNew

procedure TnsConsultationDocumentContainerNew.FillTemplateFields(const aTag: InevTag;
  const anInfo: IConsultingTemplateInfo);
var
   ...
   if (aText <> nil) then
    ...
    begin
     ...
     begin
      ...
      anItem.Owner.InsertChild(anIndex + l_Index, l_T);
     end;//for l_Index
    end;//aText.Count > 0
      ...
      try
       ...
       try
        ...
        begin
         ...
         end;//l_Tag.InheritsFrom(k2_idDocument)
         l_Ow := anItem. Owner. Owner. Owner;
         //      Контрол Ячейка Строка Таблица
         l_Ow.Owner.InsertChild(l_Ow.Owner.IndexOfChild(l_Ow), l_Tag);
         l_Ow.Owner.DeleteChild(l_Ow);
 ...
 //#UC START# *4CF3B26A011E__impl*
  ...
  if anItem.InheritsFrom(k2_idParaList) then
   FillTemplateFields(anItem, anInfo)
  else
  if anItem.InheritsFrom(k2_idLeafPara) then
  begin
   if anItem.InheritsFrom(k2_idControlPara) then
   begin
    ...
    end;//anItem.InheritsFrom(k2_idControlPara)
  end;//anItem.InheritsFrom(k2_idLeafPara)
...
begin
 ...
 if l_WasSomeHyperlinkHidden then
  if aTag.InheritsFrom(k2_idReqCell) then
  begin
   if not aTag.QT(InevObject, l_Obj) then
    Assert(false);
   l_Obj.Edit.Delete(false); 
  end;//aTag.InheritsForm(k2_idReqCell)
 //#UC END# *4CF39F58031Eafteriter*
end;//TnsConsultationDocumentContainerNew.FillTemplateFields

function TnsConsultationDocumentContainerNew.HideHyperlinks(const aPara: InevTag): Boolean;
...
//#UC START# *4CF3C730039D_4CF37A0400FB_impl*
 Result := false;
 with aPara.rAtomEx([k2_tiSegments, k2_tiChildren, k2_tiHandle, ev_slHyperlinks]) do
  if IsValid then
...
function TnsConsultationDocumentContainerNew.DoGetDocument: InevTag;
...
function TnsConsultationDocumentContainerNew.GetDocument: InevTag;
