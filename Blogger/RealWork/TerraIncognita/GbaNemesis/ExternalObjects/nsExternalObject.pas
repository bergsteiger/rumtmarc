...
{ $Id: nsExternalObject.pas,v 1.10 2013/08/02 14:12:38 morozov Exp $ }

// $Log: nsExternalObject.pas,v $
...
type
  TnsExternalObject = class(Tl3Base)
  private
  // internal fields
...
function nsDoShellExecute(const aFile: Il3CString; SetReadOnlyToFile: Boolean = False;
 const aCont: IvcmContainer = nil; IgnoreGarantURL: Boolean = False): Boolean;
 {-}
...
procedure nsSaveMultiplyDocuments(const aList: TvcmInterfaceList;
...
function TnsExternalObject.GetRandomSuffix: String;
begin
 Result := IntToHex(Random($ffff), 4);
end;

function TnsExternalObject.TempFilesList: IvcmStrings;
begin
...
procedure TnsExternalObject.CreateUniqueTempCatalog;
const
...
procedure TnsExternalObject.RemoveUniqueTempCatalog;
const
...
function TnsExternalObject.PrepareFileName(const aName     : Il3CString;
...
function TnsExternalObject.MakeTemporaryFileName(const aName             : Il3CString;
                                                 const aExtension        : String;
...
constructor TnsExternalObject.Create;
begin
 inherited Create;
 //
...
procedure TnsExternalObject.Cleanup;
begin
...
function TnsExternalObject.Execute(const aExternalObject: IExternalObject): Boolean;
var
...
var
 g_ExternalObject: TnsExternalObject = nil;

function ExternalObject: TnsExternalObject;
begin
 if (g_ExternalObject = nil) then
  g_ExternalObject := TnsExternalObject.Create;
 Result := g_ExternalObject;
end;
...
begin
 ...
 try
  ...
  Result := AnsiContainsText(l_Browser, 'iexplore.exe') or
            AnsiContainsText(l_Browser, 'chrome.exe') or AnsiContainsText(l_Browser, 'Google Chrome') or
            AnsiContainsText(l_Browser, 'firefox.exe') or
            AnsiContainsText(l_Browser, 'safari.exe');
 finally
  l_Reg.Free;
 end;
end;
 ...
 begin
  ...
  begin
   ...
   try
    ...
    Result := l3Starts(l3Cat(c_Http, l_RealURL), nsCStr(l_ArchiveURL));
   finally
    l_ArchiveURL := nil;
    l_RealURL := nil;
   end;
...
begin
 ...
 if (not IgnoreGarantURL) then
  IgnoreGarantURL := lp_IsJudgeArchiveURL(aFile);
 if not (SetReadOnlyToFile or IgnoreGarantURL) AND defDataAdapter.IsInternetAgentEnabled AND nsIsGarantURL(l3WideString(aFile)) then
 begin
  TdmStdRes.MakeInternetAgent(aFile, aCont);
  Result := true;
  Exit;
 end;//not SetReadOnlyToFile AND nsIsGarantURL(l3WideString(aFile))
 if IgnoreGarantURL and nsIsGarantURL(l3WideString(aFile)) and not IsValidDefaultBrowser then
 begin
...
begin
 Result := False;
 if Assigned(aExternalLink) then
  Result := nsDoShellExecute(nsExtractURL(aExternalLink), False, aCont)
 else
  Result := False;
end;
 ...
 try
  ...
  try
   ...
   try
    ...
    l_Generator.Start;
    try
     l_Generator.StartChild(l_Obj.TagType.ID);
     try
      l_Obj.Write(l_Generator);
     finally
      l_Generator.Finish;
     end;//try..finally
    finally
  ...
  case aFormat of
   ...
   begin
    ...
    try
     l_G.Start;
     try
      l_G.StartChild(l_Obj.TagType.ID);
      try
       l_Obj.Write(l_G);
      finally
       l_G.Finish;
  ...
  try
   ...
   try
    ...
    try
     ...
     l_G.Start;
     try
      l_G.StartChild(k2_idDocument);
      try
       for l_IDX := 0 to aList.Count - 1 do
        if Supports(aList.Items[l_IDX], IbsDocumentStreamWrapper, l_Extracter) then
        try
         ...
         try
          ...
          begin
           l_G.StartChild(k2_idTextPara);
           try
            l_G.AddPCharLenAtom(k2_tiText, l3PCharLen(l_Extracter.InternalNumber));
           finally
            l_G.Finish;
           end;//try..finally
          end;//NeedPrintTopic
...
function TnsExternalObject.MessageHook(var Msg: TMessage): Boolean;
begin
 Result := False;
