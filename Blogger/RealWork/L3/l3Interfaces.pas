...
type
 ...
 Tl3_Point = {$IfDef XE4}packed record{$Else}packed object{$EndIf}
 ...
 // public methods
   ...
   function EQ(const Pt: Tl3_Point): Bool;
 ...
 Tl3_SPoint = {$IfDef XE4}packed record{$Else}packed object{$EndIf}
 ...
 Tl3_PointBArray = array [Boolean] of Tl3_Point;
 ...
 Bottom: Tl3Inch;);
   1: (TopLeft: Tl3_Point;
 BottomRight: Tl3_Point;);
   2: (bRt: Tl3_PointBArray);
 end;//Tl3_RectUnion

 Tl3_Rect = {$IfDef XE4}packed record{$Else}packed object{$EndIf}
  {* Прямоугольник. }
 public
   R : Tl3_RectUnion;
 public
...
type
 ...
 Il3MemoryManager = interface(Il3Base)
  {* Менеджер блоков памяти. }
   ['{FBFD53BD-5898-4194-B989-161495DA44BC}']
   procedure GetMem(var P;
    aSize: Cardinal);
     {* выделяет блок памяти. }
   procedure FreeMem(var P);
     {* освобождает блок памяти. }
 end;//Il3MemoryManager
...
procedure Tl3_Point.Init(aX, aY: Long);
  {-инициализирует точку}
begin
 P.X := aX;
 P.Y := aY;
...
function Tl3_Point.IsNull: Bool;
  {-}
begin
...
function Tl3_Point.EQ(const Pt: Tl3_Point): Bool;
  {-проверяет равенство двух точек}
begin
...
procedure Tl3_Rect.Inflate(const Pt: Tl3Point);
  {overload;}
  {-увеличивает размеры прямоугольника на P}
begin
...
procedure Tl3_Rect.Inflate(X, Y: Long);
  {overload;}
  {-увеличивает размеры прямоугольника на (X, Y)}
begin
 Dec(R.Left, X);
...
procedure Tl3_Rect.OffsetPt(const Pt: Tl3Point);
  {-сдвинуть прямоугольник на Pt}
begin
...
procedure Tl3_Rect.Offset(aX, aY: Long);
  {-сдвинуть прямоугольник на (X, Y)}
begin
 Inc(R.Top, aY);
 Inc(R.Bottom, aY);
...
function Tl3_Rect.ContainsPt(const Pt: Tl3_Point): Bool;
  {-содержит ли прямоугольник точку Pt}
begin
 with Pt do
...
function Tl3_Rect.EQ(const Rt: Tl3_Rect): Bool;
  {-проверяет равенство двух прямоугольников}
begin
 Result := Tl3Point(R.TopLeft).EQ(Rt.R.TopLeft) AND Tl3Point(R.BottomRight).EQ(Rt.R.BottomRight);
end;

function Tl3_Rect.AddPt(const Pt: Tl3Point): Tl3_Rect;
  {-сдвинуть прямоугольник на Pt и вернуть Result}
begin
...
function Tl3_Rect.SubPt(const Pt: Tl3Point): Tl3_Rect;
  {-сдвинуть прямоугольник на -Pt и вернуть Result}
begin
 Result.R.Left := R.Left - Pt.P.X;
 Result.R.Right := R.Right - Pt.P.X;
...
procedure Tl3_SPoint.Init(aX, aY: Long);
  {-инициализирует точку}
begin
 P.X := aX;
 P.Y := aY;
