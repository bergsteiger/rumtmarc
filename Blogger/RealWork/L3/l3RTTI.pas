...
function L3FormatRTTIInfo(anObject: TObject;
  aSkipEvents: Boolean = True): AnsiString;
...
//#UC START# *5012C016039F_5012BFDC0300_var*
 function PropAsString(const aProp: TPropInfo): string;

  function TypeInfoAsString(aType: PTypeInfo): string;
  begin
   case aType.Kind of
  ...
  function PropValueAsString(aType: PTypeInfo; const aPropName: string): string;

   function lGetPropValue(Instance: TObject; const PropName: string; PreferStrings: Boolean): Variant;

    function lSetToString(PropInfo: PPropInfo; Value: Integer): string;
    var
     S: TIntegerSet;
     TypeInfo: PTypeInfo;
     I: Integer;
    begin
     ...
     Result := Format('\[%s] (%d)', [Result, Value]);
    end;
    ...
    case aType.Kind of
     ...
     tkClass:
      begin
       ...
       if Assigned(l_Obj)
        then Result := '$' + IntToHex(Integer(l_Obj), 8)
        else Result := 'nil';
      end;                                   
    else
     Result := lGetPropValue(anObject, aPropName, True);
 ...
 try
  ...
  try
   ...
   for I := 0 to l_SL.Count - 1 do
    Result := Result + #13#10 + l_SL[I].AsString;
  finally
   l3Free(l_SL);
  end;
 finally
