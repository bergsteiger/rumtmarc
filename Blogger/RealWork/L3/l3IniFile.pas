...
{ $Id: l3IniFile.pas,v 1.24 2013/06/17 15:36:37 voba Exp $ }
...
// - change: изменен предок TObject -> Tl3Base.
//
...
type
 ...
 public
  constructor Create(CfgName: TFileName = '');
    reintroduce;
  procedure SetCfgFile(Value: TFileName);
...
var
 UserConfig : TCfgList = Nil;
 StationConfig : TCfgList = Nil;
 ServerConfig : TCfgList = Nil;
...
procedure TCfgList.SetCfgFile(Value: TFileName);
begin
 if Length(Value) = 0 then
  Value := ChangeFileExt(Application.ExeName, CfgExt)
 else
 begin
 ...
 if Assigned(FCfgFileName) then
  l3StrDispose(FCfgFileName);
 FCfgFileName := l3StrAlloc(Length(Value) + 1);
 StrPCopy(FCfgFileName, Value);
...
begin
 StationConfig:=TCfgList.Create(aIniPath);
