...
{ $Id: l3XMLWriter.pas,v 1.18 2013/04/08 14:50:41 lulin Exp $ }
...
begin
 ...
 begin
  ...
  begin
   ...
   if (f_Last <> '') then
   begin
    SaveString(Format('<%s/>', [f_Last]), 0);
    f_Last := '';
   end//f_Last <> ''
   else
    SaveString(Format('</%s>', [l_Out]), 0);
   if (Count = 0) then
    l3Free(f_Levels);
  end;//f_Levels
 end;
...
begin
 ...
 if (f_Levels = nil) or f_Levels.Empty then
  WriteLn(aString)
 else
  WriteLn(ev_psFixLen('', f_Levels.Count + aLevelDelta, #32) + aString);
end;
