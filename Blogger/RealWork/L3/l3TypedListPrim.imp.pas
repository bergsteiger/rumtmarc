...
type
 ...
 // public methods
   ...
   function JoinWith(aList: _l3TypedListPrim_): Integer; virtual;
   ...
   {$If not defined(l3Items_IsProto)}
   function Clone(anOwner: TObject = nil): Pointer; overload; 
...
(*  if UseFind then
  ...
  begin
   ...
   begin
    ...
    Assert((ItemSlot(Result)^ = anItem) OR IsSame(ItemSlot(Result)^, anItem));
    {$IfEnd}
    {$EndIf l3Items_IsAtomic}
    DoDelete(Result);
   end;//Result >= 0
...
constructor _l3TypedListPrim_.Create(anOwner: TObject = nil);
...
{$If not defined(l3Items_IsProto)}
function _l3TypedListPrim_.Clone(anOwner: TObject = nil): Pointer;
...
//#UC START# *47BAC7A50039_47A74A5F0123_impl*
 Result := R_l3TypedListPrim_(ClassType).Create(anOwner);
