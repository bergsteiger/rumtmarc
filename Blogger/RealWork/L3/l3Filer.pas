...
{ $Id: l3Filer.pas,v 1.191 2013/07/10 16:29:35 lulin Exp $ }
...
type
  ...
  Tl3CommentState = (l3_csNone, l3_csLine, l3_csMultiLine);
  ...
  Tl3CustomFiler = class({$IfDef l3FilerIsComponent}Tl3ProgressComponent{$Else}Tl3ProtoProgressComponent{$EndIf}, Il3Rollback)
   {* Реализация компонента-обертки вокруг потока. }
    ...
    // property methods
      ...
      procedure pm_SetCOMStream(Value: IStream);
        {-}
    ...
    {public methods}
      constructor Create(anOwner: Tl3FilerOwner = nil); {$IfDef l3FilerIsComponent}override;{$EndIf}
        {-}
      procedure Open; {$IfDef OpenCloseLog} virtual; {$EndIf OpenCloseLog} // virtual for debug
        {* - открыть поток. }
      procedure Close; {$IfDef OpenCloseLog} virtual; {$EndIf OpenCloseLog} // virtual for debug
        {* - закрыть поток. }
   procedure Rollback;
     {* Откатить данные }
      ...
      function  Write(S: Tl3CustomString): Long;
        overload;
        {* - записать строку в поток. }
      function  WriteLn(S: Tl3CustomString): Long;
        overload;
...
procedure Tl3CustomFiler.pm_SetCOMStream(Value: IStream);
  {-}
var
 l_Stream : TStream;
begin
...
function Tl3CustomFiler.Write(S: Tl3CustomString): Long;
  {overload;}
  {-}
begin
 Write(S.AsWStr);
...
function Tl3CustomFiler.WriteLn(S: Tl3CustomString): Long;
  //overload;
  {-}
begin
 Result := Write(S);
