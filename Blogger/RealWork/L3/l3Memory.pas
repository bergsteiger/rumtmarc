...
{ $Id: l3Memory.pas,v 1.113 2013/07/10 15:06:08 lulin Exp $ }
...
// - new method: классу Tl3MemoryPool добавлен конструктор Make.
//
...
{$IfDef Nemesis}
 {.$Define l3MemoryPoolIsPersistent}
{$EndIf Nemesis}

{$M+}
  Tl3MemoryPoolPrim = class({$IfNDef l3MemoryPoolIsPersistent}
                            Tl3ProtoDataContainerWithCOMQI
                            {$Else   l3MemoryPoolIsPersistent}
                            Tl3SimpleDataContainer
                            {$EndIf  l3MemoryPoolIsPersistent})
   {* Кусок памяти. }
    private
    // internal fields
      f_Ptr: Tl3Ptr;
    ...
    public
    // public methods
      constructor Make(aHandle: THandle);
        {-}
      ...
      class function IsCacheable: Bool;
        override;
        {-}
      {$IfDef l3MemoryPoolIsPersistent}
      procedure Assign(aP: TPersistent);
        override;
        {-}
      {$EndIf l3MemoryPoolIsPersistent}  
(*      procedure Clear;
...
begin
 ...
 if IID.EQ(Il3MemoryPool) then
  Pointer(Obj) := Pointer(Il3MemoryPool(Tl3MemoryPoolAdapter.Create(Self)))
 else
 if IID.EQ(Il3HandleMemoryPool) then
  Pointer(Obj) := Pointer(Il3HandleMemoryPool(Tl3MemoryPoolAdapter.Create(Self)))
 else
 if IID.EQ(IStream) then
 begin
  l_Stream := Tl3MemoryStream.Create(Self);
  try
...
{$IfDef l3MemoryPoolIsPersistent}
procedure Tl3MemoryPoolPrim.Assign(aP: TPersistent);
  //override;
  {-}
begin
 if (aP Is Tl3MemoryPoolPrim) then
...
(*procedure Tl3MemoryPoolPrim.Clear;
  //virtual;
begin
 // - ничего не делаем. Правильно ли это?
end;*)
