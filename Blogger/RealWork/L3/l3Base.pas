...
{ $Id: l3Base.pas,v 1.537 2013/07/10 16:29:35 lulin Exp $ }
...
// - new method: _Tl3System.GetSizedMM.
//
...
// - new method: _Tl3System.GetBoxMM.
//
...
type
  Tl3CBase = l3CBase.Tl3CBase;
  Tl3Base = l3_Base.Tl3_Base;
  Rl3Base = class of Tl3Base;
  Pl3Base = ^Tl3Base;
  ...
  // Связанный объект автоматически освобождается при удалении строки
   ...
   protected
    procedure Cleanup;
      override;
   public
    constructor Make(const aSt: Tl3PCharLen; const anObj: Tl3Base);
      overload;
      {* - создает новую строку как копию aSt. }
  end;//Tl3ObjRefString
  ...
  Tl3ObjectHash = object(Tl3PtrHash)
    public
    // public methods
      function SetItem(ItemIndex: Long; Item: Tl3SimpleObject): PObject;
        {-}
  ...
  Tl3SystemPrim = class(Tl3MemUtils{TRefcounted}{Tl3ProtoObject})
   {* Базовый класс менеджеров ресурсов. }
    ...
    // internal fields
      ...
      f_GlobalMemUsed    : Long;
      f_ObjectMemUsed    : Long;
      f_ObjectCount      : Long;
      f_CanCache         : Bool;
      f_CacheCS          : TRTLCriticalSection;
      f_GlobalCS         : TRTLCriticalSection;
    ...
    {public methods}
      ...
      procedure GetLocalMem(var P; Size: Cardinal);
        {* - получить кусок локальной памяти размером Size. }
      procedure GetLocalMemZ(var P; Size: Cardinal);
        {* - получить кусок локальной памяти размером Size, заполненный нулями. }
      ...
      function  GlobalSize(aMem: THandle): Cardinal;
        overload;
        {* - возвращает размер куска памяти. }
      function  GlobalSize(aMem: Pointer): Cardinal;
        overload;
        {* - возвращает размер куска памяти. }
      function  GlobalAllocPtr(Bytes : LongInt;
                               Flags : Cardinal = gmem_Fixed): Pointer;
        {-}
  ...
  Tl3System = class(Tl3SystemPrim)
   {*! Класс менеджеров ресурсов. }
    ...
    // public methods
      ...
      procedure Beep;
        {-}
      function  GetMM(anInstanceSize: Integer): Il3MemoryManager;
        {-}
      function  GetBoxMM(aSize: Cardinal): Tl3Base;
        {-}
      function  GetSizedMM(aSize: Cardinal; aPartSize: Cardinal): Tl3Base;
        {-}
  ...
  Tl3LoopThread = class(Tl3ThreadContainer)
   {* Контейнер нити, выполняющий логику путем итераций. }
    private
    //internal fields
      f_Restart    : Bool;
    protected
      procedure AfterEachIteration; virtual;
    //internal methods
      procedure OneIteration;
        virtual;
        abstract;
        {* - выполнить одну итерацию логики. Для перекрытия в потомках. }
      procedure IterationDone;
        virtual;
        abstract;
        {* - нотификация о завершении итерации. Для перекрытия в потомках. }
      procedure DoExecute;
        override;
        {-}
    public
    ...
    public
    //public properties
      property Restart: Bool
        read f_Restart
        write f_Restart;
        {* - перезапустить итерацию? }
  end;//Tl3LoopThread

  Tl3NonAbsLoopThread = class(Tl3LoopThread)
    protected
    //internal methods
      procedure IterationDone;
        override;
        {-}
  end;//Tl3NonAbsLoopThread
...
function  l3Use(O: Tl3CBase): Pointer;
  overload;
  {* - увеличивает объекту счетчик ссылок. }
function  l3Use(O: Tl3DataContainerWithoutIUnknown): Pointer;
  overload;
  {* - увеличивает объекту счетчик ссылок. }
...
function  l3Set(var F; V: Tl3CBase): Bool;
  overload;
  {* - присваивает переменной F объект со счетчиком ссылок V. }
function  l3Set(var F; V: Tl3DataContainerWithoutIUnknown): Bool;
  overload;
  {* - присваивает переменной F объект со счетчиком ссылок V. }
...
begin
 if (O <> nil) then
  if not O.GetInterface(IUnknown, Obj) then
   if (O Is Tl3DataContainerWithoutIUnknown) then
   begin
    Result := Tl3DataContainerWithoutIUnknown(O).Use;
    Exit;
   end//O Is Tl3DataContainerWithoutIUnknown
   else
    Assert(false);
 Result := O;
end;
...
function l3System: Tl3System;
  {-}
begin
 if (g_l3System = nil) then g_l3System := Tl3System.Create;
 Result := Tl3System(g_l3System);
end;

var
 g_l3SystemDown : Boolean = false;
 
function l3SystemDown: Boolean;
  {-}
begin
 Result := (g_l3System = nil) OR g_l3SystemDown;
end;
...
function Tl3ObjectHash.SetItem(ItemIndex: Long; Item: Tl3SimpleObject): PObject;
  {-}
begin
 Item.Use;
 try
...
function Tl3System.GetMM(anInstanceSize: Integer): Il3MemoryManager;
  {-}
var
 l_MM : Tl3SimpleMemoryManager;
begin
 if CanCache OR (f_MMs <> nil) then
 begin
  if (f_MMs = nil) then
   f_MMs := Tl3NCObjectRefArray.Make;
  Result := Tl3SimpleMemoryManager(f_MMs[cMM]);
  if (Result = nil) then
  begin
   l_MM := Tl3SimpleMemoryManager.Create({$IfDef Nemesis}
                                           32
                                           {$Else  Nemesis}
                                           128
                                           {$EndIf Nemesis}
...
function Tl3System.GetBoxMM(aSize: Cardinal): Tl3Base;
  {-}
begin
 if CanCache OR (f_MMs <> nil) then
 begin
  ...
  if (Result = nil) then
  begin
   Result := Tl3BoxMemoryManager.Create(aSize, 64);
   try
    f_MMs[cBoxMM] := Result;
   finally
    Result := Result.Free;
...
function Tl3System.GetSizedMM(aSize: Cardinal; aPartSize: Cardinal): Tl3Base;
  {-}
begin
 if (aPartSize < cLowSizedMM) OR (aPartSize > cHighSizedMM) then
  Result := nil
 else
 begin
  if CanCache OR (f_MMs <> nil) then
  begin
   if (f_MMs = nil) then
    f_MMs := Tl3NCObjectRefArray.Make;
   Result := f_MMs[aPartSize];
   if (Result = nil) then
   begin
    Result := Tl3FixedSizeMemoryManager.Create(aSize, aPartSize);
    try
     f_MMs[aPartSize] := Result;
    finally
     Result := Result.Free;
    end;//try..finally
   end;//Result = nil
...
begin
 Result := H;
 Dec(f_GlobalMemUsed, GlobalSize(Result));
end;
...
procedure Tl3System.MemUsage2Log;
{$IfDef l3TraceClasses}
var
 I: Integer;
{$EndIf l3TraceClasses}
...
{$IfDef l3TraceClasses}
 for I := 0 to f_ClassList.Count -1 do
  if Long(f_ClassList.Objects[I]) > 12 then
...
{$EndIf l3TraceClasses}
 ...
 {$ifdef  MMTrace}
  Tl3SimpleMemoryManager(f_MMs[cMM]).f_GlobalAlloc, Tl3SimpleMemoryManager(f_MMs[cBoxMM]).f_GlobalAlloc,
 {$EndIf}
...
begin
 ...
 if (g_l3System = nil) then
  g_l3System := Self;
 InitializeCriticalSection(f_CacheCS);
 InitializeCriticalSection(f_GlobalCS);
 f_CanCache := true;
 inherited;
end;
...
begin
 ...
 if (errSt <> EmptyStr) and FShowObjectsWindow then
 begin
  {$IfDef l3TraceClasses}
  if (Application = nil) then
   Application := TApplication.Create(nil);
  ...
  try
   with l_LeakForm do begin
    memGeneral.Text := errSt;
    if (f_ClassList = nil) OR (f_ClassList.Count = 0) then begin
 ...
 {$EndIf l3TraceClasses}
 {l3FreeStubs;}
 DeleteCriticalSection(f_GlobalCS);
 DeleteCriticalSection(f_CacheCS);
end;
...
procedure Tl3SystemPrim.GetLocalMem(var P; Size: Cardinal);
  register;
  {-}
begin
 {$IfDef l3DirectUseSystemMemManager}
 l3StatMemAlloc(Size, true);
 Pointer(P) := SysGetMem(Size);
 {$Else  l3DirectUseSystemMemManager}
 System.GetMem(Pointer(P), Size);
 {$EndIf l3DirectUseSystemMemManager}
 Inc(l3MemUtils.f_LocalMemUsed, l3MemorySize(Pointer(P)));
 CheckMaxes;
end;

procedure Tl3SystemPrim.GetLocalMemZ(var P; Size: Cardinal);
  {-}
begin
 GetLocalMem(P, Size);
...
begin
 ...
 if (Result = 0) then Dec(f_GlobalMemUsed, Sz);
end;

function Tl3SystemPrim.GlobalSize(aMem: THandle): Cardinal;
  //overload;
  {* - возвращает размер куска памяти. }
begin
...
function Tl3SystemPrim.GlobalSize(aMem: Pointer): Cardinal;
  //overload;
  {* - возвращает размер куска памяти. }
begin
...
function Tl3SystemPrim.GlobalAllocPtr(Bytes : LongInt;
                                      Flags : Cardinal = gmem_Fixed): Pointer;
  {-}
var
 Sz : LongInt;
begin
 Result := Windows.GlobalAllocPtr(Flags, Bytes);
 if (Result = nil) then
  Exit;
 Assert(Result <> nil);
...
function Tl3SystemPrim.GlobalFreePtr(var Mem: Pointer): THandle;
  {-}
var
 Sz : LongInt;
begin
...
begin
 ...
 begin
  ...
  EnterCriticalSection(f_CacheCS);
  try
   ...
   LeaveCriticalSection(f_CacheCS);
  end;//try..finally
 end;//O = nil
end;
...
procedure Tl3SystemPrim.EnterGlobalCS;
  {-}
begin
 EnterCriticalSection(f_GlobalCS);
end;

procedure Tl3SystemPrim.LeaveGlobalCS;
  {-}
begin
 LeaveCriticalSection(f_GlobalCS);
end;
...
procedure Tl3LoopThread.AfterEachIteration;
begin
 Suspended := true;
end;

{ start class Tl3LoopThread }

procedure Tl3LoopThread.DoExecute;
  //override;
  {-}
begin
 while not Terminated do begin
  repeat
   ...
   if not NeedThread then
    break;
  until false;
  AfterEachIteration;
  if (ToolThread = nil) then break;
  Sleep(0);
 end;{while not Terminated}
end;

function Tl3LoopThread.NeedContinue: Bool;
  {-}
begin
 if Restart then
  Result := false
 else
  Result := inherited NeedContinue;
end;

// start class Tl3NonAbsLoopThread

procedure Tl3NonAbsLoopThread.IterationDone;
  //override;
  {-}
begin
end;
