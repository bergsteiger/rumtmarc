...
{ $Id: l3Parser.pas,v 1.62 2013/06/14 13:48:33 fireton Exp $ }
...
// - new behavior: метод Assign теперь присваивает два Parser'а.
//
...
const
  ...
  l3_DefaultParserWhiteSpace = [cc_Null .. cc_HardSpace];
    {-}
type                                                            
  Tl3CustomParser = class(Tl3ProtoPersistent{Tl3Base})
   {* Разборщик потока на токены (слова, числа, строки etc). }
    private
    // property fields
    ...
    protected
    // internal methods
      function  SkipBlanks(NTF: Tl3NextTokenFlags): Bool;
        {-}
      procedure Cleanup;
        override;
        {-}
...
function Tl3CustomParser.SkipBlanks(NTF: Tl3NextTokenFlags): Bool;
var
 l_C  : Ansichar;
 CS : Tl3CommentState;
 l_Line : Tl3String;
...
begin
 ...
 begin
  ...
  begin
   KeyWord := KeyWords.KeyWordByName[TokenLongString.AsString];
   if (KeyWord <> nil) then
