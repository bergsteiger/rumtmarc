...
{$Define l3ListOperations_imp}
 ...
 public
 // public methods
   procedure DeleteList(const aList: _ListType_);
     {* Удаляет из списка элементы содержащиеся в другом списке (aList) }
   procedure InsertList(const aList: _ListType_);
     {* Добавляет в список элементы другого списка (aList) }
   procedure IntersectList(const aList: _ListType_);
...
procedure _l3ListOperations_.DeleteList(const aList: _ListType_);
...
procedure _l3ListOperations_.InsertList(const aList: _ListType_);
...
procedure _l3ListOperations_.IntersectList(const aList: _ListType_);
