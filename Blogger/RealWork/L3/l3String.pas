...
{ Начат: 12.12.1996                   }
{ $Id: l3String.pas,v 1.306 2013/07/31 10:31:39 fireton Exp $ }
...
function l3StrToIntDef(const aString : Tl3WString; aDefault: Integer) : Integer;
...
function l3CharSetPresentExR(St: PAnsiChar; StLen: Long; const CS: TChars): Long;
  {* - определяет есть ли хотя бы один символ из множества в строке и
...
begin
 ...
 if (St <> nil) then
 begin
  I := Min(l3StrLen(St), StLen);
  while (St[I] <> #0) AND (I >= 0) do
   if (St[I] in CS) then
   begin
    Result := I;
    Exit;
   ...
   try
    ...
    try
     ...
     begin
      {$IfOpt D+}
      Inc(l_Guard);
      Assert(l_Guard < 30 * 1000, 'Есть подозрение, что зацикливаемся при поиске/замене');
      // - думаю, что 30 тыс. замен хватит :-)
      // вообще говоря - можно же l_Pos - двигать
      {$EndIf}
      if l_SF(l_S, l_BT, l_Pos) then
      begin
       if (Result = nil) then
       begin
        l3Free(l_S);
