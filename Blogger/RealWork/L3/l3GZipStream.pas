...
{ $Id: l3GZipStream.pas,v 1.12 2013/07/15 07:35:39 fireton Exp $ }
...
begin
 ...
 begin
  ...
  l_ActualRead := Read(l_Buf, l_BytesToRead);
  if l_ActualRead <> l_BytesToRead then
   raise EGZIPError.Create(sInvalidFile);
  l_CRC := zlibh.crc32(l_CRC, @l_Buf, l_BytesToRead);
  l_TotalBytes := l_TotalBytes - l_BytesToRead;
 end;
 if l_CRC <> f_Proxy.CRC32 then
