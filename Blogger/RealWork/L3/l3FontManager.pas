...
{ $Id: l3FontManager.pas,v 1.17 2013/04/04 11:33:01 lulin Exp $ }
...
type
  Tl3LogFont = class(Tl3CustomString)
  ...
  Tl3FontManager = class(Tl3Dictionary)
   {* Менеджер шрифтов. }
    private
      f_ANSIDOSFontNameRecursionCount: Integer;
    protected
    // property methods
      function pm_GetDRByName(const aName: AnsiString): Tl3PrimString;
        override;
        {-}
    protected
    // internal methods
...
var
 LF : Tl3LogFont;
begin
 LF := Tl3LogFont.Create{(Self)};
 try
  LF.f_LogFont := lpLogFont^;
  LF.f_NameLen := StrLen(LF.f_LogFont.elfLogFont.lfFaceName);
  LF.f_TextMetric := lpTextMetric^;
  if l3IsDefaultCharset(LF.f_LogFont.elfLogFont.lfCharSet) then
   LF.f_LogFont.elfLogFont.lfCharSet := CS_Effective;
  Add(LF);
 finally
  l3Free(LF);
 end;{try..finally}
...
function Tl3FontManager.pm_GetDRByName(const aName: AnsiString): Tl3PrimString;
  {override;}
  {-}

 function DoFont(pFont: PObject; anIndex: Long): Boolean;
 begin//DoFont
  if not Tl3LogFont(pFont^).IsFixed then
  begin
   pm_GetDRByName := Tl3LogFont(pFont^);
   Result := false;
  end//Tl3LogFont(pFont^).IsFixed
  else
   Result := true;
 end;//DoFont
...
begin
 ...
 begin
  try
   if ANSIEndsText(fs_Cyr, aName) then
    Result := DRByName[Copy(aName, 1, Length(aName)-Length(fs_Cyr))]
   else
   if ANSIEndsText(fs_New, aName) then
