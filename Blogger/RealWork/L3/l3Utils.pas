...
{ DLL: l3Utils.dll (*) }
{ Начат: 12.12.1996    }
{ $Id: l3Utils.pas,v 1.34 2013/03/28 15:09:44 lulin Exp $ }
...
function  l3FindNextLine(const aStr : Tl3PCharLen; WrapCol : Integer): Integer;
...
{$IfNDef nsTool}
{$IfNDef l3ConsoleApp}
function l3GetClipboardByFormatName(const aName : String): Integer;
...
function _l3FindNextLine(S : PAnsiChar; WrapCol : Integer) : PAnsiChar; register;
...
function _l3FindNextLineW(S : PWideChar; WrapCol : Integer) : PWideChar; register;
  {-find the start of the next line}
asm
  push   esi            {save}
  push   edi            {save}
...
begin
 if (aStr.SCodePage = CP_Unicode) then
  ...
  Result := _l3FindNextLineW(PWideChar(aStr.S), WrapCol) - PWideChar(aStr.S)
 else
  Result := _l3FindNextLine(aStr.S, WrapCol) - aStr.S;
end;
