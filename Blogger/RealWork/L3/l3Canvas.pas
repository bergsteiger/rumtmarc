...
{ $Id: l3Canvas.pas,v 1.262 2013/08/19 13:19:04 dinishev Exp $ }
...
// - new method: TevParaPainter.FocusRect.
//
...
(*function Tl3Canvas.GetOwner: TPersistent;
  {override;}
  {-}
begin
...
procedure Tl3Canvas.DoSetOwner(Value: TObject);
  {override;}
  {-}
begin
 f_Owner := Value;
 if (Value Is TBitmap) then
  Canvas := TBitmap(Value).Canvas;
end;
...
begin
 if (f_Font <> nil) AND (Value <> l3GlyphFontName) then
  f_Font := nil; 
 l_LogFont := (l3FontManager.Fonts.DRByName[Value] As Tl3LogFont);
 VCLFont.Name := Value;
...
begin
 ...
 begin
  ...
  Inc(f_CheckingColors);
  try
   l_LogFont := (l3FontManager.Fonts.DRByName[l_Font.Name] As Tl3LogFont);
   if (l_LogFont = nil) then
