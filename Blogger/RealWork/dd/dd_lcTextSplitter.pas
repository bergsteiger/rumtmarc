...
{ $Id: dd_lcTextSplitter.pas,v 1.7 2013/04/11 16:46:29 lulin Exp $ }
...
type
 ...
 protected
        {-}
  function DoCheckText(const aPara : Ik2Tag; aText : Tl3CustomString; const aSel : TevPair; out
...
procedure Tdd_lcTextSplitter.DoWritePara(const aLeaf: Ik2Tag);
const
...
var
 ...
 begin
  ...
  Generator.StartChild(l_P.TagType.ID);
  try
   l_P.Write(Generator);
  finally
   Generator.Finish;
...
begin
 if (Source2DossierType(f_Source) <> lc_dtVAC) and //(not f_BodyStarted) and
    (aLeaf.Attr[k2_tiText].IsValid and (aLeaf.StrA[k2_tiText] <> '') and (aLeaf.Attr[k2_tiText].AsPCharLen.SCodePage <> cp_Unicode))then
 begin
...
function Tdd_lcParaEndSearcher.DoCheckText(const aPara : Ik2Tag; aText : Tl3CustomString; const
