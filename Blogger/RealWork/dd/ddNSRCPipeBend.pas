...
{ $Id: ddNSRCPipeBend.pas,v 1.51 2013/04/11 16:46:28 lulin Exp $ }
...
procedure TddPipeBend.StartChild(TypeID: Long);
var
 l_Type : Tk2Type;
begin
 ...
 begin
  ...
  if CurrentType.InheritsFrom(k2_idBlock) then
  begin
   if (f_ExtNum > 0) then
   begin
    AddSpecialParagraph;
  ...
  If CurrentType.InheritsFrom(k2_idDictRec) and (TopObject[1].isProp) then
  Begin
   if TopObject[1].AsProp.TagIndex = k2_tiGroups then
    if f_DictRecType <> dlBases then
    begin
     f_Belongs.Clear;
     f_DictRecType:= dlBases;
    end;
  end;
end;
  {-}
...
begin
  ...
  If CurrentType.InheritsFrom(k2_idDictRec) and not CurrentType.InheritsFrom(k2_idBlock) and
     not CurrentType.InheritsFrom(k2_idDocument) then {заполнение записи: элемент словаря}
  Begin
    ...
    (TopObject[1].AsProp.TagIndex = k2_tiGroups)) then
     ...
     begin
      if (f_DictRecType = dlBases) and (Value.AsString.AsString <> 'KSE') then
      begin
...
begin
 if CurrentType.InheritsFrom(k2_idDocument) then
  if TopType[1] <> nil then // - учитываем вложенные документы
   Exit;
  if (f_ExtNum <> -1) and CurrentType.InheritsFrom(k2_idBlock)
     and not CurrentType.InheritsFrom(k2_idDocument) then
  begin
   AddSpecialParagraph;
   WriteAdditionalFile;
   f_ExtNum := -1;
