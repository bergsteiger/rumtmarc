...
{ $Id: dd_lcDocIDGenerator.pas,v 1.25 2013/04/15 05:59:22 narry Exp $ }
...
Uses
 ...
 l3FieldSortRecList, ddTypes, l3RecList, dd_lcBaseHeaderFilter, dt_Table;
...
type
 ...
 private
  ...
  function GetDiapasonAlias(aRoot: Ik2Tag): AnsiString;
  procedure ClearDocumentsList;
  procedure ExtractDateNumber(aHeaderRoot: Ik2Tag);
...
uses
   l3Base, k2Tags, Math, SysUtils, Dt_Serv, dt_AttrSchema,
 DT_Const, Dt_Query, Dt_Sab, DT_Doc, DT_LinkServ, DT_Link, DT_DictConst, DT_Dict, DT_Renum,
...
procedure TDocIDGenerator.BeforeFlushHeader(aHeaderRoot: Ik2Tag);
var
 l_DateNums, l_DN: Ik2Tag;
 i: Integer;
 l_Str: TNumberStr;
 l_S: AnsiString;
 l_ExtID: TDocID;
...
function TDocIDGenerator.GetDiapasonAlias(aRoot: Ik2Tag): AnsiString;
var
 l_Sources: Ik2Tag;
 l_Source: Ik2Tag;
 i: Integer;
...
procedure TDocIDGenerator.ExtractDateNumber(aHeaderRoot: Ik2Tag);
var
 l_DateNums: Ik2Tag;
 l_DN: Ik2Tag;
 l_Types: Ik2Tag;
...
begin
 ...
 if (l_Types.IsValid) and (l_Types.ChildrenCount > 0) then
  l_Type:= DictServer(CurrentFamily).Dict[dlTypes].FindIDByFullPath(l_Types.Child[0].PCharLenA[k2_tiName]);
...
begin
 ...
 begin
  ...
  try
   l_Q := TdtDictQuery.Create(dlSources, f_SourcesList, False, True {WithSubTree});
   l_ResQuery.AddQueryF(l_Q);

   l_Q := TdtDictQuery.Create(dlTypes, f_TypesList, False, True {WithSubTree});
   l_ResQuery.AddQueryF(l_Q);
...
begin
 ...
 begin
  ...
  l_ResQuery := TdtAndQuery.Create;
  try
   l_Q := TdtDictQuery.Create(dlSources, f_FASSourcesList, False, True {WithSubTree});
   l_ResQuery.AddQueryF(l_Q);
  ...
  //получим LC номера
  lDNTblDupLC := TdtTable.CreateDup(DictServer(CurrentFamily).GroupTbl[dlDateNums]);
  try
