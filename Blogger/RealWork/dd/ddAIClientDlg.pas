...
// $Id: ddAIClientDlg.pas,v 1.33 2013/04/11 16:46:26 lulin Exp $
...
var
 S, l_Ext, l_Filter: AnsiString;
 i: Integer;
begin
  UserConfig.SetSection('AutoImport');
  if UserConfig.ReadParamString('InitialDir', S) then
  ...
  begin
   ...
   if S <> '' then
    ShowMessage(Format('Файлы %s не соответствуют фильтру %s', [S, OpenDialog1.Filter]));
