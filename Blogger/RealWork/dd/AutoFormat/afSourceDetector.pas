...
// $Id: afSourceDetector.pas,v 1.2 2013/04/16 09:30:55 fireton Exp $
...
type
 ...
 private
  ...
  f_TypeFound: Boolean;
  function FindSource(aPara: Ik2Tag): Tdd_lcTextReaction;
  procedure GenerateTitle;
 protected
  function CheckExtractCode(aPara: Ik2Tag; out theNumber: string): Boolean;
...
uses
  ...
  m4DictScanner, DT_Const, dt_AttrSchema, m4PhraseParser, l3DateSt;
...
function TafSourceDetector.CheckExtractCode(aPara: Ik2Tag; out theNumber:
    string): Boolean;
const
 cDelo = '(Д|дело\s*)';
 cNo   = '([\№N]\s*)';
...
function TafSourceDetector.EnableWrite(const aPara: Ik2Tag): Tdd_lcTextReaction;
var
 i: Integer;
...
function TafSourceDetector.FindSource(aPara: Ik2Tag): Tdd_lcTextReaction;
var
 i: Integer;
 ...
 begin
  Result := True;
  //l3System.Msg2Log(DictServer(CurrentFamily).Dict[TDLType(aDictItemID.rDictionaryID)].GetShortName(aDictItemID.rID));
   ...
   *)
   ...
   case TDLType(aDictItemID.rDictionaryID) of
     dlSources:
      ...
      begin
       f_Source:= DictServer(CurrentFamily).Dict[TDLType(aDictItemID.rDictionaryID)].GetShortName(aDictItemID.rID);
       f_SourceFound:= True;
       l_Reaction:= lcSkip;
      end;
     dlTypes  :
      ...
      begin
       f_Type:= DictServer(CurrentFamily).Dict[TDLType(aDictItemID.rDictionaryID)].GetShortName(aDictItemID.rID);
       f_TypeFound:= True;
       l_Reaction:= lcSkip;
      end;
   end;
