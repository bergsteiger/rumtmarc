...
{ $Id: dd_lcDossierMaker.pas,v 1.26 2013/04/15 05:59:22 narry Exp $ }
...
uses
 dt_AttrSchema,
 TextPara_Const, k2Base, k2Tags, Document_Const, evdStyles, HyperLink_Const,
 ...
 Participant_Const, DT_Sab, DT_Query, l3Base,
...
begin
 ...
 begin
  l_ID:= DictServer(CurrentFamily).Dict[dlSources].FindIDByFullPath(l3PCharLen(f_Source));
...
begin
 if CurrentType.InheritsFrom(k2_idDocument) then
  case AtomIndex of
   ...
   k2_tiRelExternalHandle: f_DossierID:= Value.AsInteger;
 ...
 if CurrentType.InheritsFrom(k2_idParticipant) then // накапливаем инфу про участников
 begin
(*                     ptPlaintiff, // Истец
 ...
 if CurrentType.InheritsFrom(k2_idDictItem) and (TopObject[1].isProp) and (AtomIndex = k2_tiName) then
...
var
 l_Generate: Boolean;
begin
 l_Generate:= CurrentType.InheritsFrom(k2_idDocument) and (f_DossierID <> -1);
 if CurrentType.InheritsFrom(k2_idParticipant) then
 begin
  f_ParticipantType:= -1;
  f_Participant:= '';
 end; // CurrentType.InheritsFrom(k2_idParticipant)
 inherited;
...
procedure TlcDossierMaker.DoStartChild(TypeID: Long);
var
 i, l_ID: Integer;
begin
 if TypeTable[TypeID].InheritsFrom(k2_idTextPara) then
...
begin
 f_SourcesList := Value;
 ExpandDictIDList(dlSources, f_SourcesList);
end;
