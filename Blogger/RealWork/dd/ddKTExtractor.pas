// $Id: ddKTExtractor.pas,v 1.28 2013/04/19 13:10:10 lulin Exp $
...
type
 ...
 protected
  procedure Cleanup;
   override;
  function IsValidSource(aRoot: Ik2Tag): Boolean;
   {-}
...
var
 ...
 begin
  ...
  begin
   ...
   Assert(lDocID = Integer(f_KTList.Objects[lCurrDocItem]), 'TddKTExtractorTermFilter.CloseCache: не прокатило :(');
   ...
   lKTStr := f_KTList.Strings[lCurrDocItem];
...
//  lKTStr := f_KTExtractor.GetKTString;
  f_KTList.AddObject(f_KTExtractor.GetKTString, TObject(f_DocID));
...
begin
 ...
 if CurrentType.InheritsFrom(k2_idTextPara) and (AtomIndex = k2_tiText) and not Value.AsString.Empty then
 begin
...
var
 l_Sab: ISab;
begin
 l_Sab := GetExpandDictIDValueSab(dlSources, f_Sources);
 f_Sources.Clear;
 dtCopyValuesSabToList(l_Sab, f_Sources);
 l_Sab := nil;
end;

function TddKTExtractorFilter.IsValidSource(aRoot: Ik2Tag): Boolean;
var
 l_Sources: Ik2Tag;
 l_Source: Ik2Tag;
 i: Integer;
  ...
  if l_Sources.IsValid then
   ...
   begin
    ...
    if l_Source.Attr[k2_tiName].IsValid then
     l_Handle:= DictServer(CurrentFamily).Dict[dlSources].FindIDByFullPath(l_Source.Attr[k2_tiName].AsPCharLen);
...
var
 ...
 lListofChains : Tl3SimpleObjectRefList;
 l_Ext: AnsiString;
 l_Zip : TZipForge;
 l_FName: AnsiString;
   ...
   try
    ...
    try
     ...
     begin
      ...
      lChainList := Tl3LongintList.Create;
      try
       lListofChains := Tl3SimpleObjectRefList.Create;
       try
        for I := 0 to pred(lArrDim) do
        begin
