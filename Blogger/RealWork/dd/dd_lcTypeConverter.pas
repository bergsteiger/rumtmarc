...
{ $Id: dd_lcTypeConverter.pas,v 1.8 2013/04/12 15:02:04 lulin Exp $ }
...
begin
 if CurrentType.InheritsFrom(k2_idDictItem) and (TopObject[1].IsProp and (TopObject[1].AsProp.TagIndex = k2_tiTypes)) then
 begin
 ...
 end; // CurrentType.InheritsFrom(k2_idDictItem) and (TopObject[1].IsProp and (TopObject[1].AsProp.TagIndex = k2_tiTypes))
...
procedure Tdd_lcTypeConverterEx.DoFlushBuffer(const aLeaf: Ik2Tag; aTagId: Integer; aNeedCloseBracket : Boolean);
var
 ...
 function lp_CheckPara(const aPara: Ik2Tag; Index: Long): Boolean;
 ...
 begin
  Result:= True;
  if aPara.InheritsFrom(k2_idTextPara) and aPara.Attr[k2_tiText].IsValid then
  begin
...
begin
 ...
 begin
  ...
  begin
   ...
   aLeaf.IterateChildrenF(k2L2TIA(@lp_CheckPara));
  end;
 end;
 inherited;
end;
...
procedure Tdd_lcTypeConverter2.BeforeFlushHeader(aHeaderRoot: Ik2Tag);
var
 l_1stType, l_2ndType: AnsiString;
 l_Types, l_Sources: Ik2Tag;
 l_Pos: Integer;
 l_text: AnsiString;
begin
