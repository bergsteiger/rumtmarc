...
{ $Id: ddRelPublish.pas,v 1.20 2013/04/26 14:36:29 voba Exp $ }
...
// «апись в список документов без справки и документов без публикации
//
// Revision 1.13  2012/06/04 13:07:15  narry
// ќбновление
//
// Revision 1.12  2012/05/30 09:32:04  narry
// Ќе распознавалс€ номер статьи
//
// Revision 1.11  2012/05/30 09:05:07  fireton
// - номера статей (  367210765)
//
// Revision 1.10  2012/05/28 10:28:38  narry
// Ќе добавл€лась в журнал информаци€ об изменении
//
// Revision 1.9  2012/05/28 09:51:09  narry
// ¬озвращалс€ мусор, если не обрабатывалс€ файл
//
// Revision 1.8  2012/05/24 12:48:19  narry
// »нтервал повтор€емости в минутах (365825882)
//
// Revision 1.7  2012/05/23 07:52:48  narry
// —охран€ть список заблокированных справок (365825718)
//
// Revision 1.6  2012/05/23 07:21:01  narry
// ƒополнение к задаче по заливке публикаций (363583844)
//
// Revision 1.5  2012/05/17 10:34:47  narry
// —сылки внутри добавл€емой информации (363574319)
//
// Revision 1.4  2012/05/14 11:25:42  narry
// ѕовторное добавление информации (363574314)
//
// Revision 1.3  2012/05/14 09:29:29  narry
// јвтодополнение справок (269063217)
//
...
uses
 ...
 ddDocTextFilterer,
 l3Interfaces,
 l3LongintList, l3ProtoObject, DT_Types, l3Filer, l3Base, dt_Sab, l3Parser;
...
type
 ...
 private
  ...
  f_Text: Tl3String;
  procedure CreateRestrictedDocs;
 ...
 public
  procedure Execute(const aFileName: AnsiString; out theTotal, theWorked, theMissed: Integer;
      aMissedList: TStrings; theLocked: Tl3LongintList; aProgressor: TddProgressObject);
 end;

procedure CorrectRelatedText(const aFileName: AnsiString; out theTotal, theWorked, theMissed: Integer;
    aMissedList: TStrings; theLocked: Tl3LongintList; aProgressor: TddProgressObject);
...
uses
 ...
 DT_Serv, alcuUtils, l3LongintListPrim;
...
begin
 ...
 try
  Execute(aFileName, theTotal, theWorked, theMissed, aMissedList, theLocked, aProgressor);
 finally
  Free;
 end;
end;

procedure TddRelPublishCorrector.CreateRestrictedDocs;
var
...
begin
  ...
  try
   ...
   l_List.Assign(Tl3LongintList(ddAppConfiguration.AsObject['utRegionID']));
   l_Q := TdtDictQuery.Create(dlAccGroups, l_List);
   try
    f_RestrictedDocs:= l_Q.GetDocIdList;
    f_RDIDList:= dtMakeSortedIDListBySab(f_RestrictedDocs);
    l3System.Msg2Log('Ќайдено %d "региональных" документов', [f_RestrictedDocs.Count]);
...
procedure TddRelPublishCorrector.Execute(const aFileName: AnsiString; out theTotal, theWorked,
    theMissed: Integer; aMissedList: TStrings; theLocked: Tl3LongintList; aProgressor:
    TddProgressObject);
var
 l_Ini : TCfgList;    
begin
...
begin
 try
  ...
  begin
   DocError(f_ExtDocID, 'справка зан€та пользователем');
   ...
   begin
    ...
    begin
     ...
     try
      ...
      begin
       DocError(f_ExtDocID, '√руппа доступа не допускает редактирование');
       f_Locked.Add(f_ExtDocID);
      ...
      case f_Filter.WorkResult of
       wrNotFound     :
        begin
         DocError(f_ExtDocID, 'строка опубликовани€ в справке не найдена');
         f_Locked.Add(f_ExtDocID);
        end;
       wrAlreadyExist : ; //DocError(f_ExtDocID, 'источник опубликовани€ уже присутствует в справке');
       wrDidTheJob    :
        begin
