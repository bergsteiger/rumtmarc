...
{ $Id: ddExtObjectSupport.pas,v 1.13 2013/02/20 09:22:39 dinishev Exp $ }
...
// - Ik2RawData: IsStreamModified -> IsModified
//
...
procedure TddExtObjectMachine.AddAtomEx(AtomIndex: Long; const Value: Tk2Variant);
  ...
  {-}
begin
 if (AtomIndex = k2_tiInternalHandle) and CurrentType.InheritsFrom(k2_idDocument) then
  f_DocID := Value.AsInteger;
 inherited;
end;

function TddExtObjectMachine.NeedFlushBuffer(const aLeaf: Ik2Tag; aTagId: Integer): Boolean;
begin
...
procedure TddExtObjInserter.DoFlushBuffer(const aLeaf: Ik2Tag; aTagId: Integer; aNeedCloseBracket : Boolean);
var
...
(*    l_Prop := aLeaf.TagType.Prop[k2_tiData];
    aLeaf.AttrW[k2_tiData, Context] := Tk2Type(l_Prop.AtomType).ObjToTag(l_TStr);*)
   finally
    l3Free(l_TStr);
   end;//try..finally
  finally
...
procedure TddExtObjInserter.AddAtomEx(AtomIndex: Long; const Value: Tk2Variant); 
begin
...
procedure TddExtObjExtractor.DoFlushBuffer(const aLeaf: Ik2Tag; aTagId: Integer; aNeedCloseBracket : Boolean);
(*var
 l_Index: Integer;
 l_SStr   : IStream;
 l_EVDStr : IStream;
