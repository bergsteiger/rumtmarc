...
{=====================================
 ...
 ===================================== }
{ $Id: ddKW_r.pas,v 1.64 2013/04/11 17:14:58 lulin Exp $ }
...
type
 ...
 public
   ...
   property DictID: TDLType read f_DictID write f_DictID;
 ...
 public
  ...
  function Execute(aFileName: AnsiString; aFamily: TFamilyID; aDict: TDLType; aRenum, aClear, aAddNewItems: Boolean): boolean;
 public
      ...
      while not Parser.Filer.EOF do begin
       ...
       begin
        ...
        begin
         ...
         RootID := KWServ.GetRootKeyWord(l3PCharLen2String(l3LTrim(Filer.ReadLn),
                                                           CP_ANSI),
                                         RootNode);
        end{'KEY'}
        else
             ...
             if (l_LineParser.TokenType <> l3_ttEOF) then
              ...
              begin
               ...
               begin
                ...
                l_SubID := 0;
                KWServ.AddKeyWord(RootNode, l_DictID, l_KeyWord.AsWStr, l_DocID, l_SubID);
              end{while (l_LineParser.TokenType <> l3_ttEOF)}
             else
             if not l_Keyword.Empty then
              KWServ.AddKeyWord(RootNode, l_DictID, l_KeyWord.AsWStr, 0{l_DocID}, 0{l_SubID});
            finally
             Filer.Stream := nil;
            end;{try..finally}
           end;{with l_LineParser}
           ...
           if (l_LineParser.TokenType <> l3_ttEOF) then
            ...
            begin
             ...
             begin
              ...
              l_SubID := 0;
              KWServ.AddKeyWord(RootID, RootNode, l_DictID, l_KeyWord.AsWStr, l_DocID, l_SubID);
            end{while (l_LineParser.TokenType <> l3_ttEOF)}
           else
           if not l_Keyword.Empty then
            KWServ.AddKeyWord(RootID, RootNode, l_DictID, l_KeyWord.AsWStr, 0{l_DocID}, 0{l_SubID});
          finally
           Filer.Stream := nil;
          end;{try..finally}
         end;{with l_LineParser}
...
constructor TddDictLinkPipe.Create(anOwner: TObject);
begin
 inherited;
end;

function TddDictLinkPipe.Execute(aFileName: AnsiString; aFamily: TFamilyID; aDict: TDLType; aRenum, aClear, aAddNewItems:
    Boolean): boolean;
var
...
constructor TDeltaKWReader.Create(anOwner: TObject = nil);
begin
 inherited;
     ...
     begin
      ...
      begin
       ...
       begin
        ...
        l_Node := DictServer(CurrentFamily).Dict[dlKeyWords].FindNodeByFullPath(l3PCharLen(l_Fullpath));
        ...
        begin
         ...
         l_OldID:= DictServer(CurrentFamily).Dict[dlKeyWords].FindIdByFullPath(l3PCharLen(l_FullPath));
         l_NewID:= DictServer(CurrentFamily).Dict[dlKeyWords].FindIdByFullPath(l3PCharLen(l_FullPathNew));

         try
          if (l_NewID <> cUndefDictID) and (l_OldID <> l_NewID) then
           (l_Node as IPersistentNode).DeleteEx(l_NewID)
          else
           with DictServer(CurrentFamily).Dict[dlKeyWords] do
      ...
      begin
       ...
       try
        ...
        if DictServer(CurrentFamily).Dict[dlKeyWords].FindIdByFullPath(l3PCharLen(l_Fullpath2)) = cUndefDictID then
        begin
         with DictServer(CurrentFamily).Dict[dlKeyWords] do
      ...
      begin
       ...
       begin
        ...
        begin
         ...
         if DictServer(CurrentFamily).Dict[dlKeyWords].FindIdByFullPath(l3PCharLen(l_Fullpath2)) = cUndefDictID then
         begin
          l_ParentNode := DictServer(CurrentFamily).Dict[dlKeyWords].FindNodeByFullPath(l3PCharLen(l_NewItem));
          if l_ParentNode = nil then
           l_ParentNode := DictServer(CurrentFamily).Dict[dlKeyWords].AddDictNode(cUndefDictID, l3PCharLen(l_NewItem), l3PCharLen(cEmptyEngName), l3PCharLen, nil);

          DictServer(CurrentFamily).Dict[dlKeyWords].AddDictNode(cUndefDictID, l3PCharLen(l_OldItem), l3PCharLen(cEmptyEngName), l3PCharLen, l_ParentNode);
         end; 
         try
