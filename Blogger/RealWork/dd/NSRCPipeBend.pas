...
{ $Id: NSRCPipeBend.pas,v 1.43 2013/04/11 16:46:26 lulin Exp $ }
...
begin
 ...
 try
  if CurrentType.InheritsFrom(k2_idDictRec) and (f_DictRecType = dlBases) and (TopObject[1] <> nil) and (Ik2Property(TopObject[1]).TagIndex <> k2_tiGroups) then
  begin
...
begin
  ...
  Begin
    If (AtomIndex = k2_tiShortName) then
     ...
     begin
      if (f_DictRecType = dlBases) and not (f_BelongsDone) then
      begin
        f_Belongs.Append('\');
