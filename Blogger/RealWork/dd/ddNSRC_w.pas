...
{ $Id: ddNSRC_w.pas,v 1.307 2013/07/12 14:54:19 voba Exp $ }
...
begin
 ...
 if TT.InheritsFrom(k2_idNumAndDate) then {заполнение записи: дата.номер}
  ApplyToNumAndDate(AtomIndex, Value)
 else
 if TT.InheritsFrom(k2_idAddress) and
...
begin
 if NotAnObject and not (f_UserType in NoDocs) then
  ...
  if (f_DateNumRec.rDNType = dnChangerDate) and
     (f_UserType = utEdition) and (f_DateNumRec.rDate > 1) then
       begin
        OutCommand(kwdChDate);
        OutDate(f_DateNumRec.rDate);
       end;
  with f_DateNumRec do
  begin
    rDNType:= dnDoc;
