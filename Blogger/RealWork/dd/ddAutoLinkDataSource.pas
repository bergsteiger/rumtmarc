...
{ $Id: ddAutoLinkDataSource.pas,v 1.13 2013/04/11 17:14:58 lulin Exp $ }
...
begin
 if f_Query <> nil then
 begin
  l_Q := TdtDictQuery.Create(dlTypes, aDocType, False, False, True);
  f_Query.AddQueryF(l_Q);
 end
 else
  ErrorQueryNotStarted;
...
begin
 if f_Query <> nil then
 begin
  l_Q := TdtDictQuery.Create(dlSources, aSources);
  f_Query.AddQueryF(l_Q);
 end
 else
  ErrorQueryNotStarted;
...
begin
 ...
 f_Type := aType;
 f_StructCache := TddDocStructCache.Create(GetAutolinkStructCacheFilePath(f_Type), GlobalHtServer.TextBase[CurrentFamily], False);
end;
...
begin
 ...
 begin
  ...
  if m3IsStorageExists(l_SCFN) then
  begin
