...
//$Id: ddSectionRepair.pas,v 1.121 2013/04/11 16:46:28 lulin Exp $
...
type
 ...
 private
  ...
  procedure CalculateParagraphWidth(const aRoot: Ik2Tag);
...
procedure TddSectionRepairFilter.CalculateParagraphWidth(const aRoot: Ik2Tag);
begin
 if (f_Tool = nil) then
  f_Tool := TnevFontTool.Make;
 if (f_IC = nil) then
  f_IC := l3CRTIC;
 aRoot.Attr[k2_tiPrintFontSize]:= k2NullTag;
...
begin
 ...
 begin
 if CurrentType.InheritsFrom(k2_idDocument){ or
   (CurrentType.InheritsFrom(k2_tiParas) and TopType[1].InheritsFrom(k2_idDocument))}
 ...
 if CurrentType.InheritsFrom(k2_idDocument) and (AtomIndex = k2_tiInternalHandle) then
  ...
  if (TopType[1].InheritsFrom(k2_idSectionBreak)) then
   case AtomIndex of
 ...
 begin
  ...
  if f_Buffer.CurrentType.InheritsFrom(k2_idTextPara) then
   case AtomIndex of
  ...
  if f_Buffer.CurrentType.InheritsFrom(k2_idTableCell) then
...
var
 ...
 function _CalcWidth(const aTag: Ik2Tag; anIndex: Integer): Boolean;
 var
  l_Width, l_SegmentFinish: Integer;
  l_Segments: Ik2Tag;

  function _IterateLayers(const aLayer: Ik2Tag; anIndex: Integer): Boolean;
   function _CalcSegmentsWidth(const aSeg: Ik2Tag; anIndex: Integer): Boolean;
   var
   ...
   begin // _CalcSegmentsWidth
    ...
    begin // вычислить ширину текста между сегментами
     f_IC.Font := f_Tool.GetObjIFont(nil, aTag, False, nil);
    ...
    f_IC.Font := f_Tool.GetObjIFont(nil, aSeg, true {Это Люлин правил} {False}, nil);
    f_IC.Font.Size := l_Size;
  ...
  begin
   ...
   if aLayer.Attr[k2_tiHandle].IsValid and (aLayer.IntA[k2_tiHandle] = ev_slView) then
    aLayer.IterateChildrenF(k2l2tia(@_CalcSegmentsWidth));
  end;
 ...
 begin // _CalcWidth
  ...
  begin
   ...
   begin
    ...
    l_Width := 0;
    l_SegmentFinish := 0;
    l_Segments.IterateChildrenF(k2l2tia(@_IterateLayers))
   end
   else
 ...
 function _ChangeParaFont(const aTag: Ik2Tag; anIndex: Integer): Boolean;
 var
  l_Style: Integer;
 begin
  Result := True;
  if aTag.InheritsFrom(k2_idTextPara) then
  begin
...
begin // AdjustFontSize
 ...
 repeat
  Dec(l_Size);
  l_MaxWidth:= 0;
  f_ParaBuffer.Root.IterateChildrenF(k2L2TIA(@_CalcWidth));
 ...
 begin
  AddDoc;
  f_ParaBuffer.Root.IterateChildrenF(k2L2TIA(@_ChangeParaFont));
 end; // result
end; // AdjustFontSize
...
begin
 ...
 theScale:= c_DefaultScale;
 theOrientation:= Ord(ev_poPortrait);
 f_Buffer.Root.Attr[k2_tiZoom]:= k2NullTag;
...
begin
 Result := False;
 if (CurrentType.InheritsFrom(k2_idContentsElement, [k2_idCommentPara]) and (f_CollectedBlocks > 0)) or
     not CurrentType.InheritsFrom(k2_idContentsElement, [k2_idCommentPara]) then
  with f_Buffer do
  begin
   if CurrentType.InheritsFrom(k2_idContentsElement, [k2_idCommentPara]) then
...
begin
 ...
 begin
  if CurrentType.InheritsFrom(k2_idTableRow) and (f_NeedBreak)  then
  ...
  begin
   ...
   begin
    if (CurrentType.InheritsFrom(k2_idDocument) or
        CurrentType.InheritsFrom(k2_idContentsElement, [k2_idCommentPara])) then
     WriteCollectedObjects(CurrentType.InheritsFrom(k2_idDocument));
   end; // f_CollectedCount > 0
   if CurrentType.InheritsFrom(k2_idSectionBreak) then
   begin
    DecSkipTags;
    ResetPageParams;
   end;
...
begin
 ...
 l_Gen.StartChild(f_Buffer.Root.TagType.ID);
 try
  f_Buffer.Root.Write(l_Gen);
  f_Buffer.Root:= nil;
 finally
  l_Gen.Finish;
 end;
...
procedure TddSectionRepairFilter.StartChild(TypeID: Long);
var
 l_Type : Tk2Type;
begin
 ...
 if not f_CollectAtoms then
 begin
  if l_Type.InheritsFrom(k2_idStyledLeafPara) or (l_Type.InheritsFrom(k2_idTable)) then
  begin
   if l_Type.InheritsFrom(k2_idStyledLeafPara) and (f_CollectedObjectsCount = 0) then
   begin
    if not f_ParaBuffer.StreamOpened then
    begin
     f_ParaBuffer.Start;
     f_ParaBuffer.StartChild(k2_idDocument);
     f_MaxWidth:= 0;
    end; // not f_ParaBuffer.StreamOpened
   end //l_Type.InheritsFrom(k2_idTextPara) and f_CollectedCount = 0
   else
   if (f_CollectedObjectsCount > 0) and l_Type.InheritsFrom(k2_idTable) then
     WriteCollectedObjects(False);
   if l_Type.InheritsFrom(k2_idTable) then
   begin
    f_NeedBreak:= not l_Type.InheritsFrom(k2_idSBS);
    f_PrevRowWidth:= -108; // !!!
    f_MaxWidth:= 0;
   end;
   f_CollectAtoms:= True;
   ...
   f_Buffer.Start;
   IncSkipTags;
  end // l_Type.InheritsFrom(k2_idTextPara) or l_Type.InheritsFrom(k2_idTable)
  else
  if l_type.InheritsFrom(k2_idContentsElement, [k2_idCommentPara, k2_idDocument]) and not f_HasText then
  begin
   WritePageParameters
  end;
 ...
 if (f_CollectedObjectsCount > 0) and l_Type.InheritsFrom(k2_idSectionBreak) then
  WriteCollectedObjects(False);
 if not f_CollectAtoms then
 begin
 ...
 begin
  if l_Type.InheritsFrom(k2_idTableRow) then
  begin
   if f_NeedBreak then
    f_NeedBreak:= not l_Type.InheritsFrom(k2_idReqRow);
   f_PrevRowWidth:= -108;
  end;
  f_Buffer.StartChild(TypeID);
  if l_type.InheritsFrom(k2_idContentsElement) then
...
procedure TddSectionRepairFilter.OpenStream;
begin
 inherited;
 //f_WorkedDocs.Clear;
end;
...
begin
  ...
  begin
   ...
   begin
    ...
    //l_Default := Prop[k2_tiHeight].DefaultValue;
...
begin
  ...
  begin
   ...
   if l_Landscape <> GetActualPageOrientation {= Ord(ev_poLandscape)} then
   begin
  ...
  f_ParaBuffer.Root.Write(Generator, [l3_spfChildren]);
  f_ParaBuffer.Root:= nil;
  f_ParaBuffer.Finish;
  f_CollectedObjectsCount := 0;
  f_HasText := True;
...
begin
 if not f_HasText and CurrentType.InheritsFrom(k2_idDocument) then
 begin
...
begin
  if (((f_Style = ev_saTxtNormalOEM) or (f_Style = ev_saANSIDOS) or (f_Style = ev_saContents)) and (f_Text <> nil)) or
     ({(f_CollectedObjectsCount > 0) and }((f_Text = nil) or f_Text.Empty)) then // Накапливаем абзацы
  begin
...
begin
 ...
 begin
  ...
  begin
   ...
   begin
    if l_Orientation = ord(ev_poLandscape) then
    begin
