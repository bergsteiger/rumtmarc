...
{ $Id: ddSectionRepairUtils.pas,v 1.3 2013/04/11 16:46:28 lulin Exp $ }
...
// В функцию передаётся список топиков. Если список пустой, то поиск будет по всей базе, иначе только по тем топикам,
// что указаны. Если расстановка разделов прошла успешно, то функция вернёт True и в списке будут топики, реально
...
begin
 ...
 begin
  CT := CurrentType;
  if CT.InheritsFrom(k2_idLeafPara) then
  begin                                           
   case AtomIndex of
...
begin
 ...
 begin
  CT := CurrentType;
  if CT.InheritsFrom(k2_idLeafPara) and
...
procedure TddFTAMFilter.StartChild(TypeID: Long);
var
 CT : Tk2Type;
begin
 inherited;
 ...
 begin
  CT := CurrentType;
  if CT.InheritsFrom(k2_idTable) AND not CT.InheritsFrom(k2_idSBS) then
   ReportDocID;
  if (not f_WasOut) and CT.InheritsFrom(k2_idLeafPara) then
  begin
