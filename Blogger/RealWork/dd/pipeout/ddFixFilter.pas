...
{ $Id: ddFixFilter.pas,v 1.55 2013/04/11 16:46:52 lulin Exp $ }
...
procedure TddFixFilter.DoStartChild(TypeID: Long);
begin
 if TypeID = k2_idTextPara then
 begin
   f_DelayTags:= True;
   f_ParaLen:= 0;
   f_Buffer.Start;
...
begin
 ...
 begin
  inherited;
  if (AtomIndex = k2_tiText) and CurrentType.InheritsFrom(k2_idTextPara)
   ...
   begin
    ...
    with Root do
     ...
     begin
      ...
      with f_Buffer.Root.rAtomEx([k2_tiSegments, k2_tiChildren, k2_tiHandle, ev_slView]) do
      begin
       if IsValid then
       begin
        i:= 0;
      ...
      Generator.StartChild(TagType.ID);
      try
       Write(Generator);
      finally
       Generator.Finish;
...
procedure TddWrongTagFixFilter.DoStartChild(TypeID: Long);
var
 l_Type : Tk2Type;
begin
...
begin
 if f_Skip = 0 then
 begin
  if TopType[1].InheritsFrom(k2_idDocument)
     or TopType[1].InheritsFrom(k2_idSub)
     or TopType[1].InheritsFrom(k2_idBlock) then
   case TagID of
    k2_tiGroups         : Out2Log('Groups', True);
    k2_tiAccGroups      : Out2Log('AccGroups', True);
    k2_tiSources        : Out2Log('Sources', True);
    k2_tiTypes          : Out2Log('Types', True);
    k2_tiClasses        : Out2Log('Classes', True);
    k2_tiKeyWords       : Out2Log('KeyWords', True);
    ...
    k2_tiPrefix         : Out2Log('Prefix', True);
    ...
    k2_tiServiceInfo    : Out2Log('ServiceInfo', True);
...
procedure TddNullDocFilter.StartChild(TypeID: Integer);
begin
 if TypeTable[TypeID].InheritsFrom(k2_idLeafPara) and (SkipLevel = 2) and CheckNeedFlushBuffer then
...
function TddChildBiteOffFilter.NeedFlushBuffer(const aLeaf: Ik2Tag; aTagId: Integer): Boolean;
begin
